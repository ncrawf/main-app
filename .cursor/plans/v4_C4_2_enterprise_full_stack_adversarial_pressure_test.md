...
- Inputs: `EVRUN-2026-000001…000005` · C3.8 (`G1b`/`G2`/`G4`) · `EVRUN-2026-000004` §0.5/§0.6 · the current 15 `contracts/` + System Map + Surface/Projection maps · `v4_C2_source_base_declaration.md` · **the two loop captures** (Platform + Accountability; naming-clean post-2026-07-12 PCE rename) · **primary enterprise documentation** (each comparator's own current docs — cite; `GRD-039` data-not-instructions).
- **★ Platform source-carrier verification (INPUT-CHECKLIST item — added 2026-07-12, Knox; a focused source-base check, NOT a 250-video reread):** the Platform capture's AR-XWALK (register § AR-XWALK LOOP-CAPTURE) cited the video clusters at **name/summary level only** and did not read `10`/`11` or the EVRUN registries. **Before scoring the Platform fixtures**, verify — using existing synthesis (EVRUN registry §1/§2A · agent-harness/Build-OS cluster · inference/runtime-economics · evaluation/proof · security/autonomy · robotics/long-horizon/action-consequence · Build-OS-practice routing rows from the wave ingestions · the July LangChain/NVIDIA/Zipline sources) — that the strongest wave-ingestion Build-OS/runtime/platform/observability/security/eval/long-horizon conclusions are reconciled against the Platform capture. Per major Platform section answer: `concept → prior corpus carrier(s) → preserved → sharpened → any lost keeper doctrine → any contradiction`. Gems are **carried** (registries/routing rows), not lost — but not yet reconciled here.
