...
- Inputs: `EVRUN-2026-000001…000005` · C3.8 (`G1b`/`G2`/`G4`) · `EVRUN-2026-000004` §0.5/§0.6 · the current 15 `contracts/` + System Map + Surface/Projection maps · `v4_C2_source_base_declaration.md` · **the two loop captures** (Platform + Accountability; naming-clean — pre-release plane = **Engineering & Validation (E&V)**, interim "PCE" retired) · **primary enterprise documentation** (each comparator's own current docs — cite; `GRD-039` data-not-instructions) · **C4.3 — Care Response-Seam / Correction & Continuity Admission Test** (`v4_C4_3_care_response_seam_correction_continuity_test.md`, `required_pre_spine_input`, design-PASSED 2026-07-19).
- **★ C4.3 correction-continuity integration (INPUT addition, NOT a method change — 2026-07-19):** Task D consumes **C4.3** as a cross-cutting **correction-continuity oracle set** (candidate Law 10.1 + O1–O22 + the 24-mutation ledger, design-PASS only). Do NOT add the 24 scenarios as new primary Task-D fixtures and do NOT reopen Task-D methodology. Apply these perturbations across the EXISTING Task-D fixtures where relevant: challenged/corrected source state · correction-of-correction · degraded correction-impact coverage · effective-time authority invalidation · external correction ACK≠accepted custody · selective reopening · independent remedy/technical/clinical/recurrence horizons · no-shadow-truth / no-central-corrector. Use C4.3 in R·E·A·X·P·L scoring for: **E** = invalid correction transitions structurally prevented · **A** = correction committed by the authorized owner · **X** = partial failure, correction, recall, selective reopening · **P** = impact reconstruction + bounded uncertainty · **L** = learning without rewriting historical truth. C4.3 is a design-PASS (contracts/build/external OPEN/absent/unproven) — treat as an oracle set, not a production guarantee.
...
## §J — Change log
- `2026-07-19` — INPUT addition (NOT a method change; shell stays `shell_pending_population`): added **C4.3 Care Response-Seam / Correction & Continuity Admission Test** to §B as a `required_pre_spine_input` + cross-cutting correction-continuity oracle set, with perturbation list + R·E·A·X·P·L usage. Methodology, roster, fixtures, and grammar UNCHANGED. C4.3 is design-PASS only (paper discrimination ≠ production enforcement).
