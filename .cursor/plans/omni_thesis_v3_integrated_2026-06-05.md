...
Builds on: spine `omni_thesis_v3_integrated_spine.md` (accepted 2026-06-05); posture `omni_enterprise_posture_2026-06-03.md` (`D0THES-DEC-035` + `D0THES-DEC-036`); AI frame `D0THES-DEC-034`; concept inventory `ai_substrate_routing_spine_REV-176.md` + `ingestion/outside_learning/sources/2026-spring_ai_substrate/inventory/`.
...
```mermaid
flowchart TB
    subgraph Input["INPUT MODALITIES (interchangeable)"]
        IM1[Form intake]
        IM2[Lab result]
        IM3[Wearable signal]
        IM4[Voice transcription]
        IM5[Live AI conversation]
        IM6[Device telemetry]
        IM7[External Rx record]
        IM8[Inbound message]
        IM9[Appointment event]
        IM10[Patient self-report]
        IM11[External chart import]
        IM12[Robot session telemetry]
    end

    Input --> SE["source_event<br/>actor + time + system<br/>identity_confidence<br/>purpose_of_use<br/>consent_context"]
    SE --> MA["media_artifact<br/>audio/video/transcript/<br/>image/device_log<br/>retention-governed"]
    MA --> OBS["observation<br/>structured, coded,<br/>temporal, with provenance"]
    OBS --> ER["extraction_run<br/>AI/rule/policy-driven<br/>model_version + capability_envelope pinned<br/>per 12.8"]
    ER --> AS["extracted_assertion<br/>confidence, polarity,<br/>uncertainty,<br/>contradiction_flag,<br/>requires_confirmation,<br/>requires_clinician_review<br/>(AI CANDIDATE; not committed truth)"]
    AS --> PC[patient_confirmation]
    AS --> CR[clinician_review]
    PC --> CS[committed assertion]
    CR --> CS
    CS --> CTX["CNS context_packet update<br/>longitudinal state"]
    CTX --> CAND["candidate generation<br/>prescribe / message /<br/>schedule / escalate /<br/>order / block / coordinate"]
    CAND --> RES["policy_resolver<br/>deterministic rules<br/>policy version pinned"]
    RES --> AUTH["authorized_action<br/>actor authority verified<br/>scope checked, audited<br/>HUMAN-committed per 12.8"]
    AUTH --> EXE[execution]

    subgraph Output["OUTPUT MODALITIES (interchangeable)"]
        OM1[SMS/email/push]
        OM2[Rx to pharmacy]
        OM3[Appointment created]
        OM4[Lab order sent]
        OM5[Robot procedure auth]
        OM6[Chart materialization]
        OM7[Payment processed]
        OM8[Provider notified]
        OM9[Patient app update]
        OM10[External chart export]
    end

    EXE --> Output
    Output --> EVID["evidence_record<br/>audit, delivery_receipt,<br/>outcome lineage"]
    EVID -.feedback.-> SE
```
...
### AI owns AI/model/tool capability + lineage — NOT authority, NOT all capability

AI #12 owns the **AI/model/tool capability registry + lineage** — `ai_capability`, `ai_model_registry`, `capability_envelope`, `ai_run`, evals, traces, autonomy class — *what a model or tool may do.* Two boundaries it does **not** cross:

- **Not authority.** *What an actor is authorized to do for a represented principal* is `delegated_authority_envelope`, decomposed across the trust axis (§A: Identity/RBAC/Federation/D7). A tool may be *capable* of ordering a lab while the actor is not *authorized* to order this lab for this patient now.
- **Not all capability.** AI #12 owns AI/model/tool capability — **not every OMNI capability, and not external capability contracts.** General internal/external capability exchange (labs, Rx, payroll, insurance, supplier rails, external agents) belongs to **§C and the owning domains.** AI #12 is not the god-domain for the word "capability" (`GRD-035`).

Three bounding objects, distinct — they interact in §C without merging:

| object | bounds | owner |
|---|---|---|
| `capability_envelope` | what a **model / tool** may do | AI #12 (§B) |
| `delegated_authority_envelope` | what an **actor** may do for a represented principal | trust axis (§A) |
| `capability_contract` | what an actor/system may **invoke across a governed boundary** | Governed Capability Exchange (§C) |

AI is one subtype of non-human actor — never the home of non-human authority, never the home of general capability.
...
> - **Evidence Plane / governed learning workbench (weave — likely §8 or §B):** OMNI's `ingestion/` Evidence Plane is the *operational form of the §8 Sense loop pointed at the outside world* — it continuously captures parallels/industry/research, indexes + analyzes them (manually OR agentically), and promotes only through gates. This is how the substrate keeps getting better and stays a 2035 system instead of a 2025 website (ties §8 loops + §B AI substrate + anti-staleness `GRD-033` + capture-broad/promote-gated `GRD-036` + workbench-not-data-store `GRD-041`). Capstone line worth landing: *"OMNI improves by governed learning — broad intake, gated promotion."* Do NOT inject a body blurb mid-rework; weave deliberately when authoring §8/§B. (Nick wants this in — flagged 2026-06-06.)
