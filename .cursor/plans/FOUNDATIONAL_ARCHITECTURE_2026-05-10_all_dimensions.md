...
**Subordination, not adequacy.** This anchor binds every primitive and sibling below: §4 primitives (especially #10 currently named `outbound_jobs` and #11 AI orchestration runtime), §5 siblings (especially `external_communications/` and future rail-bound siblings), §7 four-layer epistemic model, §8 cross-cutting concerns. **The CNS subordination is locked by DL-14. Subsystem adequacy and naming (e.g., is `outbound_jobs` truly universal? is AI runtime truly a planner over multi-event state? does §1Q express the multi-actor multi-action model?) is determined by Phase 0 of the brain hardening audit at `.cursor/plans/omni_brain_hardening_d1ef429b.plan.md` — NOT by this anchor.**
...
**Primitive #10 (DL-14 subordination — binding; NO adequacy certification).** Per DL-14 invariant 5 (subsystem subordination, not adequacy). Primitive #10 is a **subsystem UNDER the OMNI CNS** (per §0 anchor at the top of this document + MAIN DL-14). The current name (`outbound_jobs`) and the current scope (patient-outbound message dispatch + rails described in this row) reflect its existing design. **Phase 0 of the brain hardening audit (`.cursor/plans/omni_brain_hardening_d1ef429b.plan.md`) will determine** whether primitive #10 is genuinely the universal action substrate the CNS requires OR whether it must be semantically broadened — and possibly renamed (e.g., `orchestration_actions`) — to host sibling action types as projections beneath: **patient outbound message, provider notification, staff task, ops alert, passive awareness marker (with `OMNI sent → unseen → seen → acknowledged` state machine), suppression/cancellation, AI plan request, lifecycle state update, no-op.** **Until Phase 0 verdict lands, primitive #10 remains formally scoped to its existing description above.** This DL-14 entry only declares its **subordination** to the CNS center of gravity, not its adequacy. Cross-link DL-14 + MAIN DL-14 invariant 5 + foundational §0 + Phase 0 stress scenarios 9-11 (passive provider awareness + staff feedback attachment + AI feedback attachment).
...
**Primitive #11 (DL-14 subordination — binding; NO adequacy certification).** Per DL-14 invariant 5 (subsystem subordination, not adequacy). Primitive #11 is a **subsystem UNDER the OMNI CNS** (per §0 anchor + MAIN DL-14). The runtime is subordinate to `§1Q` deterministic gates and never bypasses them per §4.A. The current scope reflects existing design (AI MAY summarize / classify / suggest / draft for human review; AI MAY NOT autonomously send patient-facing messages, alter history, impersonate staff, decide clinical outcomes). **Phase 0 of the brain hardening audit will determine** whether the existing AI runtime scope is genuinely a planner / recommender over the unified multi-event patient state across all actor targets (patient, provider, staff, ops, queue, AI, vendor) and all action types — OR whether it is currently scoped as marketing-copy assist / draft-suggest only, and must be broadened to a planner over multi-event state to satisfy DL-14. The Phase 0 audit also determines whether the learning-loop substrate (AI accept/edit/reject feedback with full attachment depth: proposal id, prompt version, model version, context snapshot, final action diff) exists or is missing. **Until Phase 0 verdict lands, primitive #11 remains formally scoped to its existing description above.** This DL-14 entry only declares its **subordination** to the CNS center of gravity, not its adequacy. Cross-link DL-14 + MAIN DL-14 invariant 5 + foundational §0 + Phase 0 stress scenarios 9-11.
