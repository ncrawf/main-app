...
Status: `draft_for_ratification` (created 2026-05-30, Foundation vNext; first proof contract; Nick + Knox review gate) · **corpus gem-pull (scheduling/D5 cluster) done 2026-06-01** (HIGH-bar collapse-zone review: confirmed contract is built from the fresher rule-matrix Round-5 layer + correctly rejects the superseded `encounter_container` parent framing per inv 1/`GRD-003`; 2026-05-17 operating-model/pressure-test = pre-rule-matrix evidence; 27+45 workflow scenarios = build-validation corpus §9; 4-tier attestation→RBAC + venue→Settings staged; map-change = none)
...
| 2026-05-17 scheduling **operating-model** + **architecture-pressure-test** docs — `encounter_container` / 14 `encounter_line` types / `encounter_profile` enum / Q1 encounter-container-architecture / Q15-16 encounter-identity / pressure-test "3 tentative options" | **superseded (pre-rule-matrix layer)** — NOT re-pulled (`GRD-022`/`GRD-003`) | these are the EARLIER exploration; the rule-matrix Rounds 1-5 + Round-5 closure + `service_occurrence` parent lock RESOLVED them; this contract is built from that fresher layer. Durable concepts (scheduling-as-sibling, 5 source-of-truth lanes = D5/D6/D7/MSG/CM, planned→actual→doc→charge→close→followup pipeline = dual-loop+seams, care_episode parent) already carried forward. Evidence only. |
| 27 workflow scenarios (operating-model §5) + 45-Q2 pressure-test scenarios + industry-analog set (Botox/laser-package/multi-episode/async-Rx/video/multi-provider/multi-location-federation/cancellation/no-show/bulk-reschedule/waitlist/lab→result→followup/recurring-series/package-redemption/insurance-prior-auth/manual-capture/minor-consent/cross-state/provider-coverage) | **preserve as build-validation corpus** (evidence) | the collapse-zone (planned vs actualized) validation suite — consumed at D3/D5 build to prove the model survives real flows; NOT contract text (per Artifact Index: scenario suites stay evidence, converted to per-domain pressure-tests at build) |
| 4-tier provider authorship + attestation (operating-model Q10) · 11-axis venue substrate (Q12) | **stage** → RBAC/Attestation (DL-18) · Settings/Federation (venue) | authorship-tier = attestation authority (RBAC, not drafted); venue substrate = Settings/Federation config | tracked for RBAC + Settings native passes |
...
`DL-20_care_coordination_DRAFT_2026-05-17.md` (§0 reconciliation + body, now evidence) · `designs/day_0_scheduling_rule_matrix/05_domain_service_occurrence.md` (SO-01..SO-34) + `05_3_round5_closure_verdict.md` (the controlling closure) · **`2026-05-17_omni_scheduling_operating_model_and_architecture.md` + `_architecture_pressure_test.md` + `_day_0_build_contract.md` + `2026-05-16_mindbody_architecture_understanding.md` (pre-rule-matrix layer — `encounter_container` framing superseded; 27+45 workflow scenarios = build-validation corpus)** · `omni_thesis_v2_2026-05-26.md` §7.3/§7.5.1/§7.7 · decisions `D0THES-DEC-027/028/029`.
