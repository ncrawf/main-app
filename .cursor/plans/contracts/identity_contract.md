...
| `actor` (primitive #1 + DL-16 amendment 43 4-tuple) | who/what performed an action — `staff` / `staff_with_ai_assist` / `provider` / `patient` / `system` + **`device` / `robot` / `external_system`** subtypes (thesis §12.2). Only humans may occupy §7.2 care-ownership roles |
