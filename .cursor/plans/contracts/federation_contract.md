...
**Does NOT own:** **patient IDENTITY + `patient_relationship`** (Identity — Federation governs cross-relationship GRANTS, Identity owns the identity/relationship rows); **RBAC capability/role atoms** (DL-18 — cross-brand grants COMPOSE with RBAC atoms, don't replace them); **where permeability policy is *stored/inherited*** (Settings/DL-19 substrate hosts + inherits the configurable VALUES) — **but Federation owns the cross-operator MEANING of the policy; Settings/RBAC MUST NOT reinterpret a federation policy as mere tenant config** (the values are Settings-hosted; the cross-operator semantics, admission gates, and Tier-4 requirement are Federation's); **clinical/commerce/scheduling/doc/observation truth** (they READ federation to gate cross-tenant action); **actor CAPABILITY + attestation + break-glass ENFORCEMENT** (RBAC/DL-18 — Federation owns the break-glass *topology/path*, RBAC owns the capability/attestation that authorizes it; see §6 inv 11); **policy ENFORCEMENT orchestration** (CNS Meta / Network Governance Plane enforces + audits; Federation supplies the substrate it reads).
...
11. **Break-glass = Federation topology + RBAC teeth (split).** Federation owns the cross-tenant break-glass *topology/path* (which operator boundary can be crossed in emergency, recorded with the same cross-tenant `cns_decision` audit, inv 6); the **capability to invoke it + the attestation/reason-code + the enforcement** belong to **RBAC/DL-18** (capability-gated, reason-coded) and **CNS Meta** (enforce/audit). Federation never grants break-glass *capability* — it only defines that the cross-boundary path exists and is auditable. (Knox caution; the RBAC native draft picks up the capability/attestation/audit teeth.)
