...
### §6.2 Template discipline (rules/templates §1Q engine — `REV-149`; CNS §9.2)

**Automated** patient-facing sends (system/rule/scheduled/AI-emitted) MUST use **approved templates** with `(rule_id, template_key, evidence_refs)` lineage — CI-lint forbids automated paths bypassing the template registry; missing template variable at render = **error, not silent fallback**. **Human-authored** provider/staff freehand to patients IS permitted under role authority (`requireCapability`) + audit; **AI may DRAFT it but the human approves before send** (no AI auto-send under a human's capability; `ai_drafting_assist` + `model_version` + draft-vs-sent diff logged — extends §8.3). `ai_refinement_allowed` is **template-governance (CODEOWNER PR-time), NOT patient consent** — AI refines only enumerated phrasing fields within constraints, never `prohibited_claims`/`required_variables`/`tone`. The engine spec + own-domain decision live in CNS §9.2 / `REV-149`; Messaging **renders + sends** under it. (Source gem: `audits/2026-04-30_rules_templates_framework.md`.)
