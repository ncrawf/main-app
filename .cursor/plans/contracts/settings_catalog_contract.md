...
5. **Settings DEFINES, domains EVALUATE/PRICE/COMMIT** (§3 anti-junk-drawer) — `service_policy` defined here, evaluated by D3; `catalog_item` defined here, priced by D6; permeability values hosted here, meaning owned by Federation.
...
- **`SC-SET-D3-001`** Settings → D3: service catalog + `service_policy`/gate DEFINITIONS (D3 evaluates at booking; `REV-147`).
...
- Confirm staged re-points at ratification: `SC-SET-D3-001` (service/policy defs → D3 evaluate), `SC-SET-D6-001` (catalog → D6 price), `SC-SET-MSG-001` (`pathway_sensitivity` → Messaging), Federation permeability hosting boundary.
