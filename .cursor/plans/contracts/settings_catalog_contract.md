...
- **Compatibility boundary RESOLVED** (not deferred): Settings-defined, D3-evaluated, D3-owned timing/locks — D3 §3/§4 + Settings §4a agree. Confirm remaining staged re-points at ratification: `SC-SET-D6-001` (catalog → D6 price), `SC-SET-MSG-001` (`pathway_sensitivity` → Messaging), Federation permeability hosting boundary (Settings hosts values, Federation owns meaning — §7 binding even at code level).
