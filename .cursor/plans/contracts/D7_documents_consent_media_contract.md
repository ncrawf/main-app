...
`patient_document` (unified; open `document_kind`; PII class; 7-state lifecycle; storage_uri + checksum) · `media_artifact` · `clinical_photo_detail` + `before_after_pair` · `consent_artifact` + `consent_template` + `signature_envelope` · **`patient_consents` (typed consent record + 6-toggle UI mapping, §5)** · `patient_document_linkage` (M:N, references not copies) · `evidence_record` / materialized records (notes/PDFs/packets) · `visibility_grant` / `shared_context_grant` (scoped projection) · `upload_event` / fingerprint dedupe (§6). Exact shapes per DL-22 + contract.
