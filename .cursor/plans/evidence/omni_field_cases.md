...
## FIELD-004 — Outbreak-aware acute-illness phone encounter (live evidence + pharmacist signal + fulfillment reroute) — "Dan" case

- **case_type:** `provider_workflow_case` (also live-care-resolution flavor — a real-time, multi-actor governed resolution)
- **source / provenance:** operator-provided account + Nick↔Knox trifecta discussion, 2026-07-11; captured as Evidence-Plane source **`EVSRC-2026-000252`** (`ingestion/user_operator_research/sources/2026-07/EVSRC-2026-000252_outbreak-aware-phone-encounter-live-evidence-pharmacy-reroute/` — de-identified operator account + verbatim Knox Review 001 (two rounds) + Opus repo-grounded Review 003 + delta ledger). **This row is a compact pointer — see the specimen + `EVRUN-2026-000004` §9.10 for the full decomposition; do NOT duplicate the narrative here.**
- **summary:** A single prescription write for an acute-diarrheal complaint pulled in **four+ actors** (patient · provider · live web-search AI · pharmacist) **+ a real-world event** (Michigan regional foodborne-parasite outbreak). Provider researched the outbreak + treatment live *while texting/on the phone*, committed an empiric plan ("we'll call it in"), hit a **pharmacy stockout** ("filled 500 scripts, we're out"), and rerouted to an alternate pharmacy — the clinical resolution staying open through it all. Operator: *"this is clinical medicine, there is no textbook answer here… all 4 of those actors had input for ONE SIMPLE PRESCRIPTION WRITE."*
- **why_it_matters:** Sharpest isolation yet of **streaming, mixed-initiative care resolution**: care must stay coherent while the conversation, the outside world, available resources, AND the *meaning of the planned action* (one drug = empiric/targeted/hedge guess at different tree positions) are all still changing — with authority, source-truth, and commit boundaries held stable throughout. Surfaces the **context-admission stability gate** (when is a partial/streaming fragment eligible to shape clinical context — distinct from the commit gate) as a genuinely new governance question. HARDENS existing physics (no new primitive); the low-cardinality version of the hospital arc's findings.
- **thesis / domain relevance:** thesis §1 (core exchange) · §8 (loops+gates) · §B (candidate≠commit; AI-influence lineage) · §A (permeability) · **REV-184** (branch/lineage depth + reopen triggers + world-model honesty) · **M2** comm-event fabric (`build=ABSENT`) · **Knowledge Reservoirs + Sense loop** (outbreak signal) · **`ai_decision_log`** (AI-influence disposition, 3 axes) · **`source_authority_map`** (within-utterance claim atomization) · **OFC** (false-closure ladder + `replayable_proof`) · **WI6/FIELD-002** (semantic reconciliation — organism-name meta-instance).
- **affected_domains:** CNS, Messaging, Clinical Memory, Observation, D7, OFC/P35, RBAC, Identity, AI/model-lineage, Knowledge Reservoirs, Longitudinal Intelligence.
- **required_future_handling:** carry the `EVRUN-2026-000004` §9.10 delta ledger (D1–D10) as v4-spine + C5 inputs — esp. the **context-admission stability gate** (M2 `source_event` attribute + CNS rules), **AI-influence-lineage ledger** (`ai_decision_log`), **execution-exception→Decide-reopen**, and the **false-closure/`replayable_proof`** fixture. Propose-only; dedup against §0.5 baseline; no new plane/domain/wrapper.
- **linked_open_review:** none minted; routes via `EVRUN-2026-000004` §9.10 + the pre-spine physics-of-care continuation (`HANDOFF_2026-07-11`). Open a `D0THES-REV-*` only if the spine author finds a genuine gap. **Open sub-item:** a referenced "Mike" case has no corpus entry (only FIELD-001 Alec + FIELD-003 Kyle) — confirm whether it is a fifth uncaptured specimen or a mis-remembered alias.
- **status:** `active`
