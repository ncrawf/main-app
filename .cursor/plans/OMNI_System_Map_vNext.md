...
- **OMNI is also a business-operations OS** (not only care): **Business-Ops / Workforce** (workforce scheduling, time-clock, payroll, compensation, commissions, productivity) is a future substrate domain family that consumes D3/D5/D6/RBAC events but does NOT own clinical/care truth (`REV-164`); **analytics/metrics = projection** over domain truth (never a truth source); **admin/provider access = RBAC/Boundary-Policy** (authority, not a dashboard).
- **Comparator/analogy index:** the complete classified inventory of every brand comparator + architecture analogy + feature-idea reference (~75-80) lives in `doctrine/comparator_analogy_registry.md` (thesis §3.5 = narrative; registry = full index). The product-category→domain mapping below is the orientation subset.
...
| **BIZOPS** | **Business Ops / Workforce** (future substrate family — workforce scheduling, time-clock, payroll, compensation, commissions, incentives, provider productivity) | **workforce/employee operational records, shift, clock-in/out, worked hours, pay rules/period, commission/incentive events, labor cost** (truth) | **clinical/care truth; commerce sale truth (D6); actualized clinical work (D5); authority (RBAC); analytics (projection)** | `contracts/business_ops_workforce_contract.md` (future) | ⏳ **named, not scoped — `REV-164`** (consumes D3/D5/D6/RBAC events; never corrupts care truth; Workday/ADP/Gusto + QuickBooks layer) |
