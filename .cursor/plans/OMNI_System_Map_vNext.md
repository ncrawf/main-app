...
| **CM** | **Clinical Memory / Assertion** (clinical-truth substrate — fed by many, read by all) | **`clinical_concepts` registry, `patient_clinical_assertions` (append-only), `source_authority`/`authored_by`, authority precedence, `status`/`clinical_adoption_state`, context/`context_key`, conflict/reconciliation, `patient_clinical_assertion_current` view, `patients.*` projection trigger, `recordClinicalAssertion` + adoption gate** | **raw evidence rows (producers' homes); intake construction (Intake); identity (Identity); the AI engine that generates candidates (AI — a producer that must respect adoption rules)** | **`contracts/clinical_memory_assertion_contract.md`** | ◑ **contract drafted; ratify-pending** (clinical_assertion_layer_design + 1K.5.A + shipped assertion-types/orchestrator; §7.5.3 alignment: `source_authority`/`clinical_adoption_state` ≈ `authored_by`/`status`; provider-only adoption; AI-never-confirms; open REV-151 vocab unify) |
