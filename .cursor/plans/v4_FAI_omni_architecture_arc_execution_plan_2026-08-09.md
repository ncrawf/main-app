...
**Two words, used here in one sense each, because both are already spoken for** `[KND]`. ***Method*** is used as `§4` uses it — **the governing rules for how work is done**, which is what the method lane writes. It is **not** the sense in `doctrine/omni_work_method_repertoire.md`, where a *method* is an **optional technique selected for a piece of investigation** (`METHOD-000` and its siblings): **a method lane does not pick an approach from a catalog, it revises the rules that any approach runs under.** ***Loop*** here is a **build-time collaboration cycle** — evidence, change, inheritance, better work — and has **no relation to OMNI's governed operating loops** (Sense and Act; the Care, Platform and Accountability Loop family). *Those are substrate physics; this is how the substrate gets built.* **Throughout this section *loop* is the cycle and *lane* is the work stream running inside it.**
