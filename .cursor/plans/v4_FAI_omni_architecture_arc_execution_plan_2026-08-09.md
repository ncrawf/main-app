...
### R8 in-revision — the repertoire error, second instance, caught by the operator

**`§4.4`'s destination clause routed collaboration topology to "the Arc Repertoire."** No such artifact exists — the string appeared **once in the entire repository**, in that sentence. The real artifact is `doctrine/omni_work_method_repertoire.md`, whose passport reads `manifest_or_catalog` · `derived_nonbinding` · *"describes how to investigate, never what is true. **Execution behavior = `agent_work_protocol.md` §2.1**"*, and whose decision row `D0CKPT-DEC-006` says it **cannot originate architecture or execution authority**. Lane identity, writable surfaces, freshness, inheritance and concurrency are execution behaviour, so the catalog was the one destination that sentence could not have.

**This is receipt (1) below, recurring inside the section that fences against it.** `§4.4` spends a paragraph pinning *method* away from the repertoire's sense — *"a method lane does not pick an approach from a catalog, it revises the rules that any approach runs under"* — and then, four paragraphs later, routed the topology into the catalog under an invented name. **Corrected in place at `§4.4`:** the destination is **Build OS `09` + Agent Work Protocol `§2.1`**, which is already titled *lane continuity contract + coordination overlay* and is where `AGENTS.md` routes durable-lane and coordinated multi-lane work.

**Classification — and it is deliberately NOT a method defect.** `§4.1` ① already governs this: *a label is not a disposition* · *source access is not source understanding*. The rule exists, covers the failure exactly, and was not followed — which is `§4.4`'s explicit anti-trigger (*"a single mistake an existing rule already covers"*). **Object-local correction, no method lane.** *Contrast the derivation-rule defect at the carrier's `§E4c`, where the failure survived compliance because no rule covered it at all — that distinction is what separates a lapse from a gap.*
...
**Three words, each pinned to one sense, because all three are spoken for elsewhere** `[KND]`. ***Method*** is used as `§4` uses it — **the governing rules for how work is done**, which is what the method lane writes. It is **not** the sense in `doctrine/omni_work_method_repertoire.md`, where a *method* is an **optional technique selected for a piece of investigation** (`METHOD-000` and its siblings): **a method lane does not pick an approach from a catalog, it revises the rules that any approach runs under.** ***Loop*** and ***lane*** are both used here, for different things. **The loop is the continuous cross-lane feedback cycle** — evidence, admission, inheritance, better work — and it **runs on every review whether or not anything opens**, because the signal below is answered each cycle and is usually answered *none*. **The lane is the intermittent work stream**: a method lane opens on a trigger, does bounded work and closes. *A loop that samples continuously and usually actuates nothing is still a loop; only its actuator is intermittent.* **This loop has no relation to OMNI's governed operating loops** — Sense and Act, and the Care, Platform and Accountability family — which are **substrate physics rather than build-time collaboration.** That is a standing disambiguation cost, accepted because the word is accurate; *lane* is already the estate's build-time word (`AGENTS.md` durable-lane and multi-lane work) and carries no such cost.
...
**Safety rules.** Method changes are **proposals under review, never silent self-modification.** The method lane does not adjudicate the object's substantive architecture. **A method change may not be introduced to rescue an object finding currently under review** — that inverts the coupling into laundering, and the overlapping seat is precisely the one positioned to do it without noticing. **The test is directional:** did object evidence establish the defect *independently of the finding*, or was the standard moved to fit it? **A finding that survives a properly established method change has not been laundered** — re-evaluating a conclusion under a corrected method is the loop working, and treating survival as guilt would block the legitimate direction entirely. Not every method discovery interrupts active work. **The method lane closes when its defect is addressed**, and at closeout only what proved useful is generalized.
...
**What each function receives is deliberately unequal.** The object author gets the **committed method head, the controlling sections, the exact behavioural delta and the next permitted act**, plus **bounded rationale wherever the matter is still unresolved** — *an open question cannot travel as a delta* — but **not the method deliberation**. The method author gets **the failure evidence and the trace around it** — not the object's whole source corpus. The review function gets **both heads, both diffs, both returns and the cross-lane line**, plus whatever native sources it must open to assert a finding of its own. **These are default interfaces, not access limits** — any position may open whatever it must to support a finding it is personally asserting, under `§4.1`'s per-claim rule. *This estate has already read a scope statement as an exclusion twice; a description of what is delivered by default is not a description of what may be read.* *Attention stays fluid across both lanes — noticing the connection is the entire point — while **writes and recorded state stay partitioned.** Fluid attention, partitioned authorship.*
...
**The state that travels is small, and it is named rather than improvised:** *object · object head · object state · object writer and writable surface · method-lane state · trigger · method head · cross-lane impact · required freshness or inheritance action · next permitted act · any other materially influencing lane.* **One such record per active object→method dependency edge** — the coupled work graph makes the edge the unit, not the program — and **when no lane is open it collapses to the signal itself**: inactive, no impact, continue. *Proportional to what is actually happening; a fixed length would make ordinary work pay for machinery it is not using.* Each line is there because **both lanes have to agree on it** — this is the shared state that belongs to neither, written where either can read it. Whether that set is exactly right is learned by using it; **what is not optional is that the coupling's state is written somewhere rather than carried in a conversation.**

**Known failure modes, named so they stay recognizable.** The overlapping seat is a **throughput bottleneck and a single point of vantage** — if only one position sees both lanes, its absence blinds the system. **Method churn can outrun inheritance**, leaving the object author working against instructions that moved underneath — a staleness the coupling itself creates, and the standing price of having a bridge at all. And the pattern **costs more than it returns on ordinary work**, which is why it is optional rather than owed.

**Nothing here is frozen, and the overlap is a function rather than a person.** No seat count, staffing pattern, model identity or named participant is fixed; **the number of agents is not the architecture.** **And the object count is no more fixed than the participant count** — this arc ran one object lane against one method lane because that is what the work presented. The general shape is a **coupled work graph**: several object lanes may share **one** method dependency, with Output 4, Authority, Profiles and Conformance all inheriting the same source and review method, and one object lane may consume **several** method surfaces. **Two lanes is this instance, not the pattern**, and what is declared is the **edge** between an object and a method it depends on — never a fixed topology. Whoever holds the overlap must be **replaceable**, which means **the coupling's state is shared between the two lanes and owned by neither** — so it is stated where both can read it, rather than held by whichever position happens to sit in both conversations. *Unstated cross-lane state does not merely risk being forgotten; it lets the two lanes carry silently different views of what is binding on whom, which is the specific failure a bridge exists to prevent.* The durable claim is narrow: **separate object work from method work, connect them through reviewed evidence, and let accepted method learning change future execution without rewriting the work already under judgment.**

**Two destinations, and they do not collapse.** The architecture-learning semantics of `§§4.1`–`§4.3` compile into **Architecture Operations** in `operations/`. **This section's collaboration topology does not** — lane identity, authoring and review assignment, writable surfaces, freshness and inheritance, and concurrency belong to **Build OS `09` and Agent Work Protocol `§2.1`**, whose *lane continuity contract + coordination overlay* is already the estate's home for exactly those. **★ First candidate instance drafted 2026-08-23** (`D0CKPT-DEC-010`, **`review_required` — NOT active law**): the **bounded interrupt** — assess a mid-work discovery against the parent's next act and completion condition, hold the parent in an *operational* surface with a named re-entry condition, let the existing continuity × coordination test decide any resulting topology, and **do not integrate into the parent's normative model until the candidate has survived review independent of the authoring pass** — is proposed at `agent_work_protocol.md` §2.1, with concepts at `09` Layer 2 and the anti-pattern at `06` `D0CKPT-GRD-006`. **An interrupt is an event affecting active work, not a kind of work, and it pre-decides no topology.** *Blocked from activation pending independent review (Knox, 2026-08-23); non-blocking to this arc's Authority object.* ***Not the method repertoire*** `[KND]`: `doctrine/omni_work_method_repertoire.md` is a `derived_nonbinding` optional catalog of investigation techniques whose own passport routes execution behaviour **away** from itself to `agent_work_protocol.md` `§2.1`, and `D0CKPT-DEC-006` states it **cannot originate execution authority**. Lane identity, writable surfaces and concurrency **are** execution behaviour, so the catalog is the one place this may not land — *the fence three paragraphs above governs this sentence too, and an earlier draft failed it* (`§0`). **And this is what the method lane is named for:** `§§4.1`–`§4.3` are the **arc-local home** of the same body of method law that `operations/` will hold **permanently** — *one thing at two points in its lifecycle, not two senses of the word.* `§4.4` is the exception that proves the split: it is method work, but its content is collaboration topology, so it routes to Build OS instead. **There is no `method/` folder in the selected package and none is owed** (`§0.6`). **But destinations are owner-relative, and `operations/` is not the home of all methods:** a method lane writes the governing method object named by its dependency, and **that object's rightful owner sets its destination** — an evidence-intake method would route to the Evidence Plane, for instance. **The examples settle no ownership**: where an owning system is itself unresolved in the estate, the destination is an open question for that owner rather than something this section decides. *The two destinations above are this arc's, derived from what its own method objects are.* *What survives replacement is not "use four agents" but something closer to a declarable state: work object · method-dependency version · authoring holder · review holder · writable surface · freshness position · acceptance holder.*
