...
### §4.4 — Coupled object and method lanes
...
**One word used in one sense, and one word deliberately avoided** `[KND]`. ***Method*** is used as `§4` uses it — **the governing rules for how work is done**, which is what the method lane writes. It is **not** the sense in `doctrine/omni_work_method_repertoire.md`, where a *method* is an **optional technique selected for a piece of investigation** (`METHOD-000` and its siblings): **a method lane does not pick an approach from a catalog, it revises the rules that any approach runs under.** **And these are deliberately *lanes*, not loops.** OMNI already uses *loop* for its **governed operating loops** — Sense and Act, and the Care, Platform and Accountability family — which are **substrate physics rather than build-time collaboration**, and a term that needs a permanent fence is a term that will eventually be read without one. **This structure is also dormant by default, which a loop is not:** it opens on a trigger, does bounded work and closes. ***Lane* is already the estate's build-time word** (`AGENTS.md` durable-lane and multi-lane work), so it carries no such cost. *The feedback path is described below rather than named, on `§4.3`'s reasoning.*
