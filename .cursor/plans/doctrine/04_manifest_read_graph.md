...
15. **Current Checkpoint Handoff:** `.cursor/plans/HANDOFF_2026-06-13_v4_prep_through_c3_5_arc_setup.md` (current, 2026-06-13 — v4 authoring runway: wave-2 ingestion COMPLETE; gates C2 CLOSED · C3 PASSED · C3.1 PASSED · C4.0 ACCEPTED; C3.5 EHR-gravity arc stood up [separate agent next]; **controlling plan in HOME dir `~/.cursor/plans/wave-2_source_scaffolding_654989a0.plan.md`**; do NOT author v4 prose yet). Lineage: `…06-06_evidence_plane_and_knowledge_reservoirs` → `…06-10_ai_corpus_ingestion_complete_pre_100video_scaleup` → this 06-13 v4-prep handoff. **Prior handoffs are historical-only** (06-06 Evidence-Plane/`GRD-036..042` detail + 06-10 wave-2 ingestion detail stay valid as detail). Operational state transfer; updated each Tier 2+ checkpoint per `agent_work_protocol.md` §8 **Checkpoint Closeout Rule**. **Drift-guard: this entry MUST name the same file as `AGENTS.md` `## OMNI Operating References` Current Checkpoint Handoff; if they differ, the §1 Boot Freshness Check fires → STOP.**
