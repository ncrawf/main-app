...
15. **Current Checkpoint Handoff:** `.cursor/plans/HANDOFF_2026-07-02_c3_8_charter_accepted_pre_g1a.md` (current, 2026-07-02 — C3.8 Enterprise-AI-OS Convergence Pass charter ACCEPTED [Nick + Knox]: a bounded, gated pre-spine pressure/convergence pass; gates C2 CLOSED · C3 PASSED · C3.1 PASSED · C4.0 ACCEPTED · C3.5/C3.6/C3.7 COMPLETE · REV-184 CLOSED · C4 runway ACCEPTED · Readiness Note PASSED; G1a APPROVED [Nick + Knox 2026-07-02]; NEXT = C3.8/G1b [ingest the approved 15-source shortlist → G2/G3/G4, run MANUALLY gate-by-gate with a STOP + review after EACH gate; leash `v4_C3_8_G1b_KICKOFF_PROMPT.md`; the G1b→G4 run is enterprise-hot, so a FRESH-context spine author resumes from the clean G4 ledger]; **controlling plan in HOME dir `~/.cursor/plans/wave-2_source_scaffolding_654989a0.plan.md`**; HARD STOPS: no C4 edits, no thesis/spine prose, no contract edits, no source-list sprawl). Tier-3 narrative: `docs/architecture/evolution_narrative_volume_6_2026-06-14.md`. Lineage: `…06-13_v4_prep` → `…06-14_c3_5_complete…pre_c4` → `…06-15_c4_runway_accepted…` → this 07-02 C3.8-charter-accepted handoff. **Prior handoffs are historical-only** (06-15 C4-runway/readiness detail, 06-14 arc + REV-184 closure, 06-13 v4-prep, 06-10 ingestion, 06-06 Evidence-Plane detail all stay valid as detail). Operational state transfer; updated each Tier 2+ checkpoint per `agent_work_protocol.md` §8 **Checkpoint Closeout Rule**. **Drift-guard: this entry MUST name the same file as `AGENTS.md` `## OMNI Operating References` Current Checkpoint Handoff; if they differ, the §1 Boot Freshness Check fires → STOP.**
...
10. **v4 C3.8 — ENTERPRISE-AI-OS CONVERGENCE PASS charter (accepted 2026-07-02) — consult FIRST for any C3.8 enterprise-AI convergence/pressure work, inserted pre-spine after the C4 runway.** `v4_C3_8_enterprise_ai_os_convergence_pass_plan.md` — bounded, gated pass asking whether OMNI survives AND is legible as a governed execution substrate vs Palantir/OpenAI/Anthropic/MSFT/Google/IBM (pressure sources, not gods). Six axes (ontology/semantic · **data/memory/model ownership at tenant scale = highest-risk** · policy/action governance · agent-runtime/P35/tools · eval/sim/deployment · security/zero-trust); strategic-posture + capital-allocator legibility; common-sense ownership questions (plain-language-first); pre-registered OMNI invariants; universal-vs-procurement filter; gates **G1a** (source shortlist + axis-coverage plan → STOP for Nick+Knox approval) / **G1b** (ingest, current sources) / G2 (convergence matrix + enterprise translation map) / G3 (40–80 doctrine breakers) / G4 (disposition ledger — route every finding, 6 categories; NOT a magnitude cap). Grounded agent runs G1a; a separate agent runs G1b→G4; findings fold into C4, then the spine. G1a artifact = `v4_C3_8_G1a_source_shortlist_and_axis_coverage.md` (when produced). `consult_if_routed`, `analysis_nonbinding` (`GRD-036`).
