...
1. **Permission Group is a substrate primitive (not a label on staff).** `permission_group` carries: `id`, `tenant_id` (brand-scoped per DL-10), `name`, `description`, `is_system_group` BOOLEAN (TRUE for OMNI-built-in groups; FALSE for tenant-customized), `parent_group_id` FK NULL (optional inheritance hierarchy). Mindbody (Layer 2 + Batch 16 Step 05 evidence) ships 5 system groups per brand: `External` / `Front Desk` / `Manager` / `Service Provider` / `Social Media Manager`. OMNI Day 0 ships ≥ these 5 + adds: `Provider` (clinical authorship distinct from service provider) / `Admin` (tenant admin) / `Owner` (legal entity owner) / `Compliance_Reviewer` / `Read_Only_Auditor` / `Break_Glass_Operator`. Total Day 0 seed = ~11 system groups. Tenants may add custom groups (`is_system_group = FALSE`); system groups REJECTED for tenant edit/delete (only enable/disable + atom grant override).

2. **Permission Atom is a substrate primitive (granular capability unit).** `permission_atom` carries: `id`, `atom_key` STRING UNIQUE (stable identifier; e.g., `appointment.book` / `appointment.cancel_on_behalf_of_patient` / `commerce.refund_issue` / `commerce.void_pre_settlement` / `commerce.adjustment_create` / `commerce.discount_override` / `clinical.chart_write` / `clinical.rx_prescribe` / `clinical.lab_order` / `clinical.attestation_sign` / `staff.deactivate` / `settings.edit_required_fields` / `settings.edit_payment_methods` / `audit.inspect_log` / `audit.export_phi` / `federation.modify_permeability_policy` / `break_glass.invoke` / `data_privacy.unmask_merged_client` / etc.), `domain_scope` ENUM (`scheduling` / `commerce` / `clinical` / `messaging` / `audit` / `settings` / `staff_management` / `federation` / `break_glass`), `risk_tier` ENUM (`low` / `medium` / `high` / `critical_break_glass`), `default_audit_required` BOOLEAN (TRUE for all critical_break_glass + most high), `requires_attestation_kind` ENUM NULL (`reason_code` / `dual_approval` / `provider_signature` / `legal_entity_owner_signature`), `description`, `introduced_in_version`. Day 0 seed registry: ~80 atoms across the 9 domain_scopes. **NOT a closed enum** — registry extends as new operational features land per DL-16 inv 5 + 9 + 29.
...
11. **Every permission-grant / staff-assignment / attestation emission is audited (binding).** Per DL-16 inv 30 + 38. `permission_audit_log` is a specialized projection of the universal audit log per DL-16 inv 38; records: `(actor_4_tuple, action_kind, grant_or_assignment_id, before_state, after_state, occurred_at, reason_code, attestation_id)`. Hash-chain per DL-16 inv 38. REJECTED: silent permission changes (admin updates a permission group atom grant without audit row).
