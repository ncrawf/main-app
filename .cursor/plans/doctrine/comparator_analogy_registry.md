...
| **Palantir (Foundry / AIP; Karp "we own the ontology")** | **TAKE:** executable objects / relationships / actions / dynamic-security — ontology as an operating layer (a real Lens-B borrow; C3.8 G2/G4). **REJECT:** proprietary ontology gravity · semantic lock-in · the claim that controlling the platform *representation* equals owning the domain. **OMNI counter-position:** owning a proprietary ontology *implementation* is not owning healthcare — even an *executable* ontology is a representation of the domain, not the domain physics; it does not thereby own consent / clinical-adoption / source-authority / care-responsibility / non-action / reopening / outcome-proof, which arise from healthcare itself and stay portable. **OMNI owns the governed care loops + AI-mediated-care physics that remain true across implementations** (match≠authorize · candidate≠commit · plural commit planes · non-action · execution-failure-reopens-decision · outcome-proof). **Full posture + keeper lines: `EVRUN-2026-000004` §0.5 ② "Offensive ontology posture" → thesis §3.5.** **★ AMENDED 2026-07-23 (cross-authority/neutrality residual, Knox patch #7 — replaces any "Palantir structurally cannot" reading): Palantir is NOT structurally unable to represent/operate plural-authority healthcare networks (customer-owned data · granular authority · multi-org · governed actions · consortium deployments are all technically supportable, and it may be hired by a neutral network). Its DEFAULT buyer, deployment, and value-capture gravity are institution-centric, customer-specific, config-heavy. OMNI's residual = making patient authority · professional authority · operator alpha · portability · non-action · reciprocal custody acceptance · longitudinal consequence continuity the CONSTITUTIONAL DEFAULTS of a care network — not custom policy inside one institution's platform. Falsifier: a Palantir-powered consortium builds a broadly-trusted operator-neutral patient-authority-first cross-institutional care network relied on without surrendering authority. Full: `v4_C4_residual_moat_and_network_formation_doctrine.md` §9 (FSG-09).** | C3.8 `G1b`/`G2`/`G4`; `EVRUN-2026-000004` §0.5 ② + §3 M6 + §9.10.1 D9 (named 2026-07-11); cross-authority residual amendment 2026-07-23 |
...
| **Claude / GPT(-4) / Gemini / OpenAI / Anthropic / Google** | Model registry; surface capabilities not models; `cross_org_ai_model_partner` (§12.8). **★ RECLASSIFIED/EXPANDED 2026-07-23 (Knox patch #12): beyond model supply, OpenAI (Responses API · integrated tools · MCP · hosted computer environments · sandboxes · durable skills · tracing · Agents SDK/orchestration) and Anthropic (MCP · dynamic tool discovery · long-running harnesses · managed agents · containment) are building the HORIZONTAL AGENT SUBSTRATE — models/agent-loops/tool-protocols/sandboxes/orchestration becoming capable + increasingly interchangeable infra. OMNI posture: use aggressively; count as REPLACEABLE supply, never the moat; architect for supplier SUBSTITUTABILITY but operate as though models/harnesses are NON-FUNGIBLE (route by evaluated capability/safety/latency/cost/modality/risk/version). See `v4_C4_residual_moat_and_network_formation_doctrine.md` §8 (FSG-08) + bet-memo §16 AI Alpha Firewall + `FWREG-010`/`FWREG-013`.** | thesis §12.8; horizontal-agent-substrate reclassification 2026-07-23 |
