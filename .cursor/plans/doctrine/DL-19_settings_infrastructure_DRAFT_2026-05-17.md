...
30. **Provider routing policy substrate cluster (Amendment F binding addition per Day 0 Scheduling Rule Matrix Round 2 Domain 2 BC-09 expansion + Round 2.6 dual-target guardrail).** Provider eligibility (per Domain 2 BC-08 — staff_service_assignment ∩ capability ∩ provider_license) returns the SET of qualified-and-available providers. When the set has > 1 member, OMNI needs deterministic policy for: (a) **continuity routing** — patients who have seen a provider before get "rebook with X" by default; (b) **new-lead / "any provider" routing** — tenant-controlled strategy for distributing leads across qualified providers (round-robin / first-available / weighted-pool / priority-pool-cascade / hybrid / manual-staff-mediated). Day 0 deterministic rules + weights + thresholds; AI-driven optimization explicitly DEFERRED to M3-6+ (per Knox 2026-05-17 framing — "Day 0 should not pretend to be magic"). Patient continuity is DERIVED from `encounter_line.provider_id` + `appointment_item.planned_provider_id` history (no new continuity_link substrate; queries via service_id + patient_id history).

**Dual routing target (binding cross-domain guardrail per Knox/chat 2026-05-17 Round 2.6 review):** The provider routing policy substrate cluster (provider_routing_policy / provider_pool / provider_pool_membership / provider_routing_state) is **REUSABLE across two routing targets**, NOT scoped only to scheduled appointment composer:

- **Target 1 — Scheduled appointment routing** (Domain 2 territory): used by booking composer (BC-08 / BC-09 / BC-09b / BC-09c) when a patient is creating an `appointment` row with scheduled time. Result: selected provider populated on `appointment_item.planned_provider_id` per DL-20 inv 34.
- **Target 2 — Async provider queue routing** (Domain 5 territory; substrate detail deferred): used when a patient completes intake / submits async request that needs provider review WITHOUT creating a scheduled appointment. Examples: Hims-style GLP-1 intake → provider review queue → full-time provider first, then 1099 overflow per Knox's GLP-1 pattern. Result: clinical work item / async encounter / provider queue assignment per Domain 5 substrate (NOT scheduled appointment).

**Binding rule (Round 2.6 guardrail):** appointment lifecycle (DL-15 inv 5 + amendments + DL-20 inv 33) owns ONLY scheduled appointment state. Async-only intake review MUST NOT auto-create a fake `appointment` row to fit the routing pattern. The routing POLICY is reusable; the routing TARGET substrate is different. Domain 5 (encounter creation rules) will own the async target substrate (likely an `async_clinical_work_item` or `provider_review_queue_assignment` substrate referencing this routing policy). Until Domain 5 lands, Round 2 + 2.5 substrate handles Target 1 only.

**Anti-pattern (Round 2.6 explicit rejection):** Forcing async-only Hims-style intake into `appointment.status = scheduled` to access provider routing is rejected. The substrate is REUSABLE; the target is NOT collapsed into appointment lifecycle. Cross-link to Domain 5 (encounter creation) when authored.
