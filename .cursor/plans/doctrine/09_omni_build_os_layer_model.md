...
**Unit mapping and change-after-review are part of that property.** **Every semantic change in a review object must map to an acceptance unit or to explicitly declared shared support**, and the tooling must be able to detect **unassigned changes · conflicting unit claims · units changed after their review · and landing bytes that differ from the accepted unit revision.** *Requirement only — no manifest, schema or identity scheme is designed here.*
