...
1. **Human-visible thread title** (what Nick types; optimized for scanning work):
   ...
   The visible title is **work-first and seat-free by default** — do **not** put `KNOX` / `OPUS` / `GEM` in it while each platform already implies the seat (ChatGPT ⇒ Knox, Cursor ⇒ Opus, etc.). Counterpart threads for the same work across ChatGPT and Cursor normally carry the **same visible title**, so Nick matches relay targets by reading the work. If several seats later share one workspace (e.g. Knox pulled into Cursor as read-only), the visible seat may be restored **there** as a conditional disambiguator. The visible title is not canonical agent identity.
