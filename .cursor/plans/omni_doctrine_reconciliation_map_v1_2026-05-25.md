...
## §0 Status Posture — PHASE G.2.d EXECUTED (`D0THES-DEC-024`); cluster C11 `model_version_of_record` adoption-time floor landed in DL-14 inv 10 (LOCKED INLINE DOCTRINE — first G.2 batch to materially refresh an audited operational canonical home, not a DRAFT-home DL): AI-originated `clinical_adoption`/`recordClinicalAssertion` confirmation now REJECTED unless it carries the full model_version_of_record tuple (ai_capability + ai_model_id + ai_model_version + capability_envelope_id + model_policy_version + ai_run_id + model_output_artifact_id); substrate-event half of T0-16(c) (settings-plane half was DL-19 inv 16 G.2.b). Honesty: emission-side lineage was ALREADY enforced (§1N inv 10 + §1P.19; Q7) — not re-litigated/padded; only the genuine adoption-time gap landed. Closed REV-043; REV-044 (cross-operator transmittal) stays open for G.3. **STOPPED for Nick/Knox checkpoint before G.2.e.** BEFORE/AFTER substrate-behavior table is the standing opening artifact for every G.2 batch (Nick confirmed). [Prior: G.2.c DL-21 + operator-neutrality (`D0THES-DEC-023`); G.2.b DL-19 + T0-16 floors + universal catalog (`D0THES-DEC-022`); G.2.a DL-18 + C4 (`D0THES-DEC-021`); G.1 Tier 0 → 13; F.3 ratified §20; F.2 closed §19.] G.2 batched G.2.a-f, per-batch checkpoint.
