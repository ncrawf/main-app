...
**v4 carry-forward operator-read inputs (fold in at v4 authoring; `analysis_nonbinding`, reconcile at the v4 ledger, bind nothing until promoted `GRD-036`):** two operator-read + Knox-deliberation captures staged after the v3 spine — pull these into the v4 source stack alongside #5. Catalogued + tagged `work_package = v4_thesis_source_stack`.
...
- `audits/2026-06-08_website_surfaces_public_content_operator_read.md` — website = the **Public/content surface KIND** (`OMNI_Surface_Map_vNext.md` §3; un-contracted, `REV-175`); surfaces own NO truth (`D0THES-DEC-033`). Routes: §3 (P5) · §7 · §13 · §21 · `REV-175`.
