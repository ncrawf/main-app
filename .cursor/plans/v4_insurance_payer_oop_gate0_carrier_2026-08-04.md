...
| **Status** | `gate_0_complete · review_ready_pending_integrator · analysis_nonbinding · not_promoted · no_payer_domain_minted · no_connector_as_architecture · D6_not_edited · card_3_floor_correction_returned` |
...
| # | Question | Verdict |
|---|---|---|
| **1** | **Estate sufficiency** — is the existing OMNI source estate sufficient to *frame* the problem? | **YES for framing, NO for decomposition.** Sufficient: the constitutional posture is *already accepted*, not missing (§D.1). Insufficient: no object anywhere in the estate can hold a **bilaterally contested fact** (§F.2 / Gap **G1**), and the two shipped coverage tables are a **second source of coverage truth** (§D.4 / Gap **G2**). |
| **2** | **Source-floor sufficiency** — was Card 3's floor adequate? | **NO — materially incomplete in three ways that change the result.** (a) It omits the **shipped** insurance code, so a lane obeying only the floor would report insurance "architecturally untouched" while `payer_eligibility_documents` + `patient_insurance_details` are live. (b) It omits **C4.3** (correction-continuity Law 10.1 + O1–O22), which is the exact oracle set retroactive coverage termination needs. (c) It names the pharmacy handoff **without naming the obligation inside it** — C4.6 designated Insurance as *the deliberate falsifier of its own rubric*. Full correction: §A.3. |
| **3** | **Problem decomposition** — one coherent arc or several linked problems? | **SEVERAL — five write-authority zones, not one arc.** The boundary that holds is **who may write**, not what the subject is. Zones: external assertions we mirror · bilaterally authored/contested · our assertions frozen on transmission · our versioned derived interpretations · our immutable communicative acts. Cutting on subject nouns ("eligibility", "claims", "billing") produces leaking boundaries because each contains both our assertion and someone else's, and those have **opposite correction semantics**. §C. |
| **4** | **Architectural home** — is D6 the principal home, one participating owner, or misrouted? | **D6 is correctly named and materially insufficient as "the home." It is the money/entitlement/patient-responsibility anchor — one participating owner of roughly a third of the problem.** Not misrouted; under-scoped. Three things D6 structurally cannot own are proven at §E.3. |
| **5** | **Domain decision** — new domain needed, contraindicated, or genuinely open? | **CONTRAINDICATED, and the estate's existing prohibition is correct for a stronger reason than it states.** `FWREG-017` forbids minting a payer domain on sequencing grounds. The load-bearing reason is `D0THES-GRD-026` plus C4.6's *counterparty-noun ≠ lifecycle*: "payer" names a **counterparty acting in several distinct capacities**, not a truth-owner. §E.4. **One genuinely open ownership question remains** (Gap **G1**, bilateral participation/rate truth) — open as to *owner*, not as to *whether a payer domain should exist*. |
| **6** | **Constitutional posture** — which care-vs-financing separations must enter the spine? | **Most are already accepted and need carrying forward, not deciding.** The Care Operating Model already names the payer as a non-fungible commitment plane that "commits coverage" and "does not determine clinical indication," and already models `appropriate-but-not-covered` as commercially — not clinically — inadmissible. **Three separations are genuinely new** and must be added: money-moved ≠ economically-final; counterparty-confirmed as a distinct verification gate; estimate-as-speech-act. §D.1 / §D.2. |
...
| **9** | **Task-D readiness** | **`READY_WITH_NAMED_LIMITS`.** Five limits, stated at §L.0. |
| **10** | **Task-D reliance** | Fourteen claims Task-D may rely on. §L.1. |
| **11** | **Task-D falsification** | Nine hypotheses Task-D must be free to defeat, including this carrier's central zone model. §L.3. |
| **12** | **Next gate** | **A bounded two-gate insurance arc, NOT a multi-gate program, and NOT immediately.** G1 = Coverage & Financing Ownership Decomposition (resolve Gaps G1–G3 only). G2 = deferred behind C3.9, which is the natural falsifier. Plus **one small correction that should not wait for either**. §K. |
| **13** | **2030/2035 posture** | Six things must be preserved now or the future is foreclosed; the sharpest is that **coverage must be person-scoped, not tenant-scoped, from the start** — that one is structurally irreversible later. §J.5. |

**One-sentence summary.** Insurance is not an untouched domain and not a Commerce feature: OMNI's *constitutional* posture toward payers is already accepted inside Care, its *counterparty* machinery is already accepted inside the C4.6 pharmacy rubric, and its *capture* layer is already shipped — so the real Gate-0 finding is three specific structural gaps (bilaterally contested truth, a duplicated coverage-truth source, and an absent fourth verification gate), not a missing domain.

---

## §1 — SOURCE POSTURE (contract element 2 — stated honestly)

**Read fully** (opened and read end-to-end at the pinned base):
`D6_commerce_contract.md` · `v4_C4_2_enterprise_full_stack_adversarial_pressure_test.md` (frozen Task-D method) · `v4_C4_2A_task_d_interim_closeout_and_pause.md` · `v4_pre_spine_sufficiency_and_task_d_reentry_map_2026-08-04.md` §7/§7.1 + Card 3 + the common lane output contract + the source-floor aperture clause · the exact rows `D0THES-REV-159`, `D0THES-REV-160`, `D0THES-REV-185`, `D0THES-REV-187`, `FWREG-017`, `FWREG-018` · the passports + §3 ownership boundaries + §4/§5 object tables of `identity_contract.md`, `federation_contract.md`, `D7_documents_consent_media_contract.md`, `rbac_authority_contract.md` · the shipped DDL for `payer_eligibility_documents`, `patient_insurance_details`, `eligibility_decisions`.
...
**NOT inspected** (named so the omission is not silent): `v4_C4_2B` and `v4_C4_2C` in full (Task-D interim bodies; I relied on C4.2A's terminus, which is the designated status/routing surface — **this is a real limit, disclosed at §L.0 L4**) · C4.3 in full · the C3.5 `HCASE` library · the C3.6/C3.7 arc bodies · `EVRUN-2026-000012` bodies · the full legacy three-layer map (1,000+ insurance-vocabulary hits; I read §1O.4.2 and the sibling/appendix lines, not the whole file) · Mindbody commerce raws · the majority of `docs/architecture/` evolution volumes.
...
| **Build** | **PARTIAL — capture and routing only, and better-shaped than the documentation implies.** Two shipped tables + intake questions + a routing RPC. **No** claims, adjudication, eligibility inquiry, remittance, prior-auth, appeal, COB or clearinghouse code exists in the searched tree. | §D.4, verified DDL |
...
| `.cursor/plans/v4_C4_care_operating_model_capture.md` | `9a641a0e` | **C** | cross-cutting capture · nonbinding | **REVIEW-DRAFT**, not closed | **The accepted constitutional posture** (§D.1) |
...
| `supabase/migrations/20260508120000_phase_4d_artifact_pipeline.sql` | `228b3ac9` | **F** (DDL) | migration · **shipped** | in production | `payer_eligibility_documents` |
| `supabase/migrations/20260504120000_intake_foundation_v1.sql` | `711686ea` | **F** (DDL) | migration · **shipped** | in production | `patient_insurance_details`; `eligibility_decisions` |
| `lib/intake/question-bank/universal/insurance_payment_readiness.ts` | `c01fbc2a` | **S** | code · **shipped** | in production | Intake insurance capture |
| `lib/intake/write/insurance_details.ts` | `8a86f0c8` | **S** | code · **shipped** | in production | Write path to `patient_insurance_details` |
...
1. **The shipped insurance code is omitted.** `payer_eligibility_documents` (`228b3ac9`), `patient_insurance_details` (`711686ea`), `insurance_payment_readiness.ts` (`c01fbc2a`), `insurance_details.ts` (`8a86f0c8`). Card 3 routes to D6 §12's *deferral* while live production tables already hold coverage facts. A floor-only lane would report "architecturally untouched" and miss both Gap **G2** (duplicated coverage truth) and the fact that the cheapest correction available is already sitting in shipped DDL. **This is the single most consequential omission.**
...
| **Subscriber / member** | the coverage relationship | nothing clinical | **Partially shipped** (`subscriber_name`, `subscriber_dob`, `relationship_to_subscriber`) | Handle-vs-person discipline not applied to `member_id` (§D.4) |
...
So the decomposition is by **write authority**. Five zones. This is the carrier's central proposal and the thing Task-D should attack hardest.
...
| **B — bilateral agreements** | jointly authored, routinely contested | two positions held simultaneously; disagreement is representable | **NO — Gap G1. The genuine hole.** |
...
| **D — our derived interpretations** | us, continuously | versioned, pinnable, reproducible from pinned inputs | **YES, and shipped** — `eligibility_decisions` (`rule_version` + `inputs_hash` + `input_snapshot`) |
...
**Four of six zones already have accepted OMNI patterns.** That is the strongest single finding in this carrier and it is why the answer is *not* a new domain: insurance is largely a **new instance of accepted machinery**, plus two real holes.
...
**A card is a photograph of a plastic object that may be three plan-years stale.** Any design where step 1 writes into step 4's *verified* state is already broken — and §D.4 shows the shipped `eligibility_status` enum is one field away from exactly that.

### §C.4 — THE FOURTH GATE (a genuine, named extension of accepted D7 doctrine)
...
The insurance card needs a **fourth**, and it is not any of the three:

4. **counterparty confirmation** — *did the authority that owns this fact confirm it, as of when, and through which exchange?*

It is not extraction fidelity (a perfectly extracted member ID can be void). It is not clinical adoption (a clinician adopting a coverage belief would be a category error, and a dangerous one). And it has a property none of D7's three gates have: **it can be retroactively falsified.** Coverage terminated with a backdated effective date converts a correctly-verified, correctly-adjudicated, correctly-paid claim into an overpayment — with recoupment windows documented at 18–24 months in state law. A gate that can be un-passed by a counterparty months later is a different kind of gate, and it needs an as-of, a source authority, an exchange reference, and a revocation path.

**This is the cleanest new-architecture finding in the carrier: one new gate class, extending an accepted three-gate model, rather than a new domain.**

### §C.5 — THE SIXTH RUNG (extending the accepted Rx ladder)
...
| **6 — NEW** | *nothing* | **money moved ≠ economically final** | recoupment, offsets against unrelated claims, post-payment audit, retroactive eligibility |

**Rung 6 is a genuine extension the pharmacy rubric does not have**, because a dispensed prescription is not clawed back eighteen months later while a paid claim routinely is. A provider can hold a clean 999 and a clean 277CA and know **nothing** about coverage outcome; it can hold an 835 and still not be economically done.

**Rubric verdict (discharging the C4.6 falsifier obligation): the ladder survives insurance and gains one rung.** The adapter-family / connection-instance split also survives cleanly — a clearinghouse is an **adapter family**, a payer is a **connection instance**, and Rx L2's law that "shared software NEVER implies shared authority or uniform capability" is exactly why Availity is not a payer and why a single connector cannot resolve payer heterogeneity. **Two things break** (§F.2): Zone B bilateral disagreement, and rung 6.
...
### §D.1 — The finding that reframes the lane: the constitutional posture is ALREADY ACCEPTED
...
Two honest caveats. The Care capture is **REVIEW-DRAFT, `analysis_nonbinding`, not closed**, and its own ratification posture defers C5 promotion behind a coverage-managed validation corpus that does not exist. So "already accepted" means *accepted as direction inside a nonbinding capture*, not *contracted*. And Care's capture is a **cross-cutting capture, not a truth-owning domain** — it cannot host the substrate objects.

### §D.2 — The three genuinely new separations

| # | Separation | Why it is not already covered |
|---|---|---|
| **N1** | **money-moved ≠ economically-final** | Rung 6 (§C.5). D6's money vocabulary terminates at `captured` / `refund_*` / `write_off` and has **no state for "settled but reversible for 24 months."** Nothing in the estate models an offset — money that never moves but changes two balances. |
| **N2** | **counterparty-confirmed** as a fourth verification gate | §C.4. D7 insists verification is not one word and names three gates; none is "an external authority confirmed this, as of T, and may retroactively un-confirm it." |
| **N3** | **estimate-as-speech-act** | §C.6 L6 / Gap G7. D7 owns the *artifact*; nothing owns the immutable fact that **we told this person this number on this date, pinned to these input versions.** If estimates are recomputed on read, the evidence is destroyed — and this is the single most patient-visible failure in US healthcare finance. |
...
|---|---|---|---|
| **"claim"** | **clinical assertion** — `patient_clinical_assertions`, whose shipped comment reads "Canonical **clinical claim** ledger" | **payer claim** — the 837 submission | **`claim_lifecycle`** — C3.5-F2 candidate primitive P20 (RCM sense) |
| **"eligibility"** | **clinical/pathway suitability** — shipped `eligibility_decisions` (`pathway_code`, `rule_id`, `rule_version`, result `eligible`/`review_required`/`blocked`) | **insurance coverage verification** — shipped `payer_eligibility_documents.eligibility_status` | **X12 270/271** coverage-in-force inquiry |
...
Two tables are shipped and in production. The documentation says insurance is deferred; the database says capture is live. Both are true, and the gap between them is where the finding is.
...
**Gap G2 — a second source of coverage truth.** Both tables hold `member_id` and `group_id`; `patient_insurance_details.carrier_name` and `payer_eligibility_documents.payer_name` are the same fact in two places; **there is no foreign key or linkage between them.** D6 §8.6 forbids a second source of *price*; the same principle plainly forbids a second source of *coverage*, and the estate has one in production right now. This is the most concrete, cheapest-to-fix finding in the carrier.

**A shipped positive worth reusing.** `eligibility_decisions` carries `rule_id`, `rule_version`, `inputs_hash`, `input_snapshot`, `decided_at`, `decided_by ∈ {rule_engine, provider_override, system_derived}`. That is a **versioned, reproducible, input-pinned determination** — exactly the Zone D shape benefit interpretation needs. **OMNI already ships the pattern; benefit interpretation should follow it rather than invent one.**
...
|---|---|---|---|
| Card artifact custody | **D7** | canonical, drafted; **shipped** | **SUFFICIENT** |
| Card extraction | **Observation** (extraction-fidelity gate) | canonical, drafted | **SUFFICIENT in principle**, unwired |
| Coverage identity + assertion (L1) | *nobody* — split across two shipped tables | **ABSENT** | **INSUFFICIENT — Gap G2** |
...
| Network participation + rates (L4) | *nobody*; Federation holds `provider_credentialing`, **not** payer participation | ABSENT | **INSUFFICIENT — Gap G1, the genuine hole** |
...
| Money + allocation (L10) | **D6** | canonical, drafted; partially shipped | **MOSTLY SUFFICIENT** — needs offset, refund-to-payer, multi-party allocation (§E.2) |
...
**Seven of fifteen have an existing accepted OMNI pattern that has simply never been pointed at insurance.** That is the shape of the work: mostly application, some extension, two genuine holes.
...
**But — and this is the part FWREG-017 does not contemplate — "no payer domain" is not the same as "D6 owns everything."** Gap G1 is a genuinely open *ownership* question: bilateral participation/rate truth has no owner, and the three plausible homes (Federation extension, a new bilateral-agreement owner, or D6 with an alien mechanic) are all defensible. That question belongs at G1 (§K), not here, and not to a payer domain.

### §E.5 — Verdict

**D6 is a correctly-identified, materially insufficient participating owner** — the money / entitlement / patient-responsibility anchor, roughly a third of the problem. `REV-159` should **not** close by expanding D6 to cover coverage, participation and coding; it should close by **narrowing** to what D6 genuinely owns and **naming the neighbours** for the rest. **Nothing here reopens or edits D6.**
...
- **The rung ladder** — survives and gains rung 6 (§C.5).
...
- **Break 1 — Zone B.** The rubric assumes one sovereign counterparty executing in its own lifecycle while OMNI composes a profile over the exchange. Network participation is not like that: it is **jointly authored and routinely contested**, and no rung, adapter family or profile can represent "we say in-network, they say not." **Gap G1.**
- **Break 2 — rung 6.** A dispensed prescription is not clawed back eighteen months later. A paid claim is. The ladder's top rung assumed acceptance was the end of the authority question; under insurance, money is not final.
...
| **C — Blind decomposer** | decompose from first principles; **explicitly forbidden from reading the repository or seeing the D6 hypothesis** | fully blind | **Adopted as the carrier's spine.** The five-zone write-authority model, the fourth-gate reasoning and the five-machines-not-one-case finding are its contributions. **It independently reached the "several owners, not one, and not a payer domain" conclusion without ever seeing FWREG-017** — genuine corroboration, and the strongest evidence the decomposition is not motivated reasoning. |
...
| **S9** | Primary + secondary (COB) | **No COB determination transaction exists**; ordering is data, not code; the shipped schema cannot represent it. |
...
Deliberately **not** copied from another arc's pattern. C4.4 needed five gates; C4.6 needed L2 depth; C4.5 needed a micro-pass. Insurance needs something different because most of its constitutional work is already done and most of its remaining work is *application of accepted patterns*, with two genuine holes.

### §K.1 — Recommended: a bounded TWO-gate arc, plus one immediate correction

**IMMEDIATE — `INS-CORR-COVTRUTH` (does not wait for either gate).** A bounded correction of the **shipped duplicated coverage-truth source** (Gap G2, §D.4). Runtime/implementation-lane scope: reconcile `patient_insurance_details` and `payer_eligibility_documents` to a single coverage identity; separate the four concerns fused in `eligibility_status`; replace `is_active boolean` with the effective-dated assertion the same migration already gives clinical assertions and identity verifications; record verification **source authority** and **as-of**, not just a staff user. Justified independently of the arc because it is live production code, the fix is cheap now and expensive after data accumulates, and it is a **two-way door**. **Requires Build Entry admission** — this carrier does not authorize it.

**GATE 1 — `INS-G1-OWNERSHIP` (Coverage & Financing Ownership Decomposition).** Bounded to resolving exactly three things and nothing else:
- **G1** — who owns bilaterally contested participation and rate truth (Federation extension · a new bilateral-agreement owner · D6 with an alien mechanic). **This is the only genuinely open ownership question and the one thing a payer domain would have been for.**
- **N2** — admit the **fourth verification gate** (counterparty-confirmed, with as-of, source authority, exchange reference, revocation path) as an extension of D7's accepted three-gate model.
- **N1/N3** — rung 6 (money-moved ≠ economically-final) and the estimate-as-speech-act owner (G7).

Explicit non-goals: no claims architecture, no connector design, no coding ownership, no schema, no C5, no payer domain, no vertical arcs. Consumes this carrier plus C4.3's oracle set. **Should NOT run before the immediate correction is at least scoped**, because the shipped shape is evidence about what the right decomposition is.

**GATE 2 — `INS-G2-COMPOSITION`, deferred behind C3.9.** C3.9 (plastics/medspa mixed financing, multi-site, partitioned vs non-partitioned) is the estate's **already-designated falsifier** for exactly this territory and is `shell_pending_population`. Running G2 before C3.9 would mean scoring a composition against fixtures that do not exist. **Sequence G2 after C3.9 and let C3.9 try to break G1's answer.**

### §K.2 — Explicitly NOT recommended, with reasons

| Not recommended | Why |
|---|---|
| A five-gate insurance program | Most constitutional work is already accepted (§D.1); a five-gate program would re-derive it and would exceed what Task-D needs. |
...
### §L.0 — Readiness verdict and its five named limits

**`READY_WITH_NAMED_LIMITS`.**

Ready because: the problem is framed (Verdict 1); the constitutional posture is recovered and shown to be largely already accepted (§D.1); the routing hypothesis is tested rather than assumed (§E); implementation state is verified against code rather than documentation (§D.4); the external reality is sourced to primaries with an explicit unverified register (§F); sixteen fixture families with distinct physics are named (§H); and the inherited C4.6 falsifier obligation is discharged (§F.2).
...
- **L2 — Gap G1 has no owner.** Task-D must treat bilateral participation/rate truth as **unowned**, not as tacitly D6's. A fixture requiring it (S4, S12, S16) will expose an OMNI-side gap — and per the frozen method's own rule, **that is a `NOT_READY`/named-reconciliation signal against OMNI, not against a competitor.**
- **L3 — A second source of coverage truth is live in production** (§D.4). Task-D must score OMNI's `execution_maturity` here honestly: capture ships, and it ships with a duplication defect.
...
1. **The care/financing constitutional separation is already accepted inside the Care Operating Model**, not missing — with the payer as a named non-fungible commitment plane that commits coverage and does not determine clinical indication, `appropriate-but-not-covered` as a distinct commercially-not-clinically-inadmissible state, insurer routing as an operational reroute, financial transaction-authorization as a distinct consent family, and "ten payer bots = one payer principal." (§D.1, exact passages cited.)
...
5. **The Rx rung 1–5 ACK ladder survives insurance and gains rung 6**: money-moved ≠ economically-final. (§C.5.)
...
7. **D7's three verification gates are insufficient; a fourth — counterparty-confirmed — is required**, and it is the only gate that can be **retroactively falsified**. (§C.4.)
8. **Three genuinely new separations** must enter the spine: N1 rung 6, N2 the fourth gate, N3 estimate-as-speech-act. (§D.2.)
9. **Implementation is capture-and-routing only, and carries a duplicated coverage-truth defect** (G2), verified against DDL. No EDI, claims, adjudication, remittance, prior-auth, appeal or COB code exists in the searched tree. (§D.4.)
10. **`eligibility_decisions` is a shipped, versioned, input-pinned, reproducible determination pattern** that benefit interpretation should follow rather than reinvent. (§D.4.)
...
- **F3 — That the fourth gate is needed.** If counterparty confirmation is representable as an Observation-class extraction with provenance, N2 dissolves.
- **F4 — That rung 6 is new.** If D6's `dispute_open`/`dispute_lost` plus additive adjustment already covers recoupment and offset, N1 dissolves.
- **F5 — The re-scoped wedge.** If a fixture shows "everything is tracked, exceptions surfaced early" is not purchasable, §I fails.
- **F6 — That the constitutional posture is already accepted.** The Care capture is REVIEW-DRAFT and nonbinding; if that is too weak to inherit, Verdict 6 must widen and more spine work is owed.
- **F7 — The gate recommendation.** If Task-D needs more than G1 resolves, §K is under-scoped; if it needs less, over-scoped.
- **F8 — That no payer domain is needed.** If G1's resolution requires a truth-owner that is genuinely payer-shaped, revisit — with `GRD-026` and the C4.6 precedent as the burden to overcome.
- **F9 — Card-3 floor correction.** If the three omissions turn out immaterial, §A.3 is over-stated.
...
22. **Task-D must receive a positive-readiness answer** — *rejected. `READY_WITH_NAMED_LIMITS` is given because the evidence supports it, and `NOT_READY` was a live available outcome throughout.*
...
**Catalog** (`01_master_corpus_catalog.md`) — one proposed row: this carrier; category `analysis` / gate-0 carrier; authority `analysis_nonbinding`; status `gate_0_complete · review_ready_pending_integrator · not_promoted`; domains per passport; review gate `user_knox_required`.

**Read graph** (`04_manifest_read_graph.md`) — one proposed Tier-2 route: *"Insurance / payer / coverage / out-of-pocket / mixed financing"* → this carrier → then D6 §12 + `REV-159`/`REV-160` + `FWREG-017`. Triggers: `read_before_insurance_payer_work`, `read_before_commerce_work`, `read_before_task_d_close`, `read_before_spine_authoring`, `read_before_c5_contract_authoring`. **Placement note:** the route should say the carrier is read **before** D6 §12, because a reader who starts at D6's deferral will conclude insurance is untouched and miss the shipped state.

**Decision ledger** (`03_decision_extraction_ledger.md`) — four proposed rows: **(1)** insurance decomposes by write authority into five zones, four with existing accepted OMNI patterns; **(2)** D6 = participating owner, not the home; no payer domain, on `GRD-026` + counterparty-noun grounds; **(3)** the fourth verification gate (counterparty-confirmed) extends D7's accepted three-gate model; **(4)** ACK-ladder rung 6 — money-moved ≠ economically-final — extends the accepted C4.6 ladder.
...
**Open review** (`08_open_review_queue.md`) — **`REV-159`: update disposition, do NOT close.** Record that Gate-0 ran, that D6 remains the money/entitlement anchor, that the closure condition should be **narrowed** (D6 gains offset · refund-to-payer · multi-party allocation · generalized entitlement trigger · rung 6) rather than expanded to coverage/participation/coding, and that a *new* open row is owed for Gap G1. **`REV-160`:** unchanged; note the financing-vs-`payment_method` question is now also a *substitution-of-the-obligated-party* question (§C.6 L12). **`REV-185`:** note that payer-facing economics strengthen the case for structural rather than policy enforcement, and that U10 (clinician financial visibility) belongs here. **`REV-187`:** note the coverage-analysis grid (sponsor XOR insurer) is a **worked precedent** for dual-financing allocation and should be consumed by G1, not re-derived. **Proposed new row — `REV-NNN` (number assigned by the integrator): bilateral participation + contracted-rate truth has no owner (Gap G1)** — candidate homes Federation extension / new bilateral-agreement owner / D6 with an alien mechanic; trigger `INS-G1-OWNERSHIP`.

**Future Work Registry** (`future_work_registry.md`) — **`FWREG-017`: update, do not close.** Gate-0 discharged; the four named invariants are **already accepted inside Care** and need carrying forward rather than deciding; three genuinely new separations added (N1–N3); next trigger becomes `INS-G1-OWNERSHIP`; the immediate shipped-code correction is noted as independently justified. **Proposed new rows:** `INS-CORR-COVTRUTH` (implementation-lane correction, Build-Entry gated) and `INS-G1-OWNERSHIP` (bounded Gate 1), with `INS-G2-COMPOSITION` sequenced **after C3.9**.

**Supersession / conflict ledger** (`05_supersession_conflict_ledger.md`) — **one proposed conflict row, not a supersession:** the shipped duplicated coverage-truth source (G2) versus the single-owner principle and D6 §8.6's no-second-source rule. Nothing in the estate is superseded by this carrier.

**Card-3 floor correction** — routed to the **integrator and the map owner** (§A.3), not landed here. If the envelope is amended for a successor lane, the three material omissions should be added to its floor.
...
| Review mode | full-document / acceptance review by Nick + Knox, then integrator routing |
...
- **No closure was manufactured.** `READY_WITH_NAMED_LIMITS` carries five limits; nine falsifiable claims are handed forward; fourteen items are left unresolved; `NOT_READY` was a live outcome throughout.
