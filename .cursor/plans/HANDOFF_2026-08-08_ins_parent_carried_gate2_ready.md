...
Status: **★ THE STATUS STRING BELOW IS HISTORICAL — it describes the pre-landing moment and is NOT current state.** `[HISTORICAL as of 2026-08-08 PM] insurance_parent_assembled_on_branch · E1_failed_2026-08-08_corrected_pending_rerun · main_unlanded · gate2_prepared_not_started · not_promoted`. **This file owns no live state and therefore does not restate it** (`D0CKPT-GRD-007`): resolve program state from the **2026-08-03 checkpoint**, and Gate-2 execution + acceptance state from the **Gate-2 brief's state table**.
...
successor_pointer:          Gate 2 · outside Phase A · state owned by the Gate-2 brief
                            (this file restates no Gate-2 state — D0CKPT-GRD-007)
```
...
**★ THIS SECTION NO LONGER STATES A NEXT ACTION (`D0CKPT-DEC-009` · `D0CKPT-GRD-007`).** This file is subordinate Insurance continuity and owns **no** live program state, so it must not carry one. **Resolve the current operator-selected focus and next allowed action from the 2026-08-03 checkpoint; resolve Gate-2 execution and acceptance state from the Gate-2 brief's state table.**

*(Superseded, retained for lineage: "**NONE auto-starts, and it is NOT Gate 2.**" That sentence, mirrored here from the checkpoint, is the recorded origin of `D0CKPT-GRD-005` — it retargeted a correct cold boot away from the live arc. Its 2026-08-08 PM replacement, which restated the corrected focus and Gate-2 posture in this file, was itself a single-source violation and has been removed in favour of the pointers above.)*
...
| Owed | Where | State |
|---|---|---|
| **Gate-2 execution** | `v4_INS_G2_operating_composition_and_sufficiency_brief_2026-08-08.md` | **that brief's state table owns Gate-2 execution and acceptance state — read it there** (`D0CKPT-GRD-007`) |
