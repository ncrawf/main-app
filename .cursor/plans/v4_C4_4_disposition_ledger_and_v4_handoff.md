...
7. **scope + expiry / reconsideration trigger** (see the six scope kinds below);
...
**Six scope kinds a downstream author must keep distinct** (an override is exactly one of):
`one_time_override` (this action only) · `temporary_condition` (owner-state with an expiry/reconsideration trigger) · `continuing_local_configuration` (durable owner-state until changed) · `proposed_reusable_lesson` (an S2 candidate — not yet operative) · `adopted_local_policy` (owner-state operative via an explicit S3/Settings adoption link) · `network_promoted_knowledge` (requires a **separate** cross-principal promotion gate).
...
- **One local override never silently becomes operator-wide, platform-common, or network knowledge.** Cross-principal promotion requires the separate `network_promoted_knowledge` gate (Foundry federation profile, I.6).
...
**Worked override trace (the fixture Angle A must be able to reconstruct from this packet).** Scheduling agent proposes slot X; clinic manager overrides to slot Y because a room is out of service. → proposal record (X) + authority eval (manager holds Scheduling override capability) + actual override (Y) + reason ("room 3 out of service") + versions (scheduler policy vN, model vM) + effective=now/recorded=now + scope=`temporary_condition` (expires when room returns) → **Scheduling domain S3 commits the temporary local constraint**; **S1 records the immutable decision/run receipt** → the **next Context Router draw immediately sees the Y-constraint** (fast clock, no S2 wait) → analytics later find 15 recurrences → a `proposed_reusable_lesson` S2 candidate (`subject=operations · form=lesson · epistemic=observed · authority_class=institutional_record · admission_scope=operator_local · applicability=this clinic`) enters review → if adopted, an **explicit S3/Settings adoption link** makes it operative **for that operator only** → outcome observed (S1/S3) → reconsideration. **Cross-tenant promotion is blocked** unless the separate federation promotion gate fires. (This trace is the seam; the surrounding fan-out is I.2/I.3.)
...
4. **Workflow / process / microdecision observation** → Agent Runtime / Runtime-Ops (observation evidence is S1; a standardized interaction taxonomy is required; observation is not adoption).
...
- **NEW I.3A — Operational decision-and-override lineage seam (two-speed law), controlling front-door section:** the fast operational clock (proposal · authority eval · actual action/override · reason · context/source refs · policy/model/config versions · effective & recorded time · scope + expiry · owning-domain/Settings/governance S3 commit · immutable S1 receipt · immediate next S6 draw) vs the slow learning clock (S1 evidence → candidate → recurrence/applicability → S2 admission → optional owner-policy proposal → adoption/rejection → reconsideration); six scope kinds; four hard-law falsifiers (S2 never live owner-state; runtime memory never durable authority; one local override never silently operator-wide/platform-common/network; **no "instant memory" class**); a worked override trace. Fixes the run-1 Angle A "override not traceable from the packet" gap.
...
- **Stop declaration:** **C4.4 G5 BOUNDED POST-TEST PATCH APPLIED (I.3A two-speed seam + I.9A/B/C enterprise proof interface). RAW RUN-1 EVIDENCE PRESERVED. READY FOR FRESH A/B/C RERUN. main / G3 (`e364acb`) / G4 (`07f8662`) / Runtime / AGENTS / #15 UNCHANGED.**
...
Document identity (passport):
 ...
 status: G3_ACCEPTED · G4_PASS_30_OF_32 · G5_RUN1_HOLD_BOUNDED_POST_TEST_PATCH_APPLIED_RERUN_ARMED (analysis_nonbinding test substrate) · not_accepted · not_promoted · not_merged · outer_checkpoint_#15_UNCHANGED
