...
Status: `G1_closed · G2_closed · G3_ACCEPTED · G4_KNOX_ADJUDICATED_PASS_30_OF_32 · G5_CANDIDATE_COMMITTED_FOR_AUTHORABILITY_TEST · analysis_nonbinding · not_accepted · not_promoted · not_merged · outer_checkpoint_#15_UNCHANGED` — first draft 2026-08-01; reworked (Knox HOLD, disposition quality); naming/precision close; **REFRAMED 2026-08-01 into an integrated reference + downstream authoring packet** (Part I front door + orthogonal S2 taxonomy + Foundry deployment profiles + Simulation-Lab boundary + comparator-by-mechanism + downstream authoring contracts + authorability close criterion; routing detail in Part II); then a type+freeze patch and a candidate-commit micro-patch (Knox's 7 corrections), and **Knox has remote-byte-verified the candidate commit** on branch `analysis/c4-4-g3-reference-architecture`. **This packet is repository-durable on the C4.4 analysis branch — a test substrate only, NOT G5 acceptance.** The exact candidate commit + packet blob are supplied by the **external test-freeze integrity receipt** (not self-stamped here). Nothing accepted/promoted/merged/repointed. Next authorized action = **frozen three-angle testing (Angle A constitutional/type · Angle B comparator/future-resilience · Angle C App-E downstream-authorability) followed by Knox adjudication** — see Part I §I.10 / App-E. **The G5 integration deltas (I.2 Foundry recomposition · I.4 S2 taxonomy · I.6 profiles/interface · I.7 Simulation · I.5/I.9/I.10 pilot/contracts) are pending acceptance — not independently G4-tested; see the per-section decision-state table below.**
...
**C4.4 closure criterion (added — Knox §8):** C4.4 is **not** closure-ready merely because every row is routed (zero-unrouted). It is closure-ready only when a **fresh repo-connected agent, given this packet + its exact pointers and no chat history, can produce**: (1) spine §8 outline, (2) Source Estate C5 outline, (3) Knowledge Reservoir C5 outline, (4) Foundry Pilot build-entry outline, (5) Simulation-Lab boundary charter, (6) comparator build/buy/wrap matrix — **all six PASS (PARTIAL = HOLD), without broad corpus search, new architectural classes, ownership collapse, false build claims, or an omitted App-E §E.7(e) required concept.** The full **test specification** (read set · six output schemas · rubric · explicit kill-criteria · all-PASS rule · exact prompt template) is authored in **App-E**; the **candidate packet is now committed and Knox-remote-verified**, so the test is armed — it freezes when the three packets pin their bytes to the state-normalization commit, then runs, then Knox adjudicates.
...
**Status: SPECIFICATION — repository-durable candidate committed + Knox-remote-verified; the test is armed but NOT yet frozen (final pins supplied at freeze-time).** The candidate packet is committed to the C4.4 analysis branch and Knox has completed remote-byte verification; what remains before freeze is only that the **three test packets pin their target bytes to the state-normalization commit** (candidate commit + packet blob supplied by the external test-freeze integrity receipt, not self-stamped here). The evidence sources below are already pinned by exact blob+path. Sequence: candidate committed → Knox remote-byte verification complete → freeze the three packets against the normalization commit → run three independent read-only tests → agents do NOT self-adjudicate → Knox adjudicates → specific failures patch only the implicated section → all required outputs PASS before G5 acceptance.
...
- **Next authorized action:** **frozen three-angle testing** (Angle A constitutional/type adversary → mutation ledger · Angle B comparator/future-resilience adversary → mechanism-gap ledger · Angle C App-E downstream-authorability → six outputs) against the state-normalization commit; **agents do not self-adjudicate — Knox adjudicates**; patch only validated failures at the implicated section → G5 acceptance + bounded control-plane sync + C4.4 close. The candidate + normalization commits are `analysis_nonbinding` (NOT G5 acceptance, NOT promoted, NOT merged to main, no force).
- **Stop declaration:** **C4.4 TEST CANDIDATE STATE NORMALIZED AND PUSHED. AWAITING KNOX THREE-ANGLE FREEZE. main / G3 / G4 / Runtime / AGENTS / #15 UNCHANGED.**
...
Document identity (passport):
 ...
 status: G3_ACCEPTED · G4_PASS_30_OF_32 · G5_CANDIDATE_COMMITTED_FOR_AUTHORABILITY_TEST (analysis_nonbinding test substrate) · not_accepted · not_promoted · not_merged · outer_checkpoint_#15_UNCHANGED
