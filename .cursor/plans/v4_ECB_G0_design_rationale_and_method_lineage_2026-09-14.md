...
Manifest action: `add_tier2` — **DONE on branch `cursor/ecb-g0-reconnaissance-6a09`:** catalogued in `doctrine/01_master_corpus_catalog.md` and routed at **`9x`** in `doctrine/04_manifest_read_graph.md`, alongside the carrier's `9w`, in one change. **Not yet on `main` — default-branch publication is a separate act.** *(Historical: registration stood deferred through R30 for want of an intake mandate; that deferral is history, not current state.)* See `§6`.
...
| **Cumulative source recovery, and interruption that survives** — **the working method most at risk of being reinvented in a dumber form, so it is named exactly** | **A list of reads is not recovery.** The inherited control is FAI's **five-column live recovery, maintained DURING reading** rather than reconstructed at the end: source and exact sections · consumption, reader and depth · the native finding with its authority or proposed status · the effect on the working account · the remaining read. **Two rules carry the efficiency: *no row may terminate at "read," and no row owes an empty field.*** **Detail follows consequence, not a uniform form** — a source that merely confirms earns a compact consequence record, while one that **sharpens, contradicts, splits, relocates or exposes missing law** earns the full relational chain *(section → native mechanism → current model and authority → consequence or tension → remedy and restraint → destination → next read)*, which may live inline or in a linked synthesis rather than crushed into one cell. **Interacting findings stop accumulation and produce a synthesis checkpoint.** **A bounded read yields an attributed limited finding now, with the unread remainder preserved — read completion and reconciliation completion are different.** **A replacement resumes the recorded partial account; running out of context is a legitimate stop and an illegitimate relevance judgment** | **FAI carrier `§E15.17.8` — *Internal-estate reconciliation*** *(both rules verbatim there; ECB cites it at `§13.2` as adopted behaviour)*. **ECB's adapted workbench is `§13.2`; the developing account and its checkpoints live at `§7.4`; remaining work at `§15.4`. This is a written working procedure, not an autonomous estate-survey tool** |
| **One writer, explicit transfer, exact review inputs** | **One active writer for the maintained carrier; reviewers inspect an identified snapshot and return findings the author dispositions; replacement is explicit rather than assumed.** **A moved branch head does not by itself establish changed reviewed content** — head and content blob are different facts, and this is distinct from tracking a changed *method*. **On separately owned strands of work — "lanes":** the capability is retained without copying any prior arc's staffing. **Any additional durable contribution declares its question, owner, exact input, owned output, writable boundary, dependency and disposition path BEFORE activation**, and **concurrency alone creates no parent package, common base or integrator** — coordination is earned by a real dependency or collision, sequenced or simultaneous | ECB `§12.9.7a`; AWP `§2.1`; collaboration model `§2.6` |
