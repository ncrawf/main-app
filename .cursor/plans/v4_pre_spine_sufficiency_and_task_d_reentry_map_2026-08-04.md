...
| **Operator Economics (AP/AR/COGS/procurement/media-spend/budgets/subledger)** | Demand **Gate-0** owner-gap cluster → candidate **Track B, Gate-0 OWED** (`b191d75…`) | REQUIRED-BEFORE-TASK-D | SPINE-POSTURE-NOW | **Second real gap** (not hidden in "vendor residual"). "No canonical Operator Economics owner located in the inspected set." See §4.2. |
...
### §4.2 Operator Economics Track B + Counterparty residual — **the second gap** (Gate-0 OWED)
Accepted Demand **Gate-0** found **no canonical owner** for AP · AR · COGS · procurement · media spend · budgets · vendor invoices/settlement · accounting-subledger depth, and split a candidate **Operator Economics Track B whose Gate-0 remains OWED** (3-layer: operational truth OMNI-owned / accounting-subledger depth OPEN / statutory external near-term). This is dead-center of the "enterprise destroys us on the business backend" fear and **must not be hidden inside a vague vendor receipt**. Lane = **Operator Economics Track B Gate-0 + Counterparty Residual** (payroll/HRIS boundary; ERP/accounting boundary; statutory-external vs OMNI operational truth; warranty/recall/remedy; what Task-D tests; what stays C5). **No new domain by fiat.**
...
- **Phase B — C3.9 population** (plastics + medspa multi-site, carrying insurance/OOP coexistence), consuming Insurance + Operator-Economics findings. *(C3.9-before-Task-D = Knox recommendation pending Nick acceptance via `D0THES-DEC-039`.)*
...
**Parallel-track note:** Full **Demand Track-A Gate-1** may run independently; accepted **Gate-0** posture is the minimum Task-D input **unless Nick explicitly makes Gate-1 blocking**. Gate-1 may **not** silently mutate the critical-path sequence. **H1** is tested inside Task-D first; a separate physical-realization arc opens only if Task-D leaves a material question unresolved.
...
- **Unresolved sequencing decisions (for Nick):** (a) accept C3.9-before-Task-D; (b) confirm Demand Gate-1 is parallel/non-blocking (or make it blocking); (c) confirm H1-inside-Task-D-first.
