...
Status: `CLOSED / accepted` 2026-06-13 (Nick + Knox sign-off, relayed) — full-estate reconciliation DONE (all §2 categories accounted for incl. the surfaced Cat 9 `designs/`; supersession §6 resolved against the ledger; code/ADR lane enumerated with findings; owed governance rows in §8 done). The v4 source base is the accepted estate for C3.5/C4. (Living: the v4 Source Stack still grows as new captures land — `GRD-036`; post-2026-06-13 additions are assembled in the *Post-C2 living additions* block in §2.)
...
- [x] **Wave-4 AI corpus — `EVRUN-2026-000005`:** `ingestion/outside_learning/analysis/EVRUN-2026-000005_ai-corpus-wave-4/` — 15 sources `EVSRC-2026-000253…000267` (its own run; cumulative w/ `EVRUN-000001 §2A` + `000002` + `000003`). 0 net-new + ~28 sharpenings (Build-OS runtime/eval · Knowledge-Reservoirs · §C/P35 capability topology · safety · governance/business-model), all routed. Evidence-ledger `D0OL-EVD-004`; read-graph #6a. `analysis_nonbinding` (`GRD-036`).
...
### Post-C2 living additions (per `GRD-036` — inputs that landed AFTER the 2026-06-13 close; a currency log, not a 10th category)
> These were previously visible only in the controlling-plan running banner. Assembled here so a future v4 author loads them alongside the 9 categories and nothing hides. Each keeps its natural category home; this block is the index.
- [x] **Async-care crystallization — `EVRUN-2026-000004`** (TRT `EVSRC-2026-000251` + Dan `EVSRC-2026-000252`): §0.5 SETTLED STATE = the settled identity/boundary/resolution ontology + async-care build mandate. Home: Cat 6 above; read-graph #9a item 13. `analysis_nonbinding`.
- [x] **Wave-4 AI corpus — `EVRUN-2026-000005`** (15 sources `EVSRC-2026-000253…000267`): 0 net-new + ~28 sharpenings, all routed. Home: Cat 6 above; evidence-ledger `D0OL-EVD-004`; read-graph #6a. `analysis_nonbinding`.
- [x] **Dan/TRT longitudinal continuation — `EVRUN-2026-000007`** (`EVSRC-2026-000279` + re-read `EVSRC-2026-000251` as one case series; `analysis_closed` 2026-07-16). Multi-principal/multi-actor/agent-mediated care reframe + the marquee synthesis **`OMNI Reactor` = `candidate_spine_doctrine` (named · non-ontological · UNPROMOTED · FROZEN 2026-07-16)** — OMNI's risk-adaptive **intent-to-consequence constitution**: 8 invariants + 5-layer ownership (domain owns authoritative state · principal the duty · committer authority · actor custody · OMNI continuity+proof) + care-first preamble + neighbor boundaries + **compiled-not-deployed** build law + **OMNI firewalls authority, cannot legislate liability** + Reactor(doctrine)/plant(product)/grid(network). **Required spine PRESSURE input — route like G4 findings; candidate, NOT adopted authoring doctrine; ratify only via the `_05 §I.15` gates.** Read order: `_06` (closeout) → `_05 §I.13–§I.15` (canonical carrier) → `_02 §10` → registry. Home: Cat 6 above; read-graph #9g + #9a item 15. Also carries findings `DTP-01…23` + `STH-01/02/03` + `EVAL-279-A/B` + successor charter `_04` (`EVRUN-2026-000008`, proposed-blocked). `analysis_nonbinding`.
- [x] **Accountability Loop (GRR reframe) — `v4_C4_governed_reporting_resolution_capture.md`** (Draft 2): OMNI's inward integrity loop — custody · routing · party remedy · communication · verified closure; cross-cutting **contract candidate** (orchestration rides CNS; C5-deferred; NOT a new domain; NOT incident-management). Object model = signal → admission → typed **response record** (`response_envelope`) + optional **`response_case`**; REV-184 = LINKED care-decision object, NOT the container. Required Task-D + spine input; read-graph #9c + Task-D Fixture 5 (#9b) + watch-list WI14/WI15 + FWREG-009. CLOSEOUT-READY (provisionally-stable; the pressure-out is done — it produced the two sibling loop captures below). `analysis_nonbinding` (`GRD-036`).
- [x] **Platform Loop — `v4_C4_platform_loop_capture.md`** (produced by the 2026-07-12 GRR pressure-out): the governed loop run on the platform/capability estate = **Engineering & Validation (E&V) → Release Operations → Runtime Operations**; `09`/`10`/`11` = the ORTHOGONAL OMNI Build-OS meta-system that GOVERNS E&V (not a detail child). Platform identity graph · desired-vs-observed reconciliation (Apollo-grade) · transition/custody contracts · §SBI inherited hard-laws. Required Task-D + spine input; read-graph #9c; watch-list WI15. CLOSEOUT-READY. `analysis_nonbinding` (`GRD-036`).
- [x] **Agent Runtime & Harness — `v4_C4_agent_runtime_and_harness_capture.md`** (produced by the 2026-07-12 GRR pressure-out): the **§B AI-substrate runtime layer** (agent instantiation · budget · tools · delegation · steering · compaction · recovery · governance; **conversation = execution context, NOT canonical memory**). `active_map` — map-depth, runtime BUILD deferred. Fulfills the already-named Agent Runtime/Control-Plane Doctrine; consumes v3 §B + Build-OS + AWP + registry carriers. Required Task-D + spine input; read-graph #9d; FWREG-010; watch-list WI15. `analysis_nonbinding` (`GRD-036`).
- [x] **Care Operating Model capture — `v4_C4_care_operating_model_capture.md`** (currency/index entry added 2026-07-14, Lane-6 L6-B): the THIRD loop peer + center of gravity (Care = two interlocking Sense + Act/Fulfillment loops, CNS-coordinated; REV-184 at the Sense→Act gate; "Care never closes" rejected). **STATUS: `frozen_under_audit` · `analysis_nonbinding` · NOT authoring-authoritative.** It is the artifact **under test** by the Care Forensic Inheritance Audit (Lanes 1–6) — consult it only as the correction TARGET, never as controlling authoring prose. Home: read-graph #9e (dedicated frozen-capture route). Its authoring-input status is a **gated future pointer only** (read-graph #9a item 14): the post-forensic, Nick+Knox-accepted corrected capture — NOT the current frozen draft. Listed here so the currency log is complete; this does NOT promote it into the C2 authoring source base.
...
3. **NEW: code/ADR reality-check lane** (§4) — the single genuine addition at the 06-13 close, per Nick's scope decision.
4. **Post-C2 living additions (2026-07, `GRD-036` — distinct from the at-close deltas 1–3).** Several inputs landed after close and are assembled in the *Post-C2 living additions* block in §2: the two evidence runs (`EVRUN-2026-000004` async-care · `EVRUN-2026-000005` wave-4) + the three 2026-07-12 architecture captures (**Accountability Loop** [GRR reframe] · **Platform Loop** · **Agent Runtime & Harness**). This is the `GRD-036` "living" growth the header always allowed — not a reopening of the closed 06-13 reconciliation.
