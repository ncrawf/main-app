...
- **Free-text intake extraction emitter (per `1K.5.A` narrative-evidence fan-out (b)):** runs on `intake_response` rows with `answer_type = free-text-bounded` and `narrative_intent` declared per `1K.4`. Same pattern as the lab-derived emitter per `1L.17` and document-extracted emitter per `1O.14`: produces ai_suggested clinical assertion candidates (via `recordClinicalAssertion` per `1K.5.A`, `evidence_refs` back to the raw `intake_response`), patient-concern candidates (`patient_chart_ai_reviews`), patient-question candidates (`patient_action_items` of `provider_question_pending`), and non-clinical-segment candidates (`patient_action_items` of `ops_triage_request`). Idempotent per `(intake_response_id)`. Never auto-confirms, never satisfies authority floors, never gates `clinical_required` (deterministic safety scan per `1K.5.A` narrative-evidence fan-out (a) is the safety gate; AI is assistive secondary). Extracted clinical facts become structured assertions with `evidence_refs` back to the raw response — never a parallel narrative store.
...
**Narrative free-text intake evidence (binding; foundational; per `2026-04-30_free_text_intake_pressure_test.md`):** intake answers with `answer_type = free-text-bounded` per `1K.4` are **immutable raw evidence** stored on `intake_response` exactly like every other answer (same version-pinning, same `branch_path_token`, same append-only + supersession discipline per `1K.5`, same Mode J semantics per `1K.13` — Mode J supersession applies only to the answer row itself, never to AI-extracted derivatives). They are NOT projected into `messages` / `message_thread` (no duplicate transcript — `1G` messaging SoT is reserved for actual bidirectional conversation), and the legacy `1K.5` Fix A direct projection to `patients.*` does NOT apply (narrative does not denormalize to chart columns). The Stage 1 emitter no-op rule above is preserved (narrative never auto-mints assertions). Narrative answers route instead through **three deterministic fan-outs**, each running in the same DB transaction as the source `intake_response` INSERT and fail-closed per the Stage 1 failure semantics rule above:
