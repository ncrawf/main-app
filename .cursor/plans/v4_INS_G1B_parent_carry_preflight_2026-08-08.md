...
Status: **★ WHOLLY HISTORICAL as of 2026-08-08 PM — this is the pre-carry readiness record. The carry EXECUTED and LANDED on `main` at `849f0c1`. Every state, next-action and sequencing assertion in this file describes the pre-landing moment and is SPENT; read it as the write plan that was followed, never as current state.** Live state: the 2026-08-03 checkpoint (program) and the Gate-2 brief (Gate-2). Note in particular that this file's "next authorized lane … it is NOT Gate 2" and "held behind C3.9" wording is superseded by `D0CKPT-DEC-009` — Gate-2 construction may begin and C3.9 binds at final acceptance. `[HISTORICAL] preflight_knox_accepted · pending_nick_integrator_transfer · provisional_pending_parent_integration · carry_not_executed · integrator_role_not_assumed · shared_surfaces_untouched · gate2_not_started`
