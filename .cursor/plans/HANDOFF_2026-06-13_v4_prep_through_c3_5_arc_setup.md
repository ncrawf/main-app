...
- **Maintained AS WE GO:** catalog rows (`01`) for C2 doc + C3 note + C3.1 + C4.0 + C3.5 plan-doc + kickoff + this handoff; read-graph (`04`) Tier-2 #7 (C2) + #8 (C3.5 arc hub) + this checkpoint repoint (#15 + AGENTS, reconciled); open-review (`08`) `D0THES-REV-179..183`; supersession (`05`) consulted/cited.
- **NEW boot-freshness machinery (2026-06-13, `D0CKPT-DEC-003` / `D0CKPT-GRD-001`):** run the **Boot Freshness Check** at boot (Agent Work Protocol §1 + AGENTS Agent Boot) — confirm this handoff = the one AGENTS names = read-graph Tier-0 #15 = the controlling-plan banner; STOP if they disagree. **Checkpoint Closeout Rule** (Protocol §8): no Tier-2+ work is "closed" until handoff + AGENTS + read-graph + controlling-plan banner are repointed in the SAME closeout commit (the one claiming closure — not every intermediate commit). **Work is not complete until the next agent boots to the right state.**
