# EVRUN-2026-000008 — §00 · Run Charter + Pre-Registered Adversarial Method (OPENED at method-stage)
...
Opened 2026-07-18 (Opus, at Nick + Knox direction, quadrifecta checkpoint). **Current truthful state: `methodology_and_adversary_intake` — NOT completed evidence.** Supersedes-in-approach the EVRUN-000007 `_04` proposed charter (preserved there as historical; corrected here). Predecessor: `EVRUN-2026-000007` (`analysis_closed`; froze `OMNI Reactor` = `candidate_spine_doctrine`).
...
**RUN EXECUTED + ADJUDICATED 2026-07-18 (nonbinding).** Artifacts: `_00` (this — charter + frozen rubric `§6A`) · `_01` (prior Gemini Round-1/Round-2 reports verbatim + labels) · **`_02_omni_builder_trace_and_conformance_results.md`** (Builder submission, Opus, + executable fixture `scripts/test-consequential-transition-conformance.ts`) · **`_02_incumbent_native_adversary_submission.md`** (fresh Gemini incumbent-native, verbatim) · **`_03_knox_adjudication_and_disposition.md`** (Knox ruling). **Result: R3 — cross-authority continuity residual (design/conformance only; scores Gemini 47.0 / Opus 61.2; no external acceptance; R4 unavailable). `OMNI Reactor` unchanged — frozen · candidate · non-ontological · UNPROMOTED; no definition/invariant change; naming gate OPEN.** **Do NOT repoint the 07-12 gate checkpoint** (nested nonbinding run). Nothing promoted.

**★ METHOD-HISTORY NOTE (persistence chronology — Knox integration directive step 3; supersedes the earlier "local-only/origin stale" line, which is removed as stale):** the pre-registered rubric (`§6A`) was authored + frozen **off-repository** by the continuity Opus and committed as `60315ee` (2026-07-18 16:41) **BEFORE either fresh submission existed** — i.e. genuinely pre-registered. The Builder (Opus) produced `_02` + the fixture **blind** to the fresh Adversary (and to `_01`), and committed + pushed `60315ee` + `6b5a8f8` (Builder + fixture; fixture blob `869911493ed9a550cf5376828b68ec07a6a6d3eb`) to `origin/main` **before** the Adversary/Knox outputs were relayed to the Builder. Knox adjudicated against a **stale `origin/main` snapshot (`a296b94`)** that did not yet show the rubric-lock or the fixture, and correctly flagged the fixture as not-yet-repository-verifiable; the push has since made both present on `origin/main` (`origin/main` now ends at the integration commit that adds `_02`-adversary + `_03`). No semantic amendment was made to the rubric `§6A` at any point. `assignment_source_fidelity` remains routed to `FWREG-013` (canonical parked home; no standalone eval registry minted). Full reconciliation: `_03 §R`. The immutable submissions (`_02` Builder, `_02` Adversary) are preserved verbatim; corrections to the Adversary's external claims live in `_03 §2` (Knox audit), not by editing the submission.
