# EVRUN-2026-000011 — ai-corpus wave-6 (13 new-drop videos)

> Run opened 2026-07-18 (scaffolding stage). Per-source flow: Nick pastes §1 transcript + §3 Review 001 (Knox) + optional §3 Review 002 gut note into each wave-6 source file; Opus writes §3 Review 003 (formal deep extraction), folds cross-source synthesis into THIS run's concept registry, updates coverage matrix + anchor ledger, fills each source's §4 pointers at closeout. Method = the settled registry-first two-tier read (`00_pipeline_doctrine.md`) **+ the wave-3 two-axis reality-check** (`doctrine_status × build_status`). Cumulative with `EVRUN-2026-000001 §2A` + `000002` + `000003` + `000005` + `000006` registries.
...
**Two-axis reality-check (carried from wave-3/4/5):**
- **`doctrine_status`** — `AFFIRM` / `PARTIAL` / `ABSENT` vs thesis v3 (§0→§B) + current domain contracts + post-v3 layer (C3.5/6/7/8 · REV-184/GRR · `EVRUN-000004` §0.5 · Polaris/C4.1 · wave-5 gaps · Reactor candidate).
- **`build_status`** — `present` / `partial` / `absent` in the repo (grep/inspect `app/`, `lib/`, `components/`, `middleware.ts`, `supabase/`).
- Interesting rows = the mismatches (esp. `doctrine=AFFIRM · build=absent`, the wave-2/3/4/5 dominant pattern).
