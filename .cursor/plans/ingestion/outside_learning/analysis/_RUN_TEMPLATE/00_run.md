...
> **★ CAPPING CONTRACT (BINDING — see `00_pipeline_doctrine.md` "Capping rule" + "Semantic-fidelity closeout gate").** This run is NOT "capped/closed" until BOTH hold: **(1) fidelity** — every source has `processing_status` + `semantic_fidelity_status` with an independent second-reader sign-off (zero significant-restore / zero inversion / zero undispositioned operator-flagged insight); AND **(2) candidate resolution** — the CLOSING AGENT has classified EVERY candidate (guardrail / primitive / net-new / tension) against the live repo (`DUPLICATE`+cite / `SHARPENS`+cite / `NEW` / `PARK`+trigger) BEFORE handoff, and each is either **promoted through its home gate by this agent** OR **presented to the Nick+Knox gate** as a decision-ready NEW set (recorded in an `08 D0OL-REV-NNN` row). **Deferring adjudication to "a future agent / reviewer decides distinct-vs-sharpen" is NOT a valid cap** — the wave stays `authoring_blocked`. Write a `{EVRUN}_{slug}_final_closeout.md` terminus. Precedent: Wave-6 (`EVRUN-2026-000011`).
