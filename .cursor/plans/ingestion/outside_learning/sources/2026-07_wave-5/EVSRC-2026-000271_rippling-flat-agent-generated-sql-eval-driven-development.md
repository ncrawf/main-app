...
- reviewer: Opus (restoration subagent) · type: AI assistant · at: 2026-07-18 · purpose: recover Knox Review-001 nuggets dropped/flattened/INVERTED in Review 003 · binds nothing (GRD-036/GRD-044). Append-only — Review 003 NOT modified; the Cluster-3 inversion is corrected here as a supersession.

**Method.** TARGETED RESTORATION per the Wave-5 nugget-preservation restore ledger (`EVRUN-2026-000006_…_nugget_preservation_restore_ledger.md`, §1 EVSRC-271 entry = minimum inventory). Re-read §1 transcript ([5:40]–[6:08], [15:36]–[15:53], [10:32]–[14:57]), §3 Review 001 (Knox — "What NOT to import" #7, keeper #14, #23, and the doctrine/primitive-pressure candidate list), and §3 Review 003 Cluster 3 / Cluster 7 / Cluster 8 to verify anchors and confirm each loss.

**Fidelity verdict:** **MINOR-LOSS + 1 INVERSION.** **Restored: 5** (1 inverted · 2 flattened · 2 omitted). **Weight-change: NO** — the dedup verdict ("~0 net-new DOMAIN objects") stands; the net-new statistical eval-sufficiency cluster (`eval_confidence_contract` + `minimum_detectable_regression` + `cost_uncertainty_lag_policy` + `tiered_eval_cadence`/`runtime_health_eval`) was already faithfully preserved in Review 003 Cluster 7 (no action). These are cautions / sharpenings / primitive-candidates + 1 correction, not weight changes.
...
|---|---|---|---|---|---|---|---|---|
| 1 | "Model-visible context ≠ user-visible context; OMNI still needs system instructions / policy / safety constraints / minimum-necessary filtering; context is traceable, not user-mirrored." | R001 "What NOT to import" #7; §1 [6:02] "whatever the user sees…the LLM sees the same thing" | inverted | Review 003 Cluster 3 repeated the raw Rippling claim approvingly — inverting Knox's caution into an endorsement; in a regulated multi-owner substrate this would sanction user-mirrored model context | **CORRECTION** (+ GUARDRAIL) | `06` guardrail digest · Agent Runtime context-assembly · security (minimum-necessary) | guards Cluster 3 flat-agent; pairs with `purpose_scoped_schema_view` (schema-disclosure = capability-disclosure) | propose — **supersedes the Review 003 Cluster-3 "whatever the user sees, the LLM sees" line** |
| 2 | "Flat agent, selective context, scoped capability" — context routing, not context dumping. | R001 keeper #14 ("This is context routing, not context dumping") | flattened | keeps "flat" from meaning load-every-skill / full-record / all-SOPs-in-one-prompt; flat = interaction-coordination posture, not context flatness | SHARPEN | Agent Runtime & Harness · Context Router / skills & SOPs | sharpens Cluster 3 + Cluster 7 "selective skill loading" | propose |
| 3 | `eval_coverage_debt` — unevaluated behaviors / populations accrue as tracked debt against the eval corpus. | R001 doctrine/primitive-pressure list (`eval_coverage_debt`) | omitted | names the gap between "evals passed" and "coverage complete" — statistical sufficiency ≠ coverage sufficiency; the counterweight to Wilson-interval confidence | PRIMITIVE-CANDIDATE | Build-OS Runtime Proof · Release Operations | complements net-new `eval_confidence_contract` (Cluster 7); dedup vs 215 `agent_eval_bundle` | propose (dedup-then-mint) |
| 4 | "What synthetic data misses" (rare combos · longitudinal irregularity · human ambiguity · adversarial · demographic edges) → **5 proof environments**. | R001 #23 (synthetic-misses list + fully-synthetic-fixtures / production-derived / replay-shadow / expert-reviewed / production-monitoring) | flattened | Cluster 8 kept "synthetic environments" generically but dropped the "synthetic is necessary AND incomplete" brake + the 5-environment taxonomy | SHARPEN | Build-OS Runtime Proof · D7 privacy · Release Operations | sharpens Cluster 7/8 synthetic-test-data; pairs with `production_failure_to_eval_loop` | propose |
| 5 | `skill_acceptance_suite` / `eval_repetition_budget` — owner-authored skill acceptance tests + an explicit per-eval repetition budget. | R001 candidate list (`skill_acceptance_suite`, `eval_repetition_budget`) | omitted | named candidates left un-dispositioned; `skill_acceptance_suite` operationalizes "the owner authors the acceptance criteria" (Cluster 4); `eval_repetition_budget` operationalizes cost-uncertainty-lag | PRIMITIVE-CANDIDATE | Knowledge Reservoirs / skills · Build-OS Runtime Proof / Release Operations | pairs with Cluster 4 (owner-authored skill) + Cluster 7 (`cost_uncertainty_lag_policy`) | propose (dedup-then-mint) |

**CORRECTION statement (supersession of Review 003 Cluster 3).** Review 003 Cluster 3 recorded Rippling's line *"whatever the user sees, the LLM sees"* as a concept repeated approvingly. That **inverted** Knox's Review-001 safety caution ("What NOT to import" #7). This Review-004 line **supersedes** that Cluster-3 statement (append-only — the Cluster-3 text itself is left intact per the transaction rule; it is corrected here, not edited there):
> **Model-visible context is NOT user-visible context.** Rippling's flattening (eliminating hidden cross-agent context) is a useful transparency goal, but OMNI must NOT mirror the user's message history into the model as its whole context. OMNI still requires **system instructions, policy context, safety constraints, minimum-necessary filtering, and authority-scoped context assembly.** Model-visible and user-visible context must be **traceable, not identical** — context is authority-scoped and traceable, not user-mirrored. (Pairs with Cluster 6's `purpose_scoped_schema_view`: schema/context disclosure is itself capability disclosure.)

**One-line verdict.** Review 003 read the transcript faithfully (0 fabrications) but flattened 4 Knox cautions and **inverted 1** — the flat-agent transparency line was repeated as endorsement; restored here as a correction: *simplify the agent, but never mirror unfiltered user-visible context into a regulated model, and never let "synthetic passed" or "eval passed" stand in for coverage.*
...
- `2026-07-18` — **Review 004 — semantic-fidelity restoration** appended to §3 (Wave-5 semantic-restoration transaction; Knox ruling 2026-07-18; PROPOSE-ONLY `GRD-036`/`GRD-044`). Restored 5 Knox Review-001 nuggets dropped/flattened in Review 003 (1 **inverted** · 2 flattened · 2 omitted): (1) CORRECTION + supersession of Cluster-3 "whatever the user sees, the LLM sees" (model-visible ≠ user-visible; minimum-necessary filtering; context traceable not user-mirrored); (2) "flat agent, selective context, scoped capability" = context routing not dumping; (3) `eval_coverage_debt`; (4) "what synthetic data misses" + 5-proof-environment taxonomy; (5) `skill_acceptance_suite` / `eval_repetition_budget`. Fidelity verdict MINOR-LOSS + 1 INVERSION; weight-change NO (net-new statistical eval cluster already faithfully preserved). Append-only — Review 003 / §1 / §0 NOT modified.
