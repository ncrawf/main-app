...
### Review 004 — semantic-fidelity restoration  ·  layer: `analysis_nonbinding`  ·  (agent-authored recovery pass, NOT a Nick paste)
...
**method:** TARGETED RESTORATION (WAVE-5 SEMANTIC-RESTORATION TRANSACTION; Knox ruling 2026-07-18). Read §1 verbatim + §3 Review 001 (Knox) + §3 Review 003 (Opus); verified all 5 restore anchors against §1 (**8/8 real, 0 fabrications**). The "too basic" rating is EARNED and STANDS — this pass only recovers the keepers dropped/flattened *under* it. **Fidelity verdict: MINOR-LOSS.** **Restored: 5.** **Loss-type breakdown: omitted 2 · flattened 3.** **Weight-change: NO** — the dedup verdict (ZERO net-new DOMAIN objects; source = vocabulary + AFFIRM sharpenings) is unchanged; restored items are 1 GUARDRAIL + 4 SHARPEN cautions/sharpenings, not a new weight tier.
...
|---|---|---|---|---|---|---|---|---|
| 1 | Agent iteration is an execution technique, not the enterprise operating model — it does not replace OMNI's governed two-loop model. | R001 §2 "decide–act–observe loop does not replace OMNI's governed loops" ("Agent iteration is an execution technique, not the operating model of the enterprise"); §1 [0:27] | flattened | Guards the constitutional two-loop model (Sense→Resolution→Plan/Commit→Act/Fulfill→Prove) from collapsing into single-actor agent iteration — highest value. | GUARDRAIL | thesis §8 two-loop model · `06` guardrail digest | sharpens the two-loop constitutional model / `agent_lifecycle_projection` (existing) | restored |
| 2 | Connectivity becoming universal makes authority more valuable, not less — tool sprawl, hidden write paths, cross-tenant leakage. | R001 §6 "Standard connectivity increases the need for governance"; §1 [1:27] | omitted | MCP-ubiquity strengthens, not weakens, the need for Connector Gateway / registry / capability envelopes / kill-switches. | SHARPEN | §C Connector Gateway / connector registry · Federation | sharpens MCP-as-rail + Connector Gateway doctrine (existing) | restored |
| 3 | Three-actor split: AI proposes/drafts/flags · human commits irreversible · deterministic systems validate invariants, transact, preserve lineage. | R001 §14 "division of labor, not approval theater"; §1 [3:39] | omitted | The deterministic third leg (validation / transaction / lineage) was lost; the target is intelligent division of responsibility, not maximum autonomy. | SHARPEN | human-steering architecture · §A candidate≠commit / deterministic validation | sharpens `authority_interrupt_policy` / human-steering (adds deterministic leg) | restored |
| 4 | RAG is one strategy among keyword/vector/graph/hybrid/reranked/long-context/cached; embeddings and indexes are access paths, not truth stores. | R001 §3 "RAG is one possible access strategy among…"; §1 [0:59] | flattened | Review 003 kept RAG-governance but flattened the multi-strategy routing + the access-path-≠-truth-store distinction. | SHARPEN | Context Router · Knowledge Reservoirs | sharpens Context Router (RAG = one access strategy) (existing) | restored |
| 5 | Evals ≠ verification: 11-kind assurance taxonomy — deterministic, invariants, policy-safety, retrieval-grounding, tool-call, trajectory, stochastic-repetition, human-expert, dual-control, production-monitoring, outcome-evidence. | R001 §8 "evals alone are not verification"; §1 [2:25] | flattened | Review 003 collapsed the 11 assurance kinds into "not one LLM-judge score"; the LLM-judge is one instrument among eleven. | SHARPEN | eval / assurance doctrine · Build-OS E&V | sharpens `continuous_agent_assurance` / closed eval loop (existing) | restored |

**Verdict:** MINOR-LOSS restoration — the earned "too basic / ZERO net-new DOMAIN" verdict stands; 5 keepers recovered (2 omitted, 3 flattened) so the constitutional two-loop guard, the connectivity-raises-authority sharpening, the deterministic third leg, RAG multi-strategy routing, and the 11-kind assurance taxonomy survive faithfully for downstream authoring; binds nothing (GRD-036/GRD-044).
...
- `2026-07-18` — Opus (restoration subagent) appended §3 **Review 004 — semantic-fidelity restoration** (WAVE-5 SEMANTIC-RESTORATION TRANSACTION; Knox ruling 2026-07-18) recovering 5 Knox Review-001 keepers dropped/flattened under the earned "too basic" rating: (1) agent-iteration ≠ enterprise operating model / two-loop guard [GUARDRAIL], (2) connectivity-universal → authority-more-valuable [SHARPEN], (3) three-actor split incl. deterministic third leg [SHARPEN], (4) RAG one-strategy-among-many / access-paths-not-truth-stores [SHARPEN], (5) evals ≠ verification / 11-kind assurance taxonomy [SHARPEN]. Fidelity MINOR-LOSS (2 omitted · 3 flattened); anchors 8/8 real; **no weight change** (ZERO net-new DOMAIN verdict stands). Append-only (`GRD-036`/`GRD-044`) — Review 003 / §1 / §0 untouched.
