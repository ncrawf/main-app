...
### Review 004 — semantic-fidelity restoration  ·  layer: `analysis_nonbinding`  ·  (agent-authored; append-only recovery)
...
**Method.** Transcript-native reread of the ENTIRE §1 transcript + Knox §3 Review 001 (all 30 numbered items + landing-zones + tiering tags + what-NOT-to-import) + Nick §3 Review 002 + Opus §3 Review 003 (7 clusters), against the wave-5 nugget-preservation restore ledger (`EVRUN-2026-000006_..._nugget_preservation_restore_ledger.md`, source-275 block). The governance-SYSTEM spine genuinely landed in Review 003 (Clusters 1–7, anchor-clean); this pass recovers the flattened/dropped semantic counterweights and the positive/routing halves that the "low net-new mechanism" verdict compressed away.

**Fidelity verdict: MINOR-LOSS.** Restored: **5**. **Weight-change: NO** — the Review-003 dedup verdict ("low net-new mechanism; 1 missing WRAPPER + 1 differentiator SHARPEN") STANDS; these are guardrail-routing + sharpenings + positive/counterweight restorations, not new domains and not a change to the promotion weight. The one materially new landing is **governance-by-design** surfacing as a GUARDRAIL (previously zero landing) — additive routing, not a re-tier.
...
|---|---|---|---|---|---|---|---|---|
| 1 | "Late compliance review cannot repair a system whose identity, authority, evidence, appeal were omitted from architecture." | [Knox] R001 what-NOT-import #14 ("Governance separate from product design") | omitted | Load-bearing WHY-embed-in-spine-NOW; the justification for Nick's "numerous levels"; had ZERO landing in R003. | **GUARDRAIL** | `06` guardrail digest + thesis §A/§B (embed governance in architecture now, not retrofit) | Net-new guardrail landing; inverse framing of Cluster-3 `AI_control_runtime_binding` (omission cannot be compiled in later). | propose-only (GRD-036) |
| 2 | Governed improvement: runtime evidence→finding→analysis→proposed change→validation→authorized release→monitored deploy = candidate→commit. | [Knox] R001 §27; transcript "continuous improvement cycle" [1:51] | flattened | The POSITIVE pipeline survived ONLY as the reject "continual-improvement-as-autonomous-self-modification"; the required half was lost. | **SHARPEN** | Build-OS + Release Operations + Runtime (candidate→commit lifecycle) | Restores the positive counterpart to the recorded REJECT; sharpens Cluster-6 Platform-Loop lifecycle. | propose-only |
| 3 | "Certification may open the enterprise door; trustworthy execution must justify staying inside." | [Knox] R001 §26 | flattened | Procurement / insurer / investor / regulatory-readiness value; only the cert-as-theater REJECT survived → one-sided record. | **SHARPEN** | BIZOPS / enterprise-diligence posture + thesis §A (trust) | Counterweight to REJECT "certification-as-safety-certificate"; both must coexist (value ≠ safety-proof). | propose-only |
| 4 | 12-row requirement→OMNI-home crosswalk: Scope→use-case registry; AI policy→policy registry/§C/Settings; Risk→governance profile/Polaris; Validation→Build-OS; Change→Build-OS+Release; Monitoring→Runtime+clinical; Supplier→Tool/Connector; Internal audit→assurance; Management review→governance body+decision ledger. | [Knox] R001 §25 mapping table | flattened | Reusable routing artifact — the concrete "incorporate at numerous levels" map, dispersed into cluster prose in R003. | **SHARPEN** | §C control-plane routing + v4 spine authoring aid | Consolidates the downstream-homes scattered across R003 Clusters 1–7 into one routing table. | propose-only |
| 5 | "Governance surrounds clinical AI without replacing clinical authority." | [Knox] R001 landing-zones (Care Loop — minor-medium) | omitted | Prevents the governance envelope being read as overriding clinical authority; the care-native boundary line. | **SHARPEN** | Care Operating Model + Accountability (governance ≠ clinical authority) | Sharpens Cluster-2 "management-system ≠ god-domain" specifically for the Care boundary. | propose-only |

**Directive note.** Nick's Review-002 "must be incorporated at numerous levels" remains a **reread/routing instruction**, NOT a promotion or binding directive — it flags this packet for multi-home routing at authoring time (`GRD-036`); it mints nothing.

**One-line verdict.** MINOR-LOSS restored to faithful: the governance-SYSTEM spine held, but its load-bearing **governance-by-design** justification (GUARDRAIL), its **positive** governed-improvement pipeline, its **certification-value** counterweight, the **12-row routing crosswalk**, and the **care-authority boundary** were dropped/flattened — recovered here without re-flattening; dedup verdict unchanged.
...
- `2026-07-18` — Opus Review 004 semantic-fidelity restoration appended to §3 (wave-5 restore transaction, Knox ruling; append-only — Review 003/§1/§0 untouched). Transcript-native reread; fidelity MINOR-LOSS; 5 nuggets restored (governance-by-design=GUARDRAIL [zero prior landing]; positive governed-improvement pipeline=SHARPEN; certification-opens-door value counterweight=SHARPEN; 12-row requirement→OMNI-home crosswalk=SHARPEN; care-authority boundary=SHARPEN). Weight-change: NO — R003 dedup verdict stands. PROPOSE-ONLY (`GRD-036`/`GRD-044`); binds nothing.
