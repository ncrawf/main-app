...
**Method.** WAVE-5 semantic-restoration transaction (Knox ruling 2026-07-18). Read §3 Review 001 (Knox) + §3 Review 003 (Opus) in full; verified each restored anchor against the §1 transcript passages and against the `EVRUN-2026-000006` nugget-preservation restore-ledger EVSRC-268 entry (minimum inventory = 4). Targeted restoration, not re-derivation.

**Fidelity verdict: MINOR-LOSS** (matches restore-ledger §0 scorecard for 268). **Restored: 4** (2 omitted · 1 omitted/never-dispositioned · 1 flattened). **Weight-change: YES (partial)** — the DOMAIN dedup verdict ("ZERO net-new spine/domain primitives") **STANDS**; but a **net-new GUARDRAIL (anti-identity) was missed entirely** by Review 003, and **two named candidate-primitives** (`operator_process_capital`, `context_to_capability_compilation`) were dropped/never-dispositioned — so the guardrail + primitive-candidate tally changes even though the domain-object count does not.
...
|---|---|---|---|---|---|---|---|---|
| 1 | "Don't let MCP, a vector database, an agent framework, or a current UI paradigm become the identity" (+ "don't wait for superintelligence"; "don't attempt the whole 10-yr product first") | R001 §11 keepers: "Do not make MCP…the identity / Do not wait for superintelligence / Do not attempt the entire 10-year network product first" [~L1243–1248]; transcript multicast/tunnel-vision cautionary tale [~26:34; "tunnel vision" ~26:00–28:00, transcript L59/587/789] | omitted | The strategic self-discipline guarding OMNI from letting a transient tool/framework/UI/model become its identity; wholly absent from Review 003 — the single most important 268 restore per the ledger | GUARDRAIL | `06` guardrail digest + thesis §2 (moat/strategy) | net-new guardrail (no prior registry concept); complements model-pluggability / harness-sovereignty (Cluster 7) without being covered by it | propose |
| 2 | "A refactored operating process is itself capital" — process power as a durable, non-software asset (Helmer) | R001 primitive-pressure candidate `operator_process_capital` [L1305] + dedup home "→ BIZOPS / tenant local memory and policy overlays" [L1323]; transcript "human refactoring problem and process change" [~24:34, L529], Helmer "process power" [~10:59–11:00, L261/533–535] | omitted | Names the durable ASSET created by process refactoring (the source's central proof); Review 003's net-new list dispositioned the refactoring GATE but silently dropped the capital/asset it produces | PRIMITIVE-CANDIDATE | BIZOPS / tenant local memory + policy overlays | adjacent to `process_refactoring_gate` / `workflow_recomposition_plan` but DISTINCT (names the asset, not the transition); dedup-or-mint | propose |
| 3 | Compiling governed context into agent-usable capability — a candidate Knox listed SEPARATELY from `organizational_context_externalization` | R001 primitive-pressure list `context_to_capability_compilation` [L1307], a distinct entry from `organizational_context_externalization` [L1300]; transcript "download the brain into the silicon…get the carbon to talk to the silicon" [~7:08, L183/185] | omitted (never dispositioned) | Review 003 dispositioned `organizational_context_externalization` (→ Reservoirs) but never dispositioned this separate "compile context → capability" candidate; the omission itself must be resolved | PRIMITIVE-CANDIDATE | Context Router / capability-assembly (§C capability topology) + Knowledge Reservoirs | likely dedup-adjacent to Context Router (task-typed context assembly) + capability envelope; flagged dedup-or-mint (do NOT assume dedup without the resolution pass) | propose |
| 4 | "Context is not an undifferentiated blob — preserve fact / evidence / assertion / policy / memory / rationale / relationship / authority / prediction / committed-truth" | R001 "What NOT to import blindly" #9 "Context as an undifferentiated blob" [L1365–1367]; transcript broad "context" usage + "download the brain into the silicon" [~7:08, L183] | flattened | Review 003 Cluster 3 keeps a tacit-knowledge MIXTURE (fact/history/policy/folklore/bias) but drops the epistemic-typing ENUM and the explicit "context-as-blob" caution; the typing discipline is the load-bearing point | SHARPEN | Context Router / typed-evidence doctrine (also carries a REJECT-ADD dimension — "context-as-blob" belongs in the recorded reject set) | sharpens existing typed-evidence / Context Router doctrine (context typed, provenance-linked, authority-labeled, freshness-aware) | propose |

**Verdict.** MINOR-LOSS confirmed: Review 003 was anchor-clean but omitted one net-new GUARDRAIL (anti-identity), dropped two named candidate-primitives (`operator_process_capital` omitted; `context_to_capability_compilation` never dispositioned), and flattened the epistemic-typing caution into an unequal-authority list — the "~0 net-new DOMAIN objects" verdict stands, but these cautions/candidates/guardrail do not. PROPOSE-ONLY (GRD-036/GRD-044); binds nothing.
...
- `2026-07-18` — Review 004 semantic-fidelity restoration appended to §3 (Opus restoration subagent; WAVE-5 semantic-restoration transaction, Knox ruling). MINOR-LOSS; restored 4 Knox R001 nuggets dropped/flattened in Review 003 — anti-identity GUARDRAIL (omitted, net-new), `operator_process_capital` (omitted, PRIMITIVE-CANDIDATE), `context_to_capability_compilation` (omitted/never-dispositioned, PRIMITIVE-CANDIDATE), epistemic-typing caution (flattened, SHARPEN). Append-only — Review 003/§1/§0 untouched. PROPOSE-ONLY (`GRD-036`/`GRD-044`); binds nothing; domain-dedup "ZERO net-new" verdict stands.
