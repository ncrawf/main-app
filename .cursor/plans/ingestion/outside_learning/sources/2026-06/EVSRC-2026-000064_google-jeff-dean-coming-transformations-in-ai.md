...
### Review 003 — Opus deep-read extraction  ·  layer: `analysis_nonbinding`  ·  EVRUN-2026-000001 (★SPINE — §B model-substrate origin; pairs 065/066/076/087)
- reviewer: `Opus` · type: `AI assistant` · at: `2026-06-09` · purpose: `formalize Review 001 → structured extraction → registry` · binds nothing (`GRD-036`/`GRD-044`) · [full agent extraction 4524039f]

**20 clusters. Jeff Dean/Google Chief Scientist. Tier split: durable spine (inference-budget, model-plural routing, sparse/modular CNS) vs frontier-research watch vs no-op (TPU/vendor chrome). 064 = primary origin evidence for inference-budget cluster (seeded w/ 065).**
1. **`capability_slope_vs_domain_authority`** — rising capability doesn't dissolve discrete domain authority; smarter models = better candidates, truth/consent/commits still change only via gates. §A/§B/§8/CNS(privilege escalation law)/capability-topology/Build-OS(eval-before-privilege). "do some things but not most things" 4:29. PARTIAL→spine.
2. **`inference_budget_policy` / `test_time_compute_knob`** — inference-time compute = allocatable scaling dim; default cheap/fast, escalate to deep reasoning only by risk_weighted_compute_route; THE load-bearing primitive. §B/§8/CNS(`inference_budget_policy`/`model_reasoning_escalation`/`overthinking_guard`)/Build-OS(meters/loop-caps)/inference-budget/future-watch. "shouldn't spend 10,000x… on everything" 18:17. PARTIAL→spine.
3. **`selective_deep_inference` / `reasoning_escalation_tiers`** — routine triage/classify stays SLM/fast; reconciliation/seam-failures/edge-cases/high-risk earn reasoning-class + human review. §B/CNS(tier registry fast/standard/deep/verified)/capability-topology/Build-OS/security(denial-of-wallet). "make your model much more capable" 18:05. PARTIAL→spine.
4. **`frontier_oligopoly_model_plural`** — few frontier general models, many lighter specialized derivatives; OMNI model-plural under one governed harness (pairs 076). §B/§C(`AI_model_registry`/`model_route_approval`/`model_lineage_record`)/capability-topology(`sparse_capability_network`)/Build-OS(champion-challenger)/inference-budget. "won't be 50… a handful" 6:36. PARTIAL→spine.
5. **`distillation_governed_route`** — distillation compresses frontier into fast small workers; triage/classify/intake/metadata/rubric-prescore = candidates; frontier escalates; lineage records teacher→worker. §B/CNS(distilled routes)/§C(lineage)/Build-OS(distill-then-eval gate)/capability-topology. "techniques like distillation" 6:49. PARTIAL→spine.
6. **`constellation_not_monolith` / `sparse_capability_network`** — parallel constellation of distilled models beats monolithic brain; CNS routes among specialized governed capabilities not one general agent owning truth. §7.6/§12/capability-topology/CNS/Build-OS/§8. "constellation of smaller models… parallel" 26:30. PARTIAL→spine.
7. **`sparse_modular_routing` / `heterogeneous_path_cost`** — sparse expertise, 100–1000× cost spread; activate minimal subset per context; expensive expert paths explicit+budgeted; OMNI routing intentionally irregular by stakes (pairs 087). §B/§7.6/§12/CNS(expert pathway registry)/capability-topology/inference-budget. "paths… hundred or thousand times more expensive" 27:44. ABSENT→spine.
8. **`organic_continuous_learning`** — extend capacity, compact stale via distillation, background GC vs rigid dense retrain; KR+CNS skill lifecycle (promote/distill/archive); domain commits stay discrete; FRONTIER-research not near-term bet. §B/KR(FWREG-007 do-not-canonize)/CNS(skill lifecycle)/Build-OS/future-watch. "organic continuous learning system" 28:32. ABSENT→low-authority-watch→watch.
9. **`train_infer_workload_split` / `disaggregated_serving`** — train vs inference different workload classes; CNS separates batch/deep-reasoning/offline-eval from online patient-facing routes; async/build sandboxes different SLA/cost. §B/CNS(runtime class registry)/Build-OS/capability-topology/inference-budget. "training and inference are pretty different" 16:48. PARTIAL→vocabulary→watch.
10. **`multimodal_io_candidate_not_truth`** — text/code/audio/video/images → D7 artifact → Observation → CM adoption → CNS action; multimodal strengthens Sense, never bypasses gates. §B/capability-topology/Observation/D7/Clinical-Memory/§8/Evidence-Plane. "audio or video or images or text or code" 3:39. PARTIAL→spine.
11. **`rl_post_training_scaling`** — RL/post-train steer behavior; agent policies improve via governed RL/eval w/ trace lineage before privilege expand; NOT raw online learning on PHI. §B/CNS(policy eval loops)/Build-OS(production-shaped sandbox RL)/KR(experience traces)/§8. "reinforcement learning and post-training" 3:26. PARTIAL→spine→watch.
12. **`agent_capability_path_partial_now`** — agents promising w/ clear path but partial coverage today; design maturation lanes w/ tightening gates; resists theater AND permanent-dismissal. §B/§10/CNS(maturity lanes)/capability-topology/Build-OS. "path for agents… reasonably clear" 4:38. AFFIRM→spine→watch (guardrail vocab).
13. **`virtual_agent_fleet_governance`** — operators manage agent teams (registry/scope/consult/steer/exception-queues); "50 virtual interns" = complexity warning not default (pairs 086/062). §7.6/§12/§6/CNS(fleet registry)/Build-OS/capability-topology/§A(HITL steering). "manage my team of 50 virtual agents" 25:13. PARTIAL→spine.
14. **`junior_engineer_build_agent` / `tool_using_worker`** — 24/7 junior-engineer agents = tool-use+tests+debug+docs+virtual-env not codegen alone; measured by repo-nav/test-exec/diagnosis/architecture-awareness/proof/review/rollback; aggressive timeline = urgency on proof NOT care autonomy. Build-OS(primary)/Agent-Work-Protocol/§B/CNS(build sandbox)/KR/security(tool allowlists). "level of a junior engineer" 25:50. PARTIAL→spine(Build-OS)+low-authority-watch(timeline)→promote-spec/watch-timeline.
15. **`algo_systems_co_equal_moat`** — algorithmic improvements co-equal w/ hardware; endgame not checkbook-only; OMNI moat = governed context/evals/architecture/authority not biggest cluster. §B/§10/Build-OS/CNS/comparator-Lens-B. "algorithmic improvements as important as hardware" 24:05. AFFIRM→vocabulary→watch.
16. **`simulation_surrogate_exploration`** — neural surrogates enable exploration-at-scale; sim ≠ validation; workflow/journey/staffing sims accelerate Sense + Build-OS config; all labeled simulation (pairs 088). §B/§8/Build-OS/KR/future-watch/CNS(simulate layer). "approximates the simulator… 300,000x faster" 11:11. PARTIAL→vocabulary→watch.
17. **`runtime_compute_economics` / `bandwidth_aware_design`** — architecture internalizes runtime economics (latency/retrieval/context-size/multimodal-bytes/swarms); budget data movement; ignore→inoperable at patient scale; OMNI meters+caps not builds TPUs. §B/Build-OS(meters/fleet-caps)/CNS/inference-budget/security(denial-of-wallet). "network bandwidth and memory bandwidth" 19:01. PARTIAL→vocabulary→watch (AFFIRM 066).
18. **`robot_actor_future_surface`** — physical/robotic agents = cheaper actor types; OMNI coordinates doesn't build robots; Federation admits robot/device actors w/ adapters+supervision+audit; no robotics pivot. §C/capability-topology/Federation/§10 future-watch. "physical robotic agents as well" 4:56. PARTIAL→vocabulary→watch.
19. **`hardware_tpu_vendor_chrome` — REJECT** — TPU gens/custom-silicon/CUDA-vs-TPU/Pathways/analog-inference = no-op for OMNI; Pathways virtual→physical mapping = metaphor only for CNS hiding capability backends, don't import Google runtime. §10(macro)/future-watch(analog horizon)—else none. "Ironwood… trillium" 9:09. no-op→reject(promotion)/watch(macro color).
20. **`agent_hype_guardrail` / `product_form_unsolved`** — agent hype exceeds delivery; killer daily AI product shape unknown; OMNI builds governed care OS not one chat UI/demo theater; resist shipping capability without proof. §10/§2/Build-OS(proof obligations)/guardrail-digest. "mesmerized by agents right now" 3:53. AFFIRM→vocabulary→watch.

**Net-new (064 = ORIGIN/SOURCE anchor — most fold into existing rows):** `inference_budget_policy`/`risk_weighted_compute_route`/`model_reasoning_escalation` EXISTS (064 primary origin, append anchors; pairs 065/066/087); `sparse_capability_network` EXISTS (064 confirms cl.6-7); `distilled_model_worker` = example not primitive; `simulation_surrogate` EXISTS (064 science-origin anchor; pair 088); `junior_engineer_agent`/`virtual_work_environment` = Build-OS vocab (sharpen finish-line spec); `robot_actor` EXISTS (§C/embodied corpus + trajectory anchor); `organic_continuous_learning` = WATCH candidate (strongest net-new delta but low-authority-watch until FWREG-007; reservoir skill-lifecycle metaphor extend/compact/GC, NOT model-weights continual learning on PHI); `heterogeneous_path_cost` SHARPEN inference-budget+sparse_capability_network; `train_infer_workload_split` = vocabulary (runtime class dimension). DO NOT mint: pathways_abstraction/tpu_*/vibe_coding/browser_desktop_agent(use computer-use envelope)/frontier_model_oligopoly(market commentary)/bio_sparse_activation(pedagogical). **Single registry action: bind 064 as primary spine evidence for inference-budget row + append heterogeneous-path-cost anchors to sparse_capability_network; add organic_continuous_learning as watch-only KR lifecycle tag (FWREG-007), not §B commit.** **Reread (MANDATORY before §B/CNS-contract):** inference-budget law (17:47–18:30 + pair 065 test-time + 066 finiteness + 087 energy → draft `inference_budget_policy` CNS runtime law default-cheap/escalate-by-risk/overthinking_guard); sparse modular routing (26:17–28:52 → CNS expert-pathway registry; separate spine heterogeneous-path-cost from watch organic-continual-learning); junior-engineer Build-OS finish line (25:50–30:15 + 058/084 → proof: tests+debug+docs+virtual-env not codegen demos; reject 1-yr timeline→care autonomy); multimodal candidate gate (3:32–3:46 → D7→Observation→CM walkthrough); agent fleet + attention (25:13–25:30 + 086 + 062 → fleet registry + consult/steer not 50-agent sprawl). LOWER: simulation surrogate (10:46–11:30 + 088, label sim-not-truth); train/infer split (16:41–17:52, no Google import). REJECT-pass: TPU/Ironwood/Pathways (8:41–14:38); analog inference (future-watch one-liner only); vibe-coding anecdote (20:02); Chrome/Gemini integration (21:27–22:35); "manufacture intelligence" without authority guardrail. DO-NOT-reread 064 for: datacenter/energy (use 066), model-tier taxonomy (use 076), attention-routing (use 086), trust/HITL (use 070).
