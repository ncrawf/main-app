...
### Review 003 — Opus deep-read extraction  ·  layer: `analysis_nonbinding`  ·  EVRUN-2026-000001 (★SPINE — canonical agentic design-pattern taxonomy)
- reviewer: `Opus` · type: `AI assistant` · at: `2026-06-09` · purpose: `formalize Review 001 → structured extraction → registry` · binds nothing (`GRD-036`/`GRD-044`) · [full agent extraction 7c19949b]

**22 clusters (16 spine). Andrew Ng/Sequoia. Keepers: workflow harness = first-class substrate (3.5+loop > GPT-4 zero-shot) · reflection Day-1, planning/multi-agent gated · judge OMNI agents by loop quality not first-answer polish · critique ≠ authority.**
1. **`non_agentic_vs_agentic_workflow`** — single-shot prompt→answer = anti-pattern for governed care; default = event-driven iterative loop (gather→reason→tool→critique→verify→route→approve→commit). §B/§8/CNS/Build-OS/UX/product. "without ever using backspace" 1:18. PARTIAL→spine.
2. **`iterative_agentic_loop_topology`** — outline/research→draft→self-read→critique→revise→repeat w/ iteration budget + stop criteria before commit. CNS/§8/Build-OS/domain-contracts. "much more iterative" 1:49. PARTIAL→spine.
3. **`workflow_beats_model_tier`** — GPT-3.5+loop > GPT-4 zero-shot; invest orchestration/eval/harness before next foundation model; contradicts defer-harness-until-GPT-N. §B/CNS/Build-OS/capability-topology/product/future-watch. "GPT 3.5 wrapped in workflow… outperforms gp4" 2:57/3:15. ABSENT→spine.
4. **`four_pattern_taxonomy`** — canonize: reflection · tool-use · planning · multi-agent-collaboration; bind CNS roles/Build-OS lanes/capability atoms (not ad-hoc "swarm"). CNS/§B/Build-OS/capability-topology/KR. "four design patterns" 4:18. ABSENT→spine.
5. **`pattern_maturity_tiers`** — reflection+tool-use = robust/Day-1; planning+multi-agent = emerging/gated (candidates+escalation+review, never autonomous irreversible). Build-OS(lane admission)/CNS/§8/§A/UX(experimental badges). "planning and multi-agent… emerging" 4:05. ABSENT→spine.
6. **`reflection_loop_day_one`** — re-feed output, prompt critique (correctness/soundness/efficiency), reprompt V2+; default pre-gate; improves candidate quality, NOT authority validation. CNS(`reflection_loop`/`draft_critique_revise`)/Build-OS/§A/KR/domain-contracts. "reflection is a tool… just works" 3:48. PARTIAL→spine.
7. **`coder_critic_role_separation`** — separate generation from critique (drafter vs critic, extractor vs authority-verifier, patcher vs test-critic, summarizer vs citation-checker); decomposition not theater. CNS/capability-topology(role atoms)/Build-OS/§C/domain-contracts. "coder agent… Critic agent" 5:57. PARTIAL→spine.
8. **`unit_test_feedback_loop`** — external deterministic evaluators (tests/schema/ingestion-rubrics) drive next revision; LLM proposes, tests verify; pairs no-eval-no-autoresearch (084). Build-OS/CNS/§10/capability-topology/security. "run unit test if it fails… V3" 5:22. PARTIAL→spine.
9. **`tool_use_permissioned_worker`** — tools transform talker→worker but permissioned via capability-topology (read vs write tiers); model-w/o-tools can't commit substrate truth. §C/capability-topology/CNS/security/domain-contracts. "gathering information… taking action" 6:53. AFFIRM→spine.
10. **`planning_candidate_not_commit`** — planning decomposes multi-step (HuggingGPT-style), reroutes around failures, "wow" demos but finicky; output = plans/candidates NEVER autonomous clinical/billing/consent commits. CNS/§8/§A/Build-OS/UX. "finicky they don't always work" 8:52. PARTIAL→spine.
11. **`loop_failure_recovery`** — loops enable mid-workflow correction (failure→replan/retry→recover); CNS captures episodic trace/checkpoints/failure-branches. CNS/§8/Build-OS/UX(trace view). "agentic loops… recover from failures" 8:59. PARTIAL→spine.
12. **`multi_agent_bounded_roles`** — works when roles map to real checks (drafter/critic/test-runner not cosplay); bounded under CNS w/ conversation budget+supervisor+tool-access+commit-boundaries; reject fake boardrooms. CNS/capability-topology/Build-OS/§7.6/§8/domain-contracts. "doesn't always work sometimes it's amazing" 10:25. PARTIAL→spine.
13. **`multi_agent_debate_watch`** — cross-model debate may improve some tasks; requires evidence-refs/tracing/cost-controls; not Day-1 default. CNS/§8/future-watch/capability-topology. "multi-agent debate… ChatGPT and Gemini" 10:36. ABSENT→vocabulary→watch.
14. **`async_agent_delegation`** — minutes-to-hours latency tolerance; operator = agent manager (delegate→wait→review, not poll-every-5-min); needs async runs/progress-state/inbox/checkpoints/takeover. §10/Build-OS/CNS/UX(cockpit 058). "patiently wait minutes maybe hours" 11:56. PARTIAL→spine.
15. **`instant_response_culture_caution`** — sub-second chat UX misaligned w/ agentic work; design for durable runs + progress UI not half-second feedback. UX/§10/product. "want a response right away" 11:27. PARTIAL→vocabulary.
16. **`fast_token_loop_routing`** — loop-heavy = machine-to-machine token churn; workflow-relative model choice (fast/cheap for loops, strong for final/high-risk, deterministic for policy); fast-weak+more-iterations may beat slow-strong. capability-topology/§B/Build-OS/CNS/security(never cheap-model high-consequence clinical). "fast token generation is important" 12:17. PARTIAL→spine.
17. **`iteration_budget_governance`** — cap loop rounds + token spend/lane; clinical/high-consequence requires stronger models + human gates regardless of iteration economics; pairs token_throughput_meter (084). Build-OS/security/capability-topology/CNS/§A. "go around this loop a lot more times" 12:43. PARTIAL→spine.
18. **`agentic_candidate_quality_doctrine`** — judge OMNI agents by workflow (context/tools/self-critique/verify/escalate → reviewable candidates), not first-answer smarts; no loop converts output to authorized truth without gates. §A/§B/Build-OS/CNS/Agent-Work-Protocol/clinical-safety. "who codes like that no human codes" 2:37. PARTIAL→spine.
19. **`task_expansion_staged_lanes`** — agentic workflows expand task classes; stage lane admission as harness matures (reflection/tool first), not "healthcare agent blobs." §10/Build-OS/product/future-watch. "set of tasks AI could do will expand" 11:15. PARTIAL→vocabulary→watch.
20. **`future_model_vs_harness`** — future zero-shot may approach loop performance but harness remains binding; don't defer orchestration for next model drop. §B/Build-OS/future-watch. "gp5 zero shot… closer with agentic" 13:03. PARTIAL→vocabulary→watch.
21. **`agi_journey_tempered_claim`** — agent workflows = incremental step not sovereignty; governed incrementalism under Sense+Act+gates; reject hype that agents = clinical authority. §B/future-watch. "journey rather than destination" 13:21. AFFIRM→low-authority-watch→watch.
22. **`pattern_bibliography_sibling_sources`** — extend pattern corpus via siblings 059/062/084/078/082; this = taxonomy spine, siblings supply trace/memory/runtime depth. KR/Build-OS/CNS(pairs 059/062/077). "recommended reading section" 5:40. PARTIAL→vocabulary→watch.

**Net-new (promote):** `four_pattern_taxonomy`(canonical frame), `pattern_maturity_tier`(robust-Day1 vs emerging-gated → Build-OS lane admission), `workflow_beats_model_tier`(quantified doctrine), `agentic_candidate_quality`(eval doctrine), `multi_agent_debate`(watch). SHARPEN: `workflow_relative_model_choice`(064/066 inference-budget + loop economics), `iteration_budget`(pairs 084 token_throughput), `fast_token_loop`(pairs 062 relaxed-latency), `planning_candidate_not_commit`(authority gate). EXISTS-AS canonize: `draft_critique_revise`/`reflection_loop`/`critic_agent`(capability role atoms)/`unit_test_feedback_loop`(054/084)/`async_agent_run`(058)/`agent_manager_surface`(084). EXISTS (don't re-mint): agentic_workflow→CNS loop_agent/§8; tool_using_agent→§C; planning_agent→077 planner+maturity-gate; multi_agent_collaboration→077 coordinator/specialist; agent_loop_iteration→convergence-loops/checkpoints. REJECT: ChatDev/Devin cosplay; AGI-journey framing (low-auth); recommended-reading URLs(sibling pointers not primitives). **Reread (MANDATORY):** CNS-contract canonization (3:38–4:18 taxonomy+maturity + cross-check 059/062 trace/memory/runtime — don't collapse to Ng-only); Build-OS pattern-maturity lane admission (3:48–4:26, 5:22–6:25 + Build Entry Gate v0 + Agent-Work-Protocol §6 — reflection/tool before planning/multi-agent); workflow-beats-model-tier in §B (2:31–3:18, 12:17–12:48 + 069 + 084); capability-topology role atoms (5:57–6:25, 9:33–10:48 — roles map to gates not cosplay + 077); async agent-manager UX (9:03–9:17, 11:27–12:11 + 058); authority gate insertion (4:05–4:15, 8:48–8:55 — planning/multi-agent = candidates not authority before clinical/commerce/consent contracts). Sibling dedupe: 059/062(execution substrate)/084(Build-OS harness depth)/078-082(IBM AFFIRMs)/054(unit-test-loop). DO-NOT-reread: CS229 anecdote 0:35, CV tool-use story 7:02, GPT-5 name-drop 12:51.
