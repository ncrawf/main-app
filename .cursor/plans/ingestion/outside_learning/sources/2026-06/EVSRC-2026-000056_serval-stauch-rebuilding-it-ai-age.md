...
**14 clusters. Serval/Stauch rebuilding-IT. "Boundary layer is the product" + admin/runtime split.**
1. **Boundary layer is the product** — product = governed boundary (identity/consent/permissions/approvals/authority/audit/escalation/commit), not raw model intelligence. §C/§A, CNS `capability_envelope`, Build-OS (authority gates as admission), security. "the product is the boundaries" 8:48 · "the product is the controls" 8:55. **PARTIAL → spine.**
2. **Admin/runtime agent split** — builder/config agents vs runtime/care agents; runtime may ONLY invoke published+approved capabilities (can't mint power at runtime). §C, CNS `builder_agent`/`runtime_agent`, Build-OS lanes, RBAC/Settings. "two agents they work with" 9:53 · "help desk agent can only use… published with approvals" 10:17. **ABSENT → spine.**
3. **Permissioned tool registry + scoped user gating** — registered tools w/ per-action approvals + per-user gates + audit before runtime call. §C, capability-topology Node/Grant, CNS `published_capability`, security allowlists. "expressly said are okay to use" 10:40. **PARTIAL → spine.**
4. **NL workflow compiler + workflows-on-database** — keep durable primitives; compile NL intent → auditable versioned automation code. §B, CNS `workflow_compiler`, Build-OS NL→artifact, D5/D6. "workflows on top of databases" 2:35 · "turn that into code" 3:47. **PARTIAL → spine.**
5. **Automation must be easier than the manual step** — if teaching is harder than doing once, staff stay manual; watch-and-propose beats blank-slate. §8 capture, Build-OS proposal-from-runs, UX one-click. "easier to build the automation" 5:04. **ABSENT → spine.**
6. **Slop automation + workflow dedupe/lifecycle** — frictionless NL automation → duplicate/conflicting pathways; dedupe/merge/delete + ownership+lifecycle before activation. §C, Build-OS `workflow_dedupe_gate`/`automation_lifecycle_state`, CNS meta-agent. "20th password reset workflow this week" 5:33 · "delete 10 of them" 6:02. **ABSENT → spine.**
7. **Application layer as translation buffer** — OMNI absorbs model churn so customers solve business problems. §B, §2 foil, Build-OS (model-absorption behind stable API), §6. "that translation layer" 30:25 · "downpour of capabilities" 30:19. **PARTIAL → spine.**
8. **Task-specific model routing + rollback** — route by demonstrated task fit; version changes = business changes needing evals/slow-rollout/explicit downgrade. §B, CNS `model_route_by_task`/`model_rollback_state`, Build-OS `model_release_gate`. "upgraded models and then downgraded" 13:03. **PARTIAL → spine.**
9. **Compile-once/run-many economics** — prefer amortized automation artifacts over per-request regeneration. §B, Build-OS artifact persistence + budget caps. "not in the business of reselling tokens" 14:05. **ABSENT → vocabulary.**
10. **Autonomy vs control tension** — individual wants access, org wants bounds; resolve via approved capabilities + live policy + escalation; shadow-IT parallel. §A/§C, security (shadow-AI, say-yes-with-guards), Consent/Identity. "who wants autonomy and who wants control" 31:30 · "say yes as the default" 33:22. **PARTIAL → spine.**
11. **Inside-out: automate internal ops first** — deploy on staff/internal workflows first to build trust+learn failure modes before patient-facing autonomy; pairs 083. §6, Build-OS internal lanes, Messaging-internal/Settings, security. "forced the team to use it for everything" 21:08. **PARTIAL → spine.**
12. **AI right-of-first-refusal + fewer-better topology** — question every role for AI substitution first; care: AI first pass on triage/draft/gather, humans/domains retain commit authority. §6, Build-OS `AI_first_pass_human_final_authority`, operator-topology. "AI almost gets the right of first refusal" 25:31 · "fewer, better" 35:24. **PARTIAL → spine.**
13. **Six-month software horizon + reinvention** — design for reversible architectures, frequent renames; agility > permanence. §B, Build-OS rebuild/de-scaffolding, §6. "hopefully will last six months" 29:37. **ABSENT → vocabulary → watch.**
14. **ITSM / ticket-deflection / instant-support (enterprise vocab)** — adjacent vocabulary for staff/messaging surfaces, NOT clinical core; ServiceNow-era Lens-A. §3.5, UX staff support, Messaging, NOT clinical contracts. "next generation service now" 0:48. **no-op for care → vocabulary → watch.**

**Net-new:** `builder_agent`/`runtime_agent`, `workflow_compiler`, `capture_while_doing`, `workflow_dedupe_gate`/`automation_lifecycle_state`/`slop_automation`, `model_rollback_state`/`model_release_gate`, `enterprise_boundary_layer`, `autonomy_control_tension`, `inside_out_deployment`, `AI_first_pass_human_final_authority`, `compile_once_run_many`, `six_month_rebuild_horizon`(watch). EXISTS-AS sharpen: `published_capability`/`permissioned_tool_registry`→`capability_envelope` (Capability Topology Gate). REJECT: unbounded help-desk agent, NL gen w/o dedupe/approval, casual model upgrades, unbounded AI-first-refusal into clinical authority, "boring ITSM UI at expense of care trust." **Reread:** 9:45–10:57 (builder/runtime split — cleanest articulation); 5:27–6:14 (dedupe lifecycle); 12:04–13:30 (model rollback); 8:48–9:45 (product=boundaries center-of-gravity); 31:24–33:53 (autonomy/control + shadow IT); 25:08–27:06 (AI-first-refusal — bound at care gates); EVSRC-083 (inside-out stats = low-authority corroboration only).
