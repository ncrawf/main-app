...
**14 clusters. IBM "10 use cases for AI agents." Lens-B teaching vocabulary — 0 net-new spine/primitives; AFFIRMS autonomy/AOP/front-door. Keeper: Agents are loops. OMNI makes those loops governed.**
1. **Agent loop baseline (stateful reason-plan-act vs one-shot)** — agents maintain state, decompose goals, serial/parallel execution, replan; OMNI loop terminates in action-candidate→policy-gate→domain/human-commit. §B/§8, CNS `loop_agent`/convergence-loops, Build-OS overnight/patrol loops, UX agent-inbox HITL. "reason and act autonomously to achieve goals" 0:03. **PARTIAL/AFFIRM → vocabulary → watch (merge Autonomy-loops + Agentic-patterns).**
2. **Five-part agent anatomy (goal→planner→memory→executor→action)** — OMNI care-grade insertion: goal-authorization→planner→authority-labeled-memory→executor→policy-gate→action-candidate→commit-boundary→trace/audit; IBM 'action' = OMNI pre-commit surface only. §B/§8, CNS (planner/executor roles, orchestration grammar 077), Build-OS (steps map to AOP), §A (gates between executor and commit), domain-contracts (action lands in owning domain API). "goal, planner, memory, executor and action" 8:50. **PARTIAL → vocabulary → watch.**
3. **Use cases are capability patterns, not domains (anti-'healthcare agent blob')** — extract capability patterns (sensor-driven/RAG-grounded/multi-agent/anomaly/workflow-automation/replanning), route each through CNS + envelopes + domain commit; do NOT build IBM's vertical agents. §B (GRD-026), CNS (specialist roles not vertical silos), Build-OS (AOP/BPO wedge filter), domain-contracts (owned seams D7/Obs/CM/Messaging). "analyzing lab results and managing prescriptions" 8:01 (warning anchor). **AFFIRM → vocabulary → watch (reject literal vertical agents).**
4. **Sensor-driven closed loop (IoT/device → plan → actuate → replan)** — continuous Sense→plan→act on changing streams (wearables/home-devices/clinic-sensors/lab-interfaces); IoT = Lens-B analogy for governed device actuation behind envelopes. §8 Sense triggers, CNS event-driven activation, capability-topology/§C (`device_actor`), domain-contracts (Obs/Messaging), future-watch (robot/device actors 074). "based upon changing sensor data" 3:14. **PARTIAL → low-authority-watch → watch.**
5. **RAG-grounded iterative drafting (retrieve→draft→self-critique→revise)** — OMNI: retrieved text carries authority labels; retrieval ≠ authority. §A (GRD-042), KR, CNS context-assembly (059), Build-OS ingestion pipelines, domain-contracts (D7 artifacts/CM adoption). "retrieval augmented generation or RAG" 4:32. **PARTIAL/AFFIRM → vocabulary → watch (dedupe 059/068/071).**
6. **Multi-agent coordination (coordinator + specialists + shared situational map)** — strongest IBM→OMNI analogy: coordinator + parallel specialists read/write shared map; OMNI CNS = message/lab/scheduling/commerce/doc/risk specialists over authority-labeled role-scoped context packets (not free-for-all blob). §7.6/§8, CNS (coordinator/specialist roles, `context_field`, graph grammar 077), Build-OS (fleet registry, dedup/conflict), domain-contracts (each specialist owns commit boundary), UX provider-packet/attention-cockpit. "coordinator agent, working with specialist agents" 6:26 · "shared situational map" 6:49. **PARTIAL → vocabulary → watch (dedupe 071/077/087).**
7. **Reflection / iterative self-improvement inside loop** — critique output, detect unsupported sections, re-retrieve, adjust; maps Ng reflection pattern under governance. §B, CNS `reflection_loop`/`draft_critique_revise`, Build-OS eval bundles, §A (critique ≠ commit). "self-improving where the agent learns from results" 3:17. **AFFIRM → vocabulary → watch (append to Agentic-patterns 071).**
8. **Workflow automation / multistep enterprise procedures (HR onboarding archetype)** — long multistep w/ enterprise writebacks → OMNI staff-onboarding/prior-auth-chase/lab-follow-up/doc-assembly = AOP territory. CNS `agent_operating_procedure`, Build-OS (process_readiness/accuracy-ladder), §2/§5 BPO wedge, capability-topology (`web_agent_writeback`). "execute multistep processes, like onboarding new employees" 8:09. **ABSENT in v3 but already promoted via 057/056 — 0 new pressure → vocabulary → watch (deduped → AOP).**
9. **Customer-experience / sentiment-adaptive response** — patient anxiety/tone = routing signal not authority mover; front-door + messaging w/ escalation when sentiment exceeds envelope. §7.7, UX (`front_door_agent`/messaging), CNS escalation, §A (tone adapts, gates don't move). "analyze customer tone to adjust their responses" 7:50. **PARTIAL → low-authority-watch → watch (dedupe Front-door 063).**
10. **Streaming anomaly detection (fraud archetype → safety/drift flags)** — continuous ingest + anomaly models → flag abnormal patterns (workflow drift/safety/billing); Sense-layer candidate generation not autonomous enforcement. §8 Sense, CNS risk candidates, §A (flag ≠ commit), security. "anomaly detection models to flag fraud" 7:43. **PARTIAL → vocabulary → watch (fold into Sense/safety; no-op finance vertical).**
11. **IT ops auto-remediation (alert triage→root-cause→script fix)** — Build-OS/loop-agent patrol archetype (089), not clinical path. Build-OS (`loop_agent`/kill-switch), CNS patrol workers, security, UX operator cockpit (058), §8. "automated remediation, parsing thousands of system alerts" 8:21. **PARTIAL/AFFIRM → vocabulary → watch (dedupe 089 + 058).**
12. **Dynamic replanning under changing conditions (transportation archetype)** — scheduling reshuffles/care-plan timing/follow-up deferrals; CNS real-time convergence not logistics SKU. §7.6/§8, CNS (convergence/suppression), domain-contracts (D3 scheduling), UX. "continuously recalculating optimal routes as conditions change" 8:40. **PARTIAL/AFFIRM → vocabulary → watch (merge CNS dynamic-system 087).**
13. **Vague 'healthcare multi-agent' (lab+Rx coordination) — WARNING ONLY** — IBM names lab-results + prescription subagents w/o domain seams/authority/commit; OMNI must decompose into D7/Obs/CM/Rx/fulfillment/messaging/audit — NOT a 'healthcare agent.' §A (reject), domain-contracts (decomposition mandate), CNS (specialist ownership), clinical safety. "analyzing lab results and managing prescriptions" 8:01. **AFFIRM → low-authority-watch → reject (watch as warning).**
14. **Generic enterprise verticals (banking/supply-chain/transportation)** — illustrative breadth; no OMNI mapping beyond C8-C12 patterns. §3.5 color at most. "rapid fire through the remaining seven" 7:31. **no-op → reject (register, don't route).**

**Net-new (078): NONE.** Knox candidates (`agent_goal`/`goal_authorization`/`planner`/`memory_context`/`executor`/`action_candidate`/`policy_gate`/`shared_situational_map`/`specialist_agent`/`coordinator_agent`/`dynamic_replanning`/`agent_feedback_loop`) all under existing pressure: Trust-posture/§A/Autonomy-loops; Agentic-patterns(071); CNS-orchestration(077); Context-engineering(059); CNS-dynamic-system(087); AOP(057/056); Front-door(063); Agent-workbench(058). 078 CONFIRMS taxonomy; doesn't justify new rows/spine. **Reread:** before adding goal→planner→memory→executor→action to §B/CNS prose (1:42/2:58/8:50 + Knox governed-loop correction; insert authority/commit layers — don't quote IBM 'action' as final commit); disaster-response as CNS analogy (6:26–7:23 + 071 + 087; label disaster Lens-B); any 'healthcare agent'/lab-Rx language (7:54–8:04 + mandatory domain-decomposition D7/Obs/CM/Rx); dedupe glossary vs 071/059/057.
