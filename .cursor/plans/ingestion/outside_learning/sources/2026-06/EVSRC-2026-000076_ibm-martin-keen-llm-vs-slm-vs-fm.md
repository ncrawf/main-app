...
**11 clusters. IBM "LLM vs SLM vs FM." AFFIRMS inference-budget + model-plurality; 0 net-new.**
1. **`task_model_fit` / smallest-sufficient-model escalation** — CNS chooses model class by task stakes (SLM bounded; LLM synthesis; FM agentic-escalation); deterministic validators + human gates above all tiers. §B, CNS bind `inference_budget_policy`/`risk_weighted_compute_route`/`model_reasoning_escalation`, Build-OS per-route evals+regression gates. "match the model to the task" 13:28. **PARTIAL → vocabulary (AFFIRM) → watch.**
2. **`slm_specialist_not_degraded_llm`** — small models = efficient specialists not 'worse LLMs'; OMNI SLM candidates = intake-classify/message-route/metadata-extract/dup-detect/admin-triage. §B speciation, CNS SLM-class task registry, Build-OS champion-challenger, capability-topology `sparse_capability_network`. "efficient specialists" 0:30 · "match or even beat these bigger models" 2:24. **AFFIRM → vocabulary → watch (merge model-plurality + jagged-intelligence).**
3. **`inference_cost_latency_budget` / frontier-everywhere anti-pattern** — fewer params → cheaper/faster; frontier-for-everything strategically wrong. §B, CNS `overthinking_guard`/default-cheap, Build-OS cost caps + meters, security denial-of-wallet, future-watch (066/064). "Why don't I just use Frontier models for everything?" 3:54. **PARTIAL→AFFIRM → vocabulary → watch.**
4. **`deploy_where_data_lives` / on-prem SLM governance route** — sensitive/regulated payloads (PHI/finance) may require on-prem/residency-bound SLM; compliance-by-placement. AFFIRMS 069 open/self-host + 064 state-placement + §C `AI_state_home`. §B, CNS (route includes residency), Build-OS (on-prem eval+lineage), capability-topology (residency = envelope dimension), security (external-call minimization for PHI). "running an SLM on premise" 6:32 · "data never leaves that environment" 6:32. **PARTIAL → vocabulary → watch.**
5. **`slm_classification_routing_pipeline`** — classify→tag→route = SLM-class; maps OMNI intake/message/doc routing. §B, CNS SLM-worker for Sense-triage (candidate labels only), Build-OS classification eval harness, capability-topology (read-only classify vs write/commit split). "perfect job for a small language model" 5:36. **AFFIRM → vocabulary → watch.**
6. **`llm_multi_source_synthesis`** — LLM-tier for synthesizing multiple sources w/ high variability; candidate understanding not committed truth. §B, CNS multi-source `context_assembly` (059), Build-OS synthesis-evals-with-source-grounding, capability-topology (LLM behind read envelopes), security (untrusted-ingest firewall). "synthesizes all of that data" 8:12. **AFFIRM → vocabulary → watch.**
7. **`frontier_agentic_escalation` / multi-step investigation with HITL** — frontier for multi-step agentic; TODAY co-pilot+guardrails+human-signoff not full autonomy. §B (escalation tier not default), CNS `model_reasoning_escalation`+loop-depth-cap, Build-OS incident eval in production-shaped sandbox, security (tool allowlists+kill-switch). "multi-step investigation and execution" 11:50 · "human signoff" 12:10. **PARTIAL→AFFIRM → vocabulary → watch.**
8. **`model_class_taxonomy_llm_slm_fm` / Lens-B glossary (unstable boundaries)** — LLM umbrella / SLM <~10B / FM frontier; NOT three rigid ontologies. §B glossary, CNS (competence evals override tier names), Build-OS (registry metadata tags non-binding). "not really three separate categories" 0:23. **ABSENT as taxonomy → vocabulary → watch (pair 075).**
9. **`parameter_count_routing_heuristic` — REJECT as routing law** — param counts = pedagogical shorthand ONLY; OMNI routes by task/risk/eval; tool-use erodes size=frontier. counter-doctrine only. "plenty of smaller models, they can use tools now" 3:10. **AFFIRM (eval-first > size) → no-op → reject for routing policy.**
10. **IBM watsonx / product exemplar chrome** — Granite/Mistral/watsonx-cert = vendor marketing (GRD-039). **no-op → reject.**
11. **`models_propose_domains_commit` — Knox keeper (cross-source AFFIRM)** — no model class owns OMNI authority; all tiers produce candidates; domains+humans commit. §A AFFIRM, CNS orchestration_state ≠ domain_truth, capability-topology (commit via published capabilities), security (bigger model ≠ safer action). "human in the loop here at least today" 12:16. **AFFIRM → vocabulary → watch (bind Trust-posture + HITL-permanence).**

**Net-new (076): NONE.** All fold into `inference_budget_policy`/`risk_weighted_compute_route`/`model_reasoning_escalation` (064/065/073), `AI_model_registry`/`model_route_approval`/`AI_state_home` (069/064/056), `sparse_capability_network` (069+jagged), authority spine. DO NOT mint: model_route_policy/small_model_worker/frontier_reasoning_worker/classification_service/incident_response_agent (aliases/examples). **Reread:** §B model-route prose (4:05–13:50 — examples stay illustrative not canonized workflow inventory); §C `AI_model_registry`/residency (6:25–6:54 — PHI path must exceed generic 'finance and healthcare'); pair 075 AI-stack (075 = full stack, 076 = model-tier zoom — dedupe; don't double-count IBM explainers).
