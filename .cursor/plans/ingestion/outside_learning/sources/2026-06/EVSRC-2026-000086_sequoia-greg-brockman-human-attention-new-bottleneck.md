...
### Review 003 — Opus deep-read extraction  ·  layer: `analysis_nonbinding`  ·  EVRUN-2026-000001 (★SPINE — attention-routing origin; pairs 058/052)
- reviewer: `Opus` · type: `AI assistant` · at: `2026-06-09` · purpose: `formalize Review 001 → structured extraction → registry` · binds nothing (`GRD-036`/`GRD-044`) · [full agent extraction 919d0fa3]

**22 clusters. Brockman/Sequoia — "the CNS problem in plain English." 086 = spine origin for attention-routing. Keeper: CNS's primary job is attention ROUTING — allocate scarce human judgment to decisions needing authority; agents absorb the doing.**
1. **`attention_routing_state`** — human attention = scarcest resource; CNS routes it to decisions needing authority/values/clinical-responsibility. §A/§7.6/§8/CNS/UX/product/Build-OS/capability-topology. "human attention… scarce resource" 16:22 · "doing of things now is easy" 16:26. ABSENT→spine.
2. **`doing_vs_judging_inversion`** — execution cheap, evaluation expensive; substrate value = protecting humans for judgment (alignment/consent/adoption/commit). §A/§8/§10/CNS/Build-OS/§7.6/product. "writing 20%… to 80%" 7:38 · "Is this aligned with my values" 16:30. PARTIAL→spine.
3. **`CNS_context_activation`** — CNS = live context-and-agent activation network over patients/providers/workflows/reservoirs, not dumb router; stop re-explaining the world. §7.6/§B/CNS(pairs 059/087)/KR/§8/Agent-Work-Protocol. "why are you explaining to your computer" 8:32. ABSENT→spine.
4. **`context_as_one_time_investment`** — instrument governed context once (identity/consent/CM/workflow/reservoir-lineage), compound forever; decide permeability. §7.6/§A/Build-OS/CNS/KR/§7.8/product. "one-time investment that now is the time" 9:20. ABSENT→spine.
5. **`governed_workflow_memory`** — structured permissioned workflow memory (Chronicle principle WITHOUT surveillance); reject raw screen-capture/ambient-PHI. §7.6/CNS/security(PHI default-deny)/KR/UX/§A/future-watch. "see everything you're doing… form memories" 8:10. PARTIAL→spine (healthcare guardrail).
6. **`derived_permission_invalidation` + `source_derived_lineage`** — generated projections preserve lineage to source permission/authority; when source changes, derived artifacts cascade-invalidate. §A/§8/KR(primary)/CNS/Build-OS/Agent-Work-Protocol/D7-Obs-CM/UX. "derived artifacts… invalidate that as well" 12:52. ABSENT→spine.
7. **`cheap_prototype_governance`** — agents cheaply spawn dashboards/workflows/bots; register w/ owner/permission/provenance/review/lifecycle/kill-switch or shadow-ops proliferate. §6/§7.8/Build-OS(pairs 056)/security/CNS/capability-topology/product. "cost of building a prototype is cheap now" 11:44. ABSENT→spine.
8. **`human_accountable_commit`** — agents draft/code/route/propose at scale; accountable commit (merge/adoption/policy/promotion/consent/care-action) stays human/domain-gated; "neither extreme is right." §A/Build-OS/CNS/Agent-Work-Protocol/§8/capability-topology/security. "human accountable for all code merged" 9:57. PARTIAL→spine.
9. **`agent_escalation_policy` + `agent_social_boundary`** — agents need wait-thresholds/escalation-etiquette/role-boundaries before cross-role messages/manager-pings/clinical-escalations; "reasonable"≠"appropriate" (Slack-manager anecdote). §A/§8/CNS(`proactive_agent_policy`)/Messaging/RBAC/UX/security/product. "escalated to the manager" 15:29. ABSENT→spine.
10. **`interrupt_tiers` + `approval_fatigue`** — CNS classifies actions (auto-no-op/low-risk-auto/draft/must-decide/urgent-interrupt/security-review); humans rubber-stamp, so size interrupts to attention budget. §A/§7.6/CNS(`interrupt_threshold`)/UX(agent-inbox 062)/§7.7/Build-OS/product. "approve, approve… humans default" 16:05. ABSENT→spine.
11. **`CNS_agent_fleet` + `org_of_N_agents`** — operator = CEO of governed fleet (identity/purpose/scope/tools/memory/owner/lifecycle/cost/risk/kill-switch). §6/§8/CNS(fleet registry)/Build-OS(089/058)/UX(061/062)/capability-topology/§C/future-watch. "CEO of… 100,000 agents" 25:09. ABSENT→spine.
12. **`vertical_by_vertical_adoption`** — agent adoption one care/business vertical at a time w/ domain experts; prove internally then externalize. §6/§10/Build-OS(use-case-first 073)/product/capability-topology/§2/CNS. "vertical by vertical… finance… sales… IT" 10:22. PARTIAL→spine.
13. **`agentic_coding_transition`** — doing-automation 20%→80%; Build-OS itself built/reviewed by agents under harness/eval/merge-gates/attention-routing. §26/§8/Build-OS(primary)/CNS/Agent-Work-Protocol/security/§B. "writing 20%… to 80%" 7:38. ABSENT→spine.
14. **`setup_determines_outcome`** — same model + wrong harness/context = failure; capability-topology+contracts encode the setup, not model prestige. §B/Build-OS/capability-topology/CNS/§7.6/Agent-Work-Protocol. "right setup… massive results" 5:49. PARTIAL→spine.
15. **`agentic_production_stack`** — production agentic workflows require security/observability/governance/red-team/trusted-access/responsible-release as non-optional. §A/security-AI-gateway(079-081)/CNS/Build-OS/§C/Agent-Work-Protocol. "security primitives, observability… governance" 15:14. PARTIAL→spine.
16. **`sharing_becomes_bottleneck`** — as agents cheaply create, hard problem = safe cross-boundary sharing w/ visibility/provenance/publish-governance. §7.8/§7.7/Federation/CNS/UX/product/capability-topology. "bottleneck has shifted to… sharing" 11:55. ABSENT→spine.
17. **`messy_reality_training`** — train/eval on messy real workflows + interrupted humans + adversarial inputs; healthcare IS messy reality (dogfood live care, not sandbox-only). §8/§10/Build-OS(production-shaped 054)/CNS/§A(safety-case 049)/product. "real world messy code bases, humans interrupting" 27:30. PARTIAL→spine.
18. **`intent_candidate` pairing (058)** — doing automates → limits = clear intent (in) + scarce attention to verify alignment (out); two sides of one §A gate. §A/§B/CNS(clarifying-question)/Build-OS(user_perspective_verification)/§7.6/UX/058-cross-ref. "Is this what I wanted" 16:28. PARTIAL→spine (merge w/ 058 at CNS-contract).
19. **`operator_attention_budget` pairing (052)** — scarce OPERATOR attention picks one frontier (052) while CNS routes ROLE attention across queues (086); complementary. operator-context/Build-OS(`core_before_act_two`)/§6/§7.6/Evidence-Plane. "human attention… scarce" 16:22. PARTIAL/AFFIRM→vocabulary→watch (don't inflate 052 into CNS primitive).
20. **`strategic_focus_8020` + `painful_scope_exclusion`** — even frontier labs can't do everything; OMNI explicitly renounces sprawl (Future Work Registry/de-scaffolding). §2/§10/Build-OS/product/future-watch. "painful decisions about what not to do" 24:09. AFFIRM→vocabulary→watch.
21. **`mega_agi_life_coach_foil`** — "AGI you can talk to with all context" (health/finance/career) is a FOIL not OMNI's shape; clinical authority/consent must not flatten into one life-coach agent. §2/§A(pairs 060)/§10/future-watch/security. "AGI… all this context" 23:48. PARTIAL/foil→low-authority-watch→watch (reject as destination).
22. **`compute_scarcity` + `scaling_laws` + `science_horizon`** — compute scarcity → inference-budget discipline (064/066); no-wall → authority gates strengthen as models improve; science-acceleration = horizon only. §B/§10/CNS(inference-budget)/future-watch/§A. "There's no wall" 3:25. AFFIRM(inference-budget)/no-op(scaling pedagogy)→watch.

**REJECTS (guardrails):** uncontrolled "AI sees everything" ambient capture in healthcare; raw screen surveillance as memory model; unregistered agent-built widgets/bots; social/clinical escalation w/o policy; endless approve-clicking as HITL; OpenAI AGI-% as metrics; "faster prototyping = better product." **Net-new (dedup): 086 is the spine ORIGIN — registry already claims `attention_routing_state`/`human_attention_budget`/`interrupt_threshold`/`proactive_agent_policy`/`agent_escalation_policy`/`agent_social_boundary`/`derived_permission_invalidation` from 086; this CONFIRMS+sources them, no duplicate rows.** MERGE (not new): `CNS_context_activation`→059/087 context-engineering+dynamic-CNS; `CNS_agent_fleet`→fleet-registry (089/operator-topology); `AI_generated_widget_governance`→056 publish-lifecycle. Promotable delta = BIND the attention-routing primitive family (ABSENT in v3) w/ 086 as primary evidence + sharpen `derived_permission_invalidation` as KR spine; single host name = `attention_routing_state` (tiers+budget+classifier). **Reread (MANDATORY):** attention-routing CNS-contract draft (16:05–16:42 + merge 058 intent + 062 inbox → §7.6 attention-economics, define tiers before UX); derived-invalidation cascade (12:29–13:04 + GRD-042/043 → KR walkthrough); Chronicle/ambient-memory guardrail (8:06–8:35 + 081 zero-click → reject screen-capture PHI default); escalation exemplar (15:22–15:58 → Messaging/RBAC channel policy); pairs 058 (intent-in/attention-out single §A gate) + 052 (operator-context prose only); fleet/100k (25:07–25:31 + 089/058/061 → one fleet-registry primitive). REJECT-pass: AGI-% (5:08), life-coach-AGI (23:48), science-renaissance (27:39), compute-as-moat.
