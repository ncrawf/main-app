...
| 4 | **CNS / Orchestration** | `contracts/CNS_orchestration_contract.md` | ▶ **NEXT** — prereq PARTIALLY MET: §A recovered 2026-05-31 (LI doctrine + LI pressure-test corpus + 07 evidence ledger on main as evidence; `CNF-010` resolved). Remaining: §B per-file `lib/*` runtime reconciliation (`REV-148`) at pass start | CNS ADR + DL-14/16 + rules/templates §1Q + **recovered LI doctrine (evidence, re-verify vs Tier-0/thesis-v2) + LI pressure-test corpus** |
