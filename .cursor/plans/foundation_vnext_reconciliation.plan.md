...
**Native-draft order (untouched domains — DO THIS, not retro sequential-add):** for a domain with no contract yet (Federation/RBAC/Settings/AI), do **one gather + one integrated authoring pass**, NOT "thesis-draft → backfill-map → pull-gems" in sequence (that retro order defers reconciliation + caused the drafted-domain churn). Order: **(1) gather ALL sources at once** (DL-draft + legacy system-map sections + gem audits/designs + thesis sections + shipped code + the domain's **staged items already in `08`**); **(2) run the Freshest-Authority Check over the whole set FIRST** — rank controlling-spine vs stale/superseded vs thesis-adds, before writing a line (this is what kills staleness — ranking over all layers simultaneously, not adding them one at a time); **(3) author once, integrated, under the thesis lens** (thesis governs throughout; freshest substrate = mechanics; map+gems = preserved detail; every old primitive gets a disposition); **(4) disposition sweep + thesis-doctrine pressure check.** Freshness is established by the up-front FAC, not by the order of adding sources.
...
| Observation | ✅ | ✅ | ✗ | ✅ |
| D6 Commerce | ✅ | ✅ | ✗ | ✅ |
| RBAC #9 | ✅ **drafted 2026-06-01 (native)** | ✅ native | ✅ native | ✅ native (4-way spine; `REV-169` gate; `T0-13`) |
| Settings #10 | ✅ **drafted 2026-06-01 (native)** | ✅ native | ✅ native | ✅ native (anti-junk-drawer; T0-16 floors; universal catalog) |
| Federation #11 | ✅ **drafted 2026-06-01 (native — L1-L4 in one pass)** | ✅ native | ✅ native | ✅ native (owns `REV-157` grant layer; `T0-14`) |
| AI/Model-Lineage #12 (pending core) | ✗ | native | native | native |
| OFC (gated `REV-163`) | ✗ | — | — | — |
| BIZOPS (gated `REV-164`) | ✗ | — | — | — |

**L3 corpus-pull — COMPLETE for the drafted domains (2026-06-01)** (supersedes the `✗` snapshot in the L3 column above). The ~60 gem-rich docs were swept via: cross-cutting proofs (privacy/`REV-169`, rules-templates/`REV-149`) + clusters (**CM/clinical-assertion** 8 docs · **Intake** 4 · **scheduling/D5** ~18 · **commerce/marketing**) + the **pathway/system VALIDATION sweep** (glp1/trt/hrt + system/adversarial/hybrid/module-taxonomy/build-pattern/retrievability). **Verdict: the architecture HOLDS** across every vertical/adversarial/system pressure-test — no structural flaws, no new domains. Contract patches were surgical (privacy send-policy, narrative bridge, assertion-layer sharpening, `care_management_source`); large design docs were correctly recognized as **superseded layers** (scheduling encounter-container) or **decomposed** (marketing ≠ domain) rather than re-pulled; rule/template/pathway content routed to **config-as-code seeds** (`repo/clinical-concepts`, `repo/rules`, `repo/campaigns`), scenarios to **build-validation corpus**. **The `build_pattern_assessment` audit-diminishing-returns signal is now true: further audits would surface edge cases, not architecture changes.** Remaining before build: (a) **untouched-domain native drafts** — Federation → RBAC → Settings → AI (these read their DL-drafts *natively*, §4); (b) **open scope decisions** (trifecta): `REV-163` OFC · `REV-167` tracked-clinical-objects · `REV-149`/`REV-170` rules-templates-campaigns engine · `REV-164` BIZOPS · `REV-171` hybrid `interaction_context` · `REV-141` care_commitment; (c) ratification → Build-OS re-point (`REV-158`) → build reconciliation. The detailed per-domain notes follow.
...
| 11 | **Federation / Operator / Tenant / Topology** | `contracts/federation_contract.md` | ◑ **contract drafted 2026-06-01 (first native draft; ratify-pending)** | DL-21 spine (29 inv) compiled via native FAC-first single-pass; **OWNS the cross-operator grant layer** (`patient_continuity_policy`/`federation_permeability_policy`/`shared_context_grant`/`visibility_grant`/`care_relationship`/break-glass) — closes the consume-before-owned `REV-157` (consumers re-point at ratification); operator-neutrality `T0-14` inv 8; Modes 1-3 Day-0; reconciles ladder-v0 vs cross-org (`REV-143`) |
| 9 | **RBAC / Authority / Attestation / Consent-Gate** | `contracts/rbac_authority_contract.md` | ◑ **contract drafted 2026-06-01 (native; ratify-pending)** | DL-18 spine (25 inv) compiled; extends shipped `lib/auth/capabilities.ts` (seed); the **4-way composition spine** (Federation possibility · RBAC capability · domain commit · CNS-Meta enforce); landed `REV-169` consent-gate (§7) + 4-tier attestation (§6) + break-glass teeth (§6) + cross-brand-compose (§5) |
| 10 | **Settings / Catalog / Registry** | `contracts/settings_catalog_contract.md` | ◑ **contract drafted 2026-06-01 (native; ratify-pending)** | DL-19 spine (31 inv); **anti-junk-drawer rule** (DEFINES shapes, never owns runtime facts); landed `service_policy`/gate definitions (D3 evaluates) + universal `catalog_item` (T0-15) + `pathway_sensitivity` (`SC-SET-MSG-001`) + `federation_permeability_policy` hosting (Federation owns meaning) + the **T0-16 doctrine floors** (structurally un-disableable) |
| 12 | **AI / Model Lineage / Clinical Adoption** | `contracts/ai_model_lineage_contract.md` | ▶ **NEXT (native draft — LAST)** | §9.1 model_version_of_record + §12.8; **producer/consumer of Clinical Memory (6a), NOT its owner**; lands model lineage + the AI-never-final-author-for-safety-sensitive rule + adoption authority; no DL-draft (built from thesis §9.1/§12.8 + AI references across CM/CNS/Messaging/RBAC) |
...
**Fit-check before routing (reasoned, NOT a per-doc form):** before incorporating a gem, judge *whether it belongs*, not just *where* — (a) does it **preserve/extend the core thesis** (substrate≠product · candidate≠commit · AI-bounded · human clinical authority · payload-noun≠domain · projection≠truth), or **CONFLICT** → **stop-and-surface for trifecta** (§1.5 conflict rule); (b) is it **fresh/binding** or **stale/superseded/speculative** → mark it so, do NOT incorporate a hard-won-but-stale concept just because we fought for it; (c) does it change a domain **boundary / canonical object / seam** → update **System Map vNext** thinly too (not just the contract); pure mechanics/invariants/enums → contract only. This is the reasoning that makes the pull *reconciliation*, not *transcription* — surfaced in the per-doc report + the coverage disposition, not as a mandatory table. **Disposition vocabulary:** `incorporated · routed · staged-for-native-domain · stale/superseded · conflicts-thesis→stop · evidence-only · speculative/future · rejected-with-reason`.
...
**Cadence: pause per CLUSTER, not per doc** (per-doc pausing = sludge). Proof points done (privacy/`REV-169` + rules-templates/`REV-149`). Clusters run as batches with a checkpoint at each boundary: CM/clinical-assertion → Intake → scheduling/D5 → commerce/marketing → pathway-slices → system-level → DL-drafts(feed untouched drafts). **Stop immediately mid-cluster on any thesis / System-Map / domain-ownership conflict** (don't batch through a conflict).
...
| **Pathway-slices + system-level VALIDATION sweep (~10 docs):** glp1/trt/female_hrt first-slice · system_pressure_test · adversarial_slice · hybrid_care_delivery_stress_test · system_map_alignment · module_taxonomy · build_pattern_assessment · retrievability · future_blocks · care_management_source | (validation — touches CM/CNS/Identity/rules-engine) | **VALIDATION — architecture HOLDS across every vertical/adversarial/system pressure-test** (glp1 "no foundational gaps"; HRT/system "HOLDS, no structural flaws"). Per Knox: validation, not contract-bulking. **One real contract patch:** `care_management_source` → CM §6.2. Refinements (symptom_cluster_score · external_provider_conflict_disclosure · abnormal_bleeding_workflow · surfacing_tier · ~23 GLP-1 rules + ~25 templates) = **rule-config seeds → rules engine/`repo/rules`** (`REV-149` family), NOT contract text. Scenarios = build-validation corpus. | CM §6.2 (`care_management_source`) · `REV-171` (hybrid `interaction_context`/care_session) | future_blocks → future registry; build_pattern = audit-diminishing-returns signal (we're hitting it); map-change = none |
| **Commerce/marketing cluster:** `marketing_lifecycle_growth_orchestration` · `marketing_system_pressure_test` · `dynamic_behavior_*` (+ commerce = DL-17 already consolidated in D6) | D6 · CNS · (Messaging/Settings/D7/Identity refs) | **commerce verified (D6 mature); marketing DECOMPOSED — NOT a domain (`GRD-026`)** per Knox warning. Campaign orchestration → CNS §9.3; commercial slice (promo/discount/attribution/supplement) → D6 §4/§6/§10; send → Messaging 8-gate; consent/UTM → D7/RBAC (`REV-169`); segmentation = projection; contact-changes → Identity | CNS §9.3 + §13 · D6 §10 + header · `REV-170` (campaign-engine home, tied to `REV-149`) | ~80 templates + 18 campaign types + pressure-test scenarios = build-validation corpus; map-change = none |
| **Scheduling/D5 cluster (~18 docs):** `day_0_scheduling_rule_matrix/*` (Round 1-5 + closure verdict) · `2026-05-17_operating_model` · `_architecture_pressure_test` · `_day_0_build_contract` · `2026-05-16_mindbody_architecture_understanding` | D3 · D5 (+ RBAC/Settings staged) | **mostly already-reflected + verified (HIGH-bar collapse-zone)** — D3/D5 built from the fresher rule-matrix Round-5 layer; **freshest-authority check: the 2026-05-17 `encounter_container`/`encounter_profile` parent framing is SUPERSEDED** (rejected by D5 inv 1/`GRD-003`, not re-pulled); durable concepts already carried; 27+45 workflow scenarios = build-validation corpus | D5 §9 disposition + §13 · D3 §10 + §13 (verification + superseded-layer + scenario-corpus) | **net-new staged:** 4-tier provider attestation → RBAC (DL-18); 11-axis venue substrate → Settings/Federation. map-change = none |
| **Intake cluster (4 docs):** `intake_coherence_pressure_test` · `free_text_intake_pressure_test` · `mode_j_spot_audit` · `intake_construction_audit` | Intake (+ D5/CNS/Observation seams) | **incorporated** — all fit/sharpen, no thesis conflict; net-new safety gems (named freshness profiles incl. pregnancy time-sensitivity, narrative-evidence bridge, progressive-session + Mode-J-Stage-3+, no-update-rewrites-prior-decision); cross-domain placements (care_episode anchor, per-mutation jurisdiction, report_concern) noted as seams to drafted owners | Intake §4 · §5.1 · §6 · §7.1 · §8.13-15 · §11 · §10 dispositions | **map: +2 thin seam rows** (`SC-INT-OBS-001`, `SC-INT-D5-001` — Knox caught "map-change=none" was hiding new edges); other changes were internal mechanics; cross-domain enforcement → D5/CNS/Observation ratification |
| **CM/clinical-assertion cluster (8 docs):** `clinical_assertion_layer_audit` · `_followup_audit` · `_analytics_audit` · `concept_naming_and_assertion_builder` · `authority_vs_longitudinal_confidence` · `acute_states_promotion_threshold` · `lab_authored_by_mapping` · `static_clinical_facts_audit` (+`glp1_concept_registry_analysis` partly-superseded) | Clinical Memory (+ Observation/CNS/D5 seams) | **incorporated** — all fit/sharpen the assertion spine, no thesis conflict; governing rules/objects/invariants pulled, source examples + D1-D7 concept content retained as evidence/config-as-code seed (not transcribed) | CM §5/§5.1 (axes + pattern-vs-authority + rollup) · §6.1-§6.4 (naming · context-namespaces · composite-builder/partial-data · acute-states promotion-threshold) · §8.11-13 · §10 (lab→authored_by mapping) · §11 (rollup view) · §9 dispositions · §13 | `REV-167` (acute-states precedent applies); glp1 renames superseded |
