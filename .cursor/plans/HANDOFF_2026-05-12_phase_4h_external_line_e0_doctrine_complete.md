...
- **Multi-consumer adapter placement (open architectural question with recommended-but-deferred answer).** DL-13 §7.13.13.1 + §5.3(c) + §8.1 clause 29 specify "vendor code confined behind an adapter boundary inside the relevant sibling directory" — unambiguous for single-consumer vendors (Twilio → `external_communications/`) but **silent for multi-consumer vendors**. The Stripe portability sketch ([`.cursor/plans/THOUGHT_EXPERIMENT_2026-05-12_dl13_second_rail_stripe_adapter_sketch.md`](THOUGHT_EXPERIMENT_2026-05-12_dl13_second_rail_stripe_adapter_sketch.md) §11) surfaced the question and recommends **option (c)**: per-sibling business-mapping adapters at `<sibling>/lib/<domain>-adapters/<vendor>/` + thin primitive-level shared core at `lib/<domain>/<vendor>-core/` (raw SDK + webhook signature validation + capability descriptor + vendor wire types). **Concrete instances expected**: Stripe consumed by `billing_subscription/` + future `scheduling_lifecycle/` (deposits / no-show fees) + future `retail_lifecycle/` (in-clinic POS / Stripe Terminal); a future lab vendor consumed by `labs_lifecycle/` + `procedure_lifecycle/` (surgical pathology); future EHR-export consumed by multiple charting sub-domains. **Convention deferred until first multi-consumer activation** — sketch §11 carries the recommended-answer text + the 1-paragraph DL-13 clarification proposed wording; when scheduling-deposit / POS-Terminal / second-lab-vendor work activates, the builder picks up the sketch + applies option (c) + lands the clarification at that time. Foundational §7.13.13.6 carries the deferred-item pointer.
