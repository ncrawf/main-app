...
- **[INV] One `catalog_item` substrate; per-event ownership resolved at order/interaction time** — the *same* moisturizer/supplement/Botox object participates in different tracks without becoming a different object:

| The moisturizer, by context | clinical_owner | commerce track | consent needed |
|---|---|---|---|
| front-desk retail purchase | none | commerce/fulfillment | commercial authorization + privacy; **no clinical consent** |
| provider casually recommends it | care-adjacent recommendation | commerce (separate purchase) | none for the rec; commerce for the buy |
| post-laser aftercare **protocol item** | clinically-linked plan item | + fulfillment | care-plan instruction; procedure aftercare consent if applicable |
| applied **as part of a procedure** | clinical act + occurrence | — | procedure/treatment authority |
| patient buys independently | none (self-care) until adopted | commerce | none |

*Controlling laws:* a conversation/relationship may contain clinical + care-support + operational + commercial + rights + research + informational contributions simultaneously; classification is per-contribution; clinical/operational/commerce/custody/communication ownership coexist without merging; the relationship graph is a **projection over events, not a new truth store** (EVRUN-000004). This is the medspa/HRT wedge reality — and a generic ontology's inability to natively hold *multi-track ownership on one object* is part of the moat (§19).
...
## §5a — Consent / capacity / surrogate / rights / action-specific authority (NOT one boolean; composed + action-scoped + timed)
**[INV] Consent is typed · scoped · purpose-bound · time-bound · revocable · action-specific — artifact in D7, gate in RBAC, specificity in visibility-grants — and does NOT aggregate across operators.** A general-care consent does not authorize Botox, rhinoplasty, recording, AI behavioral analysis, model training, family disclosure, or secondary data use; procedure consent does not authorize all AI/data uses.
...
- **G2 Authority / commit** — *may this become truth/action?* `candidate ≠ commit → authorized_action → owning-domain commit` (unchanged canon).
...
- **[INV] Ratification scale target (a GATE, not a now-deliverable; do not vanity-count):** ~**25–50 deeply-reviewed golden traces** → **100+ curated seed cases** → **1,000+ generated/adversarial variants** → eventual **10,000+ continuous regression traces** as the platform matures. **Coverage + proof, not row count.** The corpus lives in the pressure-test/eval system (C3.5 harness lineage), referenced here — not inlined. **Spine/thesis can proceed on the model NOW; contract ratification waits on this corpus.**

## §22 — EXECUTED golden scenarios (walked; patient-visible STATE + next obligation, not "closure")
