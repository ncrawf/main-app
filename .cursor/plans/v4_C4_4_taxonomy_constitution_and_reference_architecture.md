# v4 — C4.4 Knowledge Reservoirs & Source-Estate — TAXONOMY + CONSTITUTION (G2) · REFERENCE ARCHITECTURE (G3 — ACCEPTED, Nick + Knox 2026-08-01)
...
Status: `g2_CLOSED_2026-07-22 · nick_operator_accepted · knox_verified · G1_CLOSED · G3_ACCEPTED_Nick_Knox_2026-08-01 · analysis_nonbinding · not_promoted` — **★ G3 §R ACCEPTED 2026-08-01 (Nick operator acceptance + Knox independent remote-byte verification; Knox ruling `G3_ACCEPTED`; see §20.5 acceptance receipt).** The path was: authored → Knox review (PASS-WITH-REQUIRED-AMENDMENTS) → all 5 amendment groups + 2 normalization cleanups applied (§20.4) → Knox verified the exact pushed bytes → **accepted.** G2 body authored in a FRESH Opus agent booted from `v4_C4_4_G2_kickoff_handoff.md` (Knox G2 gate patches + final consistency + micro-close applied, §19 ledger); §R **populated at G3** by a fresh Opus agent booted from `HANDOFF_2026-07-31_c4-4-g2-landed_pre-g3.md` on branch `analysis/c4-4-g3-reference-architecture` off `origin/main a87d305`, consuming the closed G1 map + the accepted Agent-Runtime consumption bridge + frontier depth; does NOT re-derive and does NOT reopen G2. **Acceptance is within the `analysis_nonbinding` arc — nothing promoted; NOT merged to `main`; outer checkpoint #15 UNCHANGED. G4 (frozen adversary + rubric) is now OPENED for FREEZE/PREPARATION only — the G4 fixture+rubric carrier is `v4_C4_4_fixture_suite_and_adversarial_results.md`; Gemini has NOT run and G4 is NOT executed/scored.**
...
- **This document now carries both G2 and G3 (★ updated 2026-08-01).** **§§1–19 are the CLOSED G2 constitutional body** (taxonomy + constitutional boundaries + Evidence-lane reconciliation + fixture classification, preserved verbatim); **§R + §20 carry the AUTHORED + Knox-amended G3 layer** (reference architecture + completion ledger + amendment receipt). Both are `analysis_nonbinding`; they bind no thesis/contract/schema/security/surface. *(The prior "This is G2 only" wording was authoring-time and is superseded — G3 has since been authorized, authored, and amended.)*
...
> **★ G3 status:** **ACCEPTED 2026-08-01 (Nick + Knox; Knox ruling `G3_ACCEPTED` after independent remote-byte verification — see §20.5).** Authored on `analysis/c4-4-g3-reference-architecture` (off `origin/main a87d305`), Knox-reviewed (PASS-WITH-REQUIRED-AMENDMENTS), amended (Amendments 1–5, §20.4) + normalization-cleaned, then accepted. `analysis_nonbinding` · not promoted · NOT merged to `main` · outer checkpoint #15 UNCHANGED. **G4 is now OPENED for FREEZE/PREPARATION only** (frozen fixtures + rubric live in `v4_C4_4_fixture_suite_and_adversarial_results.md`; Gemini adversary NOT run; G4 NOT scored/executed). §R is built **on top of** the accepted G2 constitution (§§1–19) and does **not** reopen it. Reading order into §R: **§R.0 orientation → R.1…R.17 → §R.CORPUS (the load-bearing D7/corpus-parent verdict) → §R.FIX (F0/F-Self/F-Inv design traces) → §20 (G3 completion ledger + amendment receipt + acceptance receipt).**
>
...
> **★ Amendment-1 bounded G3 clarification (fixture-routing only; does NOT reopen the six-class constitution).** This corrected F-Self routing **supersedes the earlier G2 §14 F-Self classification** that placed OMNI's catalog/read-graph as S5 and promoted decisions/guardrails as S2. The correction is scoped to *how the F-Self fixture routes* — the six classes themselves are unchanged. **Constitutional resolution used here:** OMNI's Architecture Memory Control Plane is **S3 (Domain-Owned State) where the owning domain is the control plane itself** — committed, authoritative-within-scope governance truth. This is deliberately **NOT** a new seventh class and **NOT** a "P0 class" inside the six (P0 is a *plane*, not one of the six constitutional classes; minting one would be the back-door god-class G3 must avoid). **Bounded-deferred (owner + trigger):** whether the constitution should carry an *explicit* governance-control-plane role or continue to treat it as S3-of-the-governance-domain → **spine §8** (constitution ratification), NOT G3. **Knox linguistic caution carried forward (G4/G5 pressure point, not a G3 reopening):** spine §8 should phrase this as **"committed owner-state *governed by* the control plane,"** NOT as the control plane being a normal product domain — the S3-*role* is a governance analogue, not a claim that the Architecture Memory Control Plane is an ordinary care/business domain.
...
**★ Post-amendment normalization cleanups (2026-08-01, from Knox's amended-artifact review — "PASS with two tiny normalization cleanups"):** *(a)* the live §0 "This is G2 only" line was internally contradictory (the same §0 says G3 is authored+amended) → rewritten to "§§1–19 = closed G2 body; §R + §20 = authored+amended G3 layer"; *(b)* the G2 §14 F-Self prose still showed the stale S5/S2 mapping earlier in the file → an **inline supersession pointer** was added at §14 ("G3 fixture-routing clarification supersedes this F-Self mapping; see §R.FIX Amendment-1; the six-class constitution remains unchanged") — G2 body NOT rewritten. Also carried Knox's linguistic caution ("committed owner-state *governed by* the control plane," not a normal product domain) into the spine §8 deferral note. These are normalization, not architecture reopenings.

- **Amendment stop declaration:** Knox-required G3 amendments + the two normalization cleanups applied to their gate. STOPPED for Knox patch-verification + Nick + Knox G3 acceptance. G4 remains closed.

### §20.5 G3 acceptance receipt (2026-08-01 — Nick + Knox)

**Ruling:** **C4.4 G3 = ACCEPTED** (Nick operator acceptance + Knox `G3_ACCEPTED` after independent verification of the exact pushed remote bytes). Acceptance is within the `analysis_nonbinding` arc: **nothing promoted · NOT merged to `main` · outer checkpoint #15 UNCHANGED.**

- **Accepted architecture content blob:** `e364acbad3352457eb8c761d287e91787ea71eea` (this artifact at the reviewed content commit `6c86e2e`; branch `analysis/c4-4-g3-reference-architecture`, baseline `main a87d305`). Later branch-tip commits are docs/status stamps only and do not change this architecture blob.
- **Knox remote-byte verification (recorded):** branch is a clean linear series above `a87d305` changing exactly the six authorized G3 files; `main` remains `a87d305`; G1 byte-identical to `main`; AGENTS byte-identical to `main`; accepted Runtime bridge blob remains exactly `de5b9a1fc7bf9ff77797d28a53a6fef9ced3ed34`.
- **All five amendment groups verified present** (§20.4 matrix): (1) governance control-plane state = committed S3-role, NOT S5 projection; (2) blob/artifact/receipt/evidence-independence four-identity split; (3) federated revocation split (grant-revoke / withdrawal / lawful-deletion / recipient-local-reconsideration) + recipient sovereignty + bidirectional lawful-retention exception + profile A/B; (4) closed-package receipt-closure immutability; (5) live-state normalization + R.17 truth qualifier + named S1 custody-governance commit role.
- **Both post-amendment normalization cleanups verified present** (§20.4): the §0 "G2-only" contradiction fixed; the inline G2 §14 F-Self supersession pointer added.
- **P3 D7/Source-Estate corpus verdict:** ACCEPTED (§R.CORPUS) — D7 owns subject-agnostic per-artifact custody/integrity/visibility; the S1 Source-Estate corpus layer owns package/corpus identity, completeness, quarantine, closure, reprocessing, and received-but-unadopted truth; membership by reference; subject resolution independent of membership; corpus schema/placement deferred to C5.
- **Six-class constitution preserved** (§§1–19 not reopened; Amendment-1 superseded only the G2 §14 F-Self *fixture routing*).
- **Knox linguistic caution carried (not a reopening):** spine §8 should phrase the control-plane authority boundary as "committed owner-state *governed by* the control plane," NOT as an ordinary product domain (bounded-deferred, §R.FIX Amendment-1 note).
- **Gate transition:** **G4 OPENED for FREEZE/PREPARATION only.** The G4 fixture suite + Knox rubric are frozen in `v4_C4_4_fixture_suite_and_adversarial_results.md` (`G4_FIXTURES_AND_RUBRIC_FROZEN · Gemini_not_started · Knox_adjudication_not_started`). **G4 is NOT executed here** — Gemini adversary submission and Knox adjudication/scoring are downstream; Opus does not self-score OMNI and does not begin G5.
- **Acceptance stop declaration:** G3 accepted; G4 frozen (not executed). STOPPED for Knox freeze-verification → fresh repo-connected Gemini adversary → Knox adjudication → G5 disposition.
