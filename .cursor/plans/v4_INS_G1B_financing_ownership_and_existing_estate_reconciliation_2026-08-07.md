...
Status: `gate_1b_content_accepted_pending_cross_arc_carry · r7_byte_check_pending · not_promoted · not_integrated · no_contract_mutation · no_implementation · no_shared_surface_written · nothing_renamed`
...
| **—** | Reviewer claim that §0.3 still omits the three R5 sources | **NOT A DEFECT — already present at R6** | The maturity-labelled source table landed in the R6 commit (`c8e7fb3`) and is live. Verified by direct grep; **no change made.** Recorded because a correction applied to an already-correct surface is itself a defect |
...
**★ R5/R6 ADDITION — the three sources that changed R5's answers, with their maturity stated (a strict source posture must carry the reads that produced C39, A-Q16, A-Q17 and A-Q18):**

| Source | Depth read | Maturity — **none of these is settled architecture** |
|---|---|---|
| **`v4_C3_6C_source_authority_truth_plane_map.md`** | §1 three claims + §2 `source_authority_map` per-fact schema in full; §3/§4 tables consulted | **`analysis_nonbinding` · `populated_G1_pending_review` (2026-06-14)**; the artifact calls `source_authority_map` a **candidate net-new spine** composed from existing primitives. Produced **C39** and **A-Q16** |
| **`v4_C4_6…` §9 Sourcing Selection** + `§11` ownership row + `EVRUN-2026-000012 _02 §15.3` / `_07 §7.3 L7` | §9 selection-output passage in full; §11 row; the distinct-lifecycle and Vendor-Loop-rejection anchors | **accepted L2 build doctrine** — the strongest-status of the three. Produced the **§8.2 Sourcing row** and **A-Q18** |
| **`v4_C4_5…` `T-21`** (no false total order) + `T-13`, `T-17` | the `T-21` passage verbatim; `T-13`/`T-17` at cited lines | **`gate_0_charter_accepted` · `full_pass_not_started`** — charter law, **no pass has run**. Produced **A-Q17** |
...
| **This commit (R6)** | one **bounded propagation / byte-consistency** commit. **Two-axis verdict, F1–F7, the economic-firewall three-law conclusion, both fixtures, Task-D posture and the Gate-2 name all UNCHANGED. No new law, domain, owner, graph, fixture, negotiation object or comparator *result* minted** — output 14's new families are routed questions, not findings |
...
| **Correction receipt** | §0.5 — **55 items** (C1–C15 R1, C16–C25 R2, C26–C28 R3, C29–C35 R4, C36–C44 R5, C45–C49 R6, **C50–C55 R7**), each with its adjudicating primary source |
| **Cross-arc carry** | **§15.1** — ledger-first mechanism, four tiers, two hard constraints, integrator-owned on a separate branch. **This lane cannot land it** |
...
### §15.1 The cross-arc carry transaction — mechanism, and why "add a note to each arc" is the wrong one *(R6)*
...
> **★ R7 — THE MEASUREMENT, AND WHAT IT ACTUALLY SHOWS.** At `d592e40`: **`future_work_registry.md` holds 17 rows, not hundreds.** **`08_open_review_queue.md` holds 212 unique `REV-` rows of which 83 are CLOSED — a ~39% closure rate.** And this arc is direct evidence the surfaces work: **`REV-185` produced the economic-firewall recovery, `REV-184` the governed-resolution lifecycle, `REV-160` the financing-shape question, `FWREG-009` prevented the GRR/Accountability naming error, and `REV-212` was closed by the immediately preceding lane.** **The ledger is not a graveyard. The graveyard hypothesis is falsified by count and by closure rate.**
>
> **But `DEC-027` still failed — and the measurement isolates why.** It was **not** a discovery failure: this arc found it in one pass. It was **not** a row-quality failure: it recorded the full field list and was Nick+Knox-modified. **It failed because no downstream artifact carried an obligation to consume it.** The proof is that **`REV-160`, the row that owns the exact question `DEC-027` answered, still reads "tentative"** — the answer and the open question have coexisted in the same boot-loaded ledger for months without either being reconciled against the other.
>
> **THE LAW: rows are excellent at preserving and surfacing state, and have no mechanism whatsoever for obligating consumption.** Payload, state, discovery and backpressure are all **supply**. Nothing in the estate's current carry vocabulary creates **demand**. That is the missing fifth part, and it is why the operator's instinct — *"the next major author will be looking at the arc, not the registry"* — is **correct about where consumption occurs** even though appending to a locked arc is the wrong vehicle.
>
> **Candidate guardrail, routed not executed:** *a decision, finding or recovery with no named consumer and no consuming gate is **preserved but not carried**, and will be re-derived.* Home would be `06_guardrail_antipattern_digest.md` alongside `D0CKPT-GRD-001`; the `DEC-027` → `REV-160` pair is its worked example. **This carrier proposes it and does not mint it** — build-law change is not this lane's authority, and the operator has not authorized a build-doctrine pass.
...
| **D — BOUNDED BACKPRESSURE** | direct notes **only** into surfaces that are open or designed for inbound state: **C3.6C** (`populated_G1_pending_review`) · **C4.5 next-actions** (`full_pass_not_started`) · the **Accountability `GRR-TASKD-INPUT` Input-State Receipt** · the **Care Response-Seam Audit** when opened. Each = question ID + route + maturity, nothing more | pressure returned to the supplying arc | **C4.6 (accepted L2 doctrine) and REV-184 (signed off) get NO body edit** — they are carried at A/B/C only |
| **E — CONSUMPTION OBLIGATION ★** | each carried finding **names the artifact that must dispose of it and the gate that cannot pass without doing so** — e.g. *the C5 D6 contract pass cannot close without disposing of `REV-160`+`DEC-027`; the Gate-2 brief cannot pass acceptance without disposing of A-Q16/17/18; the spine pass cannot close without disposing of row P's economics elevation* | **demand, not supply** | **`DEC-027` had A, B, C and D and still evaporated.** Parts A–D make a finding *available*; only E makes some named author's gate **fail** if they ignore it. The estate already enforces this pattern via **AWP §9 stop reports and gate acceptance criteria** — C4.4's G4 ran 30/32 — so this reuses existing machinery rather than inventing a mechanism |
...
**The one-sentence law:** *A row owns unresolved state. The Gate-2 brief owns the payload. The read graph owns discovery. Bounded inbound pointers return pressure to open source arcs. **A named consuming gate owns the obligation to absorb it.** None is sufficient alone, and the fifth is the one `DEC-027` lacked.*
