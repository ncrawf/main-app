...
Status: `gate_1b_analysis_reconciled_r4 · pending_nick_knox_review · not_promoted · not_integrated · no_contract_mutation · no_implementation · no_shared_surface_written · nothing_renamed`
...
| **C35** | Knox's *"the architecture is gravitational because each participant can join without surrendering its truth authority"* | **ACCEPTED AS HYPOTHESIS, RECORDED AS FALSIFIER** | **The neutrality that makes OMNI trustworthy to patients and providers is exactly what makes it unattractive to the party with the most leverage.** A payer has no commercial incentive to join a system designed to stop it presenting funding preference as clinical merit. Collides with Gate-0 **U11** and the moat doctrine's four unproven N=1 bootstrap mechanisms. Added as a Gate-2 kill condition and a Task-D falsifier, **not asserted as gravity** (§10.B.10) |
...
**(1) The seam-kind taxonomy is the missing typing mechanism, and it already exists.** The Accountability/GRR capture §1 defines, "**identical in both captures**": `seam_kind = custody_handoff | work_trigger | control_request | evidence_publication | consequence_notification`, with the rule that **"only `custody_handoff` transfers custody of the source work; the others create linked work / request an action / publish evidence-or-consequence while the originating owner keeps its own obligation."** Applied to this fixture, every move in the interaction types cleanly and **none of them transfers custody of the clinical decision**:

| Move | `seam_kind` | Custody of the care decision |
|---|---|---|
| Payer: "not covered / we fund B" | **`consequence_notification`** | **stays with Care** |
| Payer/PBM: prior-auth or step-therapy requirement | **`control_request`** | **stays with Care** |
| Pharmacy: "no such strength — here is what we suggest" | **`work_trigger`** (routes to `reopen_care`) | **stays with Care** |
| Pharmacy: permitted substitution inside its envelope | **`control_request`** within a pre-authorized scope | stays with Care; the substitution's **authority basis** is recorded |
| Pharmacy: dispense event, tracking, stock assertion | **`evidence_publication`** | stays with Care |
| Clinician: authorized recommit → new executable instruction | **`custody_handoff`** of the *executable instruction* only | **the care decision remains Care's; execution custody moves** |
| Patient: preference, refusal, appeal instruction, consent | **`evidence_publication`** + a patient-plane commit | stays with Care |

**This is why the interaction cannot be modelled as a negotiation object:** four of the five seam kinds explicitly *do not* move custody, and the one that does moves only the executable instruction. A negotiation object would have to hold a custody it never receives.

**(2) The authority asymmetry between pharmacy and payer is structural, and it maps onto machinery that already exists.** Knox is right that a payer is not just another pharmacy; the consequence is sharper than stated. A **pharmacy holds execution authority and bounded professional authority** — it can act, it can substitute within scope, and it bears professional consequence for its judgment. A **payer holds no execution authority over care at all**; its only lever is funding, and it bears **no clinical liability for the outcome of the alternative it prefers.** That asymmetry has an existing home: **Clinical Memory §5's eight-rank `authority_rank` precedence**, where `authored_by` is **locked at write time**. A pharmacist's clinical judgment enters as a clinical assertion with a real professional rank. **A payer's "B is equivalent" claim is not a clinical assertion at all** — it is a policy position held as `externally_committed_truth` under GCE, and it must carry **no clinical authority rank**. Clinical Memory §5.1's rule is the exact guard: repeated low-authority input *"may only boost `priority_hint`"* and **"never promote `status`, never raise `authored_by`/`authority_rank`, never satisfy an authority floor."** *A payer may raise attention. It may never raise clinical authority.*
...
### §10.B.9 2035 — the failure mode is proposal volume, and the machinery for it already exists

When every principal has an agent, the *count* of proposals rises by orders of magnitude while authority stays fixed. **The pattern survives only because proposal is cheap and commit is expensive** — the estate's `candidate ≠ commit` law. The novel attack is therefore not authority capture but **coercion by exhaustion**: a payer agent issuing hundreds of alternative suggestions per prescription can degrade care by consuming clinician attention without ever exceeding its authority.

**That is an admission-security problem, and the machinery is already written.** Accountability §5a's `[INV] Admission-security law` permits **throttle · quarantine · correlate · challenge-for-more-evidence · restrict the processing path · route to a safe review tier for weak-identity or high-volume/coordinated signals**, while forbidding silent erasure *"merely because identity is weak or volume is high,"* treating a manipulation cluster as **a correlation decision, not a delete**, and requiring that **"urgent safety/mandated-report signals must survive rate controls."** Combined with §5's originator-identity axis (`automated_detector` and `external_agent` are typed first-class, and an `external_agent`'s claim-to-represent needs **delegation proof**) and `GRD-039` hostile-by-default, the 2035 volume problem is a **rate-and-correlation** problem inside existing doctrine — **not a new negotiation subsystem.** Recorded as a Gate-2 trace concern; **no new mechanism proposed.**

**One import worth naming (mechanism only, per Lens B).** Gate-0 §G.2 already imported **AWS sagas with design-time compensating transactions — "every forward step declares its inverse before it ships."** Applied here: a **proposal should declare its compensation path at proposal time.** A payer-driven substitution that later fails has a real inverse — `reopen_care`, re-prescribe, possibly harm remediation and an Accountability duty — and outcome 10 of §10.B.5 is exactly that path. Declaring it up front is the difference between a governed alternative and an unpriced one. **Mechanism imported; no new object.**

### §10.B.10 Why no incumbent can hold this, and the falsifier that could kill the claim

**The structural position, stated precisely.** Every incumbent in this interaction **is a party to it**: Epic is the provider's system, the payer portal is the payer's, the PBM's system is the PBM's, the lender's is the lender's, the clearinghouse carries one party's message. Palantir can model the entire resolution — but it cannot be *a party to none of them*, because its revenue comes from one of them (Gate-0 §I.2a / the moat doctrine §12 "durable asymmetry"). **OMNI's candidate position is being the only participant that is party to none while carrying the lineage for all — and that is a neutrality property, not a data-modelling property.** Which yields the sharpest formulation this arc has produced:

> **The moat and the firewall are the same thing.** `T0-14` operator-neutrality and §9 Law 2 are not ethical decoration on top of a commercial strategy — they *are* the commercial strategy. **The moment OMNI's economics ride on the outcome of a resolution, the structural position that makes it trustworthy to every other party is gone**, and OMNI becomes one more interested system modelling the others.

**And the falsifier, which is the strongest attack on the whole positive architecture.** **The neutrality that makes OMNI trustworthy to patients and providers is precisely what makes it unattractive to the party with the most leverage.** A payer has no commercial incentive to join a system whose explicit purpose is preventing it from presenting its funding preference as clinical equivalence. Knox's *"the architecture is gravitational because each participant can join without surrendering its truth authority"* is a **hypothesis, not a finding** — and it collides directly with Gate-0's **U11** (does the longitudinal continuity have a buyer?) and with the moat doctrine's **four N=1 bootstrap mechanisms**, none of which is proven. **Recorded as a falsifier at §11 and a Gate-2 kill condition, not asserted as gravity.**
...
**A-Q15 (new, R4)** **three graphs are in play and must be deduped, not multiplied** — REV-184's CNS-owned resolution graph · Accountability §6b's derived `matter_graph` over persisted correlation truth · the Care capture's `resolution_participant_graph` (projection, NOT truth). This carrier names **no fourth graph** (§10.B.8 item 3). The dedup belongs to Gate 2 / C5, not here, and it is the graph-layer instance of the seven-labels problem at §9.1.
...
| **Correction receipt** | §0.5 — **35 items** (C1–C15 R1, C16–C25 R2, C26–C28 R3, **C29–C35 R4**), each with its adjudicating primary source |
...
| **Unresolved** | A-Q1a/1b, A-Q2, A-Q4–A-Q13, **A-Q14a/b/c, A-Q15** · B-Q1–B-Q5 · C · D |
...
**Gate-1b readiness (R4).** The two-axis verdict, the F1–F7 dispositions, the three-decomposition adjudication, the agreement and commitment dispositions, the three-surface implementation reconciliation, the economic-firewall recovery, both named fixtures, the residual and future-regime matrices, the Task-D boundary and the durability map are complete and internally propagated. **Gate 1b is ready to close.** Everything remaining is either (a) integrator routing this lane cannot perform, or (b) explicitly routed to `INS-G2-OPERATING-COMPOSITION-AND-SUFFICIENCY`, OPECON-G0, C5 or their own owners. **No further Gate-1b pass is owed.**

**STOP: `review_ready_pending_nick_knox_gate1b_final`**
