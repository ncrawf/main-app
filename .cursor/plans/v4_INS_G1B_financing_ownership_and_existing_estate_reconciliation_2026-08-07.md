...
Status: `gate_1b_analysis_accepted_pending_cross_arc_carry · not_promoted · not_integrated · no_contract_mutation · no_implementation · no_shared_surface_written · nothing_renamed`
...
> **★ REVISION HISTORY.** **R0 (`8c9d818`)** was authored before four commissioned reconnaissance reports returned and before four load-bearing primary sources were read. **R1 (`59cc477`)** applied Knox's first full-carrier byte review: eight blocking defects, four of them factual errors of mine, all verified and retracted (§0.5). **R2 (`1295e5f`)** applied Knox's second full-carrier review, which passed the factual reconciliation but **failed architectural sufficiency**: the carrier was turning *"no shared truth owner"* into *"no shared financing architecture,"* and under-modelled the positive thing OMNI must become. R2 splits the verdict onto **two axes**, decomposes F3/F4 by **actor capacity** rather than operator-only, restores **derived ≠ ownerless**, replaces the coverage-scope **binary with four layers**, splits **custody from substantive commitment**, makes Accountability admission **threshold-based rather than noun-based**, adds a **future-regime stress matrix**, rewrites the positive architecture as a **governed continuity function**, and — on operator recovery instruction — **recovers the economic-firewall lineage rather than minting an eighth name for it** (§9). **R3 (`34cabe4`)** was a bounded boundary addendum: it refined Law 1's scope, broadened Law 2 to any economically interested principal, added `NICK-FIXTURE-PAYER-CONSTRAINED-THERAPY-01`, added the operating-profile anti-shadow criteria to Gate 2, and — on operator instruction — added **§15, the anti-evaporation durability map**. **R4 (`7a0052e`)** identified the fixture as an instance of C4.6's existing typed multi-principal resolution protocol, corrected A-Q14 from an owner-level over-claim into A-Q14a/b/c, and added the graph-dedup and neutrality material. **R5 (`292951b`)** was the fidelity pass: it withdrew nine R4 overclaims — four of which contradicted statements already in this carrier — demoted `seam_kind` to one of five typing axes, replaced `authority_rank` with the recovered **field-level `source_authority_map`**, reframed exhaustion as **current 2026 physics** with leverage decomposed from authority, corrected the saga import against F6, reduced neutrality to a **six-dimension deployment-specific control objective**, reframed the graph question as role reconciliation, added the missing **Sourcing selection** residual row, and added **§16, the under-pressured-arrivals register**. **R6 (`c8e7fb3`)** was a **propagation and byte-consistency pass — no conceptual change. The two-axis verdict, F1–F7, the economic-firewall three-law conclusion, both fixtures, the Task-D posture and the Gate-2 name are all UNCHANGED.** R5 corrected several claims in the sections that introduced them but left **the withdrawn versions live elsewhere in the same file** — most seriously in **Gate-2 output 12, which is the execution contract the next agent follows and still instructed them to type every move by `seam_kind` and to "dedup" the graphs.** R6 removes those residues (C45–C48), carries the three R5 sources into **§0.3** with their true maturity, renumbers the Gate-2 outputs, rewrites the stale **§15** rows and adds five new ones, adds the **military/joint-C2 and civil-aviation comparator families** to output 14 with three explicit transfer limits (operator directive), and adds **§15.1**, the cross-arc carry transaction. **R7 (`cf029f5`)** was a byte and durability pass (C50–C55): it eliminates the last three residues of retracted wording — *"graph dedup"* at §14 E13, *"the cross-role authority model"* at §16 (an R6 paired edit that failed silently), and *"one Tier-2 route"* at §13.2 — corrects *"preserved nothing"* to the accurate **preserved-but-not-carried**, retitles §0.5, and sets the status to **content-accepted-pending-cross-arc-carry** rather than closed. **Its substantive contribution is §15.1's rewrite: the estate was measured rather than argued about, the strong graveyard claim was falsified (17 FWREG rows; 212 `REV-` rows at ~39% closure; this arc built on four of them), and the carry was rebuilt as a five-part loop.** **R8 (this) is the final byte and durability-contract correction (C56–C62).** Its most important item is **C56: R7 rejected a reviewer correction as "already present" on the basis of a presence-only `grep`, and the rejection was false** — the source table had been mis-anchored into §4.4 two revisions earlier, so §0.3 genuinely lacked C3.6C, C4.6 §9 and the `T-21`-as-A-Q17 use. R8 relocates it, removes the duplicate, and **retracts the rejection.** It also narrows the measurement conclusion (**a 39% closure rate means the queue is not inert, not that orphaned consumption is solved**), withdraws *"rows have no mechanism whatsoever"* after finding that **AWP §6 already mandates disposition from a fixed enum**, withdraws the untraced claim that `DEC-027` lacked only consumption, **rebinds part E to that existing enum rather than minting an eighth vocabulary**, demotes the proposed guardrail pending dedup against `GRD-043`, and fixes the §14/§11/§15 residues. **R5–R8's dominant failure was propagation, not architecture — and §0.5 now names why.** **R9 (this) is the final bounded semantic-propagation and receipt patch — not an architecture round:** it corrects A-Q14a onto the payer-policy vs payer-employed-clinician **capacity** distinction (retiring the R4 `priority_hint` handling rule C39 had already withdrawn), rebinds part E's disposition to **each owning state surface's own lifecycle vocabulary** instead of AWP §6's future-work enum, and repoints the receipts, sweep and termini — with the **two-axis verdict, F1–F7, both fixtures, the three firewall laws, the implementation findings, the residual matrix, the comparator families, the Task-D two-level maturity, the Gate-2 name and the ownership decomposition all untouched.**
...
**★ R8 — THE THREE R5-RECOVERY SOURCES, WITH MATURITY. These belong in §0.3 and were not here until now.** At R6 this table was written into **§4.4 by mis-anchored edit**; at R7 a presence-only `grep` returned 1 and the reviewer's correction was **wrongly rejected**. The reads were real; the disclosure was in the wrong section for two revisions. **None of these three is settled architecture:**

| Source | Depth actually read | Maturity |
|---|---|---|
| **`v4_C3_6C_source_authority_truth_plane_map.md`** | **§1** (three claims) + **§2** (`source_authority_map` per-fact schema) read in full; §3/§4 tables **consulted** | **`analysis_nonbinding` · `populated_G1_pending_review` (2026-06-14)** · the artifact calls `source_authority_map` a **candidate net-new spine** composed from existing primitives. **NOT settled cross-role authority architecture.** Produced **C39** and **A-Q16** |
| **`v4_C4_6…` §9 Sourcing Selection** (+ `§11` ownership row, `EVRUN-2026-000012 _02 §15.3` / `_07 §7.3` L7) | §9 selection-output passage **read in full**; §11 row and the distinct-lifecycle / Vendor-Loop-rejection anchors **consulted** | **accepted L2 build doctrine, `analysis_nonbinding` authority** — the strongest-status of the three. Produced the **§8.2 Sourcing row** and **A-Q18** |
| **`v4_C4_5…` `T-21`** no-false-total-order (+ `T-13`, `T-17`) | the **`T-21` passage verbatim**; `T-13`/`T-17` **consulted** at cited lines. *§§0–6 were read at R2 for the temporal axis generally; the specific `T-21`-as-A-Q17-basis use is R5's* | **`gate_0_charter_accepted` · `full_pass_not_started`** — charter law, **no pass has run**. Produced **A-Q17** |
...
**R4 corrections (this pass).**

| # | R3 claim | Status | Basis |
|---|---|---|---|
| **C29** | A-Q14: *"the clinical-equivalence assertion has no located home"* | **WITHDRAWN as an owner-level claim; SPLIT into A-Q14a/b/c** | The owner split is visible and largely inherited: payer position under GCE without clinical adoption · Clinical Memory for professional assertions with `authored_by` and `authority_rank` locked at write · REV-184/CNS resolution · Care recommit · D7 artifacts. **What is uncontracted is the relation-shaped representation and the recommit seam.** *Self-finding: R3 cited Accountability §19's `governed_relation_assertion` — the estate's explicit do-NOT-mint hinge for relation-shaped assertions — in two places and still failed to connect it to A-Q14* |
| **C30** | The fixture presented as an interaction the carrier had newly assembled | **IDENTIFIED AS AN INSTANCE OF AN EXISTING PATTERN** | It is C4.6 §4's **`typed multi-principal resolution`** composed with GCE + Clinical Memory + REV-184/CNS + D7 lineage + OFC + D6 + conditional Accountability. **Pharmacy discovered the pattern; Insurance adds a principal/capacity plane and financing consequences, not new resolution physics.** Pharmacy solved it at **L2 doctrine/conformance** altitude only — not at contract/build (§10.B.7) |
| **C31** | No typing mechanism was named for the moves in the interaction | **SEAM-KIND TAXONOMY RECOVERED (not minted)** | Accountability §1 already defines `seam_kind = custody_handoff \| work_trigger \| control_request \| evidence_publication \| consequence_notification`, "identical in both captures," with **only `custody_handoff` transferring custody** — and it is **LOCKED as an invariant** at §23. Applied at §10.B.8 item 1: **four of five kinds do not move custody, and the one that does moves only the executable instruction.** This is why a negotiation object cannot model the interaction — it would hold a custody it never receives |
| **C32** | Payer and pharmacy treated as symmetric participants in the pattern | **AUTHORITY ASYMMETRY MADE STRUCTURAL** | A pharmacy holds **execution authority + bounded professional authority + professional consequence**; a payer holds **funding leverage and no clinical liability for the alternative it prefers**. Existing home: Clinical Memory §5's eight-rank `authority_rank` and §5.1's rule that low-authority input *"may only boost `priority_hint`"* and **never raise `authority_rank` or satisfy an authority floor**. **A payer may raise attention; it may never raise clinical authority** (§10.B.8 item 2) |
| **C33** | Nothing guarded against describing the interaction as a new "resolution graph" | **THREE-GRAPH DEDUP OBLIGATION ADDED (A-Q15)** | REV-184's CNS resolution graph · Accountability `matter_graph` · Care `resolution_participant_graph`. **This carrier names no fourth graph**; the dedup is a named Gate-2 failure condition. It is the graph-layer instance of the seven-labels problem at §9.1 |
| **C34** | The 2035 scale risk was framed as agent-mediated negotiation | **REFRAMED AS PROPOSAL VOLUME, WITH EXISTING MACHINERY** | The attack is **coercion by exhaustion**, not authority capture — and it is an **admission-security** problem already written: Accountability §5a permits throttle/quarantine/correlate/challenge, forbids silent erasure on volume grounds, treats manipulation clusters as correlation decisions, and requires urgent safety signals to survive rate controls; §5 types `automated_detector`/`external_agent` with delegation proof; `GRD-039` is hostile-by-default. **No new mechanism proposed** (§10.B.9) |
| **C35** | Knox's *"the architecture is gravitational because each participant can join without surrendering its truth authority"* | **ACCEPTED AS HYPOTHESIS, RECORDED AS FALSIFIER** | **The neutrality that makes OMNI trustworthy to patients and providers is exactly what makes it unattractive to the party with the most leverage.** Collides with Gate-0 **U11** and the moat doctrine's four unproven N=1 bootstrap mechanisms (§10.B.10) |

**R5 corrections (this pass).** *Four of the seven R4 items below are cases where R4 contradicted something already stated in this same carrier — a pattern worth naming: **R4 introduced overclaims while correcting overclaims.***

| # | R4 claim | Status | Basis |
|---|---|---|---|
| **C36** | `seam_kind` as "the missing universal typing mechanism" | **DEMOTED to a candidate cross-loop effect axis** | The taxonomy is `analysis_nonbinding`, defined for Care–Platform–Accountability loop boundaries, in a capture that is **not closed**. Typing is **five orthogonal axes** with **C4.6's three planes primary** (§10.B.8 item 1) |
| **C37** | "clinician recommit = `custody_handoff`" | **WITHDRAWN — self-contradiction** | C4.6: **submission ≠ acceptance**; execution custody transfers only on acceptance evidence + owning-domain commit. **The carrier cites `O10 ACK ≠ accepted-custody` and R4 contradicted it** |
| **C38** | "pharmacy substitution inside its envelope = `control_request`" | **WITHDRAWN** | That is a **professional/execution commit**. It is a request only when the pharmacy asks another authority to decide |
| **C39** | "a payer's claim carries no clinical authority rank / may only raise `priority_hint` / bears no clinical liability" | **ALL THREE WITHDRAWN** | Capacity, not entity: **a licensed clinician acting for a payer may make an attributed professional assertion** — which still acquires no treating relationship, prescribing or plan-commit authority. "No clinical liability" is a **legal conclusion this gate did not establish**; replaced with *the payer does not own the treating-clinical commit or its professional responsibility.* **And the mechanism was wrong: `authority_rank` is a "read-view tie-break, locked", not an authority model** — the estate's model is **field-level `source_authority_map`** (C3.6C), recovered at §10.B.8 |
| **C40** | "the 2035 volume attack is solved as an Accountability admission-security problem" | **WITHDRAWN on both halves** | **Operator correction accepted: exhaustion is already 2026 physics**, not an emergent agentic novelty; agents amplify **or reduce** it. And `candidate ≠ commit` stops truth capture but **does nothing about burden externalization** — a proposal is cheap for the sender and expensive for the recipient. **Ordinary payer/provider exchange must NOT route through Accountability security**; the capture's §3 keeps forward-flow native until a duty threshold. Split at §10.B.9, with the **authority / leverage / influence / burden-incidence / default-power / material-effect** decomposition so leverage is never upgraded into authority |
| **C41** | "every proposal declares its compensating path; `reopen_care` is the inverse" | **WITHDRAWN — contradicts F6** | A proposal is not a committed transaction, and **a failed therapy change often has no true inverse** (time, progression, side effect, lost opportunity, money, trust). **F6, stated in this carrier, says reversal cannot restore prior care state or lost alternatives.** Corrected import at §10.B.9 |
| **C42** | "the moat and the firewall are the same thing"; "OMNI is the only participant party to none" | **WITHDRAWN** | **OMNI is funded by someone, and this carrier already records that an OMNI operator may be provider, creditor, risk bearer, sponsor, administrator or merchant.** Neutrality is a **deployment- and capacity-specific control objective across six dimensions**, not a property of the OMNI noun. **The firewall is a necessary admission condition for a neutrality-based moat, not sufficient to create one** (§10.B.10) |
| **C43** | A-Q15 framed as "dedup the three graphs" | **REFRAMED** | They sit at different altitudes — process/lifecycle · derived correlation · participant projection — and **may legitimately coexist.** The question is graph-role and projection-relationship reconciliation; the failure is a fourth *authoritative* graph, duplicated state, or one graph owning another's lifecycle |
| **C31 (R4)** | *"seam-kind taxonomy recovered… LOCKED as an invariant"* | **SUPERSEDED BY C36 (R5)** — retained above only as lineage; the taxonomy is a candidate outside its native capture | |
| **C44** | Operator challenge: the arrivals are under-pressured and the carrier risks crowning itself | **ACCEPTED — §16 ADDED** | Three recoveries landed this pass (**`source_authority_map`** field-level authority · **Sourcing selection as a distinct lifecycle** with admissible set / preference / policy precedence / **incentive lineage** · **`T-21` no-false-total-order** for non-linear sequencing), and **the Sourcing row was missing from §8.2 entirely.** §16 records seven arrivals as **under-pressured**, states that no fixture in this carrier has been executed, and makes **external-comparator pressure a required Gate-2 output** |

**R6 corrections (this pass) — propagation only; no conceptual change.** *R5's failure mode was **partial propagation**: it corrected claims where they were introduced and left the withdrawn versions live elsewhere. That is the same class of defect as R4's, one layer down, and it is why a byte pass was owed.*

| # | Defect | Status | Basis |
|---|---|---|---|
| **C45** | **Gate-2 output 12 still carried the withdrawn R4 model** — *"type every move by `seam_kind`," "only `custody_handoff` moves custody," "fails if it introduces a fourth graph rather than deduping"* | **REWRITTEN** | **This is the most serious residue in the file: output 12 is the *execution contract the next agent follows*, so a stale instruction there would have re-committed the exact errors C36–C38 withdrew.** Now states the four primary typing axes with C4.6 planes primary, `seam_kind` admissible only as a proven cross-loop effect, and the three explicit "must NOT" prohibitions |
| **C46** | §10.B.8 item 3 still read *"three graphs… must be deduped, not multiplied"* while §12's A-Q15 had already been reframed | **REWRITTEN** | Coexistence at three altitudes may be legitimate; **the failure is a fourth *authoritative* graph, duplicated lifecycle truth, or one graph silently owning another's** — not multiplicity. Propagated to §15's row |
| **C47** | *"That is the cross-role authority model — and it answers the question far beyond payer-vs-pharmacy"* | **WITHDRAWN — over-close** | The artifact is **`analysis_nonbinding` · `populated_G1_pending_review`** and calls itself a **candidate net-new spine**; the sentence also contradicted this carrier's own §16. Replaced with *strongest current candidate for field-level authority decomposition; sufficiency across clinical, operational, administrative, supply, payer and agentic roles UNPROVEN (A-Q16)* |
| **C48** | §0.3 did not carry the three sources that produced C39, A-Q16, A-Q17 and A-Q18; §15 carried three superseded rows; Gate-2 outputs ran 13→14→12; C31 was unmarked | **ALL CORRECTED** | Source posture now states each source's **maturity** (`populated_G1_pending_review` · accepted L2 doctrine · `full_pass_not_started`) so no reader mistakes a recovery for a ratification. §15 rewritten + **five rows added** (A-Q17, A-Q18, administrative friction, Sourcing carry, multi-axis typing). Outputs renumbered. C31 marked superseded |
| **C49** | **Operator directive:** the comparator set omitted the highest-value untried family, and the proposed cross-arc carry risked *"seven lazy sentences into random other arcs"* | **BOTH ADDRESSED** | Output 14 gains **military/joint C2 under contested logistics** — the mature discipline that already separates command from support from technical authority, intent from tasking from execution, and authorization from logistics readiness — plus **civil aviation** for non-fungible safety authority under constrained flow, each with **explicit transfer limits** (*no universal commander · logistics authority is independent · mission economics do not transfer*). **§15.1 added**, which corrects the carry mechanism from artifact-append to **ledger-first** on `00_omni_opus_boot.mdc` §10 (no silent mutation of sibling truth) and AWP §5 (row-first, document-second) |

**R7 corrections (this pass) — final byte + durability contract. No conceptual change to the architecture.**

| # | Defect | Status | Basis |
|---|---|---|---|
| **C50** | **R6's "ledger-first" and the reviewer's four-part answer are both incomplete**, and the operator's graveyard objection was argued by all three parties **without measuring the estate** | **MEASURED, THEN CORRECTED — §15.1 rewritten to FIVE parts** | **The measurement falsifies the graveyard hypothesis and isolates the real failure.** At `d592e40`: FWREG holds **17 rows, not hundreds**; `08` holds **212 `REV-` rows of which 83 are CLOSED (~39%)**; and this arc was *built on* `REV-185`, `REV-184`, `REV-160` and `FWREG-009`, with `REV-212` closed by the preceding lane. **But `DEC-027` still failed — not on discovery, not on row quality, but because no downstream artifact was obligated to consume it.** The proof: **`REV-160`, the row owning the exact question `DEC-027` answered, still reads "tentative."** Payload · state · discovery · backpressure are **all supply**; **part E, a named consuming gate that cannot pass without disposing of the finding, is the demand side the estate currently lacks.** Reuses existing AWP §9 gate machinery — invents nothing |
| **C51** | §14 exception **E13** still read *"Graph dedup"* | **CORRECTED** | Third and final location of the withdrawn dedup framing (§12 fixed at R5, §10.B.8 and §15 at R6). **Three passes to eliminate one retracted phrase — evidence for C50's thesis that findings need a named consumer, not just a correction** |
| **C52** | §16's row title still read *"as the cross-role authority model"* | **CORRECTED** | An R6 paired edit **failed silently** while its sibling succeeded, so the narrowed wording landed in §10.B.8 but not in §16 — leaving the carrier asserting both. Now *"strongest current candidate… sufficiency unproven"* in both places |
| **C53** | §13.2 said *"one Tier-2 read-graph route"* while §15.1 required several | **CORRECTED to six triggered cold-entry routes**, enumerated once at §15.1 part C and referenced from §13.2 | A single Tier-2 node is only found by a reader who already knows the subject — the opposite of the requirement. Single-source law: enumerated in one place, not restated |
| **C54** | *"Gate 1b produced analysis and preserved nothing"* | **CORRECTED — too absolute** | It **did** preserve an analysis snapshot and an immutable review history. The accurate statement is that it **preserved but did not operationalize, route or canonically carry** — which is exactly the `DEC-027` distinction and must not be blurred |
| **C55** | §0.5 title still read *"(R1 + R2)"*; phase status implied full closure | **BOTH CORRECTED** *(both values below are R7-era and were superseded at R9: the title is now `(R1–R8)` and the status token is now `gate_1b_analysis_accepted_pending_cross_arc_carry`)* | Title now `(R1–R7)`. Status is **`gate_1b_content_accepted_pending_cross_arc_carry`** — the content passes, **the phase is not operationally closed** until the Gate-2 brief exists, the parent row and `REV-160`/`REV-185` updates land, the six routes land, and the durable handoff names Gate 2 |
| **—** | Reviewer claim that §0.3 still omits the three R5 sources | **~~NOT A DEFECT~~ → THIS ROW WAS WRONG. See C56.** | Retracted at R8 |

**R8 corrections (this pass) — final byte + durability contract. Architecture untouched.**

> **★ The defect class this pass finally names.** R5→R8 is four consecutive rounds whose dominant failure was **not wrong architecture but wrong propagation**: a claim corrected in the section that introduced it and left live in a receipt, an exception row, a Gate-2 output, or a source posture. **The cause is structural — this carrier restates its own state in roughly eight places** (revision history · §0.5 · §11 · §13 · §14 · §15 · §16 · §17), **which is precisely the single-source violation it invokes against other artifacts at §2.1.** It is the strongest available argument that the Gate-2 payload must be a **fresh artifact with one state surface**, not a ninth layer on this one.

| # | Defect | Status | Basis |
|---|---|---|---|
| **C56** | **R7 rejected the reviewer's source-posture correction as "already present." That rejection was FALSE, and the method that produced it was worse than the error.** | **RETRACTED · TABLE RELOCATED TO §0.3** | The maturity table added at R6 was **mis-anchored into §4.4** (line ~477) — the edit attached to a source recap inside the second-axis section, not to §0.3 (lines 35–56). At R7 the check was **`grep -c` for the table's title**, which returned 1 and was read as confirmation. **That grep could not distinguish "present in the file" from "present in the correct section," and the result was used to overrule a reviewer.** §0.3 genuinely lacked C3.6C entirely, C4.6 **§9**, and the specific `T-21`-as-A-Q17 use. Now disclosed in §0.3 with read depth and maturity; **the duplicate in §4.4 is removed** rather than left as a second source of truth |
| **C57** | *"The graveyard hypothesis is falsified by count and closure rate"* | **NARROWED** | The measurement falsifies only the **strong** claim that these surfaces are never used. It does not show consumption by the *right* consumer, timeliness, propagation into contracts, absence of rediscovery, or that ~129 open rows are not aging unowned. Methodology now stated; reopened/split rows undistinguished. **A 39% closure rate means the queue is not inert, not that orphaned consumption is solved** |
| **C58** | *"rows have no mechanism whatsoever for obligating consumption"* | **WITHDRAWN — and the sweep that would have caught it is the one this carrier demands of everyone else** | **AWP §6 already requires build-entry retrieval of matched future-work and open-review items and an explicit disposition from a fixed enum — `preserve_invariant_only \| keep_parked \| promote \| open_review \| reject_stale`** — with §5 routing, §9 stop reports, §10 Non-Loss, and the Build Entry Gate's independent retrieval requirement. **The mechanism exists.** `DEC-027` is therefore a **wiring** failure, not an absence failure |
| **C59** | R7's claim that `DEC-027` *"had A, B, C and D and lacked only E"* | **WITHDRAWN — inferred, never traced** | Supported instead: **decision state, open-review question, consuming contract, route, acceptance criteria and closure receipt were never reconciled into one closed loop.** The distribution across those six is **unreconstructed**. Consumer binding stays the most probable repair; **it is not proven to be the only defect** |
| **C60** | Part E named a **new** disposition vocabulary (*accept/narrow/reject/defer/supersede*) | **REPLACED BY AWP §6's EXISTING ENUM** | R7 was one edit from committing **the seven-labels error this carrier documents at §9.1** — minting an eighth vocabulary for a concept that already has one. Part E is now **consumer binding + disposition proof**, with the disposition taken from AWP §6 and a **required receipt**, because a finding can otherwise be "required" indefinitely with nobody proving what became of it |
| **C61** | R7 proposed minting a new guardrail for consumer binding | **DEMOTED to a package-level carry-acceptance rule** | Proposed **without the dedup this estate requires before minting**, and the dedup target exists: **`D0THES-GRD-043`** already forbids *"capturing/categorizing endlessly without routing to an outcome"* with its own outcome vocabulary, and AWP §5/§6/§10 plus Checkpoint Closeout cover adjacent ground. Guardrail promotion requires that dedup **plus evidence of recurrence beyond this packet.** No Build-OS amendment proposed |
| **C62** | §14 still said *"ledger-first, four tiers"* and *"This commit (R6)"*; §11's Task-D prohibition over-stated A-Q14; §15's rules 1–3 were split by §15.1 | **ALL CORRECTED** | The §14 carry line now states the five-part loop; the transaction label reads **R8**. Task-D may not rely on **a contracted or implemented** relation-shaped representation, authority envelope or recommit seam — **not on "the existence of a home,"** which contradicted A-Q14a's own finding that the ownership split is likely visible. §15.1 now follows the complete numbered rule list |
...
**The three R5-recovery sources and their maturity are disclosed in §0.3 (relocated there at R8, where a strict source posture belongs):**
...
| **Sourcing selection — which qualifying counterparty, and on what basis** *(R5 addition; this row was missing entirely)* | **Sourcing — a DISTINCT LIFECYCLE**, not OFC, not Care, not D6 (`EVRUN-000012 _02 §15.3`; unified Vendor Loop **rejected** at `_07 §7.3` L7) | **C4.6 §9 accepted L2 doctrine**: the selection output is a **composed, dimensioned result** — an **admissible set** (licensure/jurisdiction/503A-503B/controlled-substance/quality, each `as_of`) · `failed`/`unknown`/`stale` candidates · alternatives · applicable **preference** (patient/provider/clinic) · **policy precedence** · the **objective** used · **incentive lineage so operator-margin influence is inspectable** · the **authorized selector** and authority basis. *"Routing is **not** a single deterministic owner and quality/availability/cost/reliability is **never** reduced to one authoritative scalar."* | **Ordering/interleaving across principals over time is NOT solved (A-Q17); the payer-mandated-pharmacy case fits none of the three existing categories cleanly (A-Q18)** |
...
### §10.B.7 Disposition — the pattern is inherited, and it already has a name

**No new law, domain, owner, negotiation object or cross-cutting name.** This fixture is **an instance of the existing C4.6 §4 `typed multi-principal resolution` protocol**, composed through: **GCE** for cross-boundary identity, authority and exchange · **native source owners** for assertions and commitments · **Clinical Memory** for clinical assertions, adoption, precedence and conflict · **REV-184 / CNS** for resolution and clinical recommit · **D7 + prescription lineage** for immutable artifacts · **OFC / pharmacy** for execution · **D6** for financial consequence · **conditional Accountability** for admitted duties.

> **Pharmacy discovered the reusable resolution pattern. Insurance adds another principal/capacity plane and a set of financing consequences — not a new resolution physics.**

**What pharmacy already solved, at L2 doctrine and conformance altitude:** typed principal/capacity participation · **proposal ≠ commit** · original prescription lineage (intent ≠ signed artifact ≠ executable instruction ≠ dispense record; *"none silently overwrites the original"*) · permitted substitution vs clinical reinterpretation · **operational reroute vs `reopen_care`** · non-fungible provider/pharmacy/payer/patient authority · the episode as **a projection over distributed artifacts and commits, not an owning object** · explicit rejection of a universal `rx_negotiation` god-object · and the runnable conformance test **`C12`**, which FAILS when *"normalization and clinical change are indistinguishable"* or *"a permitted substitution lacks its authority basis."*

**What pharmacy did NOT solve, at contract/build altitude:** exact C5 homes and field sets · **relation-shaped clinical assertions** (§12 A-Q14b) · all commitment carriers (A-Q1a/A-Q1b) · final Care Response-Seam realization · implementation. **C4.6 is accepted `analysis_nonbinding` build doctrine — a powerful inherited pattern, not a ratified financing contract.**

**What was actually missing before this fixture:** none of it was preserved as a required Insurance fixture, and **none of it has been tested against the first-class mixed-financing operating profile.** This closes the first gap; Gate 2 closes the second (§13.3).

### §10.B.8 Three things this gate adds that the pattern did not already carry

**(1) The typing is multi-axis. C4.6 is primary; `seam_kind` is a candidate cross-loop effect classification, not the universal grammar.** *(R5 correction — R4 over-promoted it.)* The Accountability/GRR capture defines `seam_kind = custody_handoff | work_trigger | control_request | evidence_publication | consequence_notification`, with **only `custody_handoff` transferring custody**. That taxonomy is **`analysis_nonbinding`, defined for Care–Platform–Accountability loop boundaries, in a capture that is REVIEW-READY and not closed.** It is a **reusable candidate**, not a ratified grammar for every external counterparty act, and Gate 2 must test the reuse rather than assume it.

**The interaction needs at least five orthogonal classifications, and no one of them replaces the others:**

| Axis | Source | Maturity |
|---|---|---|
| **Principal + represented capacity** | GCE | **ratified** |
| **Speech-act kind** — assertion · observation · determination · proposal · clarification request · refusal · consent · commitment | H-N2 family (§6) | **OPEN** |
| **Three planes** — authorized outbound command · inbound evidence/external assertion · owning-domain commit via the resolver | **C4.6 §4** | accepted L2 doctrine |
| **Cross-loop effect** *where it applies* — trigger · control request · evidence · consequence · accepted custody | Accountability `seam_kind` | **candidate, unratified** |
| **Resolution stance + authority gate** | REV-184 / CNS / RBAC | signed-off law; field-set at C5 |

**Two R4 mappings were wrong and are withdrawn.** *(a)* **A clinician recommit is NOT a completed `custody_handoff`.** It produces a Care/CM commit, a new immutable artifact, and an **outbound command** — and C4.6's own rung ladder says **submission ≠ acceptance**. Execution custody transfers only when the pharmacy's acceptance evidence satisfies the applicable profile and the owning domain commits the consequence. R4 asserted a custody transfer at emission while the carrier itself cites `O10 ACK ≠ accepted-custody` — a self-contradiction. *(b)* **A pharmacy substituting inside an already-authorized envelope is a professional/execution commit, not a `control_request`.** It becomes a request only when the pharmacy is **asking another authority to decide**.

**The rejection of a universal negotiation object stands — but on the correct ground.** Not "non-custody seams make negotiation logically impossible," which was R4's over-reach. The ground is that the acts are **heterogeneous in owner, authority, correction law and terminal state**: determination · proposal · clarification · professional assessment · permitted substitution · clinical recommit · consent · coverage commitment · fulfillment · settlement. Most of them are not negotiation at all, and C4.6 §4 says so directly.

**(2) Authority is determined by capacity and relationship — never by the employer noun — and the estate's model for this is field-level, not a scalar rank.** *(R5 correction; the recovery below changes both Knox's framing and mine.)*

R4 said a payer's claim "must carry no clinical authority rank," that a payer "may only raise `priority_hint`," and that a payer "bears no clinical liability." **All three are withdrawn.** The first two mis-apply the mechanism; the third is a **legal conclusion this architecture gate did not establish** and is replaced with: *the payer does not own the treating-clinical commit or its professional responsibility.*

**The correct law:** a **payer policy / benefit / formulary position** carries no treating-clinical authority and may never become clinical adoption. But **a licensed clinician acting for a payer may make an attributed professional assertion within that professional capacity** — relevant evidence, which still acquires **no treating relationship, no prescribing authority, no patient-specific plan-commit authority, and no authority to overwrite the treating team's rationale.** The same human may act as treating clinician, consultant, pharmacist, payer medical director, utilization reviewer or agent of another principal; **those capacities cannot share one authority merely because the person holds a license.**

> **★ R5 RECOVERY — the mechanism is `source_authority_map`, not `authority_rank`.** Clinical Memory §5's eight-rank ladder is explicitly *"read-view tie-break, locked"* — **a tie-break heuristic, not an authority model.** R4 (and Knox's counter) both reasoned from it. The estate already has the richer thing: **`v4_C3_6C_source_authority_truth_plane_map.md`**, whose §1 states *"**Source authority is FIELD-LEVEL, not system-level**"* and answers where truth is housed with *"**nowhere single; per-fact, positional, governed**."* Its candidate `source_authority_map` is keyed per `fact_key` and **decomposes authority into distinct roles**: `source_originator` · `repository_of_record` (the positional source-of-truth) · `extraction` (manual / feed / AI-extracted + `model_version`) · **`adoption_authority`** (who adopts it as care truth — *"adoption ≠ source-verified"*) · **`correction_authority`** · `audit_provenance` · `publication_lineage` · `knowledge_partition`. It records itself as *"mostly composition of existing primitives"* over CNS §7.5.1's seven per-event ownership dimensions + the Observation three-gate + CM adoption, with a small net-new spine.
>
> **`source_authority_map` is the strongest current candidate for field-level authority decomposition. Its sufficiency across clinical, operational, administrative, supply, payer and agentic roles is UNPROVEN and is A-Q16.** *(R6 correction — R5 said "that is the cross-role authority model and it answers the question," which over-closed an `analysis_nonbinding` · `populated_G1_pending_review` artifact that calls itself a candidate net-new spine, and contradicted this carrier's own §16.)* What it does supply is the **decomposition shape**, and that shape already explains cases a single ladder cannot: it is why an inventory manager is the `source_originator` and `repository_of_record` for `inventory.implant.on_hand` while a physician is not and holds no authority over that fact; why a respiratory therapist originates a professional observation whose **adoption into care truth** still runs through CM's `clinical_adoption_state`; and why a payer's equivalence claim can be a real attributed assertion with a real originator and **still never reach `adoption_authority` for a care fact.** **Authority decomposes by fact and by role, not by seniority on one ladder.**
>
> **Status: `analysis_nonbinding`, C3.6 G1 `populated_G1_pending_review`, candidate net-new spine, never connected to Insurance until now.** Whether field-level `source_authority_map` plus a context envelope is sufficient — or whether an **action-specific authority envelope** (professional scope · treatment relationship · represented principal · decision type · jurisdiction · commit authority) is additionally required — is **A-Q16**, routed to Gate 2 / C5.

**(3) Three graphs are in play at three different altitudes, and the obligation is role reconciliation — not deletion.** *(R6 correction — R5 corrected A-Q15's framing at §12 but left "must be deduped, not multiplied" standing here.)* REV-184 §0.4: **CNS owns the resolution record, lifecycle and graph** — a **process/lifecycle** graph. Accountability §6b: persisted correlation truth plus a **derived `matter_graph`**. Care capture §9a: a `resolution_participant_graph` marked **projection, NOT truth**. **All three may legitimately coexist.** What must not happen is a **fourth *authoritative* graph**, duplicated lifecycle truth, or one graph silently owning another's lifecycle — which is how the **seven-labels problem** (§9.1) would recur at the graph layer. **This carrier names no new graph.** The reconciliation is **A-Q15**, routed to Gate 2.

### §10.B.9 Administrative exhaustion — already 2026 physics; agents amplify or reduce it

**Operator correction, accepted: this is not a 2035 novelty.** Effective control through imposed friction is **already part of the current physics of care** — repeated evidence demands, prior-authorization loops, appeal deadlines, fragmented channels, step requirements, clinician and staff attention consumed, and providers or patients eventually abandoning an appeal or changing the plan. R4 framed it as an emergent agentic attack. **It is the present state; agent scale is an amplifier — and possibly a countermeasure, since agents may also automate evidence gathering and appeals.**

**Vocabulary discipline, because the term matters.** The payer does **not** acquire authority through exhaustion. It acquires **effective control**, which must be decomposed and never collapsed into "authority":

| Dimension | Meaning |
|---|---|
| **Authority** | who may commit a truth, decision, plan, consent or execution state |
| **Leverage** | who can withhold money, access, capacity or another required resource |
| **Influence** | who can propose, recommend, request, challenge or frame an option |
| **Burden incidence** | who bears the time, labour, delay, cognitive cost and opportunity cost of responding |
| **Default power** | whose position becomes operative when a clock expires or nobody can continue |
| **Material care effect** | how the constraints changed the feasible set or the accepted plan |

**Collapsing these either falsely upgrades leverage into legitimate authority, or pretends formal authority is all that matters while care is being redirected through exhaustion.** Both failures are live.

**`candidate ≠ commit` does NOT solve this, and R4's claim that it did is withdrawn.** That law stops proposals becoming truth. **It does nothing about burden externalization** — and the defining asymmetry is that **a proposal is cheap for the sender and expensive for the recipient.** Nor is this an Accountability admission-security problem in the ordinary case; the capture's own §3 says **ordinary forward-flow work stays in its native workflow until a duty or consequence threshold is crossed.** R4 routed too much ordinary insurance traffic into a security mechanism. **Split it:**

- **Ordinary payer/provider exchange** — governed through **GCE** capability/profile plus **CNS** work ownership, attention routing, queueing, correlation and de-duplication, evidence-sufficiency, deadline ownership, capacity-aware batching, and **typed timeout/default consequences.** (T-13: *time is neither actor nor authority* — a default-on-silence is a **declared consequence of a governing contract**, never time acquiring agency.)
- **Manipulative, harmful or systemically degrading flow** — crosses to **Accountability only after the duty/consequence threshold** (§5.5), where §5a's admission-security machinery legitimately applies.

**The state that must survive.** When a clinician stops pursuing A and moves to B because the appeal burden became intolerable, the record must **never** read *"clinician agreed B was preferable," "patient selected B freely," "original recommendation withdrawn as incorrect,"* or *"payer's equivalence claim adopted."* It must preserve: **A remains historically valid · the payer funds B under policy P · A was not pursued further because effort, delay, deadline or expected burden exceeded available capacity · B was independently judged clinically acceptable but less preferred · the resulting plan was administrative-friction / resource-access driven.** That is **F3 + F7 + the three firewall laws operating together**, and it is §10.B.5 outcome 5 under its hardest load.

**Administrative-friction variant of this fixture (a variant, not a new fixture).** Therapy A recommended → repeated documentation and alternative requests → conflicting deadlines across fragmented channels → appeal initially intended → staff/patient capacity exhausted → appeal abandoned or delayed → therapy B selected or care deferred → **the causal reason preserved explicitly as administrative friction, never as clinical preference or patient refusal.** The Gate-2 trace must expose, **without designing a schema here**: issuing principal **and capacity** · request/proposal count and duplication · evidence demanded · response deadline · the actor expected to respond · elapsed labour and delay · **default consequence of silence** · accumulated burden **by actor** · abandonment or deferral reason · material effect on the feasible set · **whether agents reduced or increased the burden.**

**Saga import, corrected (R5).** R4 said *"every proposal declares its compensating path"* and treated `reopen_care` as the inverse of a failed substitution. **Both withdrawn — and the second contradicts F6, which this carrier states.** A proposal is not a committed forward transaction, and a failed therapy change often **has no true inverse**: time passed, symptoms progressed, a side effect occurred, an opportunity was lost, money was paid, trust changed. The correct import: **a *proposal* discloses expected tradeoffs, known reversibility, failure triggers, required monitoring and likely mitigation where knowable; a *committed consequential transition* declares its rollback/mitigation/reopen path where one exists, and names the residual irreversible consequence where one does not.** No compensating action may be claimed to restore lost care state or lost alternatives.

**Named anti-shadow failure for Gate 2:** *the operating profile FAILS if it preserves formal authority while allowing unbounded administrative burden to become hidden de facto control.*

### §10.B.10 Candidate neutrality posture and its falsifiers

**R4's categorical claims are withdrawn (R5).** R4 said no incumbent can hold this position, that Palantir cannot be party to none, that OMNI is the only participant party to none, and that **the moat and the firewall are the same thing.** All four over-reach.

**Why: OMNI is funded by someone too, and may itself be a party.** This carrier already records at §10.A.5 and §8.4 that an OMNI operator may act as **provider · federation operator · creditor · risk bearer · sponsor · administrator · merchant.** **Neutrality is therefore not an intrinsic property of the OMNI noun. It is a deployment- and capacity-specific control objective** that must be proven per deployment, and a deployment may satisfy some dimensions and fail others:

| Neutrality dimension | What must hold |
|---|---|
| **Source-authority non-capture** | visibility never becomes source authority |
| **Clinical separation** | economics never become clinical merit (§9 Law 2) |
| **Capacity separation** | one entity's several roles remain distinguishable (§10.A.5) |
| **Governance** | no single principal secretly controls the rules (`T0-14`) |
| **Compensation decoupling** | compensation is not coupled to the *outcome* of a resolution |
| **Rail replaceability** | integrations remain replaceable (`GRD-033`) |

**The candidate differentiation, stated testably:** *OMNI's architecture may preserve multi-principal truth and resolution lineage **without granting its customer, operator or financing source additional substantive authority.*** That is falsifiable per deployment; "party to none" is not.

**And the moat correction:** **the firewall is a necessary admission condition for any neutrality-based moat — it is not sufficient to create one.** A moat additionally requires product surplus, distribution, reliability, counterparties, proof, switching value and willingness to pay. R4 converted an ethical constraint into an unearned business claim; that is exactly the move Gate-0 §I.2a's Round-3 correction already caught once.

**Falsifiers, retained and sharpened.** *(a)* **The neutrality that makes OMNI trustworthy to patients and providers is precisely what makes it unattractive to the party with the most leverage** — a payer has no commercial incentive to join a system designed to stop it presenting funding preference as clinical merit. *(b)* Gate-0 **U11**: the longitudinal continuity may have no buyer. *(c)* The moat doctrine's **four N=1 bootstrap mechanisms** are all unproven. **Knox's "the architecture is gravitational because each participant can join without surrendering its truth authority" is a hypothesis, not a finding**, and is recorded as such.

**Gate-2 inversion trace required:** run the fixture with **OMNI's own operator as provider · as payer/risk bearer · as creditor · as manufacturer/sponsor**, and show which neutrality dimensions survive each inversion.
...
**Task-D MUST NOT rely on:** F1–F7 as settled spine laws · **F5 and F7 as binding** · **H-N2 as closed** · **F3 as a proven lifecycle, or as operator-only** · **F5 as implemented by generic projection doctrine** · **F7 as realized** · **agreement/party-position as solved** (one candidate owner-native disposition ≠ a contracted lifecycle) · **Accountability/GRR as ratified, as named, or as owning any financing truth** · **coverage identity ownership or scoping as decided** · claim/adjudication/patient-responsibility/coding/guarantor/sponsor-boundary mechanics as solved · **any R0/R1/R2 claim retracted at §0.5** · any deployment inference · any external market or legal fact in §10 as promotion-grade · **the §8.3 continuity function as a proven moat** · **any payer-proposed alternative as clinically equivalent, and any access-driven revision as clinically optimal** (§10.B) · **a **contracted or implemented** relation-shaped therapeutic-equivalence / acceptability representation, action-specific authority envelope, or Care recommit seam (A-Q14).** *(R8 narrowing — R7 prohibited reliance on "the existence of a home," which contradicted this carrier's own A-Q14a disposition that the ownership split is likely visible: payer position under GCE · professional clinical assertion in Clinical Memory · resolution through REV-184/CNS · recommit through Care. **What is uncontracted is the representation and the seam, not the owner.**)*

**Task-D MUST remain free to falsify:** the correction-law argument · that F3 decomposes cleanly by actor · that the generic seam suffices · that the GCE precedent transfers from pharmacy to financing · that six of seven inheritances survive contracting and building · **that the operating-composition axis is real and buildable** · that "care-financing physics" is a useful architectural layer at all · **and (R4) the neutrality-as-gravity claim — that participants will join a system precisely because it prevents them exceeding their authority. That is a hypothesis, it collides with Gate-0 U11 and the moat doctrine's four unproven N=1 bootstrap mechanisms, and the party with the most leverage has the least incentive to accept it (§10.B.10).**

---

## §12 — OPEN QUESTIONS / EVIDENCE DEPENDENCIES

**A — Repository architecture.** **A-Q1a** accepted-custody/responsibility carrier (Federation + RBAC). **A-Q1b** substantive-commitment profile/lifecycle — authorization · guarantee · sponsorship · loan approval · network/rate undertaking · contractual obligation. **A-Q2** operative-position primitive (REV-184/CNS or C4.3). **A-Q3 CLOSED** (C3.7 → D6 candidate disposition). **A-Q4** `REV-160` reconciliation — carry `DEC-027`; principal-agnostic obligor. **A-Q5 (rewritten)** the stable identity of the coverage relationship, and how the **four scopes** (source relationship · tenant custody · operator applicability · portable person projection) compose without global replication or tenant captivity. **A-Q6** symmetric `payment state ≠ care state` on the Care side. **A-Q7** unsolicited inbound external commitment within GCE. **A-Q8** **§9 Law 2 structural-enforcement mechanism + audit spec** (`REV-185`; owner CNS/Network-Governance) **and the row-P economics-axis elevation** — three dependent arcs. **A-Q9** H-N3/G7 communicative act. **A-Q10** `consequence_assessment_event` dedup (Accountability §6a ↔ C4.3). **A-Q11** Accountability/GRR naming inconsistency (catalog / FWREG-009 / WI14 / Card 2) — **not an Insurance question**. **A-Q12 (new)** the **F5 assessment-lifecycle owner** — who owns computation, rule version, source selection, validity, invalidation, recomputation, audience policy, retention, issuance and proof of a mixed-financing assessment. **A-Q13 (new)** whether a **common derived incidence assessment** is needed across F3's seven actor slices. **A-Q14 (R3, CORRECTED AT R4 — the R3 phrasing was an owner-level over-claim and is withdrawn).** R3 said *"the clinical-equivalence assertion has no located home."* **Too broad.** The **owner split is visible and largely inherited**: a payer's equivalence/preference claim is a **payer policy position held under GCE, without clinical adoption**; a pharmacist's or clinician's judgment is a **source-attributed clinical assertion in Clinical Memory** with `authored_by` locked at write *(the authority-bearing field; `authority_rank` is a **read-view tie-break**, not an authority model — C39, §10.B.8 item 2)*; **REV-184/CNS** owns resolution among the positions; **Care** owns the recommit; **D7** owns the artifacts; **OFC/pharmacy** execution; **D6** the financial consequence. What is genuinely uncontracted is narrower — **the relation-shaped representation and the recommit seam** — split three ways:

- **A-Q14a (CAPACITY-CORRECTED AT R9 — the active text carried the mechanism C39 withdrew).** The question is not one question, because a payer entity performs **two different acts** and the R4 handling rule applied to neither. **Nothing new is minted here, and A-Q16 stays open.**
  - **(A) Payer policy / formulary / benefit position.** How is a payer-authored coverage, formulary, preference or "equivalence" **policy position** preserved as an **external policy/coverage position under GCE**, given that it **does not enter Clinical Memory as an adopted clinical assertion** and **obtains no treating-clinical, prescribing or care-plan commit authority**? It **may constrain funding and feasibility** (§9 Laws 1 and 3), but **may not author therapeutic equivalence or clinical appropriateness** — that conclusion belongs to a clinical authority that independently commits it (Care L171; C4.6 §4).
  - **(B) Professional assertion by a payer-employed licensed clinician.** **A separate act from (A), and it must not be collapsed into it.** How is such an assertion preserved as an **attributed professional assertion** under the clinician's **represented principal · capacity · professional scope · jurisdiction · evidence basis · relationship (or absence of relationship) to the patient**? It is real, admissible, attributable evidence — and it acquires **no treating relationship, no prescribing authority, no patient-specific plan-commit authority, and no authority to overwrite the treating team's rationale**, merely because the asserting person holds a license.
  - **The withdrawn mechanism, named so it cannot return.** R4 held that a payer-authored claim "receives no authority rank" and that "low-authority input may only raise `priority_hint`." **`priority_hint` is NOT the universal handling rule for payer input, and this carrier no longer states that it is.** Clinical Memory §5.1's `priority_hint` rule governs **`longitudinal_pattern`/`confidence_weight` from repeated patient reports** — observability never promoting `status`, `authored_by`/`authority_rank` or an authority floor. It is a real and correct rule **about a different input class**; borrowing it as the payer-handling rule mis-applies it, and C39 withdrew that borrowing at R5. **The load-bearing guard is capacity and adoption, not a hint field:** a payer position never reaches **`adoption_authority`** for a care fact (C3.6C decomposition, §10.B.8 item 2), and adoption ≠ source-verified. **Whether field-level `source_authority_map` plus a context envelope suffices, or an action-specific authority envelope is additionally required, remains A-Q16 — OPEN, and not answered here.**
- **A-Q14b** — how does **Clinical Memory represent an attributed, patient- and context-scoped therapeutic-equivalence / acceptability / inappropriateness *relation*** among therapy A · therapy B · indication · formulation · dose · route · patient · time · evidence basis · author and professional scope? *"B is equivalent to A"* is **not a unary assertion about one concept** — Clinical Memory's substrate is `(patient, concept, context_key)`, and this is a relation between two therapies under a scope. **This is the load-bearing sub-question.**
- **A-Q14c** — how does **REV-184/Care consume that relation** to distinguish clerical/operational normalization (no reopen) · permitted change inside an existing substitution/authority envelope · clinical reinterpretation requiring `reopen_care` and an **authorized recommit**?

**Dedup before minting anything, mandatory:** against the **Clinical Memory concept/assertion substrate**; against **Accountability §19's `governed_relation_assertion`**, which is already the estate's **explicit do-NOT-mint-yet Task-D hinge for relation-shaped assertions** and which asks precisely *"generalize the assertion grammar beneath `clinical_assertion` **without** universalizing Clinical Memory's adoption model?"*; and against the **Care Response-Seam Audit**, already the named required next artifact. **Do NOT mint `clinical_equivalence_assertion` here or anywhere before that dedup.** *(R4 self-finding: R3 cited `governed_relation_assertion` in §2 and §5.4 and still failed to connect it to A-Q14 — the hinge for relation-shaped assertions was already in the carrier.)* Generic clinical-authority physics, not financing.

**A-Q15 (R4, REFRAMED AT R5).** R4 framed this as "three graphs must be **deduped**." **Wrong frame.** They sit at **different altitudes and may legitimately coexist**: REV-184's is a **process/lifecycle** graph (CNS-owned), Accountability's `matter_graph` is a **derived correlation** projection over persisted correlation decisions, and Care's `resolution_participant_graph` is a **participant projection, explicitly NOT truth.** The real question is **graph-role and projection-relationship reconciliation**: *what is canonical, what is derived, what is a projection, and how are their references related without copied truth?* **Gate 2 FAILS only if it creates a fourth *authoritative* graph, duplicates truth or lifecycle state, or lets one graph silently own another's lifecycle** — not merely because three names exist.

**A-Q16 (new, R5)** **is field-level `source_authority_map` plus a context envelope sufficient, or is an action-specific authority envelope additionally required?** The envelope would need professional scope · treatment relationship · represented principal · decision type · jurisdiction · action-specific commit authority. Clinical Memory's `authority_rank` is a **read-view tie-break, not an authority model** (§10.B.8 item 2). Owner: Clinical Memory + RBAC + C3.6's `source_authority_map` spine. **Generic authority physics, not financing — and materially bigger than this arc.**

**A-Q17 (new, R5) — the sequencing question, and it is genuinely open.** C4.6 §9 already solves *selection* (see §8.2's new Sourcing row). It does **not** solve **ordering and interleaving across principals over time**: which principal is engaged first, what happens when payer, pharmacy, inventory and patient responses arrive out of order or concurrently, how partial answers compose, and what the operative posture is mid-flight. The multi-principal exchange is **not linear** — it is concurrent, bidirectional and async/sync mixed. **The existing shape is C4.5 `T-21`**: *"**No false total order**; preserve the partial order, the source clock or temporal basis, and ordering confidence. **Exact chronology is a conclusion requiring evidence, not a display convenience**"* — plus `T-17` first-class temporal uncertainty and `T-13` (time is neither actor nor authority). **But C4.5 is `gate_0_charter_accepted · full_pass_not_started`, and its application to multi-principal care sequencing at speed has not been done anywhere.** Routed to Gate 2 as a required trace dimension and to C4.5's own pass. **This is the single largest genuinely-open item this fixture exposes.**

**A-Q18 (new, R5)** **preference vs rail vs hard requirement.** C4.6 §9 separates *admissibility criteria* (licensure/jurisdiction/503A-503B/controlled-substance/quality, each `as_of`) from *applicable preference* (patient/provider/clinic) from *policy precedence*. **Whether that three-way separation is sufficient for the financing case** — where a payer-mandated pharmacy is neither a preference nor an admissibility criterion but a **funding condition** — is untested. Owner: Sourcing + D6 + Settings.
...
10. **Kill conditions** — the generic seam cannot carry required fields · no actor will pay for the continuity (Gate-0 U11) · incumbent configuration achieves equivalent cross-counterparty coherence · exception rate or human labour destroys the economics · the profile becomes a hidden source of truth (raw 3's named risk) · **and (R4) the party with the most leverage will not participate in a system whose purpose is to prevent it presenting its funding preference as clinical merit** (§10.B.10 — the neutrality-versus-leverage falsifier).
...
12. **Cross-authority option-resolution profile (R4 addition · typing REWRITTEN AT R6).** Gate 2 must prove the operating profile can carry the **existing C4.6 typed multi-principal resolution pattern** without creating a universal negotiation or case object — and must classify every proposed change into exactly one of three branches: **(a) clerical/operational reroute — no clinical reopen · (b) permitted change inside an existing substitution/authority envelope, with its authority basis recorded · (c) clinical reinterpretation — `reopen_care` plus an authorized clinical recommit.** Every trace must preserve original intent and artifact · each source-attributed proposal or position · principal **and capacity** · patient preference and consent · operative posture while unresolved · the final recommit · execution · consequence · rationale class and correction lineage.
    **Typing is MULTI-AXIS, and no single axis may be used alone** (§10.B.8 item 1). Primary: **GCE principal + represented capacity** · **speech-act kind** · **C4.6 plane** (authorized outbound command / inbound evidence-assertion / owning-domain commit) · **REV-184 stance + authority gate**. **`seam_kind` is admissible only as a candidate cross-loop *effect* classification where its reuse is actually proven** — it is `analysis_nonbinding` outside its native capture. **Gate 2 must NOT type every move by `seam_kind`, must NOT treat clinician recommit as an accepted `custody_handoff` (submission ≠ acceptance; execution custody changes only after acceptance evidence satisfies the applicable profile and the owning consequence commits), and must NOT treat substitution inside an already-authorized envelope as automatically a `control_request`.**
    **Named anti-shadow failure: the operating profile FAILS if it becomes the authoritative owner of the resolution rather than a projection and orchestration over distributed commits.** **Second named failure — graph-role and projection-relationship (A-Q15): Gate 2 FAILS if it creates a fourth *authoritative* graph, duplicates lifecycle truth, or lets one graph silently own another's lifecycle.** Process/lifecycle, derived-correlation and participant-projection graphs **may legitimately coexist**; multiplicity alone is not the failure.
13. **Sequencing, selection and burden dimensions (R5 addition).** The trace suite must additionally expose: **(a)** the **Sourcing selection** output per C4.6 §9 — admissible set with per-candidate `as_of`, preference, policy precedence, objective, **incentive lineage**, authorized selector — for the *"which of eight qualifying pharmacies / six vendors"* case, and whether a **payer-mandated pharmacy** is representable as admissibility, preference, policy precedence or **a funding condition that is none of the three (A-Q18)**; **(b)** **non-linear ordering** — concurrent, bidirectional, async/sync-mixed exchange across patient · provider · pharmacy · payer · inventory · support staff, tested against **`T-21` no-false-total-order** with partial order, ordering basis and ordering confidence preserved (A-Q17); **(c)** **burden incidence per actor** and the default consequence of silence (§10.B.9). **Gate 2 must state plainly which of these it resolved and which it only exposed.**
14. **External-comparator pressure (R5 — operator directive; TWO FAMILIES ADDED AT R6).** The arrivals at §10.B.8's typing axes, A-Q15's graph roles, A-Q16's authority decomposition and A-Q17's sequencing **have not been pressured against any external comparator.** Gate 2 must run the estate's Lens-B discipline — **mechanism only, never the hidden ownership, hierarchy or economic assumptions** (`GRD-026`, Gate-0 §G.2/§G.3) — against these families. **Gate 2 performs the research; this carrier only routes the questions and declares no comparator result.**

    **(a) Technology (R5).** **Palantir** (ontology + Actions as the governed write path; change history ≠ belief history) · **AWS** (saga/compensation, continuation-vs-compensation by failure class) · **Anthropic/OpenAI-class agent platforms** (tool-as-contract, approval enforced in the tool not the prompt, model identity on consequential output) · **Tesla** (closed-loop sense→act under degraded sensing) · **Epic** (the configuration-as-truth failure mode).

    **(b) Military / joint command-and-control under contested logistics (R6 — operator directive; a first for this build).** This is the **highest-value untried comparator for A-Q16 and A-Q17**, because military doctrine has spent decades separating things OMNI is currently at risk of conflating. Pressure: **command authority vs support vs technical/professional authority** · **commander's intent vs tasking vs execution** · **logistics readiness vs authorization** (an order does not create supply) · **source-intelligence confidence vs decision authority** · **delegated execution and degraded communications** · **abort / replan / retask / supersede** · **conditions-based authority** · **the common operational picture as a projection, not a truth store** · **partial-order synchronization and deconfliction** (the direct A-Q17 analogue).
    **Three explicit transfer limits — the import fails if any is violated.** *(i)* **Healthcare has no universal commander.** Patient consent and refusal, clinician professional judgment in scope, pharmacist professional and execution authority, payer funding authority, facility and supply source authority over capacity and stock, and bounded legal/regulatory supersession are **non-fungible authorities, not subordinate ranks**, and may never be collapsed into one hierarchy. Military doctrine ultimately reconciles under a commander; OMNI cannot and must not. *(ii)* **Logistics authority is independent, not subordinate.** Military supply serves a unified priority; OMNI's supply, inventory and pharmacy actors are frequently **separate legal and commercial principals** whose source authority over stock is genuinely their own (§8.2's Sourcing row). *(iii)* **Mission economics do not transfer.** Military planning can accept attrition against an objective; **no OMNI composition may trade a patient's care for throughput** (§9 Laws 1–3). Import the *separations*; reject the hierarchy and the objective function.

    **(c) Civil aviation / air-traffic operations (R6 — the complementary comparator).** Tests a high-stakes system with strong authority and safety envelopes where **no single actor owns every fact or every transition**: **pilot-in-command · air traffic control · dispatch · maintenance/airworthiness · weather · airport slot and capacity · emergency authority · the safety envelope · asynchronous flow and deconfliction.** For each, ask: **what is canonical, what is advisory, what is a hard constraint, who may commit which transition, and what happens under degraded communication?** **One specific question worth isolating:** whether commercial dispatch operates a genuine **joint-release** model — two independent authorities who must *both* concur before a flight departs, neither able to override the other — because if so it is the closest operating analogue in any industry to *"treating clinician and dispatching pharmacist both hold non-fungible authority over the same act,"* and OMNI currently has **no mechanism named for dual concurrent commit authority.** Verify against primary sources before importing; **this carrier asserts it as a question, not a fact.**
...
| **R4 head (Knox-reviewed, `TWO-AXIS ARCHITECTURE PASS · SEMANTIC FIDELITY PATCH REQUIRED`)** | `7a0052e2788b401e5fa4259b64f094b3cc35f558` · blob `04b77fc6b0cca13f69bde57db490c59395cf49c1` · 1,039 lines |
| **R5 head** | `292951b4308714c31156b86c97df5bca416f6cb6` · blob `e308ed5621f07aff731c3cf5276a1a0c107b2376` |
| **R8 head** | `1bc2a93f2ecbcd995f7f4103d7ab53837046d72b` · blob `ebb34d84244356ee67605e5b0abd74eaddb0c5fb` · 1,242 lines |
| **Current transaction (R9) — bounded semantic-propagation + receipt patch; NOT an architecture round** | Three corrections and their forced propagations, nothing else. **(1) A-Q14a capacity correction** — the active text still carried the R4 mechanism C39 withdrew (*payer claim receives no authority rank · low-authority input may only raise `priority_hint`*). A-Q14a now distinguishes **payer policy/formulary/benefit position** (external policy under GCE; no clinical adoption; no treating-clinical, prescribing or care-plan commit authority; may constrain funding and feasibility but may not author therapeutic equivalence or clinical appropriateness) from a **professional assertion by a payer-employed licensed clinician** (a separate act, preservable as an attributed professional assertion under represented principal · capacity · professional scope · jurisdiction · evidence basis · relationship to the patient, acquiring no treating relationship, prescribing, plan-commit or rationale-overwrite authority). **`priority_hint` is no longer stated as the universal handling rule. No new authority object minted; A-Q16 stays OPEN.** Propagated to the §12 A-Q14 preamble, **E12**, and the §15 durability row. **(2) Part E disposition vocabulary** — R8 applied AWP §6's **future-work** enum to every carried finding; §6's own wording scopes it to matched future work. Part E now takes disposition from **the owning state surface's canonical vocabulary** (future-work → AWP §6 enum over FWREG `status`; open-review → `08`'s queue states + closure contract; decisions → `03`'s lifecycle; conflicts → `05`'s `resolution_status`; contracts/artifacts → their own review and maturity gates). **No universal disposition enum minted.** Five-part structure kept; reusable law preserved as `named consumer + consuming gate + owning-surface-native disposition + state update + stop/handoff proof receipt`. **(3) Receipts and termini** — §0.5 retitled `(R1–R8)`; the R7 sweep replaced by the **R9 active-prose sweep** above; the terminus heading de-numbered; the false *"propagation finally verified at R6"* claim replaced; `r8_byte_check_pending` removed; status and STOP repointed. **UNCHANGED and NOT reopened: the two-axis verdict · F1–F7 · both Nick fixtures · the three economic/firewall laws · the implementation findings · the residual ownership matrix · the comparator families · the Task-D two-level maturity · the Gate-2 name · the ownership decomposition. No new law, domain, owner, graph, fixture, negotiation object, vocabulary or comparator *result* minted** |
| **ACTIVE-PROSE SWEEP (R9, verified line-by-line — supersedes the R7 sweep)** | **Zero live instances in ACTIVE prose** of: `CORRECTION RECEIPT (R1–R7)` · `Gate-1b readiness (R7)` · `r8_byte_check_pending` · `may only raise priority_hint` (as a handling rule) · `without acquiring clinical adoption or an authority rank` · `rows have no mechanism whatsoever` · `ledger-first mechanism, four tiers` · `universal disposition enum` (asserted, as opposed to explicitly refused) · `graph dedup` · `must be deduped` · `type every move by seam_kind` · `LOCKED as the answer` · `no clinical authority_rank` · `as the cross-role authority model` · `one Tier-2 read-graph route` · `preserved nothing` · `content_accepted_pending_cross_arc_carry` (as the live status) · `propagation verified at R6` (as the final claim). **Every surviving occurrence was inspected in place and is one of: a quotation inside a correction row whose later correction is explicit · the revision history · this sweep receipt.** Specifically retained: **C32's R4 `priority_hint` wording, live only because C39 marks all three of its claims WITHDRAWN**; C31's R4 quotation marked `SUPERSEDED BY C36`; C45/C46/C51's quotations of the withdrawn dedup and `seam_kind` framings; C55's R7-era title/status values, now marked superseded in place; C58/C60's quotations of the R7/R8 disposition wording |
| **R6 head** | `c8e7fb30d48ec924e5107bc1aab5ef5b0f197150` · blob `39c17880de9a4b1dbccd33736fa29abdb36a29cd` |
...
| **R3 head (Knox-reviewed, `PASS WITH ONE FINAL SEMANTIC MICRO-PATCH`)** | `34cabe4c0cd1b8ad0c1e48a3cace7abbe636c655` · blob `b3d0d2d761f03c130733f88ca8bd8b74077a4b33` · 966 lines |
| **Correction receipt** | §0.5 — **62 items** (C1–C15 R1, C16–C25 R2, C26–C28 R3, C29–C35 R4, C36–C44 R5, C45–C49 R6, C50–C55 R7, **C56–C62 R8**), each with its adjudicating primary source. **C56 retracts a false rejection this carrier issued against the reviewer at R7** |
| **R7 head** | `cf029f5c4646d0d37916df70dea05cb844c8f2e6` · blob `18912da4d9b5d66bc7da53208ea175c23fe4368c` |
| **Cross-arc carry** | **§15.1** — **five-part closed loop** (payload · state · discovery · bounded backpressure · **consumer binding + disposition proof**). **R9: disposition is OWNING-SURFACE-NATIVE** — AWP §6's future-work enum for future-work rows, `08`'s queue states and closure contract for open-review rows, `03`'s lifecycle for decisions, `05`'s `resolution_status` for conflicts, each contract's own review/maturity gate for contracts. **No universal disposition enum is minted.** Reusable law = `named consumer + consuming gate + owning-surface-native disposition + state update + stop/handoff proof receipt`. Two hard constraints, integrator-owned on a separate branch. **This lane cannot land it** |
| **Under-pressured arrivals** | **§16 (R5)** — seven arrivals recorded as unpressured; no fixture in this carrier has been executed; external-comparator pressure is now a required Gate-2 output |
...
| **Unresolved** | A-Q1a/1b, A-Q2, A-Q4–A-Q13, A-Q14a/b/c, A-Q15, **A-Q16 (authority envelope) · A-Q17 (non-linear sequencing — the largest open item) · A-Q18 (preference vs rail vs funding condition)** · B-Q1–B-Q5 · C · D · **§16's seven under-pressured arrivals** |
...
| **E11** | **Payer-constrained therapy selection (§10.B)** | **An instance of the existing C4.6 typed multi-principal resolution protocol, not a new pattern.** Pharmacy solved it at **L2 doctrine/conformance** altitude (incl. runnable `C12`); Insurance adds a principal/capacity plane and financing consequences, **not new resolution physics**. Never preserved as an Insurance fixture and never tested against the operating profile — now a **REQUIRED** Gate-2 trace and the decisive joint test of all three §9 laws |
| **E12** | **Relation-shaped clinical assertion + recommit seam (A-Q14a/b/c)** | **R3's "no located home" is WITHDRAWN as an owner-level claim.** The owner split is visible and largely inherited (payer **policy/coverage position** under GCE without clinical adoption · Clinical Memory for professional assertions with **`authored_by` locked at write** · REV-184/CNS resolution · Care recommit). **R9 capacity correction:** a **payer-employed licensed clinician's professional assertion** is a **separate act** from the payer's policy position and may be preserved as an attributed professional assertion under that clinician's represented principal, capacity, scope, jurisdiction and relationship to the patient — **acquiring no treating relationship, prescribing, plan-commit or rationale-overwrite authority.** **Uncontracted: the relation-shaped representation and the recommit seam.** Dedup mandatory against Clinical Memory, Accountability §19 `governed_relation_assertion` (already a do-NOT-mint Task-D hinge) and the Care Response-Seam Audit |
| **E13** | **Graph-role and projection-relationship reconciliation (A-Q15)** *(R7 — was "graph dedup")* | Three graphs exist at three altitudes — REV-184's CNS **process/lifecycle** graph · Accountability's **derived-correlation** `matter_graph` · Care's `resolution_participant_graph` **projection**. **They may legitimately coexist; multiplicity is not the failure.** This carrier names no fourth. **Blocked: a fourth *authoritative* graph, duplicated lifecycle truth, or one graph silently owning another's lifecycle.** Reconciliation routed to Gate 2 as a named failure condition |
...
| **Payer-constrained-therapy fixture** (§10.B) | **ALL THREE, not one:** (i) C4.6 L2 typed multi-principal resolution + conformance **`C12`** · (ii) the **Care Response-Seam Audit / C5** representation question (A-Q14b) · (iii) the **INS-G2** operating-profile trace suite | the interaction is re-argued from first principles every time someone touches formulary, step therapy or substitution | `C12` exists and runs — **attach, do not rebuild. But `C12` alone is INSUFFICIENT: it tests prescription lineage, not the payer/financing composition or the equivalence relation** |
| **Multi-axis interaction typing** (§10.B.8 item 1) *(R6 rewrite)* | Gate-2 acceptance contract, **output 12** | the interaction gets modelled as a negotiation object holding a custody it never receives — **or** gets force-typed by one axis, which is how R4 produced two wrong mappings | **C4.6 planes are accepted L2 doctrine; `seam_kind` reuse outside its native capture is a CANDIDATE and Gate-2 pressure is owed** |
| **A-Q16 — field-level authority decomposition** (§10.B.8 item 2) *(R6 rewrite)* | **C3.6C** pressure route + Clinical Memory + RBAC + Gate-2 comparator output 14(b) | the estate keeps reasoning about authority through Clinical Memory's `authority_rank`, which is a **read-view tie-break, not an authority model** — and non-clinical, supply and agentic roles never get an authority home at all | `source_authority_map` is **`analysis_nonbinding` · `populated_G1_pending_review` · a candidate net-new spine**; sufficiency across clinical/operational/administrative/supply/payer/agentic roles **UNPROVEN**; **action-specific authority envelope OPEN** |
| **Payer position must never reach `adoption_authority` for a care fact** | Clinical Memory ratification + the A-Q14a disposition | a payer equivalence claim is stored as if it were a clinical assertion — **the most dangerous silent rewrite in §10.B** | the CM rule **exists and is `canonical`**; the *application to payer positions* and the **capacity distinction for a payer-employed clinician** are now stated **in this carrier only** (A-Q14a, R9) — **stated is not contracted**, and neither Clinical Memory nor any contract carries them |
| **A-Q17 — partial-order sequencing at speed** *(R6 addition)* | the **C4.5 full pass** + Gate-2 output 13(b) | the multi-principal exchange gets built as a linear pipeline, and the first concurrent payer/pharmacy/inventory response set produces a **false total order** presented as chronology | `T-21` exists as charter law; **C4.5 is `full_pass_not_started`** and the application to care sequencing exists **nowhere** — the largest open item this fixture exposes |
| **A-Q18 — admissibility vs preference vs policy precedence vs rail vs funding condition** *(R6 addition)* | **C4.6 §9 Sourcing** cross-reference + Gate-2 output 13(a) | a payer-mandated pharmacy gets forced into whichever existing class is nearest, most likely "preference," which silently converts a **funding condition** into a clinical or operator choice | C4.6 §9 separates three of the five; **the funding-condition case is unclassified** |
| **Administrative-friction / burden-incidence variant** (§10.B.9) *(R6 addition)* | the **`NICK-FIXTURE-PAYER-CONSTRAINED-THERAPY-01` variant** + Gate-2 output 13(c) | exhaustion-driven plan changes get recorded as clinical preference or patient choice — **the falsification F7 exists to prevent** — and burden incidence is never measured, so de facto control stays invisible | **variant specified, never run**; burden incidence has no home in any contract |
| **Sourcing Selection as a distinct lifecycle** (§8.2) *(R6 addition)* | **C4.6 §9** is the source; the carry target is the Gate-2 brief + the read-graph route | Insurance re-derives routing, or worse, the operating profile absorbs selection and becomes the shadow domain §13.3 exists to prevent | **accepted L2 doctrine, and it was missing from this carrier entirely until R5** |
...
| **A-Q14a/b/c relation-shaped assertion + recommit seam** | Clinical Memory ratification · **Accountability §19 `governed_relation_assertion` dedup** · the **Care Response-Seam Audit** already named as the required next artifact (Accountability §20) | the relation gets minted as a new `clinical_equivalence_assertion` object without deduping against the hinge that already exists for exactly this | **owner split likely inherited; relation representation + recommit seam UNCONTRACTED** *(R4 correction — R3 wrongly said "nowhere")* |
| **A-Q15 — graph-role and projection-relationship reconciliation** *(R6 rewrite)* | Gate-2 acceptance (named failure, output 12) → C5 | a **fourth authoritative** graph is minted, lifecycle truth is duplicated, or one graph silently owns another's lifecycle — the seven-labels problem at the graph layer | three graphs exist at three altitudes and **may legitimately coexist**; **role reconciliation not performed.** *Multiplicity is not the failure — duplicated authority is* |
...
2. **A finding that exists only in this carrier is preserved but not carried.** §13.2's routing bundle plus §15.1's five parts are the whole carry mechanism, and they are **integrator-owned**. This carrier cannot land them and does not pretend to. **If the integrator transaction never happens, the honest state is that Gate 1b preserved an analysis snapshot and immutable review history but did NOT operationalize, route, or canonically carry its findings** — a real distinction, and precisely the `DEC-027` failure mode reproduced one arc later. Say it plainly rather than discover it in six months.
...
### §15.1 The cross-arc carry transaction — the five-part closed loop *(R6 · rewritten R7 · corrected R8)*

Gate 1b recovered physics from **six** arcs that did not know their own concepts were being reused and sharpened (§16 statement 1). Two mechanisms were proposed and **both are incomplete.** R6 said *"ledger-first,"* which overstates one mechanism. The reviewer's four-part answer — payload · state · discovery · bounded backpressure — is better but is **entirely supply-side**. The operator's objection was that rows become graveyards. **All three positions were argued without measuring the estate, so R7 measured it.**

> **★ R7 — THE MEASUREMENT, AND WHAT IT ACTUALLY SHOWS.** At `d592e40`: **`future_work_registry.md` holds 17 rows, not hundreds.** **`08_open_review_queue.md` holds 212 unique `REV-` rows of which 83 are CLOSED — a ~39% closure rate.** And this arc is direct evidence the surfaces work: **`REV-185` produced the economic-firewall recovery, `REV-184` the governed-resolution lifecycle, `REV-160` the financing-shape question, `FWREG-009` prevented the GRR/Accountability naming error, and `REV-212` was closed by the immediately preceding lane.** **★ R8 NARROWING — what that does and does not prove.** It falsifies the **strong** claim that these surfaces are never used. **It does not eliminate the risk.** The counts do not show that closed rows were consumed by the *right* downstream artifact, that closure was timely, that it propagated into contracts or implementation, that findings were not independently rediscovered, or that the ~129 still-open rows are not aging without an owner. **A ~39% closure rate is evidence the queue is not inert — not evidence that orphaned consumption is solved.** *(Methodology: unique `REV-` identifiers and case-insensitive `CLOSED` occurrences at `d592e40`; reopened and split rows are not distinguished.)* **Accurate statement: the estate's registries are demonstrably useful for preservation and discovery; the narrower orphaned-consumption risk remains real.**
>
> **`DEC-027` still failed, and R8 narrows the diagnosis.** It was **not** a discovery failure: this arc found it in one pass. It was **not** a row-quality failure: it recorded the full field list and was Nick+Knox-modified. **`REV-160`, the row owning the exact question `DEC-027` answered, still reads "tentative"** — the answer and the open question have coexisted in the same boot-loaded ledger for months, unreconciled. **But R7's claim that `DEC-027` "had payload, state, discovery and backpressure, and lacked only consumption" is WITHDRAWN — that lineage was inferred, never traced.** What the evidence supports: **the decision state, the open-review question, the consuming contract, the read route, the acceptance criteria and the closure receipt were never reconciled into one closed loop.** The failure may sit in any of those — linkage, currency, route specificity, consumer ownership, gate criteria, contract disposition, or receipt — and **the distribution has not been reconstructed.** Consumer binding is the most probable and most important repair; it is **not proven to be the only defect.**
>
> **THE LAW (R8 correction — R7 said rows have "no mechanism whatsoever," which is false, and the sweep that would have caught it is the one this carrier demands of everyone else. ★ R9 SCOPE CORRECTION to R8's own repair.).** A row **does not by itself** create a downstream consumption obligation. **But the estate already has the enforcement machinery**, and R7 asserted absence without checking: **AWP §6 requires build-entry retrieval of matched future-work, open-review and guardrail rows**, plus **§5 routing**, **§9 stop reports**, and **§10 the Non-Loss Requirement**; the **Build Entry Gate** independently requires that retrieval. **The mechanism exists.**
>
> **What R8 then got wrong, and R9 corrects: AWP §6's fixed enum — `preserve_invariant_only | keep_parked | promote | open_review | reject_stale` — is scoped to *matched future-work rows*, not to every carried finding.** §6's own wording is exact: *"declare dispositions for matched **future work**."* Retrieval spans three row classes; **the enum governs one of them.** R8 bound part E to that single enum for all carried findings, which would have imposed a **future-work vocabulary on open-review rows, decision rows, conflict rows and contracts that each already have their own** — the seven-labels error committed in the opposite direction: not by minting an eighth vocabulary, but by **flattening five existing ones into one.** *(Part E below is corrected accordingly: **the consuming gate uses the canonical lifecycle/disposition vocabulary of the OWNING state surface.** No universal disposition enum is minted, and none is borrowed across surfaces.)*
>
> **So `DEC-027` was a WIRING failure, not an absence failure — and R8 also withdraws R7's claim that it "had A, B, C and D."** That lineage was **inferred, not traced.** What the evidence supports is narrower and more useful: **the decision state, the open-review question, the consuming contract, the read route, the acceptance criteria and the closure receipt were never reconciled into one closed loop.** The failure could sit in any of several places — decision-to-review linkage, review currency, route specificity, consumer ownership, gate criteria, contract disposition, or receipt — and **this carrier has not reconstructed the distribution.** Consumer binding remains the most probable and most important repair; it is **not proven to be the only defect.**
>
> **Package-level rule, NOT a guardrail (R8 demotion).** R7 proposed minting *"a finding with no named consumer and no consuming gate is preserved but not carried"* into `06`. **Withdrawn as premature** — it was proposed **without the dedup this estate requires before minting**, and the dedup target exists: **`D0THES-GRD-043`** already forbids *"capturing/categorizing endlessly without routing to an outcome"* with its own outcome vocabulary (`no-op · watch · routed · re-review-trigger · promoted · rejected`), and AWP §5/§6/§10 plus Checkpoint Closeout cover adjacent ground. It is therefore recorded as a **Gate-1b carry-transaction acceptance rule**; guardrail promotion requires dedup against those and **evidence of recurrence beyond this packet.** No Build-OS doctrine amendment is proposed tonight.

**On the artifact-append proposal specifically: the intent is right and the vehicle is wrong, on this estate's own law.**

**Three reasons, in the estate's own terms.** *(a)* **`00_omni_opus_boot.mdc` §10: "No silent mutation of sibling truth."** C4.6 is **accepted L2 build doctrine** and REV-184 is **signed-off**; appending an Insurance forward-reference to either is a mutation of an artifact this lane does not own, made without its owning gate. *(b)* **AWP §5 routes findings to canonical ledger homes** — `03` decisions, `05` supersession, `06` guardrails, `08` open review, FWREG, the read graph — precisely so cross-arc pointers do not accumulate as prose in derivation artifacts. **Row-first, document-second.** *(c)* **A note appended to an artifact is discovered only by someone already reading that artifact** — which is exactly the population that did *not* need telling. A ledger row and a read-graph route are discovered by someone **entering the subject cold**, which is the actual evaporation population.

**The correct carry has FIVE parts. None is sufficient alone, and the fifth is the one the estate currently lacks.**

| Part | Mechanism | Carries | Why this part exists |
|---|---|---|---|
| **A — PAYLOAD** | **one** Gate-2 work-package brief, `.cursor/plans/v4_INS_G2_operating_composition_and_sufficiency_brief_*.md`, with passport + catalog row + read-graph disposition in the same pass (AWP §5) | the substantive packet: two-axis result · exact refs · residual matrix · F1–F7 dispositions · both fixtures + the friction variant · A-Q12/14/15/16/17/18 · the three firewall laws · comparator plan · trace suite · acceptance and kill conditions · Task-D receipt contract | **a row cannot hold this payload**, and AWP's row-first rule explicitly permits a standalone artifact when a row is insufficient or the source is locked — **both exceptions apply here** |
| **B — STATE** | **`08_open_review_queue.md`**: update **`REV-160`** (carry `DEC-027`'s field list — the highest-value single row) and **`REV-185`** (three-law separation, Insurance as third dependent arc, administrative friction as a fourth economic-pressure pathway), plus **ONE Gate-2 parent row** grouping A-Q12/14/15/16/17/18 with closure criteria | the unresolved state anchor | **one parent row, not six thin ones.** Six lonely rows is the pattern the operator correctly fears; the measured 39% closure rate applies to rows someone owns, not to orphans |
| **C — DISCOVERY** | **`04_manifest_read_graph.md`** triggered routes for **cold entry**, not one Tier-2 node: Insurance → Gate-0 → 1a → 1b → Gate-2 · Pharmacy/formulary/substitution → C4.6 → the payer fixture · source authority → C3.6C → A-Q16 · temporal/async → C4.5 → A-Q17 · economics/ranking → REV-185 + row P · relation assertion → Accountability §19–20 → A-Q14. Plus **`05`** conflict rows and a **FWREG** entry | the paths a reader hits **without knowing the filename** | this is what makes the operator's *"the next author will be looking at the arc"* true **without mutating the arc** |
| **D — BOUNDED BACKPRESSURE** | direct notes **only** into surfaces that are open or designed for inbound state: **C3.6C** (`populated_G1_pending_review`) · **C4.5 next-actions** (`full_pass_not_started`) · the **Accountability `GRR-TASKD-INPUT` Input-State Receipt** · the **Care Response-Seam Audit** when opened. Each = incoming question ID + source ref + why it pressures that arc + required consuming gate + maturity, **nothing more** | pressure returned to the supplying arc | **C4.6 (accepted L2 doctrine) and REV-184 (signed off) get NO body edit** — carried at A/B/C only. **And an open surface is not automatically writable either: a bounded pointer lands only under that surface's own owner/gate or its designated inbound section. R7 implied the integrator simply owns edits to C3.6C and C4.5 — it does not** |
| **E — CONSUMER BINDING + DISPOSITION PROOF ★** *(vocabulary corrected R9)* | each carried finding names **(i)** the consuming artifact · **(ii)** the gate that cannot close without addressing it · **(iii)** the allowed disposition, **expressed in the canonical lifecycle/disposition vocabulary of the OWNING state surface — never a borrowed or invented one** · **(iv)** the owning state row updated on disposition · **(v)** the stop-report or handoff receipt proving which disposition was taken. **Per-surface, as they already exist:** future-work rows → **AWP §6's future-work enum** (`preserve_invariant_only \| keep_parked \| promote \| open_review \| reject_stale`) over FWREG's own row `status` (`parked \| watch \| candidate \| promoted \| rejected`) · open-review rows → **`08`'s own Queue Governance Lock states and closure contract** (`open \| open_non_blocking \| ready_for_signoff \| closed \| closed_split_to_children`, closure requiring source files checked · verdict · rationale · canonical destination or no-promotion reason · conflict/supersession linkage, at the row's own `required_reviewer` gate) · decision rows → **`03`'s decision lifecycle** (row `status` + `authority_level` + supersession linkage) · conflict rows → **`05`'s `resolution_status`** (`open_review \| split_review \| needs_catalog_update \| partially_resolved \| resolved`) under its Closure Policy Lock · contracts and artifacts → **their own review and maturity gates** (`review_gate`, `draft_for_ratification`/`analysis_nonbinding`, catalog status). Worked bindings: *C5/D6 cannot close without disposing of `REV-160` against `DEC-027`, in `08`'s and `03`'s own vocabularies* · *Gate 2 cannot close without explicit dispositions for A-Q12/14/15/16/17/18* · *the C4.5 pass cannot close without disposing of the Insurance partial-order fixture* · *the Care Response-Seam Audit cannot close without disposing of A-Q14* · *Spine/Task-D cannot close without disposing of row P's economics elevation* | **demand plus proof** — parts A–D make a finding *available*; E makes a named author's gate **fail** if they ignore it, **and produces evidence of what happened to it.** **The reusable law is the SHAPE, not one enum:** `named consumer + consuming gate + owning-surface-native disposition + state update + stop/handoff proof receipt` | **This binds to machinery that already exists rather than inventing any — and now binds to it at the right granularity.** AWP **§6** requires build-entry retrieval of matched future-work, open-review and guardrail rows, with its fixed enum scoped to **future work**; **§5** routes to canonical homes; **§9** stop reports can block closure; **§10** is the Non-Loss Requirement; the **Build Entry Gate** independently requires the retrieval; and each ledger already locks its own states. **A single universal disposition enum is NOT minted and NOT needed — imposing one would overwrite five existing owned vocabularies** (`GRD-026` at the lifecycle layer, and the §9.1 seven-labels failure inverted). **Without (v), a finding can be "required" indefinitely with nobody proving what became of it** — which is `DEC-027`'s state today |

**Two hard constraints on the transaction.** **(i) No re-derivation.** Each carry states *the question, its ID, its route and its maturity* — it never restates the finding, never claims Insurance solved the supplying arc's problem, and never upgrades a candidate's status in passing. **(ii) No status laundering.** `source_authority_map` stays `analysis_nonbinding`, `T-21` stays pre-pass, the Accountability capture stays REVIEW-READY and its naming question stays closed to this transaction. **A carry that quietly promotes a source is worse than no carry**, because the next reader inherits false confidence instead of a known gap.

**The one-sentence law:** *A row owns unresolved state. The Gate-2 brief owns substantive payload. The read graph owns discovery. Bounded inbound pointers return pressure to open source arcs. **A named consuming gate owns disposition — expressed in the owning surface's own vocabulary — and must emit proof.** None is sufficient alone.*

**Owner: the integrator, on a separate authorized branch, after the Gate-2 brief exists.** This lane writes one file and cannot land any of it.

*(Rule 3 relocated at R8 to sit with rules 1–2, above §15.1.)*

---

## §16 — UNDER-PRESSURED ARRIVALS (recorded so they cannot be mistaken for settled)

**Why this section exists.** Operator directive at R5: *"we cannot just declare that yes our 3 graphs are the right ones… let's not be too quick to crown ourselves."* Correct. Several R4/R5 arrivals were reached **by internal reasoning over the estate's own artifacts, with no external comparator pressure and no fixture execution.** They are recorded here as **arrivals under-pressured**, not as findings. **A reader who takes any row below as settled is misreading this carrier.**

| Arrival | How it was reached | What has NOT been done | Routed to |
|---|---|---|---|
| **The five typing axes** (§10.B.8 item 1) | composition of GCE + C4.6 + REV-184 + a candidate seam taxonomy | never tested against an external multi-principal workflow model; **the seam taxonomy's reuse outside its loop is asserted, not proven** | Gate 2 output 12 + 14 |
| **Three graph roles** (A-Q15) | reading three artifacts and noticing three graphs | **no comparator pressure at all.** Palantir's ontology-plus-Actions, event-sourced lineage models, and knowledge-graph practice would each answer "canonical vs derived vs projection" differently, and we have not asked | Gate 2 output 14 |
| **Field-level `source_authority_map` as the strongest current CANDIDATE for authority decomposition** (A-Q16) — *not "the cross-role authority model"; sufficiency unproven (R7)* | recovered from C3.6C, `analysis_nonbinding`, `populated_G1_pending_review` | never tested on **non-clinical roles at volume** — inventory, supply, facilities, admin, support staff — which is precisely the operator's question. Never compared to RBAC/ABAC/capability-security or enterprise-permission models | Gate 2 output 14 + C5 |
| **Sourcing selection as sufficient** (A-Q18) | C4.6 §9 read directly | **no fixture has ever run it**; the payer-as-funding-condition case is unclassified | Gate 2 output 13 |
| **Non-linear sequencing answerable by `T-21`** (A-Q17) | inference from a charter law to a use case | C4.5's full pass **has not started**; no trace exists; queueing, attention economics and concurrency at volume are untouched | Gate 2 output 13 + the C4.5 pass |
| **Two-axis verdict** (§14) | the reconciliation in §3–§4 | **no fixture executed against either axis**; Axis 2 is asserted, and its ten anti-shadow conditions are unrun | Gate 2 |
| **The neutrality posture** (§10.B.10) | corrected at R5 from a categorical claim to a testable one | the six dimensions have **never been evaluated against a real deployment**, and the inversion traces are unrun | Gate 2 output 11 + inversion trace |

**Three honest statements about this gate's method, which the estate's own guardrails require.**

1. **This arc was built from a silo and it shows.** Gate 1b reasoned outward from Insurance. Nearly every strong result — the GCE inheritance, the C4.6 composed-profile precedent, the `payment_care_firewall` lineage, `source_authority_map`, Sourcing selection, `C12`, row P's elevation — was **already in the estate and simply unconnected.** The recurring failure mode is not wrong physics; it is **physics written down in several places under several names with nobody in the room at once.** That is an argument for cross-arc pressure passes, not more arcs.
2. **Convergence between two reviewers is not corroboration.** `AGENTS.md` is explicit. Several R2–R5 arrivals are Opus-and-Knox agreement with **no third independent leg and no adversarial execution.** The estate's own precedent for high-stakes results — C4.3's quadrifecta with a white-box mutation adversary, C4.4's three-angle test, C4.6's blind Gemini leg — is **more rigorous than anything applied here.**
3. **No fixture in this carrier has been executed.** Both named fixtures are **specifications of tests, not results.** Gate-0's L1 limit still governs: *"no fixture has ever been run. Every claim here is paper."*

---

**Gate-1b analysis status — accepted pending cross-arc carry.** The two-axis verdict, the F1–F7 dispositions, the three-decomposition adjudication, the agreement and commitment dispositions, the three-surface implementation reconciliation, the economic-firewall recovery, both named fixtures, the residual and future-regime matrices, the Task-D boundary, the durability map, the cross-arc carry mechanism and the under-pressured-arrivals register are complete.

**On propagation, stated accurately.** Earlier text claimed propagation was finally verified at R6. **It was not** — R6 found four residues R5 had left live, then R7, R8 and R9 each found more, including two inside statements R6 and R7 had themselves just written. **The accurate claim is the narrow one: internal propagation is verified as of the R9 active-prose sweep recorded in §14, which is the current and final sweep for this gate.** Every prior sweep receipt is superseded by it. **Four consecutive rounds needed to converge is itself the finding** — it is the structural single-source defect §0.5's R8 note names, and it is the strongest argument that the Gate-2 payload must be a fresh artifact with one state surface rather than another layer on this one.

**Gate-1b ANALYSIS is accepted, as an *ownership reconciliation*, which is what it was chartered to be. The Insurance phase is NOT closed** — operational carry and Gate-2 preparation remain owed.

**What "the phase is not closed" means concretely.** Per §15.1, the phase closes only when **all five** land: the **Gate-2 brief** exists with its passport and catalog row · the **parent open-review row** plus the `REV-160` and `REV-185` updates exist · the **six cold-entry read-graph routes** exist · the **bounded inbound pointers** reach C3.6C, C4.5, `GRR-TASKD-INPUT` and the Care Response-Seam Audit · and each carried finding has a **named consuming gate with an owning-surface-native disposition and a proof receipt**. Until then the honest status is **`gate_1b_analysis_accepted_pending_cross_arc_carry`**, and the `DEC-027` precedent says exactly what happens if the transaction is skipped.

**It is also explicitly NOT ready as a settled architecture, and §16 says so.** Everything remaining is either (a) integrator routing this lane cannot perform, or (b) routed to `INS-G2-OPERATING-COMPOSITION-AND-SUFFICIENCY`, OPECON-G0, the C4.5 pass, the Care Response-Seam Audit, C5, or their own owners. **No further Gate-1b pass is owed. Substantial pressure work is owed at Gate 2, and this carrier names it rather than implying it is done.**

**STOP: `review_ready_pending_gate2_brief_and_cross_arc_carry`**
