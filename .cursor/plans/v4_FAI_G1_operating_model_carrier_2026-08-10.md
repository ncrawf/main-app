...
**This law is INHERITED, not authored here** `[INH]`. It is the ★★ operator-flagged constitutional finding already routed in the corpus at `EVSRC-2026-000270` — *"own the grammar of safe coordination, not everyone's reality"*, `MASTER GUARDRAIL: plurality of description ≠ abolition of the discipline of truth at commit` — reinforced by `AB-13`'s `already_present` rejection of a global event graph and a global semantic layer, wave-5 `EVSRC-2026-000271`'s **`REJECT single-company-graph`**, `INV-19`'s **no globally resolved authority graph**, `INV-21`'s **observer-scoped projections**, and `T0-14` operator-neutrality. Receipt at `§E14.9`.

**What Output 3 authors is small and is stated as such:** the **six-object decomposition** and its non-collapse laws, the **binding sites** for constraints, **typed composition with authority non-propagation**, the **conflict-as-resource** split, the **resolvability classes**, and the **rejection-ground matrix**. Everything else below is inheritance made explicit at relation grain.
...
| **6** | **`EffectiveGraphProjection` / `TraversalResult`** | a derived result for a named namespace · profile · scope · temporal mode · `effective_as_of` · knowledge cutoff · policy/ruleset revision · coverage envelope | `generation_id` + pinned input manifest `[INH §E13.9]` | **nobody** — emitted, never hand-edited | canonical truth · authority · the graph |
...
**Object 1 is the object the entry receipt got wrong.** A steward-owned relation registry would have made one seat the universal semantic owner — "inventing a cleaner-but-equally-totalizing ontology," which is the exact failure `EVSRC-2026-000270` was ingested to prevent. **Withdrawn.** Recorded as `G1-FIND-66`.
...
**Relation-type versioning.** A type definition can change; historical queries must resolve **which type revision governed each assertion and each projection**. Assertions pin the governing type revision; projections pin it in their input manifest — the same discipline as `§E9.4`'s M2 pinning `[INH]`. Fork/merge/split/supersede with version-at-time is the `living_classification`/`classification_lineage` shape already routed for Settings/Catalog `[INH]`; applying it to relation types is this pass's reconciliation, not a mint.
...
Two derivation kinds, kept apart `[INH §E7.1.5]`: `specialization` **adds**; `constraint` may only make new rules about existing content, which makes loosening **structurally impossible rather than policed**. A `specializes` edge may never relax an inherited prohibition. The residue is precisely bounded and stays routed: where a rule body is prose, no mechanism can compare two prose rules `[CAN→6]`, and `NC-8`'s semantic check remains open to Outputs 3+6 `[INH §7.3]`. Clinical Memory independently carries LOOSENING/TIGHTENING semantics on `concept_version` `[INH CM §6.1]`.
...
**Supersession is a typed transaction, never a comparison** `[INH §E7.1.2]` — six conditions, no ordering anywhere. **One prohibition survives and is re-anchored here:** a resource whose `normative_effect` is nonbinding, informative or evidentiary-only may not supersede one whose `normative_effect` is binding. *(Output 1 stated this over `normative_force`, which R6 then split into `normative_effect` ⟂ `epistemic_role[]`; the rule is unchanged and now names the current facet. `G1-FIND-68`.)*
...
**Detection may propose; adjudication is authority-bearing** `[INH]` — controllers emit candidates, and evidence never manufactures an obligation or an authority (`G1-FIND-60`, `-61`).
...
**An operational alias is a scoped, intervalled binding**, attributable like any other assertion. The semantic requirement is **conditional uniqueness** `[KND]`:
...
**This is the deliverable `§G1-CONTRACT` row 3 asks for.** Each ground names what is rejected, on what basis, and with what consequence. **Rejection is decidable and attributable; it is not the same as well-formedness.**
...
| **FX-5** | The one explicit "Edge kinds" set fuses three relation types with one adjudication state, and a fifth value was minted inline | **live**, §6.1 | **Decomposition repair** (`G1-FIND-67`); not a rejection, and **not repaired here** |
...
| **FX-7** | The surviving supersession prohibition named `normative_force` after R6 split that facet | **live**, `§E7.1.2` vs `§E8.1.1` | **R15 re-anchored** (`G1-FIND-68`) |
...
**`G1-FIND-66`** — the arc proposed a steward-owned universal relation registry, which is precisely the *"cleaner-but-equally-totalizing ontology"* its own ★★ operator-flagged source (`EVSRC-2026-000270`) exists to prevent. **Withdrawn before authoring.** The corpus had routed the correct shape — shared grammar, plural vocabularies, no global owner of truth — in wave 5, and no arc had consumed it. **This is `L-1`'s pattern in its most expensive form yet:** not a re-derivation of an existing rule, but a proposal that *contradicted* one. → `architecture_steward`.

**`G1-FIND-67`** — the estate's only explicit edge-kind declaration (`v4_pre_spine…map` §6.1) fuses three relation types with one adjudication state in one set, and a fifth value was minted inline beside it. Decomposition repair, **not performed here**; the map's rightful writer owns it. → `architecture_steward`.

**`G1-FIND-68`** — the one surviving forbidden supersession transition was stated over `normative_force`, a facet R6 subsequently split into `normative_effect` ⟂ `epistemic_role[]`. Re-anchored at `§E14.5`; **no semantic change**. → recorded.
...
| `§6.1` edge-kind decomposition repair (`G1-FIND-67`) | **that map's rightful writer** |
...
**`OUTPUT_3_PROPOSED · NOT_ACCEPTED`.** Output 3 moves from `INCOMPLETE` to `PROPOSED`; it does not move to complete, and **G1 remains incomplete**. Acceptance requires independent review and steward acceptance on the standard Outputs 1 and 2 met. `B-15` narrows to outputs 4–7 plus Output 3's acceptance. **Nothing is installed; no schema, registry, route, gate, checkpoint, lane, FWREG row or artifact is created; no graph technology is named; no validator is written.**
...
| **OUTPUT 3 · 2026-08-15** | **`§E14` authored — architecture graph semantics, `PROPOSED · NOT_ACCEPTED`.** Entered under **`KNOX_OUTPUT3_ENTRY = READY_TO_AUTHOR_WITH_EXACT_SEMANTIC_LOCKS`** after an independently reviewed entry receipt; **eleven locks applied during authoring, and one of them removed the proposal's central defect before a line was written.** **The founding law is INHERITED, not authored:** the mandatory Lane-3 `M-106` check (`§E14.9.1`) recovered `EVSRC-2026-000270` — wave 5, `full_semantic`, ★★ operator-flagged — carrying *"own the grammar of safe coordination, not everyone's reality"*, the **6-layer shared-grammar/plural-vocabularies model**, `ontology_admission_gate`, `semantic_translation_profile`, `local_concept_portability`, `living_classification`, `omitted_relation_check`, and the master guardrail **plurality of description ≠ abolition of the discipline of truth at commit**. **The raw anchor was reopened** because the claim became load-bearing, per `G1-FIND-40`'s chain. **The entry receipt's steward-owned universal relation registry is WITHDRAWN** (`G1-FIND-66`) — it was the totalizing move that source exists to prevent, and no prior arc had consumed it. Authored: the **six-object decomposition** (relation-type definition ⟂ assertion ⟂ admission decision ⟂ constraint/composition rule ⟂ conflict record ⟂ effective projection) with five non-collapse laws · relation-type ownership **following the scope of the meaning**, catalog owning no semantics · **constraint binding sites** (direct ⟂ admitted ⟂ effective) · subject/object cardinality with representational ⟂ conformance preserved · a **constraint surface** for set-level exclusivity rather than pairwise flags · **typed composition over relation pairs** with the **authority non-propagation law** and full path-result preservation · **relation bitemporality SETTLED**, discharging an Output-1 `[CAN→3]` · the temporal-mode binding and no-hindsight at relation grain · invalidation as a predicate · **conflict as a first-class resource**, precedence as a reference and never a scalar · **six resolvability classes**, splitting identity from locator · `realizes` chosen as **one** predicate, `embodies` withdrawn as a synonym · **operational-alias conditional uniqueness** built on Identity inv 9 `handle ≠ person` and AWP `L3` · **cessation/succession non-collapses** · and a **34-row rejection-ground matrix** of which **six grounds are invisible to well-formedness checking**. Three findings (`-66`, `-67`, `-68`). Frontier evaluation records one honest `current_practice_only`: admission review at machine-scale assertion volume. **`B-8` unchanged. `§6.1`'s fused edge-kind set named, not repaired. One file changed; no checkpoint, catalog, read-graph, ledger, FWREG, contract, code, workflow or new artifact; no graph technology named; no validator written; not self-accepted.** |
