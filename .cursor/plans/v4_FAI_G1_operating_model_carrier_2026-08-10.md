...
>
> **Two constraints keep it from becoming the centralizer it exists to avoid** `[KND]`. **① It references and attests; it copies no substantive state** — the inherited `receipt` law `[INH §E13.4]`. The constituent judgments stay with their issuers; the basis holds identifiers and revisions, so it is a **deterministic derivation over references** even though the judgments it references are attributed and not derivable. **② Whether the basis carries its own identity, or is simply the reference set the decision record itself holds, is a physicalization question for G2.** This output requires the **law** — exact, immutable, reconstructable, substance-free — and deliberately does **not** mandate a named wrapper object, because mandating one is how a reference set becomes a document and a document becomes an authority.
...
| `remedy_obligations[]` | that a remedy **is owed**, with the role algebra of `§E15.9.1` | the roles named there — **not one owner** | recognized, then live |
| `remedy_capability_assessments[]` | whether a remedy pathway **exists and is usable** for a subject and scope (`§E15.4.3`) | the party able to perform it, or the domain owner where feasibility is domain-determined | prospective, revisable |
...
| `compensatable_not_reversible` | no semantically valid reversal exists **and** a compensation or remedy pathway is available, required or planned |
...
> **"No path identified" is NOT a path kind.** It is a **derived absence over the assessment set and its coverage** — and it decomposes into three states that must not share one token: **no path has been established** · **coverage is incomplete, so we do not know** · **a path was assessed and is unavailable.** *Minting `none_identified` as a value inside the kind enum would put an epistemic and a coverage result inside a taxonomy of remedies — the malformed-enum defect this whole section exists to remove, committed in the field added to fix it.*
>
...
### §E15.4.5 — Scope refinement, and why characterization does not explode at fleet scale

**Scope is a tree, not a flat parameter** `[KND]`. Federation's topology nests deployment ⊃ legal entity ⊃ brand ⊃ site ⊃ location ⊃ venue, and cohorts cut across it. If every characterization record had to exist at every leaf, a single protocol change would demand tens of thousands of assessments; if one root record silently bound every leaf, that is the compression this output refuses. **Neither is acceptable, and the estate already owns the resolution.**

> **An assessment made at a coarser scope is a CANDIDATE for the finer scopes beneath it. A finer scope may adopt it, refine it, or reject it under its own authority. Absence of a finer-scope assessment is TYPED ABSENCE — it is not agreement, and it is not dissent.** `[INH §E12.7.2` typed absence · `C4.4 §R.16` local admission · `AB-05`]

**This is the local-admission law applied to characterization, and it is the third object class in this output to take it** — after peer-operator assertions and architecture releases. It is not new architecture; it is the same law, and `AB-05`'s scope extension is what licenses reaching a new object class with it.

**The consequence is the one that matters operationally** `[KND]`: **characterization cost scales with genuine divergence, not with fleet size.** One assessment at the coarse scope, plus records only where a scope's physics actually differ, plus typed absence everywhere else — and the resolver can always state, for any leaf, whether it is answering from that leaf's own record, from an inherited candidate, or from nothing. **A projection that cannot say which of those three it used is not a characterization; it is an average.**
...
| `recognized_or_adjudicated_by` | who **established that this basis applies here.** A first-class reference, not a process step |
...
| `waiver_or_discharge_authority` | who may release it, and on what terms |
| `adequacy_authority` | who determines the remedy **discharged what was owed** |
...
| `CL-38` | `permits_effective_use` read as effective use, or `effective(...)` restated with fewer conjuncts than accepted `§E13.10` requires | `§E13.10` · `§E14.1.4` · `§E15.7` |
...
| `CL-44` | a decision that cannot state the exact, immutable assessment basis it reviewed, or a basis object that copies substantive state rather than referencing it | `§E15.4.1` · `§E13.4` receipt law |
...
| `CL-50` | an obligation whose basis, recognizing authority, obligor, owed-to party, waiver authority, adequacy authority, executor and work custodian are not separately held | `§E15.9.1` |
...
| `CL-57` | a coarse-scope assessment silently binding finer scopes, or absence at a finer scope read as agreement | `§E15.4.5` · `§E12.7.2` |
| `CL-58` | a characterization projection that cannot state, per scope, whether it answered from that scope's own record, an inherited candidate, or nothing | `§E15.4.5` |
...
| 28 | **one protocol change across a nested fleet** | one coarse-scope assessment; finer scopes adopt, refine or reject under their own authority; **absence at a leaf is typed absence, not agreement**; the resolver states for any leaf whether it answered from that leaf's record, an inherited candidate, or nothing (`§E15.4.5`) |
...
| **the Lane-3 `M-106` EXISTS-AS receipt (R8 `§3.9.1`) and the frontier double-evaluation (R8 `§3.9.2`)** — both owed, neither satisfied (`G1-FIND-95`) | **a bounded pass before acceptance**, with predeclared functions and no raw-corpus rescan unless a registry result becomes load-bearing |
...
| 2 | The register is a **declared-open minimum with a coverage posture**, never a census; uncovered subjects report `unknown` and never clean (`§E15.1`) |
...
| 6a | **Scope is a tree**: a coarser assessment is a **candidate** for finer scopes, refusable and refinable under their own authority, with **typed absence** beneath — the local-admission law applied to characterization (`§E15.4.5`) |
...
**Every lock imposed at entry is recorded with its disposition, and the honest reading is that instantiating the locks FALSIFIED several of them** `[KND]`. **The prior form of this table said all locks were successfully applied. That is withdrawn.** Six locks were partly falsified by being implemented: `O4-L03` (the eleven-subject list was itself under-decomposed), `O4-L08` (the five specified dimensions were internally fused and omitted an inherited axis), `O4-L13` (the chain shape implied a universal ordering), `O4-L14` (the accepted seven-value baseline is itself a fused axis), `O4-L18` (the erasure absolute contradicted this output's own routing) and `O4-L22` (several fixture expectations over-decided results the applicable policy owns).

**Attribution is recorded precisely rather than as a blanket, because "the lock was wrong" and "the author was wrong" are different facts and both occurred** `[KND]`:

- **Lock defects, faithfully implemented.** `O4-L08` specified `reversible_with_migration` as a *value of the reversibility axis* and specified `compensation_posture` with applicability, capability, obligation, execution and adequacy fused into one enum. Both were implemented as written and both are `F-08`. **The lock carried the defect.**
- **Author defects, not shelterable behind a lock.** The **omission of accepted Output 3's semantic-compatibility axis is mine alone** — `§E14.2.1`'s three axes were in my read set and cited by me, and I substituted reversibility for compatibility while claiming to carry the axis forward. Likewise the **`effective(...)` abbreviation** in `§E15.7`, the **`permits_effective_use` conflation** in fixture 10, the **different-fact-non-conflict** sentence contradicting `§E13.11` verbatim, the **"nobody writes it"** snapshot regression, the **"generate obligations"** verb against `G1-FIND-61`, and **four absolutes each falsified by something else I wrote in the same section.** No lock required any of those.

**The generalizable result** `[KND]`: **a lock is a hypothesis about a decomposition, and authoring is the experiment that tests it.** Six locks survived contact unchanged; six did not; the ones that did not were only detectable once instantiated against real subjects. That is the review topology functioning, and it is the argument for instantiating early rather than perfecting locks in the abstract.
...
| `O4-L03` subject register | **FALSIFIED BY INSTANTIATION, then applied.** The eleven-item list bundled objects with different owners, clocks and survival physics. **Replaced by a splitting rule with no maintained count**, plus a coverage posture so removing the count did not remove the closure claim | `§E15.1` |
| `O4-L04` direct scope | **APPLIED**, count removed — the ownership column is the enumeration | `§E15.1`, `§E15.0` |
...
| `O4-L08` five-class decomposition | **LARGELY FALSIFIED, then rebuilt.** The five specified dimensions were internally fused — `reversible_with_migration` named a treatment inside the reversibility axis; `commitment_frontier` was a scalar over many events; `external_consequence` mixed exposure stage, consequence kind and epistemic posture; `compensation_posture` mixed applicability, capability, obligation, execution and adequacy — **and the specified set omitted accepted Output 3's semantic-compatibility axis, which that output explicitly routed here.** Replaced by independently owned record families with prospective/observed separation. **The fusions were the lock's; the omitted inherited axis was mine** | `§E15.4` |
...
| `O4-L13` local sovereignty | **FALSIFIED IN PART, then applied.** The `≠`-chain shape implied a universal ordering the estate does not have. **Replaced by three genuine prerequisites plus a non-implication table**; `C4.4 §R.16` extended, not duplicated; the inherited `effective(...)` restored unaltered | `§E15.7` |
| `O4-L14` skew vocabulary | **FALSIFIED IN PART, then applied with a DECLARED DEPARTURE.** The accepted seven-value baseline is itself a fused axis and is now decomposed into independently owned dimensions. **The review's further instruction to restate `reconciled` over those dimensions is DECLINED** — restating an accepted Output-2 predicate is amendment-by-summary, reserved to Output 2 by `§E13.23` and forbidden to this pass. The accepted seven are preserved as **derived profiles**, `reconciled` is left unamended, and any change to it is routed as an Output-2 reopening | `§E15.7` |
| `O4-L15` temporal / no-hindsight | **APPLIED** — `resolve(...)` as a semantic interface, not a storage schema | `§E15.11` |
| `O4-L16` distributed history | **APPLIED** — no god log; the composing sources are named without a maintained count; **the entry packet's contrary position is not carried** | `§E15.11` |
| `O4-L17` source/applicability change | **APPLIED** — all seven steps | `§E15.10` |
| `O4-L18` cessation/survival | **FALSIFIED IN PART, then applied.** Per-contract declaration holds; **the erasure law was stated as an unconditional absolute and is narrowed to "by itself,"** because a separate lawful authority-bearing retention, redaction or compelled-forgetting decision may alter payload retention — which this output's own routing already assumed | `§E15.13` |
...
| `O4-L22` proof | **APPLIED** — every named fixture obligation is present, plus four this output adds (security revocation against a sovereign member · mandatory change under an `OMNI-managed` posture · retry-then-changed-position · concurrent transitions on one fact). **Count-light by design** (`F-06`): the table is the authoritative enumeration and no total is maintained here. **FALSIFIED IN PART on expectations** — several fixtures over-decided results the applicable policy owns (local decline, offline member, impossible compensation) or asserted an incorrect predicate (`permits_effective_use` on admission-without-adoption); corrected, and three fixtures added for cross-fact collision, characterization divergence and incomplete characterization at approval | `§E15.16` |
...
**R2 instruction disposition — three narrowings and one withdrawal, each with repository evidence** `[KND]`. The R2 correction package was applied in full except where a live fact defeated an instruction; those are recorded here rather than silently implemented:

| Instruction | Disposition | Evidence |
|---|---|---|
| Pin the assessment basis in a named **assessment-basis manifest** object | **NARROWED to a law, not an object.** The requirement — exact, immutable, reconstructable, reference-only — is applied; a mandated wrapper object is not, because the same packet warns that a wrapper becomes a centralizer. **Whether the reference set carries its own identity is G2's physicalization question.** The reference-only constraint is grounded in accepted `§E13.4`'s `receipt` law — *"references and attests; copies no substantive state"* — rather than minted | `§E13.4` receipt row; `§E15.4.1` |
| Record **`none_identified`** as a correction-path kind *(this author's own earlier proposal)* | **WITHDRAWN by the author.** Putting an epistemic-and-coverage result inside a taxonomy of remedies is the malformed-enum defect this output exists to remove, proposed inside the fix for a malformed enum. Absence is **derived**, and it decomposes into three states — none established · coverage incomplete · assessed and unavailable | `§E15.4.3` |
| `owner_set_complete` **fails until** environment/deployment-target ownership resolves | **NARROWED.** Accepted `§E13.10` scopes the predicate to *"every authority **required for the consequence class**"* — so an unresolved target authority blocks the transitions that require it and no others. **Read globally it would halt the entire deployment plane until the estate settles a question Output 4 is forbidden to settle** | `§E13.10`; `§E15.1` |
| *(from R1, still standing)* Restate `reconciled` over the decomposed skew dimensions | **DECLINED, unchanged.** Amendment-by-summary of accepted Output-2 content, reserved to Output 2 by `§E13.23`. Independently confirmed by both reviewing seats | `§E13.23` |

**One review characterization narrowed, recorded because misattributing a defect's origin builds the wrong mechanism** `[KND]`: the cold pass reports that R1 *"drops accepted predecessor requirements"* from `effective(...)`. The register row states all six conjuncts correctly; `§E15.7` then **added** one. **The defect is an extension, not a drop** — and `CL-38` now says explicitly that abbreviation and extension are the same violation, which the earlier form did not.

**One earlier author claim withdrawn** `[KND]`: the R1 self-audit scored blocker 6 **complete**. It was not — `§E15.7` read *"Output 4 adds exactly one thing"* and fixture 10 called adoption *"a required conjunct."* **The independent cold pass caught what the author's own audit missed on a predicate the author had just repaired**, which is the strongest available evidence for the asymmetric topology and against self-audit as a substitute for it.
...
**`G1-FIND-95` — Output 4 shipped its first proposal with no `M-106` receipt and no frontier double-evaluation, and no surface caught it until independent review.** R8 `§3.9.1` requires the Lane-3 EXISTS-AS check against the six routing maps before any novelty claim, and `§3.9.2` requires the current-practice-versus-2030/2035 evaluation. **Outputs 1, 2 and 3 each ran both** (`§E12.8`, `§E13.21`/`§E13.20`, `§E14.9.1`/`§E14.9.2`). Output 4 ran neither — and `G1-FIND-87` then found, *by accident during the correction pass*, that the estate already held per-object release lifecycles and four treatment non-collapses that a predeclared registry check would have surfaced before authoring rather than after. **The gate obligation is not decoration: skipping it is how this arc keeps re-deriving what it already owns.** Owed before acceptance, with predeclared functions covering change characterization · lifecycle projection · commitment frontier · migration/coexistence/deprecation · additive correction and compensation · source/applicability propagation · historical reconstruction and rollback · release/adoption/skew · lifecycle idempotency and semantic collision. → `architecture_steward`.
...
**`G1-FIND-98` — `G1-FIND-75` recurred in this correction pass and was caught in-pass by a mechanical sweep rather than by vigilance, which is the only part worth generalizing.** The R1 corrections initially embedded five inline correction narratives — *"the prior formulation said…"* — into `§E15.6`, `§E15.7`, `§E15.13` and `§E15.15`, which is precisely what `G1-FIND-75` records Output-3's R1 doing and R2 stripping, under the operator's standing instruction that **model sections state current law only.** The mechanism is identical each time: **a correction pass is written for the reviewer who found the defect, so narrating the fix inline feels like accountability while making the law harder to read.** They were removed and the substance retained in the findings and the lock table, where it belongs. **What made this instance catchable was searching the authored range for the pattern's shape, not remembering the rule** — which is the argument for `FWREG-021`'s relationship validators being built rather than admired, now on its fourth occurrence across four consecutive passes. → `architecture_steward`; `FWREG-021` validator candidate.

**`G1-FIND-99` — the correction pass reproduced its own failure class at a smaller grain, and the pattern is now measurable rather than anecdotal.** R0 carried ten blocking defects; R1 repaired them at the named surface and carried eleven residuals, of which **four were defects repaired in one surface and left standing in another inside the same commit** — ownerless generation, the manufacture verb, the candidate/admission step, and the assessment-authority overreach. R2 then found the same shape again: the `§E15.1` splitting rule stated and not applied to `§E15.4.2`, the obligation row, or the register's own `observation / fleet evidence` slash. **Three consecutive passes, each stating a law and violating it within the same document.** The generalizable result is not *try harder*: it is that **a correction pass must enumerate the laws the section states and mechanically search the section for its own violations, as a declared method step rather than as attentiveness.** That sweep is what caught `G1-FIND-98` and what would have caught `-96`, `R-2`, `R-8` and `R-9` before review. It is `FWREG-021`'s relationship-validator case, now recorded for the fifth time across five passes — **and recording it a sixth time is not the remedy.** → `architecture_steward`; `FWREG-021`, parked on G2 admission.

**`G1-FIND-100` — the independent cold review was independent in its answers and dependent in its coverage, and that limit is structural rather than a criticism of the reviewer.** The external pass reconstructed the artifact without author or reviewer materials and produced two findings neither prior seat held — the assessment-basis join identity, and an `effective(...)` extension the author's own audit had scored complete. **It also missed six defects the author found**, including a direct contradiction of an accepted range. The pattern is explicable: the question set the cold pass answered was authored by a prior review seat, and it emphasized decomposition and non-regression — which is exactly where the cold pass found things — while under-weighting attribution, admission and non-derogable floors, which is exactly where it missed. **An external reviewer given a question list inherits the list-author's blind spots even when it never sees their findings.** The durable correction: a cold pass should receive the artifact and the governing sources and be asked what matters, **not a checklist authored by a seat whose coverage is one of the things under test.** → `architecture_steward`; collaboration-model routing.

**`G1-FIND-101` — the FAI arc's program state exists only on an unmerged branch, so the estate's boot surface does not know Output 4 exists.** The external pass booted on `main`, resolved the checkpoint pointer there, and reported the current gate as the Insurance Gate-2 checkpoint — which is **correct for `main` and wrong for this review object**, and must not propagate into this carrier. But the underlying fact it surfaced by accident is real and unrecorded: **`AGENTS.md` and read-graph Tier-0 #15 are repointed to the FAI checkpoint only on `cursor/fai-g1-operating-model-4933`.** A cold agent booting the repository at `main` cannot discover this arc's state at all. That is tolerable for a proposal branch and it is **not** tolerable to leave unstated, because it is `D0CKPT-GRD-001`'s condition one level up: the boot path is correct on the branch and silent everywhere else. **Consequence for sequencing: the eventual state synchronization is not complete when the branch surfaces agree — it is complete when the merge that carries them lands.** Declared, not repaired here. → `architecture_steward`.
...
**Verified live during the R1 correction pass, before amending anything.** Every blocking claim in the review was checked against bytes rather than accepted on assertion: **`§E13.11`** confirmed verbatim that *"base movement and file disjointness are not collision control"* and supplied the inherited collision envelope this output now references rather than re-derives · **`G1-FIND-61`** confirmed verbatim that accepted policy *"may automate recognition or instantiation; it never manufactures the underlying authority"* · **`§E13.10`** confirmed the full six-conjunct `effective(...)`, and confirmed that `§E15.1`'s register row had stated it correctly while `§E15.7` abbreviated it — a propagation defect **within one section**, and the third recorded drift on this exact signature after `D9` · **`§E13.4`/`§E13.9`** confirmed `generation_id` and `publication_id` carry named owners, falsifying *"nobody writes it"* · **`§E14.1.4`** confirmed `permits_effective_use` is scoped to the admission decision's own effect · **`§E14.2.1`** confirmed the three change-posture axes and the omission of semantic compatibility. **One review characterization was narrowed rather than absorbed**: the review states the proposal *"drops accepted predecessor requirements"* from `effective(...)`; the register row never dropped them, so the defect is intra-section propagation, not a wholesale narrowing — recorded because misattributing a defect's origin builds the wrong mechanism to prevent it.
...
**`OUTPUT_4_PROPOSED · NOT_ACCEPTED`** — at **R2**, applied on a three-seat disposition: an author-side self-audit finding eleven residuals, an independent cold artifact reconstruction finding four material defects (two of which no other seat held, including one the author's own audit had scored complete), and a final adjudication of `NOT_ACCEPTED · CONSTITUTIONAL_SHAPE_HELD · NO_BROAD_REARCHITECTURE · ONE_BOUNDED_R2`. **The constitutional object is unchanged and was not reopened by any seat.** R2 applied the recursive corrections: the founding statement rewritten to the records-and-profiles construction · inherited predicate names restored with no alias minted · assessment separated from approval with a reference-only assessment basis · generated projections made accountable · characterization split at record grain with treatment determinations made immutable and Output-3's vocabulary de-universalized · obligation given a role algebra with a contested-duty path · five coverage kinds separated with the model gap made non-bootstrappable and the non-derogable floor restored · `effective(...)` left neither abbreviated nor extended · divergence routed through candidate → admission and scoped · the register's own slash-row and open-ownership fill repaired · correction paths made assessed records with absence derived · cohort frontiers made representable under partial knowledge. **Three instructions were narrowed with repository evidence and one earlier author claim was withdrawn**, all recorded at `§E15.17.4`. Findings `-99`…`-101`. **R1's earlier claim that the ten blockers were corrected is withdrawn: six carried residuals, and the author's own audit missed the `effective(...)` extension that an independent pass caught.**

*(Prior R1 disposition, retained as lineage: `KNOX_OUTPUT4_REVIEW = NOT_ACCEPTED · CONSTITUTIONAL_SHAPE_HELD · NO_BROAD_REARCHITECTURE`.)* **The constitutional core is unchanged**: several typed owner-scoped lifecycle contracts joined by a minimal algebra; no universal scalar, no universal state machine, no god log, no state-derived authority, no historical erasure. **Ten blocking semantic defects were corrected in place** — the register split by object physics with the count removed and a coverage posture added · the fused character vector replaced by independently owned record families with the inherited semantic-compatibility axis restored · prospective assessment separated from observed consequence with divergence made first-class · the legacy profiles repaired and their genus/species relation named · phases renamed to non-exclusive predicates · the inherited `effective(...)` restored and the chain replaced by a partial order · the skew axis decomposed · four absolutes narrowed · the semantic-collision inheritance restored · and obligation origin corrected to recognition rather than manufacture. **Every blocking claim was verified against live bytes before amending, and one review instruction was declined on inheritance grounds and recorded rather than absorbed** (`O4-L14`, restating `reconciled`). Findings `-94`…`-97`. Output 4 moves from `INCOMPLETE` to `PROPOSED`. It does **not** move to complete. **G1 remains OPEN and INCOMPLETE**; `§G1-AUTH` remains a mandatory work package, partially converged; **`B-15` is unchanged and still spans outputs 4–7** — only acceptance narrows it, which is the distinction Output 2 established and Output 3 observed. **No carry is discharged.** **Every enumerated value set remains `[CAN]`.** **No ledger row is closed** — `AB-05`, `AB-06`, `AB-07`, `INV-11`, `INV-24`, `INV-26`, `INV-27`, `AB-20`, `AB-29` retain their recorded dispositions and blocking scope. **Nothing is installed; no schema, registry, validator, route, gate, checkpoint, lane, FWREG row, contract, workflow, code or artifact is created; no plane is minted; no vendor or technology is named; Output 5 is not started.**
