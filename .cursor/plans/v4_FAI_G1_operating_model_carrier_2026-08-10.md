...
**Human contributions are first-class and that is not a claim about authority.** They must not be lost — a one-line operator objection can redirect an arc, and a model weighting by volume would systematically under-record the operator. But **first-class is not authoritative**: the operator may be right, wrong, drifting, hyperfocused, or deliberately probing, and **being human is not one of the axes that confers force.** The same actor's messages carry different weight depending on the **act performed and the seat exercised**: speculation is evidence, an instruction is an assignment act, an objection is evidence or a blocking act depending on the grant, and an acceptance is a commitment. **This is `G1-FIND-61`'s fourth instance and the operator is not exempt from it** — a human's evidence no more manufactures authority than a detector's does. Treating operator input as *"assumed context"* loses the act; treating it as automatically authoritative invents one.
