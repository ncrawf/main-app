...
**One claim in this section was wrong, and its being wrong makes `G1-FIND-27` stronger, not weaker.** *"Nothing else in the estate depends on them"* **understated the blast radius**: the normalization sweep found the same derived count independently asserted in **two further places** — parent read-graph **route `9v`** (*"81 rows, 7 open"*) and the **PRE-0 ledger's own catalog row**. So one canonical fact was independently duplicated in **four** projections, not two, and **three** of them drifted the moment it changed. Route `9v` and the catalog row were normalized to **resolve the open set and count from the binding ledger** rather than restate them — removing the duplication instead of re-synchronizing it. The count-bearing surfaces that remain are now the ledger (canonical) and the checkpoint (a deliberate Tier-0 restatement).

**This does not discharge the mechanism.** `architecture_memory_lint` still does not exist; four hand-found divergences are precisely the evidence for building it, and the finding stands as Output 2's seed with its blast radius corrected upward.
...
| Next | **The acceptance transaction is LANDED (§7.5), and its state divergences are NORMALIZED (§7.5.1a).** Remaining, in order: ① **verify the state-normalization diff**, after which that bounded grant EXPIRES *(the acceptance-landing `integration` grant already expired on its own verification)* · ② **Output 2 opens in a FRESH context** and nothing else in G1 proceeds in this one · ③ **Output 2's first executable proof is the propagation defect this arc has now produced TWELVE times — the eleventh inside its own acceptance transaction, the twelfth found while repairing the eleventh, where ONE derived count turned out to be independently asserted in FOUR projections:** one canonical change → impact traversal → required decisions → accepted update → **regenerated projections** → divergence detection → correction work item → verification receipt. **No loop diagram earns credit without that.** |
