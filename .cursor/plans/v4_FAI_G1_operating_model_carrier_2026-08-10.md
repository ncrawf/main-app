...
| **4** | **`RelationConstraint` / `CompositionRule`** | a rule over cardinality, coexistence, incompatibility, inheritance, cycles, non-loosening, or permitted composition | its own identity + revision | the authority owning the constrained **scope** | a relation assertion · an instance claim |
| **5** | **`ConflictRecord`** *(challenge / adjudication)* | the governed object carrying participants, type, basis, evidence, owner, adjudicator, policy, status, stay, decision, interval, reopening and lineage | its own resource identity | detection may propose; **adjudication is authority-bearing** | a state value on an edge · a boolean |
...
1. **A direct assertion is not an effective edge.** Effectiveness is a *join* of origin validity, active local admission and interval containment, and those are necessary and **not sufficient** `[INH §E12.7.2]`.
...
|---|---|
| `AdmissionDecision` | `target_kind` · `target_revision_ref` |
...
2. **A foreign assertion may not enter a receiving scope's effective graph unless its predicate semantics are locally admitted** — either its **exact foreign type-definition revision** is actively admitted, **or** an **actively admitted mapping assertion** maps it to an admitted local type — **and** the assertion revision is itself admitted or applicable. **Instance admission never implies semantic admission.**
...
predicate_semantics_usable(N, type_rev, scope, as_of) :=
      own_effective_type(N, type_rev, scope, as_of)
        -- N's own effective type revision. NO self-admission exists or is required.
   OR active_type_admission(receiver=N, exact foreign type_rev,
                            permits_effective_use=true, scope, as_of)
   OR usable_mapping_to_local_semantics(receiver=N, exact foreign type_rev,
                                        scope, as_of)          -- §E14.3.3
...
**The obvious fix is wrong.** Minting one architecture-plane `maps_to` predicate would collapse **equivalent · narrower-than · broader-than · conditionally-translated-as · approximately-aligned-with · transformed-under-a-named-rule** into one edge. Those differ in **conformance consequence** — a *narrower-than* mapping cannot carry an obligation stated over the broader type, and an *approximate* mapping cannot satisfy a gate that an *equivalent* one satisfies. Under the `AB-08` warrant test a new kind is justified exactly where lifecycle, authority, ownership or **conformance semantics differ materially**, so these are distinct types. **A single `maps_to` would be `owns` again, one section after `§E14.2.2` rejected `owns`.**
...
**Usability — a mapping is not a shortcut around admission** `[KND]`. A mapping participates in `predicate_semantics_usable` only when **all** hold: the mapping assertion is **origin-valid** · its **own governing mapping type is locally usable** under `§E14.3.2` (guard ④ — kernel and mapping types are not auto-usable across namespaces) · the **exact mapping assertion revision is admitted** where the mapping is foreign · the **target type is locally usable** under the same function · and its **scope, interval and conditions hold**.

**Mapping chains do NOT compose by default** `[KND]`. `A→B` and `B→C` yield no `A→C` unless an explicit `CompositionRule` declares it, with full path lineage and **cycle-safe evaluation** — the same default-deny law as `§E14.4.5`, and the reason a mapping graph cannot silently become a transitive translation layer.
...
| `RelationConstraint` revision | recorded time **always**, **+** effective interval for **any constraint capable of changing admissibility, conformance, effective resolution, rejection or blocking**. *(No hedge: a purely advisory or evidentiary statement is **not** a consequence-bearing `RelationConstraint` — it is evidence or analysis and belongs to those objects. "Where it carries normative force" was an escape hatch that let every constraint be relabelled informational.)* |
