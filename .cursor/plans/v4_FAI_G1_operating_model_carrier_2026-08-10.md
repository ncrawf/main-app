...
**Completeness posture, because removing the count must not remove the closure claim** `[INH §E13.8]`. This register is a **declared-open minimum, not a census.** A subject not represented here reports `unknown`, never `absent`, and never clean. **High-consequence change classes remain blocked over uncovered subjects; lower-consequence classes proceed only under a named, authority-bearing bootstrap policy with declared residual risk.** Coverage expansion is measurable and reported. Claiming the register is complete would be the population-versus-sample defect `G1-FIND-74` records.
...
| `deployment` | **deployment realization** — the artifact placed in a target | per-target deployment identity | the **operator or environment owner**; `environment` / `deployment_target` ownership is **`OPEN`** in the estate and is not settled here | operator/environment authority under the 4-way composition | `landed` — **partial or unknown effect is never projected as fully landed** `[INH §E13.10]` | recorded time | **Platform / Release + Runtime Operations**, at their declared nonbinding maturity | the deployment history, including failed and partial attempts |
| `activation` | **activation** — the separate fact that a realization is in force in a target | activation identity + scope | the **operator or environment owner** | an attributed act, distinct from deployment completion | contributes to `effective(...)`; **`landed` implies nothing about it** `[INH §E13.18]` | recorded time + effective interval | **Platform**, composed here | the activation intervals |
...
| `obligation` | **obligation** — durable owed work created by any of the above | `obligation_id`; **stable, lease-protected** `[INH §E13.4]` | the **obligor** substantively; **Build OS** for work custody `[INH §E13.2]` | recognized and instantiated from a **pre-existing basis**, never manufactured `[INH G1-FIND-61]` | `remediation_active` · discharge state `[INH §E13.10]` | recorded time + due/clock | **Output 2** for the contract; **Output 4** for change-originated obligations (`§E15.10`) | the obligation, its basis, and whether it was discharged, waived by named authority, or left open |
...
| | **Prospective change assessment** | **Observed consequence projection** |
|---|---|---|
| What it is | an **attributable record with its own identity and revision**, pinned to a manifest revision and a decision context | a **derived projection** over execution and observation facts at a declared `as_of` |
| Who owns it | the assessing authority, per affected subject and scope | nobody — it is generated, under a declared mode and coverage posture |
| What it is for | **it is the approval artifact.** Plan `§3.1`'s *"a change may not be approved without a declared class"* is satisfied by pinning this record, not by storing a scalar | answering *what has actually happened, as of when* |
| Temporal rule | **an acceptance decision pins the exact assessment revision it reviewed.** Later facts never mutate it | resolved under `resolve(...)`, with mode and both bounds declared |
| What later facts may do | invalidate current usability · change the observed projection · **create correction, disclosure, migration or compensation obligations** — never rewrite what was assessed and approved | — |

> **Divergence is a first-class derived condition, not a discrepancy someone notices** `[CAN]`. Where the observed projection departs materially from the pinned prospective assessment — a change approved as reversible that crossed an external frontier, a remedy assessed as available that proved impossible — that **divergence is itself a detectable condition** that opens an attributed finding and, through the affected owner, may create revalidation, notification, disclosure, correction or compensation obligations (`§E15.10`). **Without a named divergence condition the prospective/observed split silently tolerates exactly the case it exists to surface.** Detection is mechanizable; **admission of the finding and any resulting obligation resolve to a pre-existing basis and enter through their rightful gate** `[INH G1-FIND-61]`.
...
| `required_treatment_decisions[]` | accepted Output 3's axis ② — `no_action` · `revalidate` · `remap_or_migrate` · `issue_new_assertions`, **multi-valued, per affected scope** `[INH §E14.2.1`, `[CAN→4]`] | the affected scope's owner | prospective, then confirmed |
...
| `external_effect_and_consequence_records[]` | emission, acknowledgement, demonstrated reliance, and **domain-typed consequence** — each a separate record, not one value | the emitting boundary; the affected domain for consequence type | observed |
| `compensation_or_remedy_obligations[]` | that a remedy is **applicable, required or planned**, with obligor, basis, clock and required proof `[INH §E13.4 `obligation_id`]` | the obligor; **Build OS for custody only** | prospective, then live |
| `compensation_execution_and_adequacy_records[]` | what was **executed**, and whether the affected domain judges it **adequate** — two different facts | the executing party; **the affected domain owner for adequacy** | observed |
...
1. **The profiles are non-exclusive, and two of them nest.** `compensatable_not_reversible` is a **subset** of `forward_only` — the plan listed as siblings what are actually a genus and a species, which is part of why *"exactly one"* could never hold. **A G4 test-10 author needs this**: the forward-only fixture and the compensatable fixture are not disjoint cases, and a change may legitimately match `forward_only`, `compensatable_not_reversible` and `externally_committed` at once.
...
**Applying accepted Output 2's bootstrap posture to characterization** `[INH §E13.8]`: a characterization projection **declares its coverage and its unknowns**; uncovered subjects report `unknown`, never clean; **high-consequence classes remain blocked while material characterization facts are uncovered**; lower-consequence classes may proceed only under a **named, authority-bearing policy with recorded residual risk**, which is itself an attributable decision with an owner. **An approval made on a declared-incomplete characterization is honest and auditable. An approval made on a silently-completed one is neither** — and it is the shape that lets a vendor adapter supply the missing value and thereby become the law.
...
| 10 | admission without adoption | admitted ∧ not adopted. **`permits_effective_use` may be TRUE** — it is derived over the admission decision alone. What is false is **`effective(...)`**, because a required conjunct (adoption, where the profile makes it prerequisite) is absent. No `declined` value is written |
...
| 25 | approved as reversible, later crosses an external frontier | the **pinned prospective assessment is unchanged and still shows what was approved**; the observed projection changes; **divergence** opens an attributed finding and, through the affected owner, disclosure/correction/compensation obligations (`§E15.4.1`) |
| 26 | characterization incomplete at approval time | high-consequence class **blocked**; lower-consequence class proceeds only under a **named authority-bearing policy with recorded residual risk**; unknowns remain visible and the area is not projected reconciled (`§E15.4.4`) |
...
| 5 | **Prospective assessment ⟂ observed consequence**, with acceptance pinning the assessment revision it reviewed, later facts never mutating it, and **divergence as a first-class derived condition** (`§E15.4.1`) |
| 6 | The **fact families** of characterization are independently owned, and **semantic compatibility ⟂ reversibility ⟂ treatment**, **exposure stage ⟂ consequence kind ⟂ epistemic posture**, and **compensation applicability ⟂ obligation ⟂ capability ⟂ execution ⟂ adequacy** do not collapse (`§E15.4.2`) |
| 7 | The five plan labels are **non-exclusive derived profiles**, evaluated per subject, per scope and in a declared mode; **no operation-level label without a named aggregation policy** (`§E15.4.3`) |
| 8 | A **characterization declares its coverage**; high-consequence classes block over uncovered facts; lower classes proceed only under a named authority-bearing policy with recorded residual risk (`§E15.4.4`) |
...
| 11 | **Publication, admission, adoption, deployment, activation, effective use, support and conformance are separately owned facts under a partial order with stated non-implications** — `AB-05`'s scope extension of `C4.4 §R.16` — with per-target eligibility, typed absence, the inherited `effective(...)` unaltered, and the `T0-14` constraint on the `OMNI-managed` posture (`§E15.7`) |
