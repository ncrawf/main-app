...
| the Phase-5 contract-retrofit sequence the posture names (Federation non-human actor / delegated grant / agent boundary → RBAC delegated authority → Identity non-human actor → AI #12 registry → CNS control plane → context-packet exchange) | **the posture's own sequence**, owned by each contract's owner |
