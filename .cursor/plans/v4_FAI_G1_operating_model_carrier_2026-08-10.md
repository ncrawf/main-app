...
It decomposes into work of five mechanically distinct classes. **The class is not a phase.** It is a *failure-and-identity signature*, and it is chosen precisely so that identity, idempotency, retry, recovery and authority semantics are **derived from class membership** rather than specified nine times. Ask what class an object is and its operating contract follows.
...
|---|---|---|---|---|---|---|
| **Commitment** | The **authority-bearing act** — approval, refusal, acceptance, local admission, publication authorization, integration authorization, exception grant, closure | The authoritative act did not occur | Idempotent at **submission**; the *effect* is separate | Stable; revised only by supersession | **Required**, revalidated at point of consequence | Yes |
| **Deterministic Derivation** | A pure function of pinned inputs under pinned versions, meeting a **declared determinism contract** | No current-generation result exists | Free and unlimited | **Content digest**, actor-independent | None — evidence only | Per generation |
| **Observation / evidence acquisition** | An attributed measurement of the world at a time | Coverage gap or unreachable target — **not** a wrong answer | Re-acquisition yields a **new** observation, not the same one | Observer + target + time; **never content-addressed** | None — evidence only | Per acquisition |
| **Obligation** | Durable work that is **owed**, surviving actor and session replacement | Remains open and visible | New attempt against the **same** obligation identity | Stable across re-detection (`§E13.4`) | Obligor is substantive; custody is execution-side | Only on discharge |
| **Controller** | Continuous reconciliation with no mission completion state | Coverage degrades or liveness lapses | Per-iteration, with lease and attempt | Scope + generation + instance | None — emits, never applies | **No mission terminus** |
...
|---|---|---|---|
| 1 | Proposal | Artifact + Commitment | The manifest revision is an immutable artifact; **submitting it for decision** is the commitment |
...
|---|---|---|---|---|---|
| `work_package` / lane | Obligation | operator/project | Stable across actors | n/a | **Build OS / AWP §2.1** |
...
| `observation_id` | Observation | observer + target + acquisition time | Append-only | **None — re-acquisition is a new fact** | Observer |
| `generation` · `publication_id` | Commitment | (artifact class, scope) | Monotonic | Atomic pointer swap | Admitting scope |
...
| `finding_id` | Obligation | **(detector, subject locator, defect class)** — deliberately revision-independent | Stable while unresolved | Re-detection updates; never duplicates | Detector output, admitted by governance |
...
`manifest_submitted · decision_recorded · decision_superseded · decision_withdrawn · grant_revoked_in_flight · scope_admitted · scope_declined · landing_proven · effect_outcome_recorded · generation_published · finding_opened · finding_closed · finding_reopened · obligation_created · obligation_leased · obligation_discharged · exception_granted · degraded_mode_entered · degraded_mode_exited · correction_linked`
...
**Bitemporality is inherited, not invented** `[INH]` — the estate's temporal discipline already separates effective from recorded time, and Clinical Memory already runs append-only assertions with a derived current view rather than treating the view as the history. The no-hindsight rule holds: a reconstruction of what was effective at *T* may use only what was **recorded by** *T*. The Effective Architecture Snapshot remains immutable and is **demoted from historical truth to a materialized derivation** over the history plus pinned inputs — reproducible, and content-addressed rather than actor-stamped.

## §E13.6 — Ownership and binding map — `FO-6` closed, not deferred
...
| Resource revision content | Owning steward | Artifact | Hand-authored, canonical |
...
**`G1-FIND-50` — non-waivable holders are resolved by policy, never by graph adjacency.** Patient consent, professional authority and operator sovereignty **cannot be "not found,"** because their presence is not a function of whether anyone drew an edge. Graph absence may downgrade a completeness posture; it may **never** remove a non-waivable holder from the approval condition. This single asymmetry is what stops an omitted edge from laundering a change past the authority that most needed to see it — and it is the architecture-layer form of `INV-30`'s *every affected owner*, which `G1-FIND-36` already showed CODEOWNERS cannot satisfy. `[CAN→3 · adjacency semantics · 6 · the check]`
...
| `accepted` | An authority-bearing acceptance references the **exact** current manifest revision, impact result, conformance result and decision basis |
...
| `effective(scope, as_of)` | Origin-side validity, local admission and activation, profile applicability, temporal interval and non-revocation all hold **for that scope** |
| `observed` | A trusted, sufficiently current observation covers the named target and generation; **unreachable or unknown is not observed** |
...
| `reconciled` | Declared, landed, effective, generated and observed facts agree — **or** every remaining difference is an explicitly authorized variation, transition skew, degraded state or open obligation |
...
| Can be ended | **Yes — prospectively**, by withdrawing or replacing the delegation | **No** |
| Recorded refusal | **Survives.** Ending a delegation never rewrites a recorded refusal as though it never occurred | Survives |
| Fungibility | Not routable around while the delegation stands | **Never routable around, never outvoted, never timed out** |
...
**Machine evidence is evidence.** A signature proves custody, not correctness; a tool version proves provenance, not accuracy. **The observer never auto-applies** — detection opens governed work, or a linter becomes a sovereign committer. This is the raw anchor's own position (`EVSRC-2026-000188_TK.md`: emits work items; auto-promotion prohibited), not an inference from the registry summary.

**`G1-FIND-54` — observer independence is a property of the observation set, not of any observation.** Twenty observers running the same detector against the same corpus are **one** observation with twenty timestamps, and recording them as corroboration is worse than recording one, because it manufactures confidence that nothing earned. This is `§E12.2`'s independence-per-claim rule applied to machine evidence, and it is the same law wave 6 states as *parallel agents ≠ evidentiary independence*. `[CAN→6]`

## §E13.14 — Review admission and independence requirements

**Admission before review.** A proposal that has not arrived carrying declared class and consequence · pinned base · valid schema · impact result and coverage posture · required-owner set with provenance · conformance result · collision check · rollout scope · rollback/compensation/forward-only posture · cited evidence · stated uncertainty — **does not consume owner-review capacity.** At a thousand machine-authored proposals a day, a queue that accepts everything syntactically valid is a denial-of-service weapon aimed at authority holders.
...
Identity is **namespace-qualified** — operator/project namespace + actor identity + runtime profile + run + attempt. **The display name is not the identity**; two actors may carry the same label in different namespaces without ambiguity.
...
| 3 | Four carrier status blocks (`§E10.13`, `§E11.G`, `§E11.H`, `§E12.5`, `§E12.6`) | **Typed-ambiguity cases**, held open until the current-vs-historical-quotation rule decides them. Not pre-judged |
...
| Agent identity | Session and chat identity | **Work identity survives actor replacement** | Replacement is routine; thread identity becomes irrelevant |
...
> **Scope: the CARRIER AS A WHOLE and G1 — NOT Output 1.** Output 1 is **accepted as a G1 proposal** (`§7.5`); Outputs 2–7, `§G1-AUTH`, the source floor and the model boundaries remain incomplete, so the carrier-level verdict stands unchanged. Recommended by `proposal_authoring`. Not self-accepted.
...
| **Remaining G1 blockers** | **`B-8`** source floor · **`B-13`** `AB-01` + C4.4 owner acceptance · **`B-15`** outputs 2–7. *(`B-3` closed with `AB-08`.)* |
...
| Field | Value |
|---|---|
| Artifact | FAI **G1 operating-model carrier · OUTPUT-2 AUTHORED 2026-08-13 (`§E13`, `PROPOSED · NOT_ACCEPTED`)** · Output-1 acceptance landed 2026-08-13 · seat `proposal_authoring` · state **`proposed`** · **`B-1` CLOSED 2026-08-11** |
...
**STOP: `g1_carrier_OUTPUT_2_AUTHORED · OUTPUT_1_ACCEPTED_AS_G1_PROPOSAL__1_OF_8_COMPLETE · OUTPUT_2_PROPOSED_NOT_ACCEPTED_§E13 · AB-08_adopted_narrowed · B-3_and_G-08_CLOSED · B-8_B-13_remaining · B-15_NARROWED_to_outputs_3_to_7_plus_output_2_acceptance · entry_head_50431e32851086acbbd9f4534e2cec7e7f4b5f49_FROZEN_AS_AB-08_FIXTURE · ledger_287_repair_OWED_SEPARATELY_NOT_IN_THIS_TRANSACTION · four_carrier_status_blocks_TYPED_AMBIGUITY_UNDECIDED · catalog_and_route_9v-ii_UNTOUCHED_belong_to_the_accepting_transaction · PR_17_draft · G1_INCOMPLETE · outputs_3_to_7_not_started · no_standing_integration_grant`**
