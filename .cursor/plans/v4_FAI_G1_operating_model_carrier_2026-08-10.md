...
**Two classifications this repair gets right that a three-way split could not.** **`AB-13`** is `already_present` — it is an **index proving the law was already located**, and its authority is the carriers it points at (`D0THES-GRD-026`, the no-god-domain doctrine), not the ledger row; it belongs in **A by reference**, never as the law's authority. **`INV-19` and `INV-21`** are **class C**, not inherited: their *substance* is separately supported in class A — decomposed authority by the 4-way composition and `D0THES-GRD-035`, observer-scoped projection by `T0-15`/`DEC-033` — but **the ledger rows themselves are unaccepted candidates, and `INV-21`'s expose-known-omissions clause has no class-A carrier at all.** Citing the governing carrier for the substance and the ledger row as a candidate is the accurate form; calling either "inherited accepted law" is not.
...
| **3** | **`AdmissionDecision`** *(an `applicability_decision`)* | a receiving scope's separate, attributed decision to admit, narrow, deny, defer or condition **a specific origin assertion revision** | its own decision identity `[INH §E12.7.2]` | the **receiving namespace** | a rewrite of the origin assertion · origin truth |
...
| **5** | `ConflictRecord`'s four moments — see `§E14.6` | **detected candidate conflict** ⟂ **attributed challenge** ⟂ **stay or interim disposition** ⟂ **authority-bearing adjudication**. **A detector must not manufacture a challenge or an adjudication by emitting a finding** `[INH G1-FIND-60/-61]` |
...
LOCAL ADMISSION       B's attributed, effective-dated decision over A's exact assertion REVISION
        ↓ derived
B'S EFFECTIVE GRAPH = join(origin-valid assertions, ACTIVE local admissions, in-interval, in-scope)
```

**Necessary, not sufficient** `[INH]`. An origin-accepted assertion may still be unadmitted, outside the selected profile, inapplicable to the deployment or mission, admitted only as evidence, blocked by a constraint, locally superseded, or contested and still effective.
...
**A `specializes` edge may never relax an inherited prohibition** — and that requirement is *declared and checked*, not automatically satisfied by the derivation kind. `NC-8`'s semantic check remains open to Outputs 3+6 `[INH §7.3]`. Clinical Memory independently carries LOOSENING/TIGHTENING semantics on `concept_version` `[INH CM §6.1]`.
...
issuer-side assertion decision state ⟂ receiving-scope admission ⟂ assertion lifecycle ⟂ **dependency-inheritance classification (scope-labelled to `D0CKPT-GRD-005`)** ⟂ validation result ⟂ challenge/stay ⟂ coverage/freshness posture ⟂ typed absence.
...
| revalidation on material change | w6 `285` — *"review once, inherit many — but re-review every material departure from the certified envelope"* | **`SHARPENS`** | no | `§E14.7.4` revalidation is inherited |
...
**Non-regression proof for Outputs 1 and 2 — digest-verified, not asserted.** Both accepted ranges were extracted from the entry head `a4c88d3` and from the authored tree by the same section-bounded extraction and hashed. **Output-2 accepted range `§E13`: SHA256 `2f89bb4184c47b3d49d1e71f3a5097216aadefc791d4eb71e8711ef9289b5b45`, identical before and after — and identical to the digest the `§7.6` acceptance receipt recorded at landing**, so the proof closes against the receipt's own value rather than against a fresh baseline. **Output-1 accepted range `§E10`–`§E12`: SHA256 `d2c531b7553bbf0f8dd2b924d2bfcfc4e7e19161a5ef2fc8fd15c7fa41e3f87a`, identical before and after.** `§E14` was inserted between `§E13.27.8` and `§E5` and touches neither range. Every `§E14` reference to either output is a **consumption, or the discharge of a carry those outputs themselves routed to Output 3** — never an amendment. `§E14` discharges `relation bitemporality [CAN→3]`, `edge authority state [CAN→3]`, `G1-FIND-45` no-hindsight, the dispute lifecycle, and `G1-FIND-65`'s `realizes` routing; it **re-anchors** one Output-1 rule to its current facet name with no semantic change (`G1-FIND-69`). Receipt at `§E14.9.6`.
