# v4 — FAI — G1 OPERATING-MODEL CARRIER (R8.4)
...
Status: **`g1_carrier_R8.4 · state=proposed · B-1_CLEARED · OUTPUT_1_CONDITIONALLY_ACCEPTED__TWO_STRUCTURAL_PROOFS_DISCHARGED__READY_FOR_OPERATOR_STEWARD_ACCEPTANCE · AB-08_SEMANTIC_DISPOSITION_ACCEPTED__LEDGER_RECEIPT_PENDING · three_binding_carries_recorded · 0_of_8_outputs_complete`**
...
|---|---|---|---|---|---|---|
| 1 | **artifact metamodel** | **`§E10` + `§E11` + `§E12` (`AMM-C4.1` core + closeout + micro-correction + acceptance boundary `§E12.7` + Output-1 Lane-3 checksum `§E12.8`) — the CURRENT candidate.** `§E6.5`–`§E6.12` (R4) · `§E7` (R5) · `§E8` (R6) · `§E9` (R7) = **evidence history, superseded** · spec `§E6.3` | Artifact Index ✔, governance taxonomy ✔, R8 §3.1 ✔ **+ §3.9.1/§3.9.2 ✔**, System Map ✔, Surface Map ✔, `03`/`05`/`06`/`08` schemas ✔ · **Lane 3: ownership-scoped `M-106` ✔ + Output-1 eight-function checksum ✔ (`§E12.8`, ten rows, no material core change) · full G1 capability-by-capability check PARTIAL (`B-8`) · NO raw source anchor reopened** · **Lane-1 probes, none adopted: 42010 ✔ SEI ✔ FHIR ✔ IHE ✔ AUTOSAR ✔ Backstage ✔ CODEOWNERS ✔ AWS ✔ Palantir ✔** | **`AMM_C4_1_CORE_REVIEWED · CLOSEOUT_AMENDMENT_COMPLETE · READY_FOR_STEWARD_ACCEPTANCE_AS_A_PROPOSAL`** (`§E11.H`). Fifth candidate, four reviews — **the central model survived; source discipline and surface propagation were the failures, both repaired in R8.1.** Ownership-scoped Lane-3 `M-106` complete (one exact match, four sharpens, one adjacent-and-conflated, one tension — `§E11.B`); the four vendor mechanisms recorded as **durable ⟂ adapter ⟂ residue**, not as a verdict. **R6's 3/11/11 aggregate and all three of its `PASS`es are WITHDRAWN** — a factual error (`§E9.0`) plus two verdicts resting on inference. Published instead: the **eight-relation resource-governance grammar** (`§E9.3`), the **meta-kernel termination rule** (`§E9.4`), four more value collapses repaired (`§E9.5`), and a **eight-case per-resource governance enumeration** (`§E9.6`) — **no replacement aggregate** | **`resource_steward` declared for ZERO of seven real resources**; of eight governance relations OMNI declares at most three for any resource, never the same three · placement predicate `[CAN→3]` · relation bitemporality `[CAN→3]` · prose non-loosening `[CAN→6]` · approval-policy content `[CAN→3]` · **carried to Outputs 3/5/6, NOT settled: operationalization vocabulary · `derogability` · risk-acceptance schema · `independence_assessment` · procedural⟂substantive conformance · edge authority-state · the no-hindsight-leak constraint on `as_of` resolution** (`§E12.7`–`§E12.8`) · the focused fixture is **weak evidence where I chose the cells and strong only where an estate fact filled them** · catalog row = a **pending acceptance-transaction edit**, named once rather than counted | **both structural proofs are discharged; `architecture_steward` + operator record acceptance of the structural core at an exact head.** **`AB-08_SEMANTIC_DISPOSITION_ACCEPTED__OPERATOR_STEWARD_LEDGER_RECEIPT_PENDING`** (`§E12.5`) — one ledger receipt, no further fixture; descriptor-schema installation is **G2's** transaction against R8 §3.1 |
...
| Field | Value |
|---|---|
| **Output 1** | **`AMM_C4_1_CONDITIONALLY_ACCEPTED · TWO_STRUCTURAL_PROOFS_DISCHARGED · READY_FOR_OPERATOR_STEWARD_ACCEPTANCE`.** Boundary at `§E12.7`: **the facet DECOMPOSITION is core; every enumerated VALUE SET plus the operationalization vocabulary, `derogability`, the risk-acceptance schema and `independence_assessment` are CARRIED to Outputs 3/5/6.** **Proof 1 (`§E12.7.1`) — cross-namespace identity is INHERITED from Identity/Federation/`§R.16`, not invented; a global identity scheme would violate `T0-14` operator-neutrality.** **Proof 2 (`§E12.7.2`) — five of six assertion states are already covered by existing facets; ONE structural hook added (the resolver admits only `accepted` + in-interval assertions, excluding rather than deleting the rest); `disputed` and typed confidence routed to Output 3.** **Three binding carries recorded (`§E12.7.3`).** **Stated without self-flattery:** the core survived four reviews, and R8.1/R8.2 each shipped constructions inside passes labelled cleanup (`G1-FIND-43`, `F-12`) |
...
| **`AB-08`** | **`SEMANTIC_DISPOSITION_ACCEPTED__OPERATOR_STEWARD_LEDGER_RECEIPT_PENDING`** — independently accepted on the merits; normalized at `§E11.G`, narrowed at `§E12.5`. **One ledger receipt remains; no further fixture** |
| **Minted** | **nothing** |
| **Output 2** | **NOT STARTED** |
...
| Field | Value |
|---|---|
| **Output 1** | **`CONDITIONALLY_ACCEPTED · TWO_STRUCTURAL_PROOFS_DISCHARGED · READY_FOR_OPERATOR_STEWARD_ACCEPTANCE`**, bounded by `§E12.7`: **the facet decomposition is core; every value set and the four R8.1/R8.2 constructions are CARRIED to Outputs 3/5/6.** Both reviewers conditionally accepted the core; **the two proofs are discharged by INHERITANCE plus one structural hook, not by new architecture** |
| **`AB-08`** | **`ACCEPTED_IN_SUBSTANCE__PENDING_OPERATOR_STEWARD_RECEIPT`** |
...
**The relief review asked for *"globally collision-safe"* identity via a globally unique scheme. That framing is corrected and the correction protects a Tier-0 guardrail:** OMNI issuing global architecture-resource identities would make OMNI the identity authority across operators, which **`T0-14` operator-neutrality forbids** — *"OMNI-operated operators are tenants like any third-party Brand… no privileged"* path. **What is required is collision-safe, origin-preserving references plus explicit cross-namespace identity and lineage assertions.**

**And the estate already owns this law, deliberately:** the Identity contract holds *"one canonical row per person **within an identity namespace** (deployment / org PHI boundary)"* and marks **cross-namespace identity `OPEN/deferred`** rather than minting an MPI · *handle ≠ person* · Federation admits cross-boundary **possibility**, RBAC decides capability, the owning domain commits · **`C4.4 §R.16`: *"federated publication ≠ universal trust (locally admitted)"*, `publish → admit → use-under-grant → revoke`.** **The architecture-resource model inherits that and adds nothing.**
...
**One structural hook is added — and only this one:** **the effective-architecture resolver admits an assertion only when `governance_decision_state = accepted` AND its interval contains the `as_of`. Non-admitted assertions are EXCLUDED, never deleted** `[KND]`. Rejected, disputed and withdrawn edges remain visible as evidence and history. **The state machine, conflict semantics and filtering algorithm stay Output 3.**
...
> **Composing the two proofs yields the invariant neither review stated, and it is the one that makes them a single answer: `governance_decision_state` is NAMESPACE-SCOPED, not global.** When B mirrors A's resource it mirrors A's assertions too — **`accepted` in A's namespace, `candidate` in B's until B's own admission decision.** That is `§R.16`'s publish≠adopt applied to edges, and without it a mirror silently imports another operator's committed architecture as though locally binding. *(`G1-FIND-46`.)*
...
| **B-3** | **`AB-08` = `ACCEPTED_IN_SUBSTANCE__PENDING_OPERATOR_STEWARD_RECEIPT`** (`§E11.G`, narrowed at `§E12.5`). **What remains for the ROW is one operator/steward ledger receipt — nothing else.** No further fixture. **Output-1 acceptance is a SEPARATE transaction** (`AMM-C4.1` ready for steward acceptance as a proposal); policy composition is Output 3/5 and operationalization conformance is Output 6 — **neither is an `AB-08` closure criterion** | **ledger `blocking_scope` = blocks G1**; owner `architecture_steward` |
...
**`G1-FIND-46` (new, R8.4 — `§E12.7.2`). `governance_decision_state` is NAMESPACE-SCOPED, not global** — the invariant that falls out of composing the two acceptance proofs and that neither reviewer stated. **When operator B mirrors A's resource it mirrors A's relation assertions too: `accepted` in A's namespace, `candidate` in B's until B's own admission decision.** That is `C4.4 §R.16`'s *publication ≠ adoption* applied to edges. **Without it, a mirror silently imports another operator's committed architecture as though it were locally binding** — which is the architecture-layer form of the exact collapse Identity/Federation exist to prevent. → Output 3.
...
| Field | Value |
|---|---|
| Artifact | FAI **G1 operating-model carrier R8.4** · seat `proposal_authoring` · state **`proposed`** · **`B-1` CLOSED 2026-08-11** |
...
| Ledger | 40 rows verdicted. **ZERO closed.** **`AB-08` REMAINS OPEN — but its semantic answer is now `INDEPENDENTLY_REVIEWED__READY_FOR_OPERATOR_STEWARD_CLOSURE`** (`§E10.13`), which is the first substantive result of this arc an independent reviewer has endorsed on the merits. **Still not closure** — the row is the steward's and this seat holds `proposal_authoring`. The surviving substantive conclusion — **adopt neither “17 classes” nor “5 tiers”** — is now three passes old and unchallenged. **Closure still blocked on the placement predicate `[CAN→3]` and relation bitemporality `[CAN→3]`** |
...
| Verdict | **`NOT_CONVERGED__OUTPUT_1_ON_FIFTH_CANDIDATE_AWAITING_REVIEW__AB-08_SEMANTIC_ANSWER_READY_FOR_CLOSURE__OUTPUTS_2_TO_7_AND_SOURCE_FLOOR_INCOMPLETE`** *(R4's completion label stays withdrawn; the source floor and model boundaries remain incomplete exactly as R3 stated)* |
| G1 blockers | **still four, and still NOT peers.** **B-3** now reads *`AB-08` `ACCEPTED_IN_SUBSTANCE`, pending an operator/steward receipt; `AMM-C4.1` ready for steward acceptance as a proposal* — **running a fixture is not disposing a ledger row**, and R4's proposed closure was withdrawn under review. The estate governance failures are separately routed and are **not** closure criteria · **B-8** source floor incl. unread **Lane-2** carriers — **unchanged**; the narrow `AB-08` floor was read, the floor was not · **B-13** `AB-01` generalized form + C4.4 owner acceptance — **unchanged** · **B-15** = *outputs 2–7*, i.e. **the rest of G1**. *(B-2 → open item with owner; B-6 → G2/G4)* |
| Owed and NOT done — declared | **`B-8` is unchanged and one Lane-3 search is not Lane 3 consumed** — R8 §3.9.1 requires the EXISTS-AS check *for every operations capability*, and it has now run for exactly one (ownership). The **catalog row** is a **pending acceptance-transaction edit** — named once, not counted every pass; repointing belongs to the accepting transaction and this pass may write only this carrier. **Route `9v-ii` needs NOTHING** — it carries path, trigger, read rule, authority, lifecycle and promotion condition and **no content description**; R4's claim that it needed repointing is **withdrawn**, and acting on it would have pushed Output-1 narrative into a routing surface |
| Next | **Stop for a diff-only check, then the acceptance transaction:** ① operator + `architecture_steward` record acceptance of the **Output-1 structural core** at an exact head · ② the **`AB-08` ledger row** moves `open → adopted_narrowed` · ③ Output 1 becomes **1 of 8** · ④ the catalog row is repointed **by its rightful writer** · ⑤ **route `9v-ii` STAYS conditional/proposed — six outputs and `§G1-AUTH` remain incomplete, so accepting one cell does not promote the carrier** · ⑥ PR #17 stays draft · ⑦ **Output 2 opens in a fresh context, and its FIRST executable proof is the defect this arc demonstrated eight times: one canonical change, updated once, with every dependent surface regenerated or flagged divergent — not a prettier loop diagram.** *(Prior wording retained below.)* ① **`AB-08` can close now on its own merits** — operator/steward closure on the five-function semantic answer (`§E10.13`), separately from `AMM-C4.1`; ①b the steward accepts, amends or rejects **`AMM-C4.1`**; ② **Output 2 — the architecture-operations loop as a transaction with actors, gates and artifacts** — begins only after that, and it inherits `AMM-C1`'s resource/carrier/descriptor split as its object model; ③ outputs 3–7 inherit the obligations Output 1 was forbidden to settle (precedence · conflict · non-loosening semantics · applicability evaluation · profile composition · source-change propagation · effective resolution · conformance proof) plus `NC-8`'s unresolved semantic check. **`G1-FIND-13` and `G1-FIND-14` are repairs to files this seat may not write; they belong to the steward and to G2, not to Output 2.** **A clean acceptance carrier is assembled after the cells close** — earlier it is another revision by another name |
...
**Load order:** `AGENTS.md` → checkpoint → route `9v` (plan R8 → charter R9 → ledger R5) → `9v-ii` this carrier. Gate sequence **only** in plan §5; closure conditions **only** in the ledger's `blocking_scope`. **Inside this carrier, read `§G1-CONTRACT` → `.b` → `.c` first. For Output 1 the CURRENT path is `§E10.2`–`§E10.10` (the `AMM-C4.1` core) → `§E11` (closeout) → `§E12` (micro-correction).** **`§E6`–`§E6.4` (the probe), `§E6.5`–`§E6.12` (R4), `§E7` (R5), `§E8` (R6) and `§E9` (R7) are evidence history — open them for provenance or contradiction recovery, never as the result.** **For external/enterprise correspondence, C3.8 G2/G4 is the required read. For build-side operating model, Build OS `09`/`10` — at the maturity they declare, not as settled truth.**

**Stop condition:** superseded when the steward and affected domain owners accept, amend or reject.

**STOP: `g1_working_dossier_R8.4 · B-1_CLOSED_2026-08-11 · 40_rows_verdicted_zero_closed · OUTPUT_1_CONDITIONALLY_ACCEPTED__TWO_STRUCTURAL_PROOFS_DISCHARGED__READY_FOR_OPERATOR_STEWARD_ACCEPTANCE · proof1_cross_namespace_identity_INHERITED_not_invented_T0-14_protected · proof2_five_of_six_assertion_states_already_covered_one_structural_hook_added · governance_decision_state_is_NAMESPACE_SCOPED · three_binding_carries_with_descriptor_precondition · AB-08_SEMANTIC_DISPOSITION_ACCEPTED__LEDGER_RECEIPT_PENDING · route_9v-ii_STAYS_conditional_six_outputs_incomplete · G1_CONTRACT_0_of_8_outputs_complete · output_2_not_started`**
