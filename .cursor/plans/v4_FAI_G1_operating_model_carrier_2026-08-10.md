...
**★ CORRECTED IN R2 — R1's seal contradicted itself in three consecutive sentences.** It said *every object has one work class and one record type*, then *no object is asserted to belong to exactly one class*, then described a published snapshot as a derivation **plus** a commitment. Those cannot all hold. The defect was applying a *work* taxonomy to *things*rather than to *doing*.

**The sealed formulation:**

> **Work classes classify acts and durable work items — never artifacts.** An atomic act or durable work item has **exactly one primary work class**. An **architecture operation composes many such acts** and therefore has none of its own. **Artifacts and records are produced, referenced, admitted, superseded or published *by* acts; they are not forced into a work class because an act touched them.**

Applied to the case R1 got wrong: **snapshot generation** is a deterministic derivation · **the snapshot** is an artifact · **publication authorization** is a commitment · **publication execution** is a separately identified effect · and **"the published effective snapshot"** is a **relation among those four facts** — not one composite object occupying several classes at once. Persisted things carry a record type (`artifact` for content; `record` for the durable trace of an act, result or attestation); **record type is orthogonal to work class and is not a second classification of the same thing.**
...
| **Obligation** | Durable work that is **owed**, surviving actor and session replacement | Remains open and visible | New attempt against the **same** obligation identity | **★ R2 — generic:** stable `obligation_id` · exact obligor and basis · attempts under one obligation · discharge/expiry/supersession history. *(R1 put finding-specific family/occurrence semantics here; those are right for a detector finding and wrong for a review obligation, propagation work, verification debt, a legal clock or an owner-response obligation. They now live only in the finding row at `§E13.4`.)* | Obligor is substantive; custody is execution-side | Only on discharge |
...
**★ R2 — this summary is now a pointer rather than a restated range.** The authoritative list of Output-2 findings is the `§7.3` register; **restating the range here was the third copy of a derived enumeration and it went stale within one pass** (R1 added `-59` and `-60` and this line still read `47`–`58`). Output-2 findings: `G1-FIND-47` onward, **enumerated in `§7.3`**, plus R2's additions recorded there. The prose below states the *substance* of each, which is not derived and does not drift:
...
> **Evidence never manufactures an obligation, an authority, or an assignment. Admission is always a governed act — by an authorized holder, or by an already-accepted deterministic policy whose acceptance was itself an attributed commitment.**

| Instance | Evidence | Governed act |
|---|---|---|
| Detection | Observation, candidate finding | Evidence admission → substantive obligation (`§E13.13`) |
| Proposal | Submission event, preflight result | Proposal admission → decision obligations (`§E13.3`) |
| Staffing | Context health, budget, error rate, uncertainty, unavailability | **Assignment decision** → holder binding (below) |
...
**`G1-FIND-62` — independence is a property of the appointment structure, not of the reviewer's model identity.** If one authority appoints both the author and the reviewer, sets both their contexts, and relays between them, the review is **structurally dependent** however different the two models are. That is the current bootstrap profile exactly, and it is recorded as a **live limitation** rather than resolved: `§E12.2` already scoped independence to a *claim*; R2 extends it to the *appointment*. At scale, an external party asking whether a review was independent is asking about the appointment graph, not the model roster. `[CAN→6 · independence assessment]`
...
**Context-draw receipt** must reference: exact source identities and revisions included · sources **excluded, deferred, unavailable or deliberately withheld**, with reason and policy basis · target function and actor · context policy and runtime-profile revision · purpose and authority scope · context/token/cost budget · the structured relay or distillation used · evidence pointers · freshness and invalidation posture · **and a route to reopen the original source.**

### §E13.27.4 — Staged disclosure is a distinct relationship, and the live fixture is this pass

The operator withheld a large speculative context packet from the authoring run, routed it to the independent review function first, and forwarded a distilled result. **That is not delegation.** Six relationships that all move context and differ in authority, custody and independence:
...
| **Independent review** | Reviewer receives a bounded packet and returns evidence or a blocking decision under a review grant |
...
| **Broadcast** | Information published without custody or authority transfer |

**Staged disclosure is legitimate** — it preserves reviewer independence, prevents premature anchoring, and bounds context load. **Its risks are structural and must be receipted:** the router can bias the author by selective disclosure · required evidence can be withheld · the distillation can flatten the source · **and the author cannot know what was excluded.** I am the author in this fixture and cannot verify the exclusion set, which is precisely why the receipt fields in `§E13.27.3` are obligations rather than conveniences. **A relay or distillation is evidence and never supersedes its source**; disclosure may never withhold evidence a required authority holder needs in order to decide lawfully.

### §E13.27.5 — The realization chain, run as a falsifier

The operator's music framing was tested against the model as a **falsifier, not vocabulary**. No musical term enters OMNI. Nine links, tested honestly:
...
| authorized transformation ≠ **role-scoped execution projection** | **GAP — closed by `§E13.27.3`** |
...
**Eight of nine already held; one was genuinely absent** — the role-scoped projection given to an actor, and its staleness behaviour. That is the whole yield, and it is recorded as a mechanism rather than a metaphor.

**One rights non-collapse the chain does add.** `§E13.16` separated four facts; the realization chain shows **credit was absorbing two more**: `contribution lineage ⟂ credit ⟂ economic/IP ownership ⟂ custody ⟂ semantic authority ⟂ accountable commitment ⟂ legal/professional liability` — **seven distinct relations to one work.** A performer's contribution confers no commit authority over the composition; a custodian of the master owns no meaning; identical content implies neither shared identity nor shared ownership (already Output-1 law). **Output 2 preserves the distinctions and designs no IP or licensing policy**, which routes to enterprise/legal architecture and later graph semantics. `[CAN→3 · relation semantics · enterprise/legal]`

### §E13.27.6 — Boundary for this section
...
Not *"the surfaces were fixed."* **Every current Output-2 assertion of a derived value must either be the owning source, resolve to it, or be a pinned quotation.** A future checker must flag both `AO-SELF-1` episodes — `ee1fa6e` and `eb8a2da` — and pass the corrected head, **differing only in being right**, which is the negative-control standard `ledger:289` sets for `AB-08`. **Two failed correction episodes over one defect class, by one author, under two independent reviews, is the strongest available evidence that `§E13.7`'s rule cannot be discharged by care.**
...
| Field | Value |
|---|---|
| Artifact | FAI **G1 operating-model carrier · OUTPUT-2 R2 (`§E13`, `PROPOSED · NOT_ACCEPTED`)** · Output-1 acceptance landed 2026-08-13 · seat `proposal_authoring` · state **`proposed`** · **`B-1` CLOSED 2026-08-11** |
...
**STOP: `g1_carrier_OUTPUT_2_R2 · OUTPUT_1_ACCEPTED_AS_G1_PROPOSAL__1_OF_8_COMPLETE · OUTPUT_2_PROPOSED_NOT_ACCEPTED_§E13 · R2_CORRECTION_APPLIED_ALL_CLAIMS_MECHANICALLY_VERIFIED · AO-SELF-1_EPISODE_2_eb8a2da_ENUMERATED_BY_DERIVED_VALUE_NO_TOTAL_PUBLISHED · WORK_CLASSES_CLASSIFY_ACTS_NOT_ARTIFACTS · SUBMISSION_CREATES_ADMISSION_REQUEST_NOT_DUTIES · EVIDENCE_ADMISSION_HOMED · FACT_AUTHORITY_AXES_SPLIT · DEGRADED_EXPIRY_CONDITIONED · §E13.27_ASSIGNMENT_AND_RUN_CONTEXT_ADDED · G1-FIND-61_ONE_LAW_THREE_INSTANCES · NO_SCHEDULER_MINTED · NO_MUSIC_VOCABULARY · TRIFECTA_NOT_CANONIZED · AB-08_adopted_narrowed · B-3_and_G-08_CLOSED · B-8_B-13_remaining · B-15_NARROWED_to_outputs_3_to_7_plus_output_2_acceptance · AB-08_FIXTURE_FROZEN_AT_50431e32851086acbbd9f4534e2cec7e7f4b5f49 · AO-SELF-1_FIXTURE_FROZEN_AT_ee1fa6e05b4313e49aa361e75a2dea11b491de18_blob_3b6b49f43a84a481084f7f1ea28831f4fa455c30 · ledger_287_repair_OWED_SEPARATELY_NOT_IN_THIS_TRANSACTION · AB-08_ambiguity_set_ENUMERATED_BY_IDENTITY_five_sites_two_tokens_UNDECIDED · verbatim_set_ENUMERATED_AND_PINNED_no_hand_count_survives · platform_and_accountability_FULL_DEPTH_READ_REQUIRED_BEFORE_ACCEPTANCE · FO-6_ANSWERED_CLOSES_ON_ACCEPTANCE · catalog_and_route_9v-ii_UNTOUCHED_belong_to_the_accepting_transaction · PR_17_draft · G1_INCOMPLETE · outputs_3_to_7_not_started · no_standing_integration_grant · NOT_SELF_ACCEPTED`**
