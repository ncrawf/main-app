# v4 — FAI — G1 OPERATING-MODEL CARRIER (R8.2)
...
Status: **`g1_carrier_R8.2 · state=proposed · B-1_CLEARED · OUTPUT_1_AMM_C4_1_READY_FOR_STEWARD_ACCEPTANCE_AS_PROPOSAL · AB-08_ACCEPTED_IN_SUBSTANCE__PENDING_OPERATOR_STEWARD_RECEIPT · 0_of_8_outputs_complete`**
...
| Field | Value |
|---|---|
| **Output 1** | **`AMM_C4_1_READY_FOR_STEWARD_ACCEPTANCE_AS_PROPOSAL`** (`§E12.6`). **Stated without the self-flattery R8.1 carried:** the central model survived, **and R8.1 also shipped two new constructions, one of which carried a value collapse** — so this was never only housekeeping (`G1-FIND-43`) |
...
```
derogability ∈ non_derogable · derogable_with_named_authority · derogable        [INH INV-30]
```

**`INV-30` already supplies the `non_derogable` set:** an independently liable principal's refusal · a patient right · a professional duty. **A `risk_acceptance_decision` whose `risk_scope` touches a `non_derogable` obligation is an undrawable edge**, not a policy warning.
...
**And `accountable_human_judgment` is renamed and defined**, because as a bare label it means nothing: **`required_attributable_judgment` — a named seat must render a determination, recorded with its basis and attributable to its holder. The obligation is discharged by the determination existing and being attributable, never by its content being correct.** It composes with `audit` and `sample_review`.
...
**The pair is necessary and insufficient.** Routed exactly as before — Outputs 3 and 6 — but the wording is corrected now so no future pass freezes a static actor-pair label.
...
| Field | Value |
|---|---|
| **Output 1** | **`AMM_C4_1_READY_FOR_STEWARD_ACCEPTANCE_AS_PROPOSAL`** |
| **`AB-08`** | **`ACCEPTED_IN_SUBSTANCE__PENDING_OPERATOR_STEWARD_RECEIPT`** |
...
| Field | Value |
|---|---|
| Artifact | FAI **G1 operating-model carrier R8.2** · seat `proposal_authoring` · state **`proposed`** · **`B-1` CLOSED 2026-08-11** |
...
**STOP: `g1_working_dossier_R8.2 · B-1_CLOSED_2026-08-11 · 40_rows_verdicted_zero_closed · OUTPUT_1_AMM_C4_1_READY_FOR_STEWARD_ACCEPTANCE_AS_PROPOSAL · AB-08_ACCEPTED_IN_SUBSTANCE__PENDING_OPERATOR_STEWARD_RECEIPT · E11B_tally_corrected_8_rows · operationalization_plural_residual_risk_extracted_derogability_added · independence_scoped_to_claim · six_propagation_surfaces_repaired · load_order_repointed_to_E10_E11_E12 · catalog_row_is_a_pending_acceptance_transaction_edit · G1_CONTRACT_0_of_8_outputs_complete · output_2_not_started`**
