# v4 — FAI — G1 OPERATING-MODEL CARRIER (R8)
...
Status: **`g1_carrier_R8 · state=proposed · B-1_CLEARED · output_1_AMM_C4_1_done_NOT_complete · lane3_EXISTS_AS_run · 0_of_8_outputs_complete`**
...
> **Output 1 has run three times against three candidates and two independent reviews.** R4 (`§E6.5`–`§E6.12`) ran the `§E6.3` fixture on `AMM-C1`; **review found three rules the candidate violated.** R5 (`§E7`) produced `AMM-C2`; **review found the same class of defect one level down — the axes were decomposed and the values inside them were not — plus an aggregation that mixed resources, clusters and scenarios in one denominator.** R6 (`§E8`) produces **`AMM-C3`**: every facet answers one question, relation assertions are intervalled so `as_of` is answerable at all, the evidence is split into **three tables with one unit each**, and **three holdouts were predeclared before running**. Current result: T1 **22 of 22** non-holdout resources representable, holdouts **3 of 3 with zero amendments**; `estate_validation` **3 `PASS` · 11 `FAIL` · 11 `NOT_INSTANTIATED` of 25 tested logical resources**. **R7 (`§E9`) produces `AMM-C4`** after the third review found a **factual error** in R6 — the `05` owner column I cited is in an `analysis_nonbinding` child table, not the canonical contract, because my search pattern sampled instead of enumerating. `AMM-C4` withdraws that finding and restates it stronger (**zero of four** canonical ledger contracts record an accountable owner), installs the **eight-relation resource-governance grammar**, and **terminates the assertion/descriptor recursion** R6 opened. **R8 (`§E10`) produces `AMM-C4.1`** — nine bounded corrections, three traces, and **the mandatory Lane-3 `M-106` EXISTS-AS check that R8 §3.9.1 requires and that this arc had never run.** It recovers seven ownership/accountability concepts the AI corpus had **already routed**, fails one of my own findings' novelty claim against wave 2, and labels all four vendor mechanisms **`current_practice_only`** under §3.9.2's frontier test. **Output 1 is NOT complete and is NOT self-certified** — five candidates, four reviews, and **none of mine has yet survived a reader who did not build it.** It closes no ledger row, accepts nothing, installs nothing, and starts no later output.
...
|---|---|---|---|---|---|---|
| 1 | **artifact metamodel** | **`§E8` (`AMM-C3` + three-table rerun + predeclared holdouts)** · R5 `§E7` and R4 `§E6.5`–`§E6.12` preserved, superseded · spec `§E6.3` | Artifact Index ✔, governance taxonomy ✔, R8 §3.1 ✔ **+ §3.9.1/§3.9.2 ✔**, System Map ✔, Surface Map ✔, `03`/`05`/`06`/`08` schemas ✔ · **Lane 3: six AI-corpus registries ✔ (routing maps; raws not rescanned)** · **Lane-1 probes, none adopted: 42010 ✔ SEI ✔ FHIR ✔ IHE ✔ AUTOSAR ✔ Backstage ✔ CODEOWNERS ✔ AWS ✔ Palantir ✔** | **`AMM_C4_1_DONE · NOT_COMPLETE · AWAITING_INDEPENDENT_REVIEW`.** Fifth candidate, fourth review. **Lane-3 `M-106` EXISTS-AS run for the first time in this arc** (`§E10.11`): seven ownership concepts the corpus had already routed, `G1-FIND-37`'s novelty claim failed, all four vendor mechanisms labelled `current_practice_only`. **R6's 3/11/11 aggregate and all three of its `PASS`es are WITHDRAWN** — a factual error (`§E9.0`) plus two verdicts resting on inference. Published instead: the **eight-relation resource-governance grammar** (`§E9.3`), the **meta-kernel termination rule** (`§E9.4`), four more value collapses repaired (`§E9.5`), and a **eight-case per-resource governance enumeration** (`§E9.6`) — **no replacement aggregate** | **`resource_steward` declared for ZERO of seven real resources**; of eight governance relations OMNI declares at most three for any resource, never the same three · placement predicate `[CAN→3]` · relation bitemporality `[CAN→3]` · prose non-loosening `[CAN→6]` · approval-policy content `[CAN→3]` · the focused fixture is **weak evidence where I chose the cells and strong only where an estate fact filled them** · catalog row `status` four revisions stale | `architecture_steward` accepts, amends or rejects **`AMM-C4.1`**; **`AB-08`'s semantic answer is independently reviewed and ready for operator/steward closure per `§E10.13`**; descriptor-schema installation is **G2's** transaction against R8 §3.1 |
...
| `AB-08` | artifact taxonomy granularity | **REMAINS OPEN — but the SEMANTIC ANSWER is now independently reviewed and endorsed: `READY_FOR_OPERATOR_STEWARD_CLOSURE`** (`§E10.13`). Five candidates, four reviews; false single count REJECTED | **the question is malformed as a single class count** — 10 governance categories classify *carriers*, 13 Artifact Index roles assign *homes*, `primary_kind` types *resources*, and finer granularity is `part_of` rather than more classes. Passport ⟂ descriptor survives; **`plane` ⟂ `viewpoint_or_view` is withdrawn as a single axis** (two kinds + one relation, plus `home_plane` ⟂ `constrains_planes[]`); **`authority_level` dissolved into four facets with NO ordering; lifecycle into four; naming into two (`§E7.1`)**; the kind registry is **steward-governed and open, not closed at seventeen (`§E7.2`)** | **§E7 (`§E7.8` = the current disposition)** · R4 first run §E6.5–§E6.12 · original probe §E6 · §E-note B | **operator/steward may close the row on the five-function semantic answer** (`§E10.13`) — **it is no longer hostage to the rest of the metamodel**: policy composition, transaction scoping, governance effectivity and the frontier form of enforcement are **not** taxonomy-granularity questions, and the estate governance failures never were. **`AMM-C4.1` acceptance is a SEPARATE steward act** |
...
| Field | Value |
|---|---|
| **Output 1** | **`AMM_C4_1_DONE · NOT_COMPLETE · AWAITING_INDEPENDENT_REVIEW`.** Fifth candidate, fourth review |
...
| **The obligation discharged** | **R8 §3.9.1 Lane-3 `M-106` EXISTS-AS — never run by this arc until now** (`§E10.11`), plus §3.9.2's frontier double-evaluation applied to my own four vendor sources. **All four labelled `current_practice_only` with the gap named** |
| **`AB-08`** | **`AB-08_SEMANTIC_ANSWER_INDEPENDENTLY_REVIEWED__READY_FOR_OPERATOR_STEWARD_CLOSURE`** — a state change, not a closure |
| **Minted** | **nothing** |
| **Output 2** | **NOT STARTED** |
...
| **B-3** | **`AB-08` fixture RUN FIVE TIMES (R4–R8) — the row REMAINS OPEN, but its semantic answer is now independently reviewed and READY FOR CLOSURE (`§E10.13`).** Running a fixture is not disposing a ledger row, and R4's proposed closure was withdrawn under review. **What remains for the ROW: an operator/steward closure act. What remains for OUTPUT 1 is separate and larger: `AMM-C4.1` acceptance, policy composition, and the frontier form of enforcement.** The estate governance failures are **separately routed and are NOT `AB-08` closure criteria.** No new blocker minted | **ledger `blocking_scope` = blocks G1**; owner `architecture_steward` |
...
| Field | Value |
|---|---|
| Artifact | FAI **G1 operating-model carrier R8** · seat `proposal_authoring` · state **`proposed`** · **`B-1` CLOSED 2026-08-11** |
...
| **Why this keeps happening — the two root causes** | **`G1-FIND-29`, corrected and enumerated:** **ZERO of four** canonical ledger contracts record an accountable owner, and the three that record anything each record a **different** adjacent relation — `05` `winning_authority` (semantic authority), `06` `enforced_by` (enforcement), `08` `required_reviewer` (review), `03` nothing. **The estate partitioned the concept across four surfaces without naming the partition.** • **`G1-FIND-34`, the deeper one: OMNI's authority model answers a question about ACTS and I kept applying it to ARTIFACTS.** §4.6 asks who bears the consequence of an act; an artifact has no consequences, it has integrity, currency and content. **The prior authority work was misapplied by type, not forgotten** — which is why the good care-authority prose never transferred, and why each pass reads as though ownership were being discovered |
...
**STOP: `g1_working_dossier_R8 · B-1_CLOSED_2026-08-11 · evidence_restored_and_fenced · 40_rows_verdicted_zero_closed · OUTPUT_1_AMM_C4_1_DONE_NOT_COMPLETE_AWAITING_INDEPENDENT_REVIEW · LANE_3_M106_EXISTS_AS_RUN_FOR_THE_FIRST_TIME · seven_ownership_concepts_already_routed_by_the_corpus · G1-FIND-37_novelty_claim_FAILED · all_four_vendor_mechanisms_current_practice_only · AB-08_SEMANTIC_ANSWER_INDEPENDENTLY_REVIEWED_READY_FOR_OPERATOR_STEWARD_CLOSURE · three_traces_PASS · G1_CONTRACT_0_of_8_outputs_complete · 4_G1_blockers · output_2_not_started`**
