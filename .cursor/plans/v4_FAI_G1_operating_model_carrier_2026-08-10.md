...
| Field | Value |
|---|---|
| Artifact | FAI **G1 operating-model carrier** · Output 1 `ACCEPTED_AS_G1_PROPOSAL` · **Output 2 `ACCEPTED_AS_G1_PROPOSAL` (`§E13`, receipt `§7.6`)** · ****Output 3 `ACCEPTED_AS_G1_PROPOSAL` (`§E14`, receipt `§7.7`)** · **3 of 8 complete**** · **2 of 8 complete** · seat `integration` **expired on verification; none held for this pass** · carrier state **`proposed`** · **`B-1` CLOSED 2026-08-11**. *(No revision label: correction rounds are the rows below.)* |
...
**Stop condition:** the Output-1 and Output-2 acceptance conditions have **fired** (receipts `§7.5`, `§7.6`). **Output 3 is authored and awaits independent review; no acceptance condition has fired for it, and proposal authorship cannot fire one.** This carrier does not own program progression — **resolve the current next action from the Tier-0 checkpoint.** Outputs 3–7 and `§G1-AUTH` supersede their own sections when their rightful holders accept, amend or reject them.
