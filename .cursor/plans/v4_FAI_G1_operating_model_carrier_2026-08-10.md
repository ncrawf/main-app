...
Status: **`g1_carrier · state=proposed · B-1_CLEARED · OUTPUT_1_ACCEPTED_AS_G1_PROPOSAL__1_OF_8_COMPLETE · OUTPUT_2_PROPOSED_NOT_ACCEPTED_§E13_R2 · AB-08_CLOSED_adopted_narrowed · AO-SELF-1_TWO_EPISODES_FROZEN · G1_INCOMPLETE`** *(★ R2 — the token omitted Output 2 entirely while the posture paragraph below carried it; `D3` in `§E13.26.3`. The revision label is dropped rather than maintained: it was a derived value restated in a frontmatter field.)*
...
|---|---|---|---|
| 1 | Proposal | **★ R2: Artifact + submission Event → admission request → (after admission) Obligations** | The manifest revision is an immutable artifact; **submission is an attributed, permissioned event that creates an *admission request*, not owner duties.** R0 called submission a Commitment, contradicting *proposal is never commitment*. **R1 fixed the class and left the duty bypass:** submission still instantiated decision obligations immediately, so a malformed, duplicate, incomplete or hostile submission could place durable duties on authority holders **before the preflight gate ran.** Sequence: `manifest artifact → attributed submission event → admission request / candidate work → machine preflight and classification → admitted proposal → decision obligations instantiated for the resolved required-owner set`. **A rejected intake remains an attributed event with a result and creates no owner-review duty** |
...
| 8 | Observation / drift | **★ R2:** Controller emitting Observations, evidence and **candidate** findings | No terminus; **never auto-applies, and never manufactures a substantive obligation** — admission is required (`§E13.13`). R1 corrected this in the detailed section and left *"and Obligations"* standing here |
...
| **★ R2** `evidence_admission_id` | Commitment, **or** Deterministic Derivation under an already-accepted policy | (decision purpose, observation event revision) | Superseded on re-evaluation; never edited | Deterministic where policy-driven; attributed where judged | **Admitting authority** for the decision the evidence serves (`§E13.13`) |
...
**Evidence Admission is homed, not an anonymous bridge. No new universal object is minted; it binds to an existing family** as either **(a)** an *authority-bearing evidence-admission decision* (Commitment), or **(b)** a *deterministic admission result under an already-accepted policy* (Deterministic Derivation), whose governing commitment is the prior attributed acceptance of that policy. Either way it carries: `evidence_admission_id` · the **admitting authority or governing policy revision** · the **exact observation-event revision** admitted · the **decision purpose and scope** the evidence serves · ruleset/policy basis · temporal bounds (`effective_as_of` / `recorded_as_of`) · validity and **supersession on re-evaluation**. **Admission is per-decision, not global:** the same observation may be admissible for a low-consequence check and inadmissible for a landing gate, which is a feature and not an inconsistency. `[CAN→6 · admission criteria]`
...
Not *"the surfaces were fixed."* **Every current Output-2 assertion of a derived value must either be the owning source, resolve to it, or be a pinned quotation.** A future checker must flag both `AO-SELF-1` episodes — `ee1fa6e` and `eb8a2da` — and pass the corrected head, **differing only in being right**, which is the negative-control standard `ledger:289` sets for `AB-08`. **Three failed episodes over one defect class, by one author, under three independent reviews, is the strongest available evidence that `§E13.7`'s rule cannot be discharged by care.** Each pass was more careful than the last and each still shipped restated derived values. **The conclusion is not that the author must try harder; it is that a derived value restated by hand is unmaintainable at any level of diligence, which is precisely what the rule asserts and what G2 must mechanize.**
...
| Axis | Independent when | Current bootstrap profile |
|---|---|---|
| **Appointment** | Author and reviewer are not appointed by the same authority | **Dependent** |
| **Organizational / economic** | No shared employer, funder or commercial interest | **Dependent** |
| **Context / information** | Reviewer inputs are not selected by the reviewed party or a shared router | **Dependent** |
| **Evidence / source** | Conclusions rest on separately obtained sources | **Partial** — the reviewer reads the live repository directly |
| **Method / model** | Different reasoning path, model or technique | **Independent** |
| **Decision / appeal** | The verdict is not revocable at the reviewed party's discretion, and appeal is defined | **Partial** — the verdict blocks, and the delegator may withdraw the delegation |
| **Conflict of interest** | No stake in the reviewed outcome | **Independent** |
...
| Object | What it is |
|---|---|
| **Content unit / payload** | The exact words, artifact, excerpt or structured packet — stored **once**, content-addressed |
...
**The successor's continuity view is a generated projection over this graph**, so `G1-FIND-63` applies: **it is invalidatable and must be pinned.** That settles the operator's practical handoff question on architectural grounds rather than preference — **point the successor at a committed handoff artifact rather than pasting a generated prompt**, because a committed artifact can be invalidated when its constituents advance and a pasted prompt cannot. **A pasted prompt is an unpinned projection that goes stale silently inside the successor's context**, which is `G1-FIND-63` in its most common everyday form.

**The most valuable projection is divergence, not union.** A successor does not need both transcripts; it needs **what the two seats concluded differently from the same payload**, plus the unresolved obligations. Full transcripts remain conditional forensic evidence and never default boot (`§E13.17`).

**Human interjections are first-class attributed contributions.** They are variously evidence, questions, assignment instructions, design contributions, objections or acceptance decisions, and **their force derives from the act and the seat exercised — never from length.** A one-line operator objection that redirects an arc is a larger contribution than a page of narration; a model that weighted by volume would systematically under-record the operator. Treating operator input as *"assumed context"* is the same error as treating a transcript as authority: it loses the act.
