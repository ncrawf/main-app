...
**Added by `G1-FIND-10` — one representational case and one negative control, both testing the same thing:**

- **Same source, different contextual force, no duplication.** One external source resource must be representable as *informative comparator* in one context, *internally adopted standard* in another, *contractual requirement* for one deployment, *incorporated regulatory requirement* for a specific regulated product, and *not applicable* elsewhere — **without copying or mutating the source.** Candidate shape, **fixture material not a final enum**: `external_source` → `applicability_claim` (context · profile · jurisdiction · product · `as_of`) → `normative_force` → `disposition` → `translated_to` → `verified_by`.
- **Operator variation is the same shape.** An operator requirement — *"our pharmacies only do it this way"* — must resolve as **inside a declared variation point (configuration)** or **outside it (an architecture change)**, per charter §6.2. **If the metamodel can represent a regulation's contextual force but not an operator's, it is incomplete.**
- **Negative control:** a raw standard entering the reservoir **must not** become binding OMNI architecture by ingestion. Auto-promotion **must fail.**
...
**★ The operator's Henry Ford case is the SAME question shape, and it already has an accepted answer.** *"Henry Ford says our pharmacies only do it this way"* and *"does IEC 62304 apply here"* are both **an external party's requirement meeting an OMNI context**, needing an attributed applicability decision. One is a customer policy, one is a regulation; the machinery is identical. And the mechanism is **already accepted at G0** — charter **§6.2**, by name:
...
So the operator's fear — *engineers and agents just go build it for them* — **has a named mechanism, accepted, unimplemented.** The gap is not that nobody thought of it; it is that **variation points, operator/deployment profiles, regulatory obligations and comparator lessons have no shared representation, so none of them resolves for a given build.**
