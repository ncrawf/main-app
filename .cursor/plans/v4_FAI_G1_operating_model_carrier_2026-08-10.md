...
**Founding law, proposed.**

> **OMNI has no singular sovereign canonical graph. It has separately governed relation-type definitions, attributable direct relation assertions, receiving-scope admission decisions, separately governed constraints and composition rules, first-class conflict/adjudication records, and generated effective graph projections. Canonical direct assertions remain distributed among their rightful owners; every effective graph is a scoped, temporal, generated projection that owns no source truth.**

**Its provenance is split three ways, because collapsing them is how external material becomes constitutional law by tagging** (`GRD-036`, `G1-FIND-40`, `F-11`):

| Class | Content | Force |
|---|---|---|
| **INHERITED — accepted or governing OMNI constraints** `[INH]` | `AB-13` **no global event graph, no global semantic layer, no global policy decision point** (`already_present`) · `INV-19` **no globally resolved authority graph** — attributed claims + verifier-specific acceptance projection (`adopted`) · `INV-21` **every evidence graph is observer-scoped and must expose known omissions** (`adopted`) · `T0-15` **projection ≠ authority** · `T0-13` per-event ownership dimensions do not collapse · `D0THES-GRD-035` **no single domain owns a cross-cutting concern** · `D0THES-GRD-026` payload-noun ≠ domain · `T0-14` operator-neutrality · `C4.4 §R.16` **local admission — federated publication ≠ universal trust** · `§E13.7` **one authoritative owner per fact within a declared scope** · `D0CKPT-GRD-005` transitivity, direction and inherited-edge classification | **binding or accepted where their own carrier says so.** The sentence above is a *composition* of these, not a new law |
| **CANDIDATE — Output-3 synthesis** `[KND]` | the **six-object decomposition** and its non-collapse laws · the `RelationTypeDefinition` declaration contract · constraint **binding sites** · **typed composition over relation pairs** with authority non-propagation · relation-type **evolution / non-reinterpretation** · the **six resolvability classes** · relation-level **cessation and succession** semantics · the consolidated **rejection-ground surface** | **proposed. Nothing here is settled by having been written** |
| **EXTERNAL — corroboration and anti-re-derivation evidence only** | `EVSRC-2026-000270` (*"own the grammar of safe coordination, not everyone's reality"*; the 6-layer shared-grammar model; `MASTER GUARDRAIL: plurality of description ≠ abolition of the discipline of truth at commit`; `ontology_admission_gate`; `semantic_translation_profile`; `local_concept_portability`; `living_classification`; `omitted_relation_check`) · `EVSRC-2026-000271` **`REJECT single-company-graph`** | **evidence, `omni_analysis_nonbinding`, `promote-after-reconciliation`.** It **defeated the novelty** of this arc's registry proposal and **exposed a contradiction**; it is **not** OMNI's binding architectural authority and confers no force here |

> **The discipline this repairs, stated plainly:** `M-106` establishes that a concept already exists in the evidence estate. **It does not establish that the concept is correct, accepted, governing or universally scoped** (`G1-FIND-40`). A prior draft of this section labelled the founding law `[INH]` with `EVSRC-2026-000270` as its principal basis — which contradicted `§E14.9.1`'s own statement of that rule two pages later, and is `F-11` serial source capture with the reopened source as the altitude. **Corrected: the law is inherited from the governing OMNI sources above; the external source corroborates and disproves novelty.**
...
| **declared change posture** | every revision declares one of: **compatible** (prior assertions remain valid and comparably interpretable) · **revalidation-required** (prior assertions stay interpretable but their admission or conformance must be re-evaluated) · **migration-required** (prior assertions need an explicit new assertion or mapping) · **superseded** (the type is retired with lineage; existing assertions remain historically resolvable). **The final lifecycle vocabulary is `[CAN→4]`; the declaration requirement is Output 3's** |
...
*(A prior draft of this section wrote "loosening is structurally impossible rather than policed." That is the exact wording `§E8.0` item 8 accepted **in full** as overstated and `§E8.6` corrected — *"the same error this carrier criticizes elsewhere: claiming a mechanism **discharges** an obligation."* Reproducing it regressed an accepted correction. `G1-FIND-70`.)*
...
> **This would settle on Output-3 acceptance. Output 1's carry remains OPEN until then.** A prior draft wrote *"is SETTLED … discharging the `[CAN→3]`"* — **authorship supplies the proposed answer and cannot make it accepted by writing it.** That is the defect `§7.5` recorded against this arc once already (*"closure came from the acceptance transaction rather than from authorship"*) and `§E13.6` recorded a second time (*"a proposal cannot close its own falsifier"*). This is the third instance; `G1-FIND-72`.

**The four temporal modes bind at relation grain** `[INH §E13.5]`. `historical_as_now_reconstructed(effective_as_of)` **may** use later-recorded, earlier-effective assertions; `known_as_of`, `decision_context_replay` and `evaluation_against_the_knowledge_horizon_then` **may not**. **No-hindsight binds the last three and not the first**, which discharges `G1-FIND-45` at relation grain. Every graph projection declares: temporal question/mode · `effective_as_of` · `recorded_as_of` or knowledge cutoff · resolver scope and profile · whether later evidence is admissible — and because a projection is content-addressed, **mode and both bounds sit inside its input manifest** `[INH]`. **The resolution algorithm is Output 5's; C4.5 owns temporal architecture.**

**Supersession is a typed transaction, never a comparison** `[INH §E7.1.2]` — six conditions, no ordering anywhere.

**The one surviving prohibition, re-anchored over the current orthogonal facts** `[INH]`. Output 1 stated it over `normative_force`; R6 split that field because it fused a **force** with an **epistemic use**, and the accepted result at `§E8.1.1` (as further narrowed at `§E9.5`) is:

```
normative_effect  ∈ binding · states_norm_unbound · no_norm          the force it carries
epistemic_role[]  ∈ evidence · analysis · reference · implementation_proof   how it may be USED
```

> **A resource whose `normative_effect ∈ {states_norm_unbound, no_norm}` may not supersede one whose `normative_effect = binding`.**

**`epistemic_role[]` does not enter this prohibition at all.** Whether something is evidence, analysis or reference is orthogonal to whether it carries binding force, and the four facts the transaction actually composes stay separate: **predecessor and successor `normative_effect`** ⟂ **predecessor and successor `epistemic_role[]`** ⟂ **the governing authority and decision condition** (`§4.6`, `§4.5`) ⟂ **the exact supersession policy** — which is `§E7.1.2`'s six conditions, unchanged. **No value set is minted here; the values above are the accepted Output-1 ones** `[INH]`.

*(A prior draft wrote the prohibition as `normative_effect` being "nonbinding, informative or evidentiary-only" — which re-fused `informative` and `evidentiary` into the force facet that R6 had removed them from, and used `nonbinding`, a value the split replaced. **`F-08` committed inside the finding that exists to repair a facet-naming defect — the third time this arc has decomposed an axis and then re-collapsed its values one level down.** `G1-FIND-71`.)*
...
| **stay / interim disposition** | an authority competent to suspend | **suspends** — the only one of the four that changes force without resolving |
...
**Revalidation on material change is inherited** `[INH]` — wave-6 `285`: *"review once, inherit many — but re-review every material departure from the certified envelope."*
...
| R15 | nonbinding supersedes binding | supersession transaction | `§E14.5` | reject |
...
|---|---|---|---|---|
| relation-type semantics + who owns a predicate's meaning | **`EVSRC-2026-000270`** (w5, ★★ ULTRA, operator-flagged) — *"own the grammar of safe coordination, not everyone's reality"*; the **6-layer model** `canonical coordination grammar → domain contracts → operator overlays → preserved source vocabularies → versioned mapping assertions → actor projections` = *"shared grammar, plural vocabularies"*; **MASTER GUARDRAIL: plurality of description ≠ abolition of the discipline of truth at commit** | **`EXACT_EXISTS_AS`** | **YES — reopened**, Cluster 3 read verbatim | **Decisive.** The founding law at `§E14.0` is inherited, not authored. **It also defeats the entry receipt's steward-owned registry** — that proposal was the totalizing move this source exists to prevent (`G1-FIND-67`). L1 grammar and the domain-noun federation model stay routed to the **Federation contract**, `promote-after-reconciliation`, *"do NOT mint a new god-domain"* |
| foreign semantics admitted into a local scope | **`ontology_admission_gate`** (w5, 270) — *"EXISTS-AS analog: `GRD-036` applied to semantics"* | **`EXACT_EXISTS_AS`** | yes, same read | `§E14.2` admission of foreign relation types is inherited |
| cross-namespace mapping without equality | **`semantic_translation_profile`** + **`local_concept_portability`** + **L5 `versioned mapping assertions`** (w5, 270) | **`EXACT_EXISTS_AS`** | yes, same read | `§E14.2`'s *"a mapping is an assertion, not an equality"* is inherited; the hospital-procedure ≠ medspa-treatment case is the source's own |
...
| as-of integrity | `point_in_time_context_integrity` (w2) — *"no-future-leak live + no-hindsight-leak on replay/eval"* | **`SHARPENS`** *(inherited from `§E12.8`)* | no | `§E14.5` discharges it at relation grain |
...
**Neither accepted proposal is reopened, amended or contradicted.** `§E14` **consumes** Output 1's identity levels, `RelationAssertion`, typed endpoints, `part_of` split, cardinality corrections, supersession transaction, meta-kernel seal and governance grammar; it **discharges** three carries Output 1 explicitly routed here (`relation bitemporality`, `edge authority state`, `G1-FIND-45` no-hindsight) and **re-anchors one** (`G1-FIND-69`, no semantic change). It **consumes** Output 2's event/history contract, four temporal modes, invalidation predicate, completeness posture, operational predicates, fact-ownership split and binding map, and discharges the `realizes` routing from `G1-FIND-65`. **No byte of `§E13` is altered**, and the accepted `§E10`–`§E12` range is untouched. Proof at `§9`.
