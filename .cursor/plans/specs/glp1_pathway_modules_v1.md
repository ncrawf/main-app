...
**Total: 35 questions defined in this spec. Module 21 pharmacologic acknowledgments (~4 questions) deferred to future Rx Confirmation flow spec.**
...
- Any patient with MTC or MEN-2 personal OR family history: session closes with `denied_contraindication_absolute` per `Section 1Q.15` — downstream modules (19, 20) don't render
- Any patient with pancreatitis history: session closes with `denied_contraindication_absolute` per `Section 1Q.15` — downstream modules don't render
- Module 16 (motivation + educational + commercial_confidence) renders for all patients regardless of GLP-1 history; positioned in pathway file AFTER Module 13 weight history. Module 17 (eating disorder screen) renders for all patients; positioned AFTER Module 15. Module 18 (cv_safety_extended) renders for all patients; positioned AFTER Module 17 and BEFORE Modules 19-20 so thyroid/MEN-2 hard-stop fires before downstream modules are wasted work. Module 19 (bariatric_surgery_extended) renders for all patients who clear the Module 18 hard-stop; positioned AFTER Module 18 and BEFORE Module 20. Module 20 (gi_safety_extended) runs LAST in the Layer D stack for this spec; pharmacologic acknowledgments that would have been Module 21 are deferred to a future Rx Confirmation flow spec (see Deferred scope section).
...
**Composition position:** pathway file `repo/intake/pathways/glp1.ts` inserts this module after Module 17 (eating disorder screen) and before Modules 19-20 (bariatric + GI safety). Runs in the Stage 1 safety cascade per `Section 1J` — atoms here gate eligibility BLOCKs before the patient continues. MTC/MEN-2 personal and family atoms captured here feed the future Rx Confirmation flow's MEN-2 black box acknowledgment (deferred from this spec — see Deferred scope section).
...
**Composition position:** pathway file `repo/intake/pathways/glp1.ts` inserts this module after Module 19 (bariatric surgery extended). Module 20 is the LAST module in the Layer D intake stack for this spec; pharmacologic acknowledgments that would have been Module 21 are deferred to a future Rx Confirmation flow spec. Runs in the Stage 1 safety cascade per `Section 1J` — atoms here gate eligibility for pancreatitis history. Placed after Module 19 so the post-bariatric signal is available when the provider reviews gastroparesis or gallbladder atoms (post-bariatric anatomy alters the baseline interpretation).
...
- Layer D Phase 2.2.2 COMPLETE: 8 modules / 35 questions defined in this spec (Module 21 pharmacologic acknowledgments deferred — see **Deferred scope**)
- Stage 2 grand total (Phase 1 + Phase 2.2.2): 20 modules / 74 questions defined (Layer D counts exclude deferred Module 21 questions)
- Per-patient render varies: GLP-1-naive non-diabetic cis-male or post-menopausal ~25-26 questions; GLP-1-naive reproductive-potential patient ~26-27 questions; current GLP-1 patient +5; past GLP-1 patient +7; add 2 more for ED diagnosis history; add 1 more for family sudden death under 40; add 1 more for T2D retinopathy; add 1 more for gallbladder anatomy branch
...
- Phase 2.2.2 complete. Next: Phase 3 GLP-1 pathway composition file (`repo/intake/pathways/glp1.ts`) wiring all layers (A demographics, B clinical_core, C domain baselines, D pathway-specific Modules 13-20) into the runtime pathway definition

## Next deliverable

Phase 2.2.2 COMPLETE (this checkpoint: Modules 17-20 safety extensions; Module 21 deferred — see **Deferred scope**):

1. ~~`mod.pathway.glp1.eating_disorder_screen_v1`~~ — **COMPLETE** (4 questions: symptom screen + diagnosis history + diagnosis subtype + current treatment; Q17.3 subtype drives differentiated routing — AN hard stop, BN flag, BED distinct flag cohort)
2. ~~`mod.pathway.glp1.cv_safety_extended_v1`~~ — **COMPLETE** (4 questions: personal history + family history of MTC/MEN-2/Long QT + family sudden death under 40 Y/N + heart-related Y/N/unsure conditional; MTC + MEN-2 personal OR family history fire `denied_contraindication_absolute` hard stop per `Section 1Q.15`)
3. ~~`mod.pathway.glp1.bariatric_surgery_extended_v1`~~ — **COMPLETE** (1 question: weight-loss surgery multi-select with 5 options + none; extends Module 7 baseline; flag cohort for drug choice + dose considerations)
4. ~~`mod.pathway.glp1.gi_safety_extended_v1`~~ — **COMPLETE** (3 questions: personal pancreatitis/gallbladder/gastroparesis multi-select + gallbladder anatomy conditional + diabetic retinopathy conditional on T2D; pancreatitis personal history fires `denied_contraindication_absolute` hard stop per `Section 1Q.15`)

**Phase 2.2.2 closed: 8 GLP-1 Layer D modules / 35 questions complete in this spec.** Module 21-equivalent pharmacologic acknowledgments are **DEFERRED** to a future Rx Confirmation flow spec (Hims does not run dedicated pharmacologic consent screens at intake).
...
**Interface with this spec:** Module 18 and Module 20 atoms remain the source of truth for *history* that drives eligibility and provider review; the deferred flow captures *consent / attestation* for prescribing and pharmacy, not duplicate history questions.
