...
**★ PRE-SPINE HOLD (2026-07-04):** before any spine prose, the spine author MUST clear `v4_C4_spine_watch_list.md` — a living list of operator-surfaced pre-spine posture items + a pending Knox-chat capture. Currently open: **(1) Patient Companion / External Agent Interop posture** (external patient AI = candidate-generating actor, not authority — lands in surfaces/P35/Federation/REV-184); **(2) AI Aggregation / Local Care Exchange posture** (governed front-door/exchange rail, NOT extractive aggregator — ties C3.7 firewall + G4 §3 five loops + §2.1 moat); **(3) the full Knox trifecta discussion capture is OWED** (Nick provides → capture + extract). Spine draft 0 does NOT start until this list + the capture are complete (Nick + Knox). These do NOT reopen C3.8.
