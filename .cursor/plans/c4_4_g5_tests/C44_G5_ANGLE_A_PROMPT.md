...
*Repinned for the run-2 Angle-A RERUN: run-1 targeted commit `865afad` / packet blob `d01978f` (superseded); the bounded post-test patch was `bcb2b19` / `674fffa` (superseded by the Knox pre-flight micro-patch `f7db832` / `a12bd0e`, which was itself the run-2 target and is now superseded by the run-2 R2 metadata/state patch `13f0d98` / `7c83e7e` — footer artifact-identity + App-E E.1 external-freeze correction only; the Angle-C source/authoring sections are proven byte-unchanged, so this is a valid same-constitutional-prompt Angle-A rerun). Verify against the pins below.*
...
- the repository branch/commit resolves to the target commit `13f0d98`;
- the G5 file resolves to the exact G5 packet blob `7c83e7e`;
...
- **G3 acceptance carrier (verified separately):** `13f0d98:.cursor/plans/v4_C4_4_taxonomy_constitution_and_reference_architecture.md` resolves to `76a8cdf`.
...
1. `.cursor/plans/v4_C4_4_disposition_ledger_and_v4_handoff.md` — at target commit; expected blob `7c83e7ef0e7bbc29f14a5341edd1fbfed6bf07c8`.
