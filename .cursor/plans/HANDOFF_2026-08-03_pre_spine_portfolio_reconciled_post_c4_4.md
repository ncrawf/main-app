...
- **★ OPECON is NOT a prerequisite to beginning Gate 2 (`D0CKPT-DEC-009`).** Gate-1b §13.3 rejected "Starting OPECON here" and routes the F3 operator slice **outbound** to OPECON, which makes OPECON a downstream *consumer* of Insurance, not an upstream dependency. Whether populated C3.9 requires completed OPECON output is **`UNADJUDICATED`** and is decided at C3.9 activation — it is **not** inherited from the 2026-08-04 phase diagram. Do not construct that chain again; see `D0CKPT-GRD-004`.
