...
### §4.1a — ★ MECHANICS ADJUDICATED + REWRITTEN (2026-08-05) — supersedes both the "CLOSED" reading and the freeze below

**★ STATUS.** The independent review is **complete and adjudicated**. Fresh Knox and fresh Opus reviewed the frozen bytes **independently, read-only, without coordinating**, and **both returned `MATERIAL_REWRITE`**. Nick delegated adjudication to Opus and authorized the write. `agent_work_protocol.md` §2.1 has been **materially rewritten as a proportional durable-work contract** and the boot/routing contradiction is resolved.

**What the rewrite establishes** (decision `D0CKPT-DEC-007`; supersession `D0CKPT-SUP-001`; guardrail `D0CKPT-GRD-003`):
...
- **NEXT AUTHORIZED ACTION = Knox review of the landed rewrite.** `D0THES-REV-212` stays **OPEN**; closure = Knox reviews and Nick accepts.
...
- **★ 2026-08-05 — LANE LAUNCH STILL HELD (mechanics now adjudicated).** The independent mechanics review is complete and §2.1 has been rewritten (§4.1a; `D0CKPT-DEC-007`), but `D0THES-REV-212` stays **OPEN** pending Knox review of the landed rewrite and Nick's acceptance. No lane may be activated, the base must not be rotated, and no lane branch may be written to until then.
