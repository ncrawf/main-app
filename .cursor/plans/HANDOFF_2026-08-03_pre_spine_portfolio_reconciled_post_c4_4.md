...
| Current holder | **`VACANT` (2026-08-08 PM, Nick-authorized).** Held open deliberately under the AWP §2.1 integrator-transfer law: *"a retired, exhausted or replaced holder MUST NOT strand its lanes."* **Blockers while vacant:** no shared control-plane surface may be landed by a lane; the next holder must run a freshness + collision check on assumption and record it here. **Prior holders, history only — neither is a current holder:** `THREAD LOCK: INS-G1B-PARENT-CARRY \| seat=OPUS \| visible="Insurance · Parent carry"` (assumed 2026-08-08, work complete — its carry landed on `main` at `849f0c1`, so leaving it recorded as the live holder pointed the role at a finished thread) and, before it, `THREAD LOCK PRESPINE-PHASEA-INTEGRATOR \| seat=OPUS \| visible="Pre-spine · Phase-A integration"`. |
