/**
 * Phase 4H-templates-discipline commit 7 — render module for
 * tmpl.clinical_decision.followup_needed_v1.
 *
 * Per ADR Section 7.5 cutover discipline + Section 1Q.5 Template
 * object shape. Mirrors prior render modules.
 *
 * Wording strategy:
 *   - Email body literals byte-identical to legacy.
 *   - Brand text (logo + footer + SMS prefix) sources from the
 *     brand_short_label slot. NO inline brand interpolation in body.
 *   - SMS body matches legacy "MAIN: More info needed. <url>".
 */

import { getEmailTheme } from '@/lib/notifications/emailTheme'

export interface FollowupNeededRenderInputs {
...
export function renderFollowupNeededEmail(
  inputs: FollowupNeededRenderInputs,
): RenderedEmail {
  ...
  // lib/notifications/patientMessages.ts case 'followup_needed'.
  // tier_1 existence_only — references "info needed", "case moving",
  // "steps in dashboard" without naming protocol, dose, condition,
  // or pathway.
  const subject = 'We need a bit more information'
  const previewText = 'Action needed to keep your case moving.'
  const eyebrow = 'Action needed'
  const heading = 'Additional info needed'
  const intro = 'We need a little more information before we can move forward.'
  const detail = 'Please complete the requested steps in your dashboard.'
...
/**
 * Renders the followup_needed SMS. Brand prefix sources from the
 * typed brand_short_label slot.
 *
 * Legacy: `MAIN: More info needed. ${short}`.
 * New:    `${brand_short_label}: More info needed. ${dashboard_url}`.
 */
export function renderFollowupNeededSms(
  inputs: FollowupNeededRenderInputs,
): RenderedSms {
  return {
    body: `${inputs.brand_short_label}: More info needed. ${inputs.dashboard_url}`,
  }
}
