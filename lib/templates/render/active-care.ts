/**
 * Phase 4H-templates-discipline commit 5 — render module for
 * tmpl.clinical_decision.active_care_v1.
 *
 ...
 *
 * Wording strategy:
 *   - Visual structure mirrors prior render modules (case-approved,
 *     awaiting-clinical-review, etc.).
 *   - Brand text (logo + footer + SMS prefix + INTRO LINE) sources
 *     from the brand_short_label slot. NOTE: this is the first
 *     migration where the brand label is embedded INSIDE the body
 *     copy (not just in prefix/footer). Legacy intro: "You are now
 *     in active care with MAIN." — the typed slot produces "You
 *     are now in active care with ${brand_short_label}." For
 *     brands.slug='main' the rendered output is byte-identical.
 *   - Email body literals (subject / preview / eyebrow / heading /
 *     detail) are byte-identical to legacy because legacy wording
 *     was already tier_1 compliant.
 *   - SMS body matches legacy "MAIN: Active care. <url>" pattern.
 *
 * Per ADR Section 7.5: parity is GOVERNED behavioral equivalence,
 * not byte-for-byte preservation. For this Template the legacy
 * wording happens to be governance-compliant for brands.slug='main',
 * so byte-level holds organically for the existing brand.
 *
 * Per ADR Section 7.6 (rule execution scope): this render module
 * is imported by lib/rules/runtime/dispatcher.ts. The dispatcher's
...
export interface ActiveCareRenderInputs {
...
export function renderActiveCareEmail(
  inputs: ActiveCareRenderInputs,
): RenderedEmail {
  ...
  // lib/notifications/patientMessages.ts case 'active_care'.
  ...
  const subject = 'You are in active care'
  const previewText = 'Welcome to active care.'
  const eyebrow = 'Care update'
  const heading = 'Welcome to active care'
  // Brand label embedded inline per legacy (the intro is the FIRST
  // typed slot to substitute brand mid-body rather than just at
  // prefix/footer).
  const intro = `You are now in active care with ${brandLabel}.`
  const detail = 'Your dashboard includes your current plan, check-ins, and next steps.'
...
/**
 * Renders the active_care SMS. Brand prefix sources from the typed
 * brand_short_label slot, replacing the legacy hardcoded "MAIN:"
 * prefix per ADR Section 7.5 multi-tenant rule.
 *
 * Legacy: `MAIN: Active care. ${short}` where short = dashboard URL.
 * New:    `${brand_short_label}: Active care. ${dashboard_url}`.
 *
 * For brand_short_label = "MAIN" (the existing brand's slug.toUpperCase()),
 * the rendered output is byte-identical to legacy.
 */
export function renderActiveCareSms(
  inputs: ActiveCareRenderInputs,
): RenderedSms {
  return {
    body: `${inputs.brand_short_label}: Active care. ${inputs.dashboard_url}`,
  }
}
