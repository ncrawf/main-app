...
- **Boot-surface check (runs on every `npm run lint`, TS or not):** `npm run check:boot-surfaces` validates the two current-checkpoint pointer regions against the Boot-Surface Rule below — one pointer line per surface, both naming the same checkpoint, under the length cap, and free of copied focus / next-action / gate / lane / integrator state, commit identifiers and arc-specific state. **It is a mechanical safeguard, not a doctrine linter — keep it narrow.** If it fails, move the content to its owning surface; do not widen the validator to accommodate a violation.

## Boot-Surface Rule — what may NEVER be written here (`D0CKPT-GRD-007`)

**`AGENTS.md` and `04_manifest_read_graph.md` Tier-0 #15 are DISCOVERY surfaces. They route; they do not report.** Every agent reads them before it reads any arc, so a sentence placed here outranks, in practice, whatever is actually happening — and it keeps outranking it long after the moment that justified it has passed.

**Mechanically enforced scope, stated honestly.** `npm run check:boot-surfaces` (also a CI status check) enforces the **current-checkpoint pointer blocks** — the regions delimited by the `CHECKPOINT-POINTER:START` / `CHECKPOINT-POINTER:END` HTML-comment markers in these two files (written here without their comment wrappers on purpose, so this sentence does not itself register as a second marker pair — the validator caught exactly that when this rule was first written). Inside a block: exactly one line, under a length cap, no continuation text, every backticked token drawn from a small allowlist (checkpoint paths that must exist on disk, plus a handful of named tokens), no commit identifiers, no negative directives, and both blocks naming the same checkpoint. **The rest of these two files is governed by the rule below but is NOT mechanically audited** — do not claim it is. **"Cannot merge" is true only once the CI status is marked required in branch protection**, which is a repository setting.

**Permitted here:** stable pointers (file paths, owning surfaces, route names) · timeless law and work-class routing · the drift-guard requirement that both surfaces name the same checkpoint.

**PROHIBITED here — put it in the owning surface instead:**

| Never on a boot surface | Owner |
|---|---|
| Operator-selected focus · current arc · "what we're working on" | current checkpoint |
| Next allowed / next authorized action | current checkpoint |
| Gate, phase or arc execution + acceptance state | that gate's own carrier or brief |
| Lane state · integrator holder or vacancy | checkpoint §4.2 |
| Any SHA, blob, branch head or base pin | the single owning row (AWP §2.1 Single-source law) |
| Dependency, blocker or acceptance conditions | the work horizon in the active pre-spine map |

**A negative next-action directive is absolutely prohibited.** Never write *"it is NOT `<arc>`"*, *"do not work on `<arc>`"*, or any instruction whose correctness depends on today's portfolio posture. Such a sentence converts one moment's sequencing choice into a standing order to every future agent, and it cannot expire on its own. **This rule exists because exactly that happened on 2026-08-08:** this file carried *"NEXT AUTHORIZED ACTION … it is NOT Gate 2"*, and a correct cold boot abandoned the live Insurance arc because the universal boot path told it to.

**If you are about to add current state to a boot surface, you have found a defect in the owning surface — fix that instead.** If the state genuinely has no owner, say so and stop; do not park it here. **Repairing a stale value in place is not compliance** — replacing one stale paragraph with tomorrow's stale paragraph preserves the mechanism that failed. Delete the duplicated state and leave a pointer.
...
- **Current Checkpoint Handoff — `.cursor/plans/HANDOFF_2026-08-09_work_horizon_closed_insurance_gate2_startable.md`. POINTER ONLY.** Resolve current focus, next allowed action, lane and integrator state, gate state, dependency and acceptance conditions, and every commit identifier at that checkpoint and the surfaces it names. Nothing about current work may be restated on this line (Boot-Surface Rule above; guardrail `D0CKPT-GRD-007`; enforced by `npm run check:boot-surfaces`). Historical lineage, provenance only — never boot from these: `.cursor/plans/HANDOFF_2026-08-03_pre_spine_portfolio_reconciled_post_c4_4.md` · `.cursor/plans/HANDOFF_2026-07-19_taskd_interim_checkpointed_evrun12_active.md` · `.cursor/plans/HANDOFF_2026-07-12_wave4_closed_grr_pre_plastics_and_task_d.md`. **Drift-guard:** read-graph Tier-0 #15 must name the same checkpoint file; if it does not, the Boot Freshness Check fires and you STOP.
