...
## What OMNI is (boot anchor — do not make the operator re-explain this)

OMNI is a **governed care + business operating system**. Two lenses, recorded once (canonical: thesis §3.5; full index: `.cursor/plans/doctrine/comparator_analogy_registry.md`). **Do not re-derive these — cite them.**

- **Lens A — the product stack OMNI collapses into one substrate** (what we replace/match/beat): Hims/Ro (telehealth+Rx) · Hims-Labs/Quest (labs) · Epic/Cerner (EMR) · Shopify (commerce) · WordPress (site/content) · Mindbody (scheduling/memberships) · RingCentral (comms) · ActiveCampaign (marketing) · Oracle/Workday/QuickBooks (back-office) · Stripe (payments) · and the ChatGPT-screenshot-then-confirm workaround it displaces. The win = one coherent substrate with shared identity/consent/authority/evidence, not feature parity.
- **Lens B — the substrate/build concepts OMNI borrows** (how great systems work): Amazon (ordered things→fulfillment→exceptions = Act loop) · Tesla (signals→state→safety-monitor→feedback = Sense loop) · NASA/Houston (telemetry-vs-command, authority gates, go/no-go, audit) · Airport/ATC (scheduling/routing/queues/handoffs/capacity = Planning axis) · Airplane-as-object (redundancy, preflight checklists, fail-safe, black-box) · Stripe (idempotency, immutable audit, payment-state separation) · FHIR/HL7/DICOM/LOINC (structured ingestion standards) · **Anthropic** (the build-OS engineering discipline — doctrine-first, guardrails, decision records, review gates, no silent drift) · Apple HealthKit (contributions-to-a-record review queue) · Salesforce/Zendesk (one-interaction→many-intents) · Mirth/Rhapsody (route-then-store ingestion).
- **A ≠ B:** Lens A = products we replace; Lens B = engineering/data-flow concepts we borrow. OMNI = all of both, at once, plus healthcare **authority gates** (clinical adoption, consent, eligibility, provider authority) between sense and act that none of them carry. **Operating model:** two interlocking governed loops (Sense + Act) + authority gates (thesis §8). **Payload-noun ≠ domain** (`D0THES-GRD-026`): labs/Rx/commerce/etc. are use-cases threading the model, not domains — decompose before naming.
