...
- **Current Checkpoint Handoff — `.cursor/plans/HANDOFF_2026-08-03_pre_spine_portfolio_reconciled_post_c4_4.md`. POINTER ONLY — resolve ALL current state there**, per the **Boot-Surface Rule** above: operator-selected focus · gate/arc state · next allowed action · lane and integrator state · dependency and acceptance conditions · every SHA, blob and branch head. This line deliberately carries **no** current-state value. It previously carried a ~13,400-character status blob including the next-action directive *"it is NOT Gate 2"*, which outlived its moment and steered a compliant cold boot off the live Insurance arc — the origin of `D0CKPT-GRD-005` and the reason `D0CKPT-GRD-007` now forbids current state here. Nothing was lost: the checkpoint owns every fact the blob restated. Historical lineage, provenance only — **never boot from these**: `.cursor/plans/HANDOFF_2026-07-19_taskd_interim_checkpointed_evrun12_active.md` · `.cursor/plans/HANDOFF_2026-07-12_wave4_closed_grr_pre_plastics_and_task_d.md`. **Drift-guard:** read-graph Tier-0 #15 MUST name the same checkpoint file as this line; if they differ, the Boot Freshness Check fires → STOP.
