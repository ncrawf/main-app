...
- **Current Checkpoint Handoff — `.cursor/plans/HANDOFF_2026-08-03_pre_spine_portfolio_reconciled_post_c4_4.md`. POINTER ONLY.** Resolve current focus, gate state, next allowed action, lane and integrator state, dependency and acceptance conditions, and every commit identifier at that checkpoint. Nothing about current work may be restated on this line (Boot-Surface Rule above; guardrail `D0CKPT-GRD-007`; enforced by `npm run check:boot-surfaces`). Historical lineage, provenance only: `.cursor/plans/HANDOFF_2026-07-19_taskd_interim_checkpointed_evrun12_active.md` · `.cursor/plans/HANDOFF_2026-07-12_wave4_closed_grr_pre_plastics_and_task_d.md`. **Drift-guard:** read-graph Tier-0 #15 must name the same checkpoint file; if it does not, the Boot Freshness Check fires and you STOP.
