...
> **Two different tiers — do not conflate them.** This artifact is a **checkpoint Tier-3 output** (Agent Work Protocol §8: a Tier-3/Tier-4 stop requires a narrative volume) that is routed at **manifest/read Tier 2** (`add_tier2` · `consult_if_routed`). The Control Plane's locked `manifest_action_enum` has no `add_tier3` value, and the read graph has routing Tiers 0, 0.5, 1 and 2 — not 3. *Preservation tier ≠ routing tier.*

> **Operating/Maintenance Contract** (required for governed-stream artifacts — Control Plane Enforcement Rule 7 + `## Governed Stream Artifact Operating Contract Rule`, which names *volumed artifacts* explicitly). This contract is carried **by pointer, not duplicated**: `agent_work_protocol.md` §5 **Narrative Handling Rule** (arc action `same_arc_addendum` vs `new_volume`; lifecycle `active_open` / `snapshot_locked` / `closed_superseded_by:*`) + §8 **Narrative Minimum** and **Prior-Narrative Consultation Obligation** and **Guardrail Extraction Obligation** + §9 stop-proof requirements. **This artifact:** `new_volume`, state `active_open`; a same-arc continuation is a labelled addendum here, a distinct later arc spawns a new volume; it never becomes binding and never supersedes an earlier volume; catalog/read disposition `add_tier2 · consult_if_routed · no_new_route_needed`.
